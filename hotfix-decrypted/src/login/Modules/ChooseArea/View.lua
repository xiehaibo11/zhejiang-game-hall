...
function ChooseAreaView:onEnter()
    XH.NewThrowDataManager:AnalyseStartData("选区页面")
end
...
function ChooseAreaView:close()
    XH.NewThrowDataManager:AnalyseStartData("关闭选区")
    --数据统计
    local data = {}
...
-- 用户路径选区曝光
function ChooseAreaView:pushShowData()
    if XH.lobby and XH.lobby:getModule("UserPath") then
        ...
        XH.lobby:getModule("UserPath"):throwDataOnce(XH.NewThrowDataDefine.xyh24081402 , eventData)
    end
end

-- 用户路径选区完成
function ChooseAreaView:pushCloseData()
    if XH.lobby and XH.lobby:getModule("UserPath") then
        local eventData = {
            duration = os.time() - self._viewStartTime,
            customize =  {
                choice = XH.lobby:getModule("UserPath"):firstStart()
            }
        }
        XH.lobby:getModule("UserPath"):throwDataOnce(XH.NewThrowDataDefine.xyh24081403 , eventData)
    end
end

return ChooseAreaView  �2  