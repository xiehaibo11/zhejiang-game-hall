...
ThirdDefine.IAP_PAYRESULT = {
    ...
    PAYRESULT_ORDER_ID = 215,  --订单号回调
}

--推送SDK
ThirdDefine.NOTIFY_SDK_CODE = {
    NOTIFY_STATE_OPEN = 16004, --开启了通知权限
    NOTIFY_STATE_CLOSE = 16005, --关闭了通知权限
}
...
ThirdDefine.SAVEL_IMAGE_TO_GALLERY = {
    SAVE_IMAGE_SUCCESS =  1124,--	保存图片到相册成功
    SAVE_IMAGE_FAIL = 1125,--	保存图片到相册失败
}
...
ThirdDefine.REPLUGIN_CALLBACK_CODE = {
    ...
    PLUGIN_OPEN_PLUGIN_SUCCESS = 510000,
    PLUGIN_OPEN_PLUGIN_FAIL = 510001,
}
...
-- Solar SDK 回调
ThirdDefine.SOLAR_SDK_CALLBACK_CODE = {
    SOLAR_INIT_SUCCESS = 163001, -- 初始化成功
    SOLAR_INIT_FAIL = 163002, -- 初始化失败
    SOLAR_ATTRIBUTION_SUCCESS = 163003, -- JSON 归因成功，需对返回值进行字符串判空
    SOLAR_ATTRIBUTION_FAIL = 163004, -- errorCode 归因失败
    SOLAR_DISTINCT_ID = 163005, -- distinct_id 设备唯一ID，需对返回值进行字符串判空
    SOLAR_VISITOR_ID = 163006, -- visitor_id 访客 ID，需对返回值进行字符串判空
    SOLAR_ACCOUNT_ID = 163007, -- account_id 账号 ID，需对返回值进行字符串判空
    SOLAR_PRESET_PROPERTIES_JSON = 163008, -- 预置属性完整 JSON 数据，需对返回值进行字符串判空
}
