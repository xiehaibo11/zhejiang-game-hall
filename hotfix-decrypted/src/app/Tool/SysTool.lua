...
function SysTool:getCopyString()
    if device.platform == "windows" then
        return un.Device.getCopyString()
    end
    if device.platform == "android" then
        local luaj = require("cocos.cocos2d.luaj")
        local className = "org/cocos2dx/lua/DeviceUniverseExtend"
...
local b50 = "cbadefghjiklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789*$"
-- encoding
function SysTool:base64_encode50(data)
    return ((data:gsub('.', function(x) 
        local r,bit='',x:byte()
        for i=8,1,-1 do r=r..(bit%2^i-bit%2^(i-1)>0 and '1' or '0') end
        return r;
        ...
        local c=0
        for i=1,6 do c=c+(x:sub(i,i)=='1' and 2^(6-i) or 0) end
        return b50:sub(c+1,c+1)
    end)..({ '', '==', '=' })[#data%3+1])
end

-- decoding
function SysTool:base64_decode50(data)
    data = string.gsub(data, '[^'..b50..'=]', '')
    return (data:gsub('.', function(x)
        if (x == '=') then return '' end
        local r,f='',(b50:find(x)-1)
        for i=6,1,-1 do r=r..(f%2^i-f%2^(i-1)>0 and '1' or '0') end
        return r;
    end):gsub('%d%d%d?%d?%d?%d?%d?%d?', function(x)
        if (#x ~= 8) then return '' end
...
-- 截屏旋转
function SysTool.takeScreenshotAndRotate(callback, screenshotName)
    local fileFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp/" .. screenshotName
    local directoryFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp"
    if cc.FileUtils:getInstance():isDirectoryExist(directoryFullPath) then
        cc.FileUtils:getInstance():removeDirectory(directoryFullPath)
        cc.FileUtils:getInstance():createDirectory(directoryFullPath)
    ...
    local screenSize = cc.Director:getInstance():getWinSize()
    local target = cc.RenderTexture:create(screenSize.width, screenSize.height, _G.kCCTexture2DPixelFormat_RGBA8888, gl.DEPTH24_STENCIL8_OES)
    target:setKeepMatrix(true)
    target:begin()
    cc.Director:getInstance():getRunningScene():visit()
    target:endToLua()

    local function onSaveToFile(ret)
        if ret then
            -- 读取图片
            local image = cc.Sprite:create(fileFullPath)
            local size = image:getContentSize()
            -- 创建一个新的 RenderTexture
            local rotatedTarget = cc.RenderTexture:create(size.height, size.width, _G.kCCTexture2DPixelFormat_RGBA8888, gl.DEPTH24_STENCIL8_OES)
            rotatedTarget:setKeepMatrix(true)
            image:setPosition(size.width / 2, size.height / 2)
            image:setAnchorPoint(0.5, 0.5)
            local scaleX = screenSize.width / screenSize.height
            local scaleY = screenSize.height / screenSize.width
            image:setScaleX(scaleY)
            image:setScaleY(scaleX)
            image:setRotation(90)
            rotatedTarget:begin()
            image:visit()
            rotatedTarget:endToLua()
            rotatedTarget:saveToFile("tmp/" .. "rotated_" .. screenshotName, cc.IMAGE_FORMAT_JPEG, false)
        end
    end
    target:saveToFile("tmp/" .. screenshotName, cc.IMAGE_FORMAT_JPEG, false)
    local retry = 6
    local callbackHanlder
    callbackHanlder = function()
        local fileFullExPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp/" .. "rotated_" .. screenshotName
        local exist = cc.FileUtils:getInstance():isFileExist(fileFullExPath)
        if exist then
            callback(true, fileFullExPath)
        elseif retry >= 0 then
            XH.SysTool.performWithDelayGlobal(callbackHanlder, 0.5)
            retry = retry - 1
        else
    ...
    local retryEx = 6
    local callbackHanlderEx
    callbackHanlderEx = function()
        local exist = cc.FileUtils:getInstance():isFileExist(fileFullPath)
        if exist then
            onSaveToFile(true)
            XH.SysTool.performWithDelayGlobal(callbackHanlder, 0.5)
        elseif retryEx >= 0 then
            XH.SysTool.performWithDelayGlobal(callbackHanlderEx, 0.5)
            retryEx = retryEx - 1
        else
            callback(false, nil)
        end
...
-- 截图区域
function SysTool.captureScreenshotArea(callback, fileName, rect)
    local fileFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp/" .. fileName
    local directoryFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp"
    if cc.FileUtils:getInstance():isDirectoryExist(directoryFullPath) then
        cc.FileUtils:getInstance():removeDirectory(directoryFullPath)
    ...
    -- 1. 先截取整个屏幕
    local fullScreenRT = cc.RenderTexture:create(winSize.width, winSize.height)
    if not fullScreenRT then
        if callback then callback(false, "创建RenderTexture失败") end
        return
    end
    ...
    fullScreenRT:begin()
    scene:visit()
    fullScreenRT:endToLua()
    
    -- 2. 获取全屏纹理
    local fullTexture = fullScreenRT:getSprite():getTexture()
    
    -- 3. 计算要裁剪的区域（转换为纹理坐标）
    local texWidth = fullTexture:getPixelsWide()
    local texHeight = fullTexture:getPixelsHigh()
    
    -- 确保区域在屏幕范围内
    local clipX = math.max(0, math.min(x, winSize.width - width))
    local clipY = math.max(0, math.min(y, winSize.height - height))
    local clipWidth = math.min(width, winSize.width - clipX)
    local clipHeight = math.min(height, winSize.height - clipY)
    
    -- 计算纹理坐标系中的矩形
    local texRect = cc.rect(
        clipX / winSize.width * texWidth,
        clipY / winSize.height * texHeight,
        clipWidth / winSize.width * texWidth,
        clipHeight / winSize.height * texHeight
    )
    
    -- 4. 创建裁剪后的精灵
    local croppedSprite = cc.Sprite:createWithTexture(fullTexture, texRect)
    if not croppedSprite then
        if callback then callback(false, "创建裁剪精灵失败") end
        return
    end
    
    -- 5. 渲染裁剪后的内容到新纹理
    local finalRT = cc.RenderTexture:create(clipWidth, clipHeight)
    if not finalRT then
        if callback then callback(false, "创建最终RenderTexture失败") end
        return
    end
    
    -- 调整精灵位置，使其填满整个纹理
    croppedSprite:setPosition(clipWidth/2, clipHeight/2)
    croppedSprite:setFlippedY(true)  -- 确保不翻转
    ...
    finalRT:saveToFile("tmp/" .. fileName, cc.IMAGE_FORMAT_JPEG, false)
...
-- 截图节点
function SysTool.captureScreenshotNode(callback, fileName, node)
    local fileFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp/" .. fileName
    local directoryFullPath = cc.FileUtils:getInstance():getWritablePath() .. "tmp"
    if cc.FileUtils:getInstance():isDirectoryExist(directoryFullPath) then
        cc.FileUtils:getInstance():removeDirectory(directoryFullPath)
    ...
    local targetWidth = math.floor(size.width * scale)
    local targetHeight = math.floor(size.height * scale)
    local render = cc.RenderTexture:create(targetWidth, targetHeight, _G.kCCTexture2DPixelFormat_RGBA8888, gl.DEPTH24_STENCIL8_OES)

    -- 保存当前位置和锚点
    local originalPos = cc.p(node:getPosition())
    local originalAnchor = node:getAnchorPoint()
    
    -- 设置节点位置到renderTexture中心
    node:setAnchorPoint(cc.p(0.5, 0.5))
    node:setPosition(cc.p(targetWidth/2, targetHeight/2))
    node:setScale(scale)
...
function SysTool.isSupportSetOrientation()
    if device.platform == "windows" then
        return false
    end
    if device.platform == "ios" and XH.SysTool:CompareToBundleVersion("1.3.42") then
        return true
    end
    if device.platform == "android" and XH.SysTool:CompareToBundleVersion("1.5.3") then
        return true
    end
    return false
end

function SysTool.setOrientation(orientation)
    if not SysTool.isSupportSetOrientation() then
        return
    end

    if device.platform == "windows" then
        if orientation == SysTool.Orientation.SCREEN_ORIENTATION_PORTRAIT then
            cc.Director:getInstance():getOpenGLView():setFrameSize(576*0.8,  1024*0.8)
        elseif orientation == SysTool.Orientation.SCREEN_ORIENTATION_LANDSCAPE then
            cc.Director:getInstance():getOpenGLView():setFrameSize(1024,  576)
        end
        return
    end

    local param = (orientation == SysTool.Orientation.SCREEN_ORIENTATION_PORTRAIT) and "portrait" or "landscape"
    if device.platform == "ios" then
        local luaoc = require("cocos.cocos2d.luaoc")
        luaoc.callStaticMethod("AppController", "setOrientation", { ["orientation"] = param })
        return
    end

    if device.platform == "android" then
        local luaj = require("cocos.cocos2d.luaj")
        luaj.callStaticMethod("org/cocos2dx/lua/AppActivity", "setOrientation", { param }, "(Ljava/lang/String;)V")
    end
end
...
-- 获取时区偏移量（单位：秒）
function SysTool.get_timezone_offset()
    local now = os.time()
    local utc = os.time(os.date("!*t", now))
    return os.difftime(now, utc) -- 返回与 UTC 的秒数差
end

-- 是否通过卓易通安装
function SysTool.isInstalledViaZhuoyitong()
    if device.platform == "android" then
        local luaj = require("cocos.cocos2d.luaj")
        local ok, ret = luaj.callStaticMethod("org/cocos2dx/lua/AppActivity", "isInstalledViaZhuoyitong", {}, "()Z")
        if ok and ret then
            return true
        end
    end
...
-- 是否是官包
function SysTool.isOfficialPackage()
    if device.platform == "android" then
        return tonumber(SysTool.getChannelID()) == 30002
    end
    return true
end
