...
function GameNewTypeConfiger:isShowNewType(gameID)
	...
	if allCfg and allCfg.showNewType then
		for _, v in pairs(allCfg.showNewType) do
			if v == gameID then
				return true
			end
		end
	end
    if allCfg and allCfg.showNewTypeLobby and allCfg.showNewTypeLobby[tostring(XH.areaData:getLobbyID())] then
        for _, v in pairs(allCfg.showNewTypeLobby[tostring(XH.areaData:getLobbyID())]) do
			if v == gameID then
				return true
			end
		end
