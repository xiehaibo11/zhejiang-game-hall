...
// xyid=2338
message PageRecallPlayerExRequest {
  int32 tenant_id = 1;                // 租户ID
  int32 area_id = 2;                  // 区域ID
  int32 club_id = 3;                  // 比赛场ID
  int32 page_no = 4;                  // 分页页码
...
// xyid=2339
message PageRecallPlayerExReply {
  repeated Player player = 1;  // 玩家信息列表
  int32 curPage = 2;           // 当前页码
  int32 totalPage = 3;         // 总页数
  CommonResp cr = 4;
...
// xyid=2213
message LimitedTimeOfferRequest{
    int32 ask_id = 1;
    int32 tenant_id = 2;
    int32 area_id = 3;
    int32 user_id = 4;
...
// xyid=2214
message LimitedTimeOfferReply{
    CommonResp resp = 1;
    int32 strategy_id = 2;//策略id,若是0表示无策略,使用道具的时候,将该值带到param中,例如strategy_id=3;
    string name = 4;//名称
    string now = 5;//当前时间
    int32 can_discount_endtime = 6;//可以折扣结束时间
    int32 discount = 7;//扩大100倍,例如10表示1折,100表示无折扣
}

// xyid=2215
message CaiYunFreeUsePropsRequest{
  int32 ask_id = 1;
  int32 tenant_id = 2;
  int32 area_id = 3;
  int32 user_id = 4;
  int32 props_id = 5;
  int32 props_cnt = 6;
  string props_param = 7;
}
// xyid=2216
message CaiYunFreeUsePropsReply{
  CommonResp resp = 1;
  int32 props_id = 2;
  int32 props_cnt = 3;
  string props_param = 4;
}
]]  �M  