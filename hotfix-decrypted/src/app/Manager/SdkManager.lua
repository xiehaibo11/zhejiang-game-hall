...
SdkManager.EVENT_SAVE_IMAGE_CALLBACK = "EVENT_SAVE_IMAGE_CALLBACK"
...
SdkManager.EVENT_SOLAR_SDK_CALLBACK = 'EVENT_SOLAR_SDK_CALLBACK'
...
function SdkManager:ctor()
    ...
    self._pushNotificationsEnabled = true
...
function SdkManager:onSdkCallBack(code, msg)
    ...
    elseif self:hasIndexOf(XH.ThirdDefine.SAVEL_IMAGE_TO_GALLERY, code) then
        self:onSaveImageCallback(code, msg)
...
--        self:onOpenUrlCallBack(code, msg)
    ...
    elseif self:hasIndexOf(XH.ThirdDefine.NOTIFY_SDK_CODE,code) then
        self:onNotifyCallback(code, msg)
    elseif self:hasIndexOf(XH.ThirdDefine.SOLAR_SDK_CALLBACK_CODE,code) then
        self:onSolarSdkCallback(code, msg)
    else
...
function SdkManager:onSolarSdkCallback(code, msg)
    self:dispatchEvent( { name = self.EVENT_SOLAR_SDK_CALLBACK, data = { code = code, msg = msg } })
end
...
function SdkManager:onIapPayResult(code, msg)
    ...
    if Creator2Interface and Creator2Interface.isBuy then
        Creator2Interface.buyItemsCalBack(code, msg)
        return
    end
...
function SdkManager:onAMapCallback(code, msg)
    ...
    elseif code == XH.ThirdDefine.AMAP_CALLBACK_CODE.AMAP_SUCCESS then
        local tData
        local ok, _ = pcall(function()
            tData = cjson.decode(msg)
        end
        )
        if ok and tData then
            ...
            XH.playerData:setServerLocation(tData["longitude"], tData["latitude"], sCountry .. sProvince .. sCity .. sDistrict .. sStreet)
            self:setLastGpsInfo()
        end
    end
...
function SdkManager:onSaveImageCallback(code, msg)
    self:dispatchEvent( { name = self.EVENT_SAVE_IMAGE_CALLBACK, data = { code = code, msg = msg } })
end
...
function SdkManager:onSharedSdkCallback(code, msg)
    ...
    if code == XH.ThirdDefine.SHAREDSDK_CALLBACK_CODE.WX_FLAG_SHARERESULT_FAIL and msg == "微信未安装！" then
        self:copyLastShareInfo()
    else
        self:setLastShareInfo()
    end
end
...
function SdkManager:onSysfuncSdkCallback(code, msg)
    ...
    if code == XH.ThirdDefine.XH_SYSFUNC_SDK_CODE.SYSFUC_ADDRESS_SUCCESS then
        ...
        if ok and json then
            ...
            local gcj_lat, gcj_lng = mapTool.wgs84_to_gcj02(json.latitude, json.longitude)
            print("SYSFUC_ADDRESS_SUCCESS", json.latitude, json.longitude, gcj_lat, gcj_lng)  -- 输出 GCJ-02 坐标
            json.longitude = gcj_lng
            json.latitude = gcj_lat
            XH.playerData:setServerLocation(json.longitude, json.latitude)
        end
        XH.lobby:getModule("Lobby"):reqLocation()
        local gpsMsg = "{}"
        local gpsCode = -1
        self:onAMapCallback(gpsCode, gpsMsg)
    elseif code == XH.ThirdDefine.XH_SYSFUNC_SDK_CODE.SYSFUC_ADDRESS_FAIL_2 and device.platform == "android" then
        self._bNotGpsServer = true
    end
    if code ~= XH.ThirdDefine.XH_SYSFUNC_SDK_CODE.SYSFUC_ADDRESS_SUCCESS then
        self:reqGpsDetailInfo()
    end
end

function SdkManager:needReqLocation()
    local minDis = 200
    local longitude, latitude = XH.playerData:getServerLocation()
    local lastLongitude, lastLatitude, lastCity = self:getLastGpsInfo()
    if longitude and latitude and lastLongitude and lastLatitude and lastCity and lastCity ~= "" then
        local dis = math.ceil(XH.SysTool.calLantitudeLongitudeDist(longitude, latitude, lastLongitude, lastLatitude))
        print("yuhang get dis " .. dis)
        if dis < minDis then
            XH.playerData:setServerLocation(longitude, latitude, lastCity)
            return false
        end
    end
    return true
...
function SdkManager:setLastGpsInfo()
    local longitude, latitude, city = XH.playerData:getServerLocation()
    if longitude == nil or latitude == nil or city == nil or city == "" then
        return
    end
    local data = {
        longitude = longitude,
    ...
    XH.userDefault:setValue(XH.userDefault.KEY_ID.KW_SCORING_ASSISTANT, json.encode(data))
end

function SdkManager:getLastGpsInfo()
    local localData = XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_SCORING_ASSISTANT, "")
    -- print("yuhang get getLastGpsInfo " .. localData)
    if localData and localData ~= "" then
        local json
        local ok = pcall(function()
            json = cjson.decode(localData)
        end)
        if ok and json then
            return json.longitude, json.latitude, json.city
        end
    end
    return nil, nil, nil
end
...
function SdkManager:startLocationByAmap(accuracy)
    if device.platform == "windows" then
        local msg = "{\"latitude\":20.290883,\"longitude\":200.117307}"
        local code = 1101
        self:onSysfuncSdkCallback(code, msg)
        return
    end
    local accuracy = XH.sdkManager.AmapAccuracy.MID
    if device.platform == "android" then
        -- 如果本地已经有经纬度说明获取成功
...
function SdkManager:reqGpsDetailInfo()
    local accuracy = XH.sdkManager.AmapAccuracy.MID
    if device.platform == "android" then
...
function SdkManager:startShare(info, func, obj)
    ...
    -- 加入中转URL
    local scene, mini_scene = XH.lobby:getModule("Share"):getShareScene()
    if info.actID then
        info.url = string.format("%s?url=%s&areaid=%d&scene=%s&act_id=%d&share_code=%s&share_uid=%d&wordsid=%d", UrlConf.URL_SHARE_LUCKY_BAG, string.urlencode(info.url), XH.areaData:getAreaID(), string.urlencode(scene), info.actID, string.urlencode(info.code), info.shareUID, info.wordsid)
        XH.lobby:getModule("Share"):resetShareScene()
    elseif info.url and UrlConf.URL_SHARE_TRANSFER and scene and mini_scene and not string.find(info.url,UrlConf.URL_SHARE_TRANSFER) then
        info.url = string.format("%s?url=%s&areaid=%d&scene=%s&subscene=%s", UrlConf.URL_SHARE_TRANSFER, string.urlencode(info.url), XH.areaData:getAreaID(), string.urlencode(scene), string.urlencode(mini_scene))
        if info.from_user_id then
            info.url = string.format("%s&from_user_id=%d",info.url, info.from_user_id)
        end
        XH.lobby:getModule("Share"):resetShareScene()
    end
    -- 防止多次创建热情Share对象，调用多次回调
    if self._reqShare then
        self._reqShare:stop()
    end
    local ReqShare = require("app.Req.SDK.ReqShare")
    self._reqShare = ReqShare.new()
    if func and obj then
        self._reqShare:addReqCallBack(obj, func)
    end
    self._shareUrl = info.url
    self._reqShare:start(info)
    if CF and CF.isInGameScene and CF.isInGameScene() then
        ...
        XH.lobby:getModule("RecallNew"):notifyReBackTaskRequest(RecallNewConfig.ReBackTaskType.Rtt_ShareGame)
    end
end

function SdkManager:getShareUrl()
    return self._shareUrl or ""
end
...
--func 回调方法
--obj 回调方法所在的self对象
function SdkManager:addToponAds(name, showType, leaveGoldGame)
    if device.platform == "windows" or name == nil or name == "" then
        print("windows not support topon")
        return
    end
    local leaveGoldGame = leaveGoldGame == nil and true or leaveGoldGame
    if name == XH.ThirdDefine.TOPONSDK_AD_TYPE.REWARDEVIDEO and showType == "show" and leaveGoldGame then
        self:closeGoldGame()
    end
    local adInfo = {}
    adInfo.adName = name
...
function SdkManager:callFunctionWithMap(funcName, data, isTransform)
    ...
    elseif device.platform == "android" then
        ...
        if XH.isEmbeddedApp() then
            self:callFunctionEmbedded(funcName, data, isTransform)
            return
        end
...
function SdkManager:callFunctionEmbedded(funcName, data, isTransform)
    print("yuhang callFunctionEmbedded " .. funcName)
    dump(data)
end
...
function SdkManager:onAddPermissionCallback(code, msg)
    ...
    local setPermissionState = function(perm,value)
        ...
        self:pushPermissionData(perm,value)
    end
...
-- 是否支持热力引擎
function SdkManager:isSupportSolarAndroid()
    if device.platform ~= "android" or not XH.SysTool:CompareToBundleVersion("1.6.2") then
        return false
    end
    local channelID = tonumber(XH.SysTool:getChannelID())
    return SOLAR_SUPPORT_CHANNELS[channelID] == true
end

function SdkManager:isSupportSolarIOS()
    if device.platform ~= "ios" or not XH.SysTool:CompareToBundleVersion("1.3.52") then
        return false
    end
    return true
end
...
function SdkManager:checkCanUseReYunIOS()
    if device.platform ~= "ios" or XH.SysTool:CompareToBundleVersion("1.3.52") or (not XH.SysTool:CompareToBundleVersion("1.3.34")) then
        return false
    end
    return true
end
...
function SdkManager:isSupportBuy()
    if XH.isEmbeddedApp() then
        return true
    end
    ...
    if DEBUG then
        ...
        if device.platform == "android" then
            ...
            local sigs = "(Ljava/lang/String;)Ljava/lang/String;"
            local strClass = "com.bianfeng.platform.executor.AppConfig"
            local _, ret = luaj.callStaticMethod(strClass, "getMetaDataValue", { "TD_CONFIG_ID" }, sigs)
            local testPackageIDs = { 7353, 8953, 9040 } -- 测试浙江游戏大厅的packageid
            for i = 1, #testPackageIDs do
                if testPackageIDs[i] == tonumber(ret) then
                    return true
                end
            end
        end
        XH.TipTool.showToast("测试环境不允许充值哦~", 2)
        return false
    end
    return true
end

function SdkManager:reqNotifyState()
    if device.platform == "ios" then
        XH.sdkManager:xhCallOCFunc("XHsysfunc", "sysfunc_get_notification_status", {})
    end
end

function SdkManager:onNotifyCallback(code, msg)
    if code == XH.ThirdDefine.NOTIFY_SDK_CODE.NOTIFY_STATE_OPEN then
        self._pushNotificationsEnabled = true
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.gt24101602, { item_id = "开启" })
    elseif code == XH.ThirdDefine.NOTIFY_SDK_CODE.NOTIFY_STATE_CLOSE then
        self._pushNotificationsEnabled = false
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.gt24101602, { item_id = "关闭" })
    end
end

function SdkManager:isPushNotificationEnabled()
    return self._pushNotificationsEnabled
end

function SdkManager:pushPermissionData(perm, value)
    if XH and XH.lobby and XH.lobby:getModule("UserPath") then
        local blockLableMap = {
            [self.AndroidPermission.ACCESS_LOCATION] = "位置权限",
            [self.AndroidPermission.RECORD_AUDIO] = "录音权限",
            [self.AndroidPermission.READ_PHONE_STATE] = "访问电话状态",
            [self.AndroidPermission.WRITE_EXTERNAL_STORAGE] = "写入外部存储",
        }
        local eventData = {
            block_label = blockLableMap[perm] or "未知",
            customize = {
                state = value and "成功" or "失败"
            }
        }
        XH.lobby:getModule("UserPath"):throwDataOnce(XH.NewThrowDataDefine.xyh24081413, eventData)
    end
end

function SdkManager:setLastShareInfo(info)
    self._lastInfo = info
end

function SdkManager:copyLastShareInfo()
    if self._lastInfo == nil then
        return
    end
    if string.match(un.Device.getDeviceName(), "HUAWEI") or string.match(un.Device.getDeviceName(), "huawei") then
        local strText = (self._lastInfo.copyTextWhenNoWX or self._lastInfo.textTitle) .. '\n' .. (self._lastInfo.text or "") .. '\n' .. (self._lastInfo.url or "")
        XH.SysTool.copyString(strText)
        XH.TipTool.showToast("邀请信息已复制成功，快去粘贴分享给您的好友吧！", 2)
        XH.SysTool.performDelayOnce(function()
...
function SdkManager:openYmnUrl(url)
    if url == nil then
        return
    end
    local reqFunc = {
        ["functionName"] = "sysfunc_open_safari",
        ["args"] = {
            ["url"] = url,
        }
    }
    XH.sdkManager:xhCallOCFunc("UserInterface", "callFunction", reqFunc)
end
