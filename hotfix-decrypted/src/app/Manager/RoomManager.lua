...
function RoomManager:onEnterGame(data,roomMode,reconnect,bInAppID)
	...
	param.is50 = data.is50 or false
	if bInAppID then
		param.appID = data.inappid or data.gameappid
	end
	if param.is50 then 
		param.appID = 88888888
	end 
...
function RoomManager:leaveRoomFunc(roomData, bCanBack, gameLeaveData)
	local srsGroupId = roomData._srsGroupID
	local appID = roomData._appID
	if gameLeaveData and gameLeaveData.srsGroupID and gameLeaveData.appID then
		srsGroupId = gameLeaveData.srsGroupID
		appID = gameLeaveData.appID
	end
	if not bCanBack then
		-- 千变双扣50房间升场到33时，roomData(gameStartData数据不会重新设置变化)
		XH.roomManager:reqLeaveRoom(appID,srsGroupId)
	end
	self:reqLeaveDispatchQueue(roomData)
	self._inGame = false
	self:dispatchEvent({name = self.EVENT_GAME_SCENE_EXIT, data = {gameid = roomData._gameID}})
	...
	local gameSrsID = XH.areaData:getSrsGroupIDbyAppID(appID)
	if gameSrsID and gameSrsID ~= XH.areaData:getSrsGroupID() and gameSrsID ~= XH.areaData:getGoldSrsGroupID()then
