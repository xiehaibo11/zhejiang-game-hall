...
PlayerData.EVENT_PLAYER_TIME_ROOMCARD_CHANGED = "EVENT_PLAYER_TIME_ROOMCARD_CHANGED"
...
function PlayerData:getReqConfig()
    return {
        ...
        ReqPlayerLimitedTimeRoomCard = {reqPath = "app.Req.PlayerMessage.ReqPlayerTimePropInfo", callBack = self.onPlayerLimitedTimeRoomCardCallBack},
...
function PlayerData:setPlayerDrop(propsList)
    ...
    self:dispatchEvent({name = PlayerData.EVENT_PLAYER_DROP_CHANGED, msg = propsList})
end

function PlayerData:setPlayerTimeDrop(timeProps)
    self._timeProps = timeProps
    self:dispatchEvent({name = PlayerData.EVENT_PLAYER_TIME_DROPS_CHANGED, msg = timeProps})
end
...
--限时房卡数据
function PlayerData:setPlayerLimitedTimeRoomCard(timeRoomCard)
    self._timeRoomCard = timeRoomCard
    self:dispatchEvent({name = PlayerData.EVENT_PLAYER_TIME_ROOMCARD_CHANGED})
end

function PlayerData:getPlayerTimeRoomCard()
    return self._timeRoomCard
end
...
function PlayerData:getStrSR()
    return XH.StringTool.numberToString(self._tPlusData.nSR or 0)
end
...
function PlayerData:clearFreezeData()
    ...
    local propsList = { XH.areaData:getPropDiamndID(), XH.areaData:getPropRoomCardID(), XH.areaData:getPropBindRoomCardID() }
    for _, v in pairs(propsList) do
        local freezeCount = self:getFreezePropCountByID(v) or 0
...
function PlayerData:getRealPhone()
    if self._phoneBindStr and self._phoneBindStr ~= "" then
        return self._phoneBindStr
    elseif self._bindUserData and #self._bindUserData.bind_phone >= 11 then
        return self._bindUserData.bind_phone
    end
    return ""
end
...
function PlayerData:flushPlayerLimitedTimeRoomCard()
    local srsGroupID = XH.areaData:getSrsGroupID()
    self:startReq("ReqPlayerLimitedTimeRoomCard", XH.areaData:getPropList().limitedTimeRoomCardID, srsGroupID, 500)
end

function PlayerData:onPlayerLimitedTimeRoomCardCallBack(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        self:setPlayerLimitedTimeRoomCard(data.items)
    end
end
    ...
    if type == XH.Req.TYPE.SUCCESS then
         ...
         if data.level then
            if data.level == 1 and data.status == 2 and self:getProtectedInwhitelist() == 0 then
                ...
                if XH.lobby:getModule("Lobby"):getIsForeignIP() or XH.lobby:getModule("IdentityCheck"):isSpecialCode() or XH.isEmbeddedApp() then
                    return
                end
         ...
         if self:getProtectedInwhitelist() ~= 0 then -- 白名单中的用户
            XH.lobby:getModule("RealNameSupplement"):reqGetRealName()
         end
    else
        self:setProtected(0)
    end
...
function PlayerData:getFreeRoomCard(withLimiteTime)
        ...
        if areaConfig[lobbyid] then
            ...
            if propList then
                if self._props[propList.freeRoomCardID] and XH.areaData:getPropFreeRoomCardRatio() then
                    ...
                    if withLimiteTime and XH.lobby:getModule("Lobby"):isSupportTimeCard() then
                        tmpRoomCardCount = tmpRoomCardCount + tonumber(self._props[propList.limitedTimeRoomCardID] or 0) / XH.areaData:getPropLimitedTimeRoomCardRatio()
                    end
                    return tmpRoomCardCount
                end
            end
...
function PlayerData:getLimitedTimeRoomCard()
    if XH.areaData:isRemodelArea() then
        local lobbyid = XH.userDefault:getValue(XH.userDefault.KEY_ID.LOBBYID, 0)
        if areaConfig[lobbyid] then
            local propList = areaConfig[lobbyid].propList
            if propList then
                if self._props[propList.freeRoomCardID] and XH.areaData:getPropLimitedTimeRoomCardRatio() then
                    return tonumber(self._props[propList.limitedTimeRoomCardID] or 0) / XH.areaData:getPropLimitedTimeRoomCardRatio()
                end
            end
        end
    end
...
function PlayerData:getRemodelRoomCard()
    ...
    if XH.areaData:isRemodelArea() then
        tmpRoomCardCount = tmpRoomCardCount + self:getFreeRoomCard(true)
    end
    return tmpRoomCardCount
end
...
function PlayerData:getGoldHeadUrl()
    return XH.lobby:getModule("GoldNew"):getGoldHeadUrl()
end
