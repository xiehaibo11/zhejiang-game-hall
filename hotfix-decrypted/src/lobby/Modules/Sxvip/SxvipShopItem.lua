...
function SxvipShopItem:onBuy(sender , eventType)
    ...
    XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.hy24090602, {page_item_id=sender.customData.name, block_item_id='大厅商城'})
    -- 未实名不能购买 海外玩家不用实名认证
    ...
    if device.platform == "android" or device.platform == "windows" then
        XH.lobby:getModule("Sxvip"):reqBuy(sender.customData, nil, nil, '大厅商城')
    else
