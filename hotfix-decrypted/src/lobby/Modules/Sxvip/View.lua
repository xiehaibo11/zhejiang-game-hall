...
function SxvipView:ctor(params)
    ...
    if not self._defalutMenu then
        self._defalutMenu = XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_SXVIP_DEFALUT_VIEW, SxvipConfig.MENU_DEFINE.DAILY_GIFT , "Sxvip")
    end
    -- 进入会员界面后，重新请求vip状态
    ...
    self:initView()
    self:updateAllView()
    self.KW_BTN_GIVE:setVisible(not XH.lobby:getModule("Sxvip"):isHideSendMenu())
end
...
function SxvipView:onStaticConfigSucc()
    self:updateTop()
end
...
function SxvipView:ON_KW_BTN_PAY(sender , eventType)
    ...
    XH.viewManager:openView("SxvipShopView", nil, self:getMenuData().throwDataID)
    XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.zyt2024032808, {block_item_id='特权详情页'})
    self:checkCloseSelf()
end
...
function SxvipView:checkCloseSelf()
    if self._defalutMenu == SxvipConfig.MENU_DEFINE.FEED_BACK then
        self:close()
    end
end
...
function SxvipView:onCleanup()
    ...
    XH.lobby:getModule("Sxvip"):clearStatisticsDataNew()
end
