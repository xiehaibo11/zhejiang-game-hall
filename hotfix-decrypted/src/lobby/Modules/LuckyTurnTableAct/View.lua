...
function LuckyTurnTableActView:ctor(param)
    ...
    param = param or {}
    self._isAutoPop = param.auto == true
	LuckyTurnTableActView.super.ctor(self, param)
    self:initData()
    self:initView()
    XH.NewThrowDataManager:throwData("ws26070201")
end

function LuckyTurnTableActView:initData()
    self._haveActInfo = false
    self._chances = 0
...
function LuckyTurnTableActView:initView()
    ...
    if self._btnAwardKgg and not self:isAdBtnSupported() then
        self._btnAwardKgg:setVisible(false)
    end
    -- self:showShareTips()
    self:showStandbyAni()
...
function LuckyTurnTableActView:isAdBtnSupported()
...
function LuckyTurnTableActView:onTouchEventShare(send, eventType)
    -- TODO 分享
    XH.lobby:getModule("Share"):setShareScene("大厅","活动幸运转盘")
...
function LuckyTurnTableActView:onTouchEventStart(send, eventType)
    XH.NewThrowDataManager:throwData("ws26070203")
    if self._haveActInfo == false then
        XH.TipTool.showToast("暂无活动")
        return
    end
...
function LuckyTurnTableActView:updateAwards(event)
    ...
    for i = 1, #awards do
        if self["_award" .. i] then
            ...
            if awards[i].pid == XH.areaData:getPropList().limitedTimeRoomCardID then
                value = value / XH.areaData:getPropLimitedTimeRoomCardRatio()
            end
...
function LuckyTurnTableActView:showGotAward(event)
    ...
    -- _awardsData 依赖奖品列表事件先到达，若抽奖结果先回则直接忽略动画，避免崩溃
    if type(self._awardsData) ~= "table" or #self._awardsData == 0 then
        return
    end
...
function LuckyTurnTableActView:onGetActData(event)
    if not event.data or next(event.data) == nil then
        return
    end
    self._haveActInfo = true
    ...
    if data.advertisementCount and self:isAdBtnSupported() then
...
-- 新增看广告统一埋点
function LuckyTurnTableActView:throwADData(id)
    local conventionData = {}
    conventionData.block_item_id = "转盘活动"
    XH.NewThrowDataManager:throwData(id, conventionData)
end

return LuckyTurnTableActView �C  