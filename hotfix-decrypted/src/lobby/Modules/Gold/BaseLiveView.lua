...
function BaseLiveView:onTouchEventGetToponAaward(send, eventType)
    if self._toponActInfo.limitCnt >= self._toponActInfo.maxLimtCnt then
        ...
        if self:isNewBankupt() then
            ...
            XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.jblb2024052904, conventionData, { choice = "", info = "" })
        end
    end
end
...
function BaseLiveView:onTouchEventReqBreakShare()
    ...
    XH.lobby:getModule("Share"):setShareScene("大厅", "破产礼包")
...
function BaseLiveView:getExchangeProductNew()
    local flag = self._roomFlag
    local confID = XH.goldConfigManager:getIndexByGameid(self._gameID)
...
function BaseLiveView:isBankruptBuy()
    ...
    if self._gameID == 30579 then
        info = XH.lobby:getModule("YGiftBankruptcyV2"):getPlayerActInfo()
    end
    if info then
        return true
    end
...
-- 新增看广告统一埋点
function BaseLiveView:throwADData(id)
    local conventionData = {}
    conventionData.block_item_id = "破产"
