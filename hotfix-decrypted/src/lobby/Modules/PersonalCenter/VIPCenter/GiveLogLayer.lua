...
function GiveLogLayer:ctor()
    ...
    local logInfo = XH.lobby:getModule("PersonalCenter"):getVipLogInfo(self._page,KW_MAX_LOG_NUM, nil, self._chooseType)
    if logInfo then
...
function GiveLogLayer:flush(data)
    XH.lobby:getModule("PersonalCenter"):getVipLogInfo(1, KW_MAX_LOG_NUM, true, self._chooseType)
end
...
function GiveLogLayer:getLogInfoByType(chooseType)
    ...
    return self._logInfoTable.logs
end
...
function GiveLogLayer:onTouchEventNextPage(send, eventType)
    ...
    XH.lobby:getModule("PersonalCenter"):getVipLogInfo(self._page, KW_MAX_LOG_NUM, true, self._chooseType)
end
...
function GiveLogLayer:getMaxPage()
    if self._logInfoTable.total_page == 0 then
        return 1
    end
    return self._logInfoTable.total_page
end
