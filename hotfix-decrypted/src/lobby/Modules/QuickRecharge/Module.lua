...
-- @param scene 场景名称
-- @return 无
function QuickRechargeModule:reqSpecialSceneGoods(scene, param)
    self._scene = scene
    self._param = param
...
function QuickRechargeModule:respSpecialSceneGoods(req, type, data)
    ...
    if type == XH.Req.TYPE.SUCCESS then
        ...
        if param and string.find(param, "throwprop=1") and string.find(param, "isVipProp=1") then
            XH.viewManager:openView("QuickRechargeVipView", nil, data)
        else
