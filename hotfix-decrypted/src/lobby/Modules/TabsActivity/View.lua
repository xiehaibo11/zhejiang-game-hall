...
function ActivityView:dealDailySignTab()
    ...
    for i = #self._acts, 1, -1 do
        ...
        elseif self._acts[i].icon_code == "DAILYSIGN" and XH.lobby:getModule("ToponAct"):getSignActId() == nil then
            -- 如果没有签到actid也移除
            table.remove(self._acts, i)
        elseif self._acts[i].icon_code == "DAILYSIGN" and not XH.lobby:getModule("RecallNew"):isSignTaskFinish() then
            -- 回归期间也隐藏签到
            table.remove(self._acts, i)
        end
    end
...
function ActivityView:dealLuckyTurnTableTab()
    if XH.areaData:getLobbyID() ~= XH.LOBBY_ID.TAIZHOU then
        return
    end
    local bShow = XH.lobby:getModule("LuckyTurnTableAct"):hasWhiteAct()
    for i = #self._acts, 1, -1 do
        if not bShow and self._acts[i].icon_code == "LUCKYTURNTABLE" then
            table.remove(self._acts, i)
        end
    end
end
    ...
    if self._firstTab == KW_FIRST_TAB_TYPE.ACTIVITY then
        ...
        if viewTmp then
            ...
            if node.setInsertMode then
                node:setInsertMode("BoxLobby")
            end
            self:adaptActSize(node, viewTmp.scale)
