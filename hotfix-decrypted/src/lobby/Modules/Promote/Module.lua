    ...
    if type == XH.Req.TYPE.SUCCESS then
        ...
        if data.draw_type == PromoteDefine.DRAW_TYPE.FIRST or data.draw_type == PromoteDefine.DRAW_TYPE.SECOND then
            ...
            if task ~= nil then
                ...
                local disableShareDouble = true -- todo 后期可配
...
function PromoteModule:flushPromoteActIcon()
    self:dispatchEvent({name = self.EVENT_FULSH_PROMOTE_DAY_ICON, data = {icon = LobbyConfig.ICON_ID.PROMOTE_ACT, isShow = false}}) --self:isOnlineAct()
end
...
function PromoteModule:reqSpreadTask(teaNumber, owner_numid, isOwner, isAdmin, isFirstInTea)
    ...
    local leader_numid = owner_numid
    if player_type ~= PromoteDefine.PLAYER_TYPE.ADMIN then
        owner_numid = 0
    end
    self:startReq("ReqSpreadTask", teaNumber, player_type, owner_numid, isFirstInTea, 10, leader_numid)
end
...
function PromoteModule:updateTeaIcon()
    ...
    local text = ""
    if isOwner and self:getBaseInfo().owner_join_act ~= 1 then
        return
    end
    if (not isOwner) and self:getBaseInfo().player_join_act ~= 1 then
        return
    end
    if (not isOwner) and self:getTaskInfo().player_join_act ~= 1 then
        return
    end
...
function PromoteModule:hasActPlayModes()
    ...
    if PromoteConfig[XH.areaData:getLobbyID()].IGNORECREATE then
        return true
    end
    if PromoteConfig[XH.areaData:getLobbyID()].TWOMODEALLGAME then
        for i = 1, #playModes do
            if playModes[i].bIsEnable then
...
function PromoteModule:getActPlayTables()
    ...
    for _, info in pairs(tableInfos) do
        if PromoteConfig[XH.areaData:getLobbyID()].TWOMODEALLGAME then
            if info.count ~= PromoteConfig[XH.areaData:getLobbyID()].limitCnt and info.state == 0 and info.size == 2 then
        ...
        if info.count ~= PromoteConfig[XH.areaData:getLobbyID()].limitCnt and info.state == 0 and info.gameid == PromoteConfig[XH.areaData:getLobbyID()].GAMEID and string.find(info.data, PromoteConfig[XH.areaData:getLobbyID()].RULE) and info.size == 2 then
            for i = 1, #info.nNumid do
                if info.nNumid[i] == 0 then
                    if fit2Tables ~= nil then
                        if fit2Tables.createTime < info.createTime then
    ...
    local tableInfos = teaData:getTableInfos()
    for _, info in pairs(tableInfos) do
        if info.count ~= PromoteConfig[XH.areaData:getLobbyID()].limitCnt and info.state == 0 and info.gameid == PromoteConfig[XH.areaData:getLobbyID()].GAMEID and string.find(info.data, PromoteConfig[XH.areaData:getLobbyID()].RULE) and info.size == 4 then
            local nLeft = 0
            for i = 1, #info.nNumid do
                if info.nNumid[i] == 0 then
                    nLeft = nLeft + 1
...
function PromoteModule:getActPlayModes(playerCount)
    ...
    if PromoteConfig[XH.areaData:getLobbyID()].TWOMODEALLGAME then
        for i = 1, #playModes do
            if playModes[i].anCount[1] ~= PromoteConfig[XH.areaData:getLobbyID()].limitCnt and playModes[i].bIsEnable then
                for j = 1, #playModes[i].anPlayerCount do
                    if playModes[i].anPlayerCount[j] == 2 then
                        return playModes[i]
                    end
    ...
    for i = 1, #playModes do
        if playModes[i].anCount[1] ~= PromoteConfig[XH.areaData:getLobbyID()].limitCnt and playModes[i].nGameId == PromoteConfig[XH.areaData:getLobbyID()].GAMEID and playModes[i].bIsEnable and string.find(playModes[i].acData, PromoteConfig[XH.areaData:getLobbyID()].RULE) then
            for j = 1, #playModes[i].anPlayerCount do
                if playModes[i].anPlayerCount[j] == 2 then
                    return playModes[i]
...
function PromoteModule:quickCreatePlayMode(teaNumber)
    ...
    if lastPlayMode ~= "" then
        ...
        if setMode.gameid ~= PromoteConfig[XH.areaData:getLobbyID()].GAMEID then
            lastPlayMode = ""
        end
    end
    if lastPlayMode == "" then
        ...
        if string.find(setMode.data, PromoteConfig[XH.areaData:getLobbyID()].RULE) == nil then
            ...
            setMode.data = string.gsub(setMode.data, "';TabeleColor", ","..ruleName.."';TabeleColor")
        end
    end
    local teaData = XH.teaHouseManager:getTeaHouseData()
    if teaNumber == nil then
...
function PromoteModule:checkTeaHouseShow()
    ...
    if TeaHouse.manager.teahouseMain and TeaHouse.manager.teahouseMain:getIsFirstInTea() then
        return
    end
    local teahouseInfo = self:getPendTeahouseInfo(teahouseData:getTeaNumber())
    if not teahouseInfo or not teahouseInfo.nTeaOwnerNumid then
    ...
    if teahouseData:isOwner() and self:getBaseInfo().owner_join_act ~= 1 then
        return
    end
    if (not teahouseData:isOwner()) and self:getBaseInfo().player_join_act ~= 1 then
        return
    end
    if (not teahouseData:isOwner()) and self:getTaskInfo().player_join_act ~= 1 then
        return
    end
