...
function IMCreateRoomView:addGameBtnOnce(textStr, gameId)
    ...
    if GameNewTypeConfiger:isShowNewType(gameId) then
...
-- 与 TeaHouseCreateBoxRoomView 相同：ruleTextMergeGroup 为基名或 基名_序号，同基名合并、序号定段内顺序
function IMCreateRoomView:parseRuleTextMergeGroup(raw)
...
function IMCreateRoomView:getRuleTextMergeBaseKey(raw)
    local base, _ = self:parseRuleTextMergeGroup(raw)
    return base
end

function IMCreateRoomView:getRadioOptionRuleDisplayText(nodeInfo)
...
function IMCreateRoomView:isRuleTextMergeBaseActive(mergeBase, unShowRuleData)
...
function IMCreateRoomView:shouldSkipRadioForRuleTextMerge(nodeInfo, unShowRuleData)
...
function IMCreateRoomView:getRuleTextMergeRadioPartForBase(mergeBase)
    if not mergeBase or mergeBase == "" then
        return "", nil
    end
    for _, nodeTable in pairs(radioTable) do
...
function IMCreateRoomView:buildRuleTextMergedSegment(checkBoxInfo)
