...
function JoinBoxRoomModule:onJoinBoxRoomCallBack(reqJoin, type, data)
	...
	elseif type == XH.Req.TYPE.FAIL then
		...
		-- 如果是包厢&道具不足
		elseif data.errorcode == XH.RoomProtocol.ERRORCODE.NOT_ENOUGH_PROP and data.msgData and data.msgData.roommode == XH.ROOM_TYPE.BOX_ROOM then
			XH.lobby:getModule("BoxQuickRecharge"):quickBuyBoxRoomcard(false)
		else
			XH.TipTool.showTip({
				type = XH.TIP_LAYER_TYPE.OK,
				funcOK = func
...
function JoinBoxRoomModule:quickJoinRoom()
	if not self._lastJoinData or not next(self._lastJoinData) then
		return
	end
	self:reqJoinBoxRoom(self._lastJoinData.roomID, self._lastJoinData.type, self._lastJoinData.bSeer)
end

return JoinBoxRoomModule  f  