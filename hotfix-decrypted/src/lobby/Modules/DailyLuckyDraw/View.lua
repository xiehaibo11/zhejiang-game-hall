...
function DailyLuckyDrawView:getBindingInfo()
    return {
        ...
        ["_KW_PANEL_LIST"] = {varName = "_panelListParent"},
        ...
        ["_KW_BLOCK_BG"] = {varName = "_imgBlockBg1"},
        ["_KW_BLOCK_BG_2"] = {varName = "_imgBlockBg2"},
        ["_KW_BLOCK_BG_3"] = {varName = "_imgBlockBg3"},
...
function DailyLuckyDrawView:ctor(webToponData, params)
    DailyLuckyDrawView.super.ctor(self)
    self._params = params or {}
    self._enterAni = self._params.enterAni == nil or self._params.enterAni == true
    XH.UITool.gray(self._btnTimelimits, true)
    XH.UITool.gray(self._btnNoTimes, true)
    -- 初始化数据
    self:initData()
...
function DailyLuckyDrawView:initAni()
    ...
    if self._enterAni then
        XH.SpineManager:playAniWithComplete(self._nodeBgAni, path, "zzb_hdzx_mfcj", "cx", false, "loop", true, function()
            local aniInfo = {{self._btnGetPrize, "loop", "btn2"}, {self._btnTimelimits, "loop", "btn2"}, {self._btnNoTimes, "loop", "btn2"}}
        ...
        XH.SpineManager:playAni(self._nodeBgAni, path, "zzb_hdzx_mfcj", "loop", true)
        local aniInfo = {{self._btnGetPrize, "loop", "btn2"}, {self._btnTimelimits, "loop", "btn2"}, {self._btnNoTimes, "loop", "btn2"}}
        local jsonFilePath = path .. "zzb_hdzx_dlyl1.json"
        for _, v in ipairs(aniInfo) do
            v[1]:tryAddLuaComponent(cc.ext.CompSpineAction,
    ...
    local pathSG = "animation/Lobby/Base/NewGoldHall/Shop/"
    XH.SpineManager:playAniWithComplete(self._btnGetPrize:getChildByName("KW_ANI_NODE"), pathSG, "zzb_ty_ansg", "animation", true)
    XH.SpineManager:playAniWithComplete(self._btnTimelimits:getChildByName("KW_ANI_NODE"), pathSG, "zzb_ty_ansg", "animation", true)
    XH.SpineManager:playAniWithComplete(self._btnNoTimes:getChildByName("KW_ANI_NODE"), pathSG, "zzb_ty_ansg", "animation", true)

    local aniInfo = {{self._btnGetPrize, "cx", "btn2"}, {self._btnTimelimits, "cx", "btn2"}, {self._btnNoTimes, "cx", "btn2"}}
    local jsonFilePath = path .. "zzb_hdzx_dlyl1.json"
    for _, v in ipairs(aniInfo) do
        v[1]:tryAddLuaComponent(cc.ext.CompSpineAction,
                                {jsonFilePath = jsonFilePath, animationName = v[2], isLoop = false, boneName = v[3], slotName = v[3]})
    end
    self._btnHelp:runAction(cc.Sequence:create(cc.FadeOut:create(0), cc.DelayTime:create(0.5), cc.FadeIn:create(0.3)))
    if self._enterAni then
        self._panelListParent:setScale(0)
        self._panelListParent:runAction(cc.ScaleTo:create(0.3, 1))
    end
end

function DailyLuckyDrawView:setInsertMode(scene)
    self._btnClose:setVisible(false)
    self._imgBlockBg1:setVisible(false)
    self._imgBlockBg2:setVisible(false)
    self._imgBlockBg3:setVisible(false)
...
function DailyLuckyDrawView:reqGetTodayInfo()
    XH.lobby:getModule("DailyLuckyDraw"):reqTaskProtol(TaskDefine.TASK_ID.TASK_TYPE_ZHEJAING_DAILY_LUCKY_DROP,
                                                       DailyLuckyDrawConfig.GetTaskConfig.getInfo, "getPeoInfoCallBack")
end
...
function DailyLuckyDrawView:startAni()
    local moveFunc = function()
        ...
        for i = 1, PRIZE_PANELS_NUMS do
            ...
            self["_panelPrizeView" .. i]:runAction(cc.MoveTo:create(ANI_DELAY_TIME, cc.p(getPositionX, self["_panelPrizeView" .. i]:getPositionY())))
        end
        -- 移动次数增加
...
function DailyLuckyDrawView:onToponAdsShowCallBack(event)
    ...
    if (code == XH.ThirdDefine.TOPONSDK_CALLBACK_CODE.TOPON_FLAG_SDK_JLSUCCESS) then
        ...
        XH.lobby:getModule("DailyLuckyDraw"):reqTaskProtol(TaskDefine.TASK_ID.TASK_TYPE_ZHEJAING_DAILY_LUCKY_DROP,
                                                           DailyLuckyDrawConfig.GetTaskConfig.getPrize, "getPrizeCallBack")
    end
end
...
-- 新增看广告统一埋点
function DailyLuckyDrawView:throwADData(id)
    local conventionData = {}
    conventionData.block_item_id = "免费抽奖活动"
