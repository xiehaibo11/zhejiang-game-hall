...
local ShiSanZhangMiniUtil = require("lobby.Modules.CreateBoxRoom.ShiSanZhangMiniUtil")
...
local KW_IMG_RETURN_CARD = "KW_IMG_RETURN_CARD"
local KW_IMG_SERVER_FREE = "KW_IMG_SERVER_FREE"
...
function CreateBoxRoomView:getBindingInfo()
    return {
        ...
        ["_KW_BTN_FEEDBACK"] = { varName = "_KW_BTN_FEEDBACK",type = XH.UI_TYPE.BUTTON , onTouchEnded = "onTouchEventFeedBack" },

        ["_KW_BTN_BOXACT"] = { varName = "_KW_BTN_BOXACT",type = XH.UI_TYPE.BUTTON , onTouchEnded = "onTouchShowBoxAct" },
        ["_KW_BTN_BOXACT_LEFTTIME"] = { varName = "_KW_BTN_BOXACT_LEFTTIME" },
        ["_KW_BTN_CREATE_FREE_ACT"] = { varName = "_KW_BTN_CREATE_FREE_ACT", onTouchEnded = "onTouchEventCreateBoxRoomActFree"},
        ["_KW_TXT_CREATE_TIPS_ACT_COUNT"] = { varName = "_KW_TXT_CREATE_TIPS_ACT_COUNT" },
        ["_KW_TXT_CREATE_TIPS_ACT_TIME"] = { varName = "_KW_TXT_CREATE_TIPS_ACT_TIME" }
    }
end
...
function CreateBoxRoomView:getProxyEvents()
    return {
        ...
        { module = XH.lobby:getModule("UnlimitBoxAct"), eventKeyName = "FLUSH_ACT_INFO", callBack = "onUpdateUnlimitBoxActBtn" },
        -- 玩法触达：IOP 配置更新后刷新创建面板红点
        { module = XH.lobby:getModule("GamePlayReach"), eventKeyName = "EVENT_CONFIG_UPDATED", callBack = "onGamePlayReachConfigUpdated" },
    }
end
...
function CreateBoxRoomView:initUI()
    ...
    if not self._isGuideState then
        self:onUpdateUnlimitBoxActBtn()
        XH.lobby:getModule("UnlimitBoxAct"):judgeIsShowPopAct()
    end
end
...
function CreateBoxRoomView:createGameListBtn(gameID, gameName)
    ...
    if XH.lobby:getModule("Mark"):isShowMarkGame(gameID) then
        ...
        if not GameNewTypeConfiger:isShowNewType(gameID) then
            local newMark = Mark.new(gameID)
            newBtn:addChild(newMark)
            newMark:setPosition(cc.p(0 , 0))
        end
    end
    if self:isShowNewPlayType(gameID) or GameNewTypeConfiger:isShowNewType(gameID) then
        ...
        -- 推广活动默认勾选
        if GameNewTypeConfiger:isShowNewType(gameID) or (PromoteConfig[XH.areaData:getLobbyID()] ~= nil and PromoteConfig[XH.areaData:getLobbyID()].GAMEID == gameID) then
            XH.UITool.setVisible(newBtn, KW_IMG_NEW_PROMOTE, true)
        else
            XH.UITool.setVisible(newBtn, KW_IMG_NEW_PLAY_TYPE, true)
        end
    end
    if XH.lobby:getModule("LimitFreeAct"):isTargetGameID(gameID) then
        if XH.lobby:getModule("LimitFreeAct"):isServerFree() then
            XH.UITool.setVisible(newBtn, KW_IMG_SERVER_FREE, true)
        else
            newBtn:loadTextures("com_btn_label_normal2.png","com_btn_label_normal2.png","com_btn_label_select.png",ccui.TextureResType.plistType)
            XH.UITool.setVisible(newBtn, KW_IMG_RETURN_CARD, true)
        end
    end
    -- 玩法触达：有 IOP 推送时在玩法按钮右上角展示红点
    XH.lobby:getModule("GamePlayReach"):attachGameRedDot(newBtn, gameID)
    return newBtn
end

function CreateBoxRoomView:createShiSanZhangMiniListBtn()
    local tag = ShiSanZhangMiniUtil.BTN_TAG
    local newBtn = self._btnModel:clone()
    XH.UITool.setText(newBtn, KW_TEXT_MODEl, ShiSanZhangMiniUtil.getListBtnDisplayText())
    newBtn:setTag(tag)
    newBtn:setName(tostring(tag))
    XH.UITool.setVisible(newBtn, KW_IMG_ACTIVITY, false)
    XH.UITool.setVisible(newBtn, KW_IMG_LEFT_TIME, false)
    XH.UITool.setVisible(newBtn, KW_IMG_NEW_PLAY_TYPE, false)
    XH.UITool.setVisible(newBtn, KW_IMG_NEW_PROMOTE, true)
    XH.UITool.setVisible(newBtn, KW_IMG_SERVER_FREE, false)
    XH.UITool.setVisible(newBtn, KW_IMG_RETURN_CARD, false)
    return newBtn
end
...
function CreateBoxRoomView:initGameList()
    ...
    if ShiSanZhangMiniUtil.isAreaEnabled() then
        self._listViewGames:addChild(self:createShiSanZhangMiniListBtn())
    end
    local firstGameId = 0
...
--- 打开界面默认选中玩法时：有红点则弹触达窗，弹窗成功后红点消失；已触达过的玩法不再弹窗
function CreateBoxRoomView:tryGamePlayReachOnDefaultGame()
    ...
    gamePlayReachModule:onGameSelected(self._gameID, GamePlayReachConfig.LOCATION.BOX_CREATE_GAME, {
        gameId = self._gameID,
        listView = self._listViewGames,
    })
end

--- IOP 配置变更回调：刷新红点；若默认玩法尚未触达则尝试弹窗
function CreateBoxRoomView:onGamePlayReachConfigUpdated()
    XH.lobby:getModule("GamePlayReach"):refreshCreatePanelGameRedDots(self._listViewGames)
...
function CreateBoxRoomView:onReqTaskFreePropCallBack(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        ...
        self:runCountDown()
        local hasAct = XH.lobby:getModule("UnlimitBoxAct"):getRemainTime(0) > 0
        if hasAct then
            self._btnCreateBoxRoom:setVisible(false)
            self._panelCreateFree:setVisible(false)
        else
            self._btnCreateBoxRoom:setVisible(false)
...
function CreateBoxRoomView:onTouchEventChooseGameDynamic(send, eventType,isAutoTouch)
    ...
    if gameID == ShiSanZhangMiniUtil.BTN_TAG then
        if not isAutoTouch then
            ShiSanZhangMiniUtil.openMiniProgram()
        end
        return
    end
    ...
    self._mutiGameTable = MutiVersionGameConfig.getMutiGameTable(gameID, XH.areaData:getLobbyID())
    self._tmpLobbyID = nil
    self._isMutiVersionGame = false
    ...
    self._gameID = gameID
    self:initPromoteTips()
    -- 玩法触达：手动切换玩法时，有未读红点则弹窗并在成功后消红点
    if not isAutoTouch then
        XH.lobby:getModule("GamePlayReach"):onGameSelected(gameID, GamePlayReachConfig.LOCATION.BOX_CREATE_GAME, {
            gameId = gameID,
            listView = self._listViewGames,
        })
    end
    ...
    self:throwPushData(gameID)
    self:showFreeLine(gameID)    

    self:initFourToTwo(gameID)
    self:initNewPlayTypeUI(gameID)
    self:initFreeServer(gameID)
    if XH.lobby:getModule("LimitFreeAct"):isInFreeTime(self._gameID) then
        self._btnCreateBoxRoom:loadTextures("com_btn_mfcj_new.png","com_btn_mfcj_new.png","com_btn_mfcj_new.png",ccui.TextureResType.plistType)
    else
        self._btnCreateBoxRoom:loadTextures("com_btn_create_box_room.png","com_btn_create_box_room.png","com_btn_create_box_room.png",ccui.TextureResType.plistType)
    end
...
function CreateBoxRoomView:throwPushData()
    --多版本合规台炮麻将、嘉兴麻将,上抛埋点
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.lys24101803, {game_id = tostring(XH.areaData:getAreaID() .. "_" .. gameID)})
end
...
function CreateBoxRoomView:loadGameRuleDynamic(gameid,lobbyid)
    ...
    -- 玩法触达：规则加载完成后，在匹配的规则项右侧挂红点（最多3个）
    XH.lobby:getModule("GamePlayReach"):refreshRuleRedDots(self, self._gameID, radioTable, checkBoxTable)
end
    ...
    if self._radioItem and self._checkBoxItem then
        ...
        for optionIndex = 1, tmpCount do
            ...
            if lineData[optionIndex]["haveTips"] then
                ...
                if optionIndex ~= tmpCount then
                    ...
                    if optionIndex >= 3 then
                        ...
                        XH.UITool.setPositionY(optionNode, KW_PIC_TIPS_PAOPAO, optionNode:getContentSize().height + 20)
                    end
                end
            end
            if lineData[optionIndex]["limitFree"] then
                if XH.lobby:getModule("LimitFreeAct"):isTargetGameID(self._gameID) then
                    cc.SpriteFrameCache:getInstance():addSpriteFrames("cocosStudio/hall/Image/LimitFree/LimitFree.plist")
                    local limitFreeNode = self:createFlagAfterNode(optionNode, "limit_free_img_4.png")
                    if limitFreeNode then
                        limitFreeNode:setPosition(cc.p(lineData[optionIndex]["limitFree"].x, lineData[optionIndex]["limitFree"].y))
                    end
                end
            end
            ...
            if lineData[optionIndex]["dropdown"] then
                ...
                local tipNode = XH.UITool.seekNodeByName(optionNode, KW_PANEL_TIPS)
                if tipNode then
                    tipNode:setLocalZOrder(99)
                end
            end
...
function CreateBoxRoomView:updateDefaultChoseWithCache()
...
function CreateBoxRoomView:handleFirstSelectAutoChooseRadio(send, eventType, isDefault)
...
function CreateBoxRoomView:getCacheDropDownState(nodeName)
...
function CreateBoxRoomView:setCacheDropDownState(nodeName, value)
    local extraParam = self:getCacheButtonStateExtraParam(nodeName)
...
function CreateBoxRoomView:getCacheButtonState(nodeName)
    --去除xml不能出现的字符
    if self._gameID == 30153 and string.find(nodeName, "FengDingType") then
        local oldParam = {
            ["FengDingType='20';"] = "FengDingType='64';",
            ["FengDingType='40';"] = "FengDingType='64';",
            ["FengDingType='80';"] = "FengDingType='128';",
        }
        for key, value in pairs(oldParam) do
            local extraParam = self:getCacheButtonStateExtraParam(key)
            if XH.userDefault:getValue(XH.userDefault.KEY_ID.GAMERULE_CACHE_BUTTON_STATE, false, extraParam) then
                if value == nodeName then
                    -- 老的按钮是对应当前按钮的。就转化一下状态，设置为显示，并删除之前缓存的key值
                    local extraParam2 = self:getCacheButtonStateExtraParam(nodeName)
                    XH.userDefault:setValue(XH.userDefault.KEY_ID.GAMERULE_CACHE_BUTTON_STATE, true, extraParam2)
...
function CreateBoxRoomView:migrateTrustTimeCache()
...
function CreateBoxRoomView:onTouchEventCreateBoxRoom(send, eventType)
    self:_createRoomNormal()
end

function CreateBoxRoomView:_createRoomNormal(addRule)
    addRule = addRule or ""
    ...
    gameRule = gameRule .. string.format("RoomFee='%d';", tonumber(self._roomFee or 0)) .. addRule
    --多版本合规台炮麻将、嘉兴麻将
    if self._gameID == MutiVersionGameConfig.MutiVersionGameID.JiaXingMahjong 
    or self._gameID == MutiVersionGameConfig.MutiVersionGameID.TaiPaoMahjong then
        for _, v in pairs(categorieTable) do
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.cg24011201, {block_label = "包厢", game_id = tostring(XH.areaData:getAreaID() .. "_" .. self._gameID)})
...
function CreateBoxRoomView:updateShowCostInfo()
    ...
    -- 观星台房间配置的显示免费情况下，不显示回归首次免费UI
    if XH.lobby:getModule("RecallNew"):isFreeRoom() and not (XH.lobby:getModule("LimitFreeAct"):isInFreeTime(self._gameID) and XH.lobby:getModule("LimitFreeAct"):isServerFree()) then
        self._imgAreaCostHua:setVisible(true)
        self._imgAreaCostText:setVisible(true)
        self._imgLimitAct:setVisible(true)
...
function CreateBoxRoomView:showFreeLine(gameid)
    self._panelLine:setVisible(XH.lobby:getModule("CreateBoxRoom"):isLimitTimeFree() or XH.lobby:getModule("LimitFreeAct"):isInFreeTime(gameid))
end
...
function CreateBoxRoomView:initFreeServer(gameID)
    XH.UITool.setVisible(self._btnCreateBoxRoom, "KW_IMG_FREETIPBG", false)
    XH.UITool.setVisible(self._btnCreateBoxRoom, "KW_IMG_FREETIPBG2", false)
    local limitFreeModule = XH.lobby:getModule("LimitFreeAct")
    -- 未在免费时段内，显示免费Tips
    if limitFreeModule:isInFreeDateServer(gameID) and not limitFreeModule:isInFreeTime(gameID) then
        local freeStr = limitFreeModule:getShareStrByTime(nil, 1)
        local limitFree = limitFreeModule:getActConfig()
        if limitFree.showendtip then
            local datestr = os.date("%d", limitFree.endtime)
            freeStr = datestr .. "日前" .. freeStr
        end
        XH.UITool.setText(self._btnCreateBoxRoom, "KW_TEXT_FREE", freeStr .. "限时免费")
        XH.UITool.setVisible(self._btnCreateBoxRoom, "KW_IMG_FREETIPBG", true)
        local textSize = XH.UITool.getContentSize(self._btnCreateBoxRoom, "KW_TEXT_FREE")
        XH.UITool.setContentSize(self._btnCreateBoxRoom, "KW_IMG_FREETIPBG", textSize.width + 36, 79)
    end
    -- 在免费时段内，显示免费Tips
    local limitFreeModule = XH.lobby:getModule("LimitFreeAct")
    if limitFreeModule:isInFreeTime(self._gameID) and limitFreeModule:isServerFree() then
        self._promoteTips:setVisible(false)
        XH.UITool.setVisible(self._btnCreateBoxRoom, "KW_IMG_FREETIPBG2", true)
        local freeStr = limitFreeModule:getShareStrByTime(nil, 1)
        XH.UITool.setText(self._btnCreateBoxRoom, "KW_TEXT_FREE2", freeStr .. "限时免费")
    end
end
...
function CreateBoxRoomView:initPromoteTips()
    local isVisible = self:isShowPromoteTips()
    self._promoteTips:setVisible(isVisible)
    self:onUpdateUnlimitBoxActBtn()
end

function CreateBoxRoomView:isShowPromoteTips()
    return PromoteConfig[XH.areaData:getLobbyID()] ~= nil and PromoteConfig[XH.areaData:getLobbyID()].isFree and self._gameID == PromoteConfig[XH.areaData:getLobbyID()].GAMEID and XH.lobby:getModule("Promote"):isOnlineAct()
end

function CreateBoxRoomView:onTouchEventFeedBack()
    local gameNameList = XH.areaData:getAreaGameNameList()
    XH.viewManager:openView("FeedBackView", nil, gameNameList[self._gameID])
end

function CreateBoxRoomView:onTouchEventCreateBoxRoomActFree()
    XH.lobby:getModule("UnlimitBoxAct"):judgeIsFullGold(function() 
        self:_createRoomNormal("wxdj='1'")
    end)
end

function CreateBoxRoomView:updateUnBoxLeftTime()
    local leftTime = XH.lobby:getModule("UnlimitBoxAct"):getRemainTime(2)
    if leftTime <= 0 then
        cc.Director:getInstance():getScheduler():unscheduleScriptEntry(self._unboxTimeScheduler)
        self._unboxTimeScheduler = nil
        leftTime = 0
    ...
    self._KW_BTN_BOXACT_LEFTTIME:setString(string.format("%02d", hour) .. ":" .. string.format("%02d", min) .. ":" .. string.format("%02d", sec))
    self._KW_TXT_CREATE_TIPS_ACT_TIME:setString(string.format("%02d", hour) .. ":" .. string.format("%02d", min) .. ":" .. string.format("%02d", sec))
end

function CreateBoxRoomView:onUpdateUnlimitBoxActBtn()
    local isShow = XH.lobby:getModule("UnlimitBoxAct"):getRemainTime(2) > 0
    self._KW_BTN_BOXACT:setVisible(isShow)
    if isShow and self._unboxTimeScheduler == nil then
        self:updateUnBoxLeftTime()
        self._unboxTimeScheduler = cc.Director:getInstance():getScheduler():scheduleScriptFunc(function()
            if self and not tolua.isnull(self) then
                self:updateUnBoxLeftTime()
            end
        end, 1, false)
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.zjq25060901)
    end
    self._KW_BTN_CREATE_FREE_ACT:setVisible(false)
    self._btnCreateBoxRoom:setVisible(true)
    local limitFreeModule = XH.lobby:getModule("LimitFreeAct")
    if limitFreeModule and limitFreeModule:isInFreeTime(self._gameID) and limitFreeModule:isServerFree() then
        return
    end
    local isActive = XH.lobby:getModule("UnlimitBoxAct"):getRemainTime(0) > 0 and not self:isShowPromoteTips()
    self._KW_BTN_CREATE_FREE_ACT:setVisible(isActive)
    self._btnCreateBoxRoom:setVisible(not isActive)
    if isActive then
        self._KW_TXT_CREATE_TIPS_ACT_COUNT:setString(tostring(XH.lobby:getModule("UnlimitBoxAct"):getLeftCnt()))
    end
end

function CreateBoxRoomView:onTouchShowBoxAct()
    XH.viewManager:openView("UnLimitBoxActView")
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.zjq25060902)
end

function CreateBoxRoomView:onCleanup()
    if self._unboxTimeScheduler ~= nil then
        cc.Director:getInstance():getScheduler():unscheduleScriptEntry(self._unboxTimeScheduler)
        self._unboxTimeScheduler = nil
    end
end
