...
function CreateBoxRoomModule:onCreateBoxRoomCallBack(reqCreate, type, data)
	...
	if type == XH.Req.TYPE.SUCCESS then
		...
		XH.lobby:getModule("BoxGuide"):resetTryWxdjk()
	elseif type == XH.Req.TYPE.FAIL then
		...
		if data.errorcode == XH.RoomProtocol.ERRORCODE.NOT_ENOUGH_PROP then
			...
			-- 检测是否有免费创建房间
			XH.lobby:getModule("BoxGuide"):reqBoxCreateFreeCheck()
		else
...
function CreateBoxRoomModule:isActivityGame(gameid)
	...
	if XH.lobby:getModule("LimitFreeAct"):isInFreeDateServer(gameid) then
		return false
	end
...
function CreateBoxRoomModule:isOnlyShowActGame(gameid)
	...
	for _, v in pairs(showGameList) do
		if v == gameid then
			return true
		end
	end
...
-- 快速创建上次玩法
-- 是否使用无限对局的方式
function CreateBoxRoomModule:quickCreateLastGame(useWxdj)
	if not self._lastGameData or not next(self._lastGameData) then
		return
	end
	if useWxdj then
		if not string.find(self._lastGameData.gameRule, "wxdj='1';") then
			self._lastGameData.gameRule = self._lastGameData.gameRule .. "wxdj='1';"
		end
	end
	XH.TipTool.showLoading()
	self:startReq("ReqCreate", self._lastGameData.gameID, self._lastGameData.conditions, self._lastGameData.gameRule, self._lastGameData.roomMode, self._lastGameData.srsGroupID, self._lastGameData.vipPropId, 15)
end

return CreateBoxRoomModule   m'  