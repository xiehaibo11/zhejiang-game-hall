...
local BuyCodeUI = NG.GAME.gameClass("BuyCodeUI", NG.ViewBase)
...
local windImg = {
    [NG.GAME.GameDefine.MAH_VALUE.FENG_DONG] = "ma_icon_dong.png",
    [NG.GAME.GameDefine.MAH_VALUE.FENG_NAN] = "ma_icon_nan.png",
    [NG.GAME.GameDefine.MAH_VALUE.FENG_XI] = "ma_icon_xi.png",
    [NG.GAME.GameDefine.MAH_VALUE.FENG_BEI] = "ma_icon_bei.png",
}
...
function BuyCodeUI:getBindingInfo()
    return {
        ...
        ["_KW_BTN_EXIT"] = {varName = "_exitBtn", type = NG.UI_TYPE.BUTTON, onTouchEnded = "onExitBtnClicked"},
...
function BuyCodeUI:createMahLayer()
    ...
    for i = 1, 10 do
        ...
        self._mahArray[i]:setMahValue(NG.GAME.GameDefine.MAH_VALUE.BACK)
...
function BuyCodeUI:arrageMaMahPosition()
    ...
    if pos then
        for index = 1 , #self._maPaiArray do
            ...
            local moveAction = cc.MoveTo:create(0.1,cc.p(pos[index]* 133,0))
...
function BuyCodeUI:showBuyTextLayer()
    for i = 1,self._huCount do
        ...
        if windTemp then
            if windTemp == NG.GAME.GameDefine.MAH_VALUE.FENG_DONG then
                self["windPosition"..i]:loadTexture("ma_dongfeng.png", ccui.TextureResType.plistType)
            elseif windTemp == NG.GAME.GameDefine.MAH_VALUE.FENG_NAN then
                self["windPosition"..i]:loadTexture("ma_nanfeng.png", ccui.TextureResType.plistType)
            elseif windTemp == NG.GAME.GameDefine.MAH_VALUE.FENG_XI then
                self["windPosition"..i]:loadTexture("ma_xifeng.png", ccui.TextureResType.plistType)
            elseif windTemp == NG.GAME.GameDefine.MAH_VALUE.FENG_BEI then
    ...
    for i = 1,#self._maPaiArray do
        for j = 1,#self._huDirection do
            if self._betMahDirection[i] == self._huDirection[j] then
                ...
                NG.UITool.playJsonAnimationByName(self._maPaiArray[i],"maima_ani","res/animation/Mahjong/Base/maima_ani/maima_ani.ExportJson",1, "Animation1", cc.p(70,96))
