...
local UIMahComb3D = NG.GAME.gameClass("UIMahComb3D", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.ViewBase.UIMahCombBase")
...
function UIMahComb3D:_addIconDirection(lieMahs)
    local mah = nil
    if self:getCombFlag() == NG.GAME.GameDefine.COMB_FLAG.MKONG or self:getCombFlag() == NG.GAME.GameDefine.COMB_FLAG.TKONG then
        mah = lieMahs[4]
    elseif self:getCombFlag() ~= NG.GAME.GameDefine.COMB_FLAG.CKONG then
        mah = lieMahs[2]
    end
