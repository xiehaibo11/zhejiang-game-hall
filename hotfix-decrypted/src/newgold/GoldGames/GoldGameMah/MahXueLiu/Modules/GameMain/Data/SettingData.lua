local SettingData = NG.GAME.gameClass("CardLogicData", "newgold.GoldGames.GoldGameBase.Data.SettingData")
local MahSettingConfig = NG.GAME.gameRequire("Modules.GameMain.MahLayer.Config.MahSettingConfig")
...
function SettingData:ctor()
    ...
    local defaultCfg = { 100, 1, 100, 0, 2, 1, 1, 1, 1, 0, 1, 1, 2 }
    for i=1,#defaultCfg do
        self._curDefault[i] = defaultCfg[i]
    end
end
...
function SettingData:getConfigByKey(configKey, indexKey)
    ...
    local conf = MahSettingConfig[XH.areaData:getAreaID()]
...
---获取出牌显示大牌
function SettingData:getOutStyle()
    local style = self._settings[SaveKeyTab.OutStyle]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.OutStyle)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.OutStyle))
    end
    return style
end
...
---获取手牌高度
function SettingData:getCardHeight()
    local style = self._settings[SaveKeyTab.CardHeight]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.CardHeight)
        style = userDefault:getFloatForKey(saveKey, self:getNormalData(SaveKeyTab.CardHeight))
    end
    return style
end

---获取手牌厚度
function SettingData:getCardWidth()
    local style = self._settings[SaveKeyTab.CardWidth]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.CardWidth)
        style = userDefault:getFloatForKey(saveKey, self:getNormalData(SaveKeyTab.CardWidth))
    end
    return style
end

---获取手牌牌花大小
function SettingData:getCardWordSize()
    local style = self._settings[SaveKeyTab.CardWordSize]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.CardWordSize)
        style = userDefault:getFloatForKey(saveKey, self:getNormalData(SaveKeyTab.CardWordSize))
    end
    return style
end

---获取手牌牌花(字体)
function SettingData:getWordType()
    local style = self._settings[SaveKeyTab.WordType]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.WordType)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.WordType))
    end
    return style
end

---获取手牌牌背(颜色)
function SettingData:getBackType()
    local style = self._settings[SaveKeyTab.BackType]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.BackType)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.BackType))
    end
    return style
end

---获取手牌牌身(圆方)
function SettingData:getBodyType()
    local style = self._settings[SaveKeyTab.BodyType]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.BodyType)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.BodyType))
    end
    return style
end

---获取手牌牌面(亮暗)
function SettingData:getFaceType()
    local style = self._settings[SaveKeyTab.FaceType]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.FaceType)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.FaceType))
    end
    return style
end

--获得牌面亮度设置
function SettingData:getMahLightStyle()
    local style = self._settings[SaveKeyTab.MahLightStyle]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.MahLightStyle)
        style = userDefault:getIntegerForKey(saveKey, self._curDefault[configTab.MahLightStyle] or 0) --todo
    end
    return style
end

--获得桌布样式
function SettingData:getMahTableStyle()
    local style = self._settings[SaveKeyTab.TABLE_STYLE]

    if style == nil then
        local saveKey = self:getFullSaveKey(SaveKeyTab.TABLE_STYLE)
        style = userDefault:getIntegerForKey(saveKey, self:getNormalData(SaveKeyTab.TABLE_STYLE))
    end
    return style
end
...
function SettingData:getIsMahlayer3D()
    ...
    if style == nil then
        ...
        style = userDefault:getBoolForKey(saveKey, self._curDefault[SaveKeyTab.MahIs3D] == 0 and false or true)
    end
    return style
end
...
function SettingData:setMahlayerDimension(cusData)
    ...
    if style and not NG.SysTool.support3DMahLayer() then
        NG.TipTool.showTip({
            type = NG.TIP_LAYER_TYPE.OK,
            funcOK = function()
                NG.SysTool.downloadNewVersionApp()
            end,
        }, "当前游戏版本不支持3D场景,\n是否进行更新？")
        return
    end
...
function SettingData:setMahDialectVoiceType(type)
    if type ~= nil then
        self._settings[SaveKeyTab.MahDialectType .. NG.GAME.roomTableData:getGameID()] = type
        local saveKey = self:getFullSaveKey(SaveKeyTab.MahDialectType, NG.GAME.roomTableData:getGameID())
...
function SettingData:getMahDialectVoiceType()
    local type = self._settings[SaveKeyTab.MahDialectType .. NG.GAME.roomTableData:getGameID()]
...
function SettingData:getIsClearModel()
    ...
    if style == nil then
        ...
        style = cc.UserDefault:getInstance():getBoolForKey(saveKey, self._curDefault[SaveKeyTab.ClearModel] == 1 and true or false)
    end
    return style
end
