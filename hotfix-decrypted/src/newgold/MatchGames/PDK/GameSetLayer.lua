...
GameSetLayer.RESOURCE_BINDING = {
    ...
    ["Panel_1/bg/Button_small_card"] = {
        varname="smallCardBtn",
        events={
            {event="touch",method="on_checkbox_touch"}
        }
    },

    ["Panel_1/bg/Button_big_card"] = {
        varname="bigCardBtn",
        events={
            {event="touch",method="on_checkbox_touch"}
        }
...
function GameSetLayer:refreshCheckBox()
    ...
    local isSamll = g_UserDataTable.gameCardCfgIndex == 1
    self.smallCardBtn:setSelected(isSamll)
    self.bigCardBtn:setSelected( not isSamll)
...
function GameSetLayer:on_checkbox_touch(event)
    ...
    if event.name == "ended" then
        ...
        elseif sender == self.smallCardBtn then 
            g_UserDataTable.gameCardCfgIndex = 1
            GlobalEventManager:onEvent("cardFrontSizeChanged")
        elseif sender == self.bigCardBtn then 
            g_UserDataTable.gameCardCfgIndex = 2
            GlobalEventManager:onEvent("cardFrontSizeChanged")
