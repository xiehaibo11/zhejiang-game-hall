...
function GameLayer:getCSBPath()
    return "res/cocosStudio/DoubleKou/GameLayer/CSB/NewGameLayer.csb"
end
...
function GameLayer:initNewUIPos()
    if not CF.roomData:isNewUI() then
        return
    end
    self._gamePlayBtns:setPositionY(self._gamePlayBtnsNewPos:getPositionY())
    self._chaoDiBtn:setPositionY(self._chaoDiBtnNewPos:getPositionY())
    self._clockPanel2:setPositionY(self._gamePlayBtnsNewPos:getPositionY())
end
...
function GameLayer:getProxyEvents()
    local eventTable = {
        ...
        {module = CF.game:getModule("GameLayer"), eventKeyName = "EVENT_GAME_NO_OUT_GONG", callBack = "onGameNoOutGong"},
        ...
        {module = CF.game:getModule("GameLayer")._gameData, eventKeyName = "EVENT_PLAYER_SEAT_CHANGED", callBack = "onPlayerSeatChanged"},
        {module = CF.game:getModule("WinLost"), eventKeyName = "EVENT_WINLOST_SHOWWATCHHAND", callBack = "onShowWatchHandCard"},
        {module = CF.game:getModule("GameLayer"), eventKeyName = "EVENT_SHOW_SPECIAL_BOMB_GOND", callBack = "onGameShowSpecialBomb"},
...
function GameLayer:initCardLayer()
    ...
    --     CF.game:getModule("WinLost"):onClockNotify()
    -- end, 2, false)
end
...
function GameLayer:startOutCardClock(localSeat, nTime, firstTime)
    ...
    if nTime >= 0 then
        ...
        local callback =
            ...
            nTime = nTime - 1
                outCardTimeText:setString(string.format("%02d", nTime))
            if nTime < 5 and CF.soundManager then
...
function GameLayer:onPlayerSeatChanged(event)
    local msg = event.msg
    local maxPlayer = CF.roomData:getMaxPlayer()
    local nowseat = CF.roomData:seatToLocal(msg.nowseat % maxPlayer)
    local otherseat = CF.roomData:seatToLocal(msg.otherseat % maxPlayer)
    local nowStatus = self["_chaoDiBtnPlayBack" .. nowseat]:isVisible()
    self["_chaoDiBtnPlayBack" .. nowseat]:setVisible(self["_chaoDiBtnPlayBack" .. otherseat]:isVisible())
    self["_chaoDiBtnPlayBack" .. otherseat]:setVisible(nowStatus)
end
...
function GameLayer:onGameNoOutGong(event)
    local lobbyID = CF.areaData:getLobbyID()
    if lobbyID ~= CF.LOBBY_ID.WENZHOU then
        return
    end
    local msg = event.msg
    local tNoOutGong = msg.tNoOutGong
    for seat = 0, CF.roomData:getMaxPlayer() - 1 do
        local localSeat = CF.roomData:seatToLocal(seat)
        local playerTipPanel = self["_playerTipPanel" .. localSeat]
        if tNoOutGong[seat] > 0 and playerTipPanel then
            local boomLostSprite = cc.Sprite:create()
            if boomLostSprite then
                boomLostSprite:setSpriteFrame("doublekou_boom_lost_txt.png")

                local myTextBMFont = ccui.TextBMFont:create()
                myTextBMFont:setFntFile("res/cocosStudio/Common/Font/boom_moneylost_nub-export.fnt")
                myTextBMFont:setAnchorPoint(cc.p(1, 0.5))
                myTextBMFont:setPositionX(-5)
                myTextBMFont:setString("+" .. tNoOutGong[seat])
                myTextBMFont:setCascadeOpacityEnabled(true)
                boomLostSprite:addChild(myTextBMFont)
                playerTipPanel:addChild(boomLostSprite)

                local removeAction =
                    cc.CallFunc:create(
                    function()
                        boomLostSprite:removeFromParent()
                    end
                )
                boomLostSprite:runAction(cc.Sequence:create(cc.DelayTime:create(0.2), cc.FadeOut:create(0.8), removeAction))
                --setCascadeOpacityEnabled方法未生效，myTextBMFont不会随boomLostSprite渐隐，故此处临时特殊处理
                myTextBMFont:runAction(cc.Sequence:create(cc.DelayTime:create(0.4), cc.FadeOut:create(0.8)))
            end
        end
    end
end
...
function GameLayer:onGameShowSpecialBomb(event)
    if not event.msg then
        return
    end
    self._cardLayer:onGameShowSpecialBomb(event.msg)
end

function GameLayer:onGameWinLostNew()
    local winLostData = self._gameData:getWinLostData()
    if not self._winLostLayer or tolua.isnull(self._winLostLayer) then
        local GameWinLostLayer = CF.gameRequire("Modules.WinLost.ViewNew")
        self._winLostLayer = GameWinLostLayer.new()
        self._winLostLayer:showSelf()
    end
    ...
    CF.SysTool.performWithDelayGlobal(function()
        if not self or tolua.isnull(self) or not self._winLostLayer or tolua.isnull(self._winLostLayer) then
            return
        end
        self._winLostLayer:setVisible(true)
        self._winLostLayer:playAnimation()
        CF.SysTool.performWithDelayGlobal(function()
            if not self._winLostLayer or tolua.isnull(self._winLostLayer) then
                return
            end
            if CF.roomData:isGoldRoom() then
                CF.game:getModule("ChangeLevel"):show()
                self._winLostLayer:updateH5Game()
                self._winLostLayer:showShareBtn()
                self._winLostLayer:startSchedule()
            end
        end, 2)
        CF.soundManager:playSoundEnd()
