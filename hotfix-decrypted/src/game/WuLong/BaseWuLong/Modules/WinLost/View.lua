...
local KW_PATH_RESOUCE_FONT = "res/cocosStudio/WuLong/Font"
--字体
local KW_LOST_TEXT_FNT = "Txt_shu-export.fnt"
local KW_WIN_TEXT_FNT = "Txt_yin-export.fnt"

function GameWinLostLayer:getCSBPath()
    return "res/cocosStudio/WuLong/GameLayer/CSB/GameWinLost.csb"
end

function GameWinLostLayer:getBindingInfo()
    return {
        ["_KW_BG_"] = { varName = "_KW_BG_", beginIndex = 1, endIndex = 4 },
        ["_KW_TEXT_PLAYER_MCF_"] = { varName = "_KW_TEXT_PLAYER_MCF_", beginIndex = 1, endIndex = 4 }, -- 名次分
        ["_KW_TEXT_PLAYER_JS_"] = { varName = "_KW_TEXT_PLAYER_JS_", beginIndex = 1, endIndex = 4 }, -- 奖数
        ["_KW_TEXT_PLAYER_JF_"] = { varName = "_KW_TEXT_PLAYER_JF_", beginIndex = 1, endIndex = 4 }, -- 奖分
        ["_KW_TEXT_PLAYER_JIANF_"] = { varName = "_KW_TEXT_PLAYER_JIANF_", beginIndex = 1, endIndex = 4 }, -- 捡分
        ...
        ["_KW_IMG_TAG_"] = { varName = "_KW_IMG_TAG_", beginIndex = 1, endIndex = 4 },
        ["_KW_IMG_ZF_TAG_"] = { varName = "_KW_IMG_ZF_TAG_", beginIndex = 1, endIndex = 4 },
        ["_KW_TEXT_NICKNAME_"] = { varName = "_playerNickName", beginIndex = 1, endIndex = 4 },
        ["_KW_TEXT_NUMID_"] = { varName = "_KW_TEXT_NUMID_", beginIndex = 1, endIndex = 4 },
        ...
        ["_KW_GAMERULE"] = { varName = "_KW_GAMERULE" },
    }
end
...
function GameWinLostLayer:setWinLostData(winLostData)
    self._KW_GAMERULE:setString(CF.roomData:getGameRule())
    self._winLostData = winLostData

    self._sWinOrder = winLostData:getFinishOrder()
    self._iScore = winLostData:getScore()
    self._iBoomCount = winLostData:getBoomCount()
    self._iBombRewardScore = winLostData:getBombRewardScore()
    self._iPickupScore = winLostData:getPickupScore()
    self._tScore = winLostData:getFinalScore()
    self._nChaoDiSeats = winLostData:getChaoDiSeats()

    self:updateContinueBtnState()
    self:updatePlayerScore()

    for i = 1, 4 do
        self["_KW_IMG_TAG_" .. i]:setVisible(#self._nChaoDiSeats <= 0)
        local isFind = false
        if #self._nChaoDiSeats > 0 then
            for j = 1, #self._nChaoDiSeats do
                if self._nChaoDiSeats[j] == i - 1 then
                    isFind = true
                    break
                end
            end
        end
        self["_KW_IMG_ZF_TAG_" .. i]:setVisible(isFind)
    end

    local isGold = CF.roomData:isGoldRoom()

    for i = 1, #self._sWinOrder do
        local playerData = CF.roomData:getPlayerDataBySeatId(i - 1)
        if playerData then
            self:onUpdatPlayerUI(self._sWinOrder[i], playerData)
        end
    end
...
function GameWinLostLayer:onUpdatPlayerUI(index, playerData)
    ...
    local playerNumID = self["_KW_TEXT_NUMID_" .. index]
    ...
    playerNumID:setString("ID:" .. playerData:getNumberID())

    self["_KW_BG_" .. index]:setVisible(playerData:getNumberID() == XH.playerData:getNumberID())
...
function GameWinLostLayer:updatePlayerScore()
    for i = 1, #self._sWinOrder do
        local seat = i - 1
        local idx = self._sWinOrder[i]
        local playerScore = self._tScore[i] or 0
        self["_KW_TEXT_PLAYER_MCF_" .. idx]:setString(self._iScore[i]) -- 名次分
        self["_KW_TEXT_PLAYER_JS_" .. idx]:setString(self._iBoomCount[i]) -- 奖数
        self["_KW_TEXT_PLAYER_JF_" .. idx]:setString(self._iBombRewardScore[i]) -- 奖分
        self["_KW_TEXT_PLAYER_JIANF_" .. idx]:setString(self._iPickupScore[i]) -- 捡分
        self["_playerScoreText" .. idx]:setString(playerScore) -- 总分
        if playerScore > 0 then
            self["_playerScoreText" .. idx]:setString("+" .. playerScore)
            self["_playerScoreText" .. idx]:setFntFile(KW_PATH_RESOUCE_FONT .. "/" .. KW_WIN_TEXT_FNT)
            self["_playerScoreText" .. idx]:setScale(0.8)
        else
            self["_playerScoreText" .. idx]:setString(playerScore)
            self["_playerScoreText" .. idx]:setFntFile(KW_PATH_RESOUCE_FONT .. "/" .. KW_LOST_TEXT_FNT)
            self["_playerScoreText" .. idx]:setScale(1)
        end
