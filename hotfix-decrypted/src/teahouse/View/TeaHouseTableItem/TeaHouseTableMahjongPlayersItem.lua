...
---@class TeaHouseTableMahjongPlayersItem : TeaHouseTableMahjongItem
local TeaHouseTableMahjongPlayersItem = class("TeaHouseTableMahjongPlayersItem", TeaHouseTableMahjongItem)
...
function TeaHouseTableMahjongPlayersItem:ctor(tableInfo)
    TeaHouseTableMahjongPlayersItem.super.ctor(self)
    ...
    self:setAutoChange(string.match(tableInfo.data or "", "AutoChange='1';"))
    self:setQueMen(tableInfo.gameid, tableInfo.data or "")
    if self.updatePromoteIcon then
        if TeaHouse.PromoteConfig[XH.areaData:getLobbyID()] ~= nil and TeaHouse.PromoteConfig[XH.areaData:getLobbyID()].GAMEID == tableInfo.gameid and string.find(tableInfo.data,TeaHouse.PromoteConfig[XH.areaData:getLobbyID()].RULE) then
...
function TeaHouseTableMahjongPlayersItem:getTableInfo()
    return self._tableInfo
end

function TeaHouseTableMahjongPlayersItem:getPlayerTableInfo(playerInfo)
...
function TeaHouseTableMahjongPlayersItem:addPlayerNumID(playerInfo)
    local playerCount, playerSeat = self:getPlayerTableInfo(playerInfo)
    local playerMapping = TeaHouseTableMahjongItem.PlayersMapping[playerCount]
...
function TeaHouseTableMahjongPlayersItem:initPlayers()
...
function TeaHouseTableMahjongPlayersItem:addPlayerInfo(playerInfo)
    local playerCount, playerSeat = self:getPlayerTableInfo(playerInfo)
    local playerMapping = TeaHouseTableMahjongItem.PlayersMapping[playerCount]
    if playerMapping == nil then
        return
...
function TeaHouseTableMahjongPlayersItem:removePlayerInfo(playerInfo)
...
function TeaHouseTableMahjongPlayersItem:updateState()
    -- 在房间中，不能观战
    local position = TeaHouse.BridgeData.getPlayerPosition()
    if position.gameID == 0 then
        local isCanWatch = TeaHouse.manager.teahouseMain:isSupportWatch(self._tableInfo.gameid) and self._tableInfo.state == TeaHouse.TeaHouseProtocol.TableInfo.FLAG.ST_GAME
        if not TeaHouse.manager.teahouseMain:isVipExpire() then
            self._KW_BTN_WATCH:setVisible(isCanWatch)
        else
            self._KW_BTN_WATCH_VIP:setVisible(isCanWatch)
        end
    else
...
function TeaHouseTableMahjongPlayersItem:onTableClicked(send, eventType)
    ...
    if self._tableInfo.gameid == 30371 and not string.find(self._tableInfo.data, "LaGangHuType") then
        zhRule = "拉杠胡固定3倍," .. zhRule
    end
    if #zhRule > 0 then
        zhRule = zhRule .. "\n"
    end
    zhRule = zhRule .. "("
    local bFirst = true
    local playCountConf = TeaHouse.SysTool.getPlayCountConfByGameID(self._tableInfo.gameid, self._tableInfo.data,{self._tableInfo.size})
    if playCountConf then
        for _, v in pairs(playCountConf.listData or {}) do
            if v.Value == self._tableInfo.count then
                if self._tableInfo.gameid == 30030 then -- 宁波慈溪麻将特判
                    if string.find(self._tableInfo.data, "isspecial='1';") and not string.find(v.TextShow, "冲击") then
                        zhRule = zhRule .. (bFirst and "" or ",") .. v.TextShow
                        bFirst = false
                    elseif not string.find(self._tableInfo.data, "isspecial='1';") and string.find(v.TextShow, "冲击") then
                        zhRule = zhRule .. (bFirst and "" or ",") .. v.TextShow
                        bFirst = false
                    elseif string.find(v.TextShow, "一节") then
                        zhRule = zhRule .. (bFirst and "" or ",") .. v.TextShow
                        bFirst = false
                    end
                else
...
function TeaHouseTableMahjongPlayersItem:onSeatClicked(send, eventType, seatID)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseTableMahjongPlayersItem:onCloseClicked(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseTableMahjongPlayersItem:onTableSeeClicked(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseTableMahjongPlayersItem:onTableVipSeeClicked()
    local data = {
        block_label = '比赛场牌桌',
...
return TeaHouseTableMahjongPlayersItem   �A  