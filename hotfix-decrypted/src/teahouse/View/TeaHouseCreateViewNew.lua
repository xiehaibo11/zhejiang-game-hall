---@class TeaHouseCreateViewNew : View
local TeaHouseCreateViewNew = class("TeaHouseCreateViewNew", TeaHouse.View)
local MODE_PAYTYPE_NAME = {
    QUANZHUWIN = "扣领队划入比赛场的卡",
    YUFU ="扣玩家在比赛场的可用房卡",
    LOBBY ="扣玩家自己的大厅卡",
    LIUTONG = "流通模式",
    SHAOXINGGUANZHU = "大厅卡领队模式",
    FANGFEI = "其他模式",
}
local KW_TEXT_TIP  = "KW_TEXT_TIP"
local KW_BTN_STATE = "KW_BTN_STATE"
local KW_RECOMMEND_IMG = "KW_RECOMMEND_IMG"
TeaHouseCreateViewNew.RAW_RES_BINDING_CSB = {
    csb = "res/cocosStudio/TeaHouse/CSB/TeaHouseSetupView_New.csb",
    binding = {
        ...
        ["_KW_CHECKBOX_COSTTYPE_LAST"] = { tag = "_KW_CHECKBOX_COSTTYPE_LAST", name = "_checkBoxCostTypeLast", class = "checkBox" },
        ...
        --引导
        ["_KW_GUIDE_HIDE_PANEL"] = { tag = "_KW_GUIDE_HIDE_PANEL", name = "_guideMaskPanel", class = "panel" },
        ["_KW_GUIDE_NODE"] = { tag = "_KW_GUIDE_NODE", name = "_guideTipPoint", class = "node" },
        ["_KW_GUIDE_PANEL"] = { tag = "_KW_GUIDE_PANEL", name = "_panelGuide", class = "panel" },
        ["_KW_LISTVIEW_PANEL"] = {tag = "_KW_LISTVIEW_PANEL",name = "_listPanel",class = "scrollview"},
        
        ["_KW_TEXT_MODULE_TYPE"] = { tag = "_KW_TEXT_MODULE_TYPE", name = "_textModuleType", class = "text" },
    }
}

TeaHouseCreateViewNew.KW_TEAHOUSE_MODE = {
...
TeaHouseCreateViewNew.KW_MODE_TEXT_STR = {
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU] = "领队模式",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU] = "预付模式",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI] = "其他模式",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG] = "比赛场流通",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY] = "扣玩家卡模式", --扣玩家大厅卡(包含购买卡和赠送卡)
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU] = "大厅卡领队模式" 
}

--paytype为0时，不同模式下叫法不同
TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR = {
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU] = "冠军消耗",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU] = "冠军消耗",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG] = "",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI] = "冠军消耗",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY] = "冠军消耗",
    [TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU] = "冠军消耗"
}
...
TeaHouseCreateViewNew.KW_DEFAULT_EVERYDAY_COSTLIMIT = 888888
TeaHouseCreateViewNew.KW_DEFAULT_RECHARGE = 0

TeaHouseCreateViewNew.KW_SELECT_SETUP_NODE_TAG = {
    BASE = 1,
    PLAYMODE = 2
}

function TeaHouseCreateViewNew:ctor(data)
    self._itemIndex = 1
    self._modeItems = {}
    self:parseParams(data)
    self:initData()
    TeaHouseCreateViewNew.super.ctor(self)
    self:initView()
    self:reqData()
    --创圈活动判断，是否需要显示引导
    self:initGuide()
...
function TeaHouseCreateViewNew:initData()
...
function TeaHouseCreateViewNew:reqData()
    TeaHouse.manager.teahouseSetup:reqPlayerTeaHouseCost()
    --没有转型的地区需要请求创建比赛场人员的风信后台卡数量
    local useBuyedProp = self._info.useBuyedProp
    if useBuyedProp then--别的区都是true，只有温茶会下发false
        if (TeaHouse.BridgeData.isRemodelArea() or TeaHouse.BridgeData.isNotTeaAgentMode()) then
            self._textTotalRoomCard:setString("个人账户可用房卡：" .. TeaHouse.BridgeData.getRoomCard())
        else
...
function TeaHouseCreateViewNew:initGuide()
    ...
    if not XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_RECORD_GUIDE_ENTRY) and guideLayerState ~= 2
    and teahouseData:getOwnerTeahouseCount() == 1 then--当前玩家作为领队的比赛场数为0，并且没有领过奖
        self:createFirstGuide()
    end
end

function TeaHouseCreateViewNew:initView()
...
function TeaHouseCreateViewNew:onEnter()
    self:initEvents()
end

function TeaHouseCreateViewNew:initEvents()
...
--比赛场模式选择节点初始化
function TeaHouseCreateViewNew:initNodeModeSelect()
    ...
    if SetupConfig.IsOpenModeGuanZhu then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU
        end
    ...
    if SetupConfig.IsOpenModeYuFu then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU
        end
    ...
    if SetupConfig.IsOpenModeLiuTong then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG
        end
    ...
    if SetupConfig.IsOpenModeFangFei then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI
        end
    ...
    if SetupConfig.IsOpenModeLobby then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY
        end
    ...
    if SetupConfig.IsOpenModeShaoXingQuanZhu then
        if not firstMode then
            firstMode = TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU
        end
...
function TeaHouseCreateViewNew:initItem()
    local teahouseData = TeaHouse.manager.teahouseData
    local SetupConfig = TeaHouse.manager.configManager.SetupConfig
    SetupConfig = self:filterModeRemodelTeaHouse(SetupConfig)
    if SetupConfig.IsOpenModeGuanZhu then
        self:addItem(false,MODE_PAYTYPE_NAME.QUANZHUWIN,SetupConfig.IsOpenGuanZhuRecomend)
    end
    if SetupConfig.IsOpenModeYuFu then
        self:addItem(false,MODE_PAYTYPE_NAME.YUFU,SetupConfig.IsOpenYuFuRecomend)
    end
    if SetupConfig.IsOpenModeLobby then
        self:addItem(false,MODE_PAYTYPE_NAME.LOBBY,SetupConfig.IsOpenLobbyRecomend)
    end

    if SetupConfig.IsOpenModeLiuTong then
        self:addItem(false,MODE_PAYTYPE_NAME.LIUTONG,SetupConfig.IsOpenLiuTongRecomend)
    end

    if SetupConfig.IsOpenModeFangFei then
        if not TeaHouse.LOBBY_ID.NINGBO == TeaHouse.BridgeData.getLobbyID() and not TeaHouse.LOBBY_ID.YUYAO == TeaHouse.BridgeData.getLobbyID() then
            self:addItem(false,MODE_PAYTYPE_NAME.FANGFEI,SetupConfig.IsOpenFangFeiRecomend)
        end
    end

    if SetupConfig.IsOpenModeShaoXingQuanZhu then
        self:addItem(false,MODE_PAYTYPE_NAME.SHAOXINGGUANZHU,SetupConfig.IsOpenShaoXingGuanZhuRecomend)
    end
end

function TeaHouseCreateViewNew:addItem(itemState, itemName,isShowRecomend)
    ...
    --是否已经添加过
    for k, v in pairs(self._modeItems) do
    ...
    local recomedImg = ccui.Helper:seekWidgetByName(item, KW_RECOMMEND_IMG)
    if recomedImg then
        recomedImg:setVisible(isShowRecomend)
    end
end

function TeaHouseCreateViewNew:updateItemState(itemName)
    if TeaHouse.manager.teahouseMain:isShowPhoneBindTip("绑定手机后可以升级至高级比赛场，解锁该功能，是否绑定？", self._nTeaNumber) then
    ...
    for _,v in ipairs(self._modeItems) do
        if v.BtnName ~= itemName then
            ...
            if itemName == MODE_PAYTYPE_NAME.QUANZHUWIN then
                self:onBtnEventModeGuanzhu()
            elseif itemName == MODE_PAYTYPE_NAME.YUFU then
                self:onBtnEventModeYufu()
            elseif itemName == MODE_PAYTYPE_NAME.LOBBY then
                self:onBtnEventModeLobby()
            elseif itemName == MODE_PAYTYPE_NAME.LIUTONG then
                self:onBtnEventModeLiutong()
            elseif itemName == MODE_PAYTYPE_NAME.SHAOXINGGUANZHU then
                self:onBtnEventModeShaoXingQuanzhu()
            elseif itemName == MODE_PAYTYPE_NAME.FANGFEI then
                self:onBtnEventModeFangFei()
            end
        end
    end
    ...
    self:savePayTypeData(self._selectMode, payTypeTable, false)
end

--比赛场备注节点初始化
function TeaHouseCreateViewNew:initNodeTeaHouseNote()
...
--每日消耗上限节点初始化
function TeaHouseCreateViewNew:initNodeEveryDayCostLimit()
    ...
    self._textEveryDayCostLimit:setString(tostring(TeaHouseCreateViewNew.KW_DEFAULT_EVERYDAY_COSTLIMIT))
...
--比赛场充值节点初始化
function TeaHouseCreateViewNew:initNodeRecharge()
    ...
    self._textRecharge:setText(tostring(TeaHouseCreateViewNew.KW_DEFAULT_RECHARGE))
...
--陌生人可见节点初始化
function TeaHouseCreateViewNew:initNodeStrangerCanSee()
    local SetupConfig = TeaHouse.manager.configManager.SetupConfig
...
--自动补房卡节点初始化
function TeaHouseCreateViewNew:initNodeAutoAddRoomCard()
...
function TeaHouseCreateViewNew:initAutoInput()
    local inputNodes = {}
...
function TeaHouseCreateViewNew:initNodeOther()
    self:initNodeStrangerCanSee()       --陌生人可见
    self:initNodeAutoAddRoomCard()      --自动补房卡
end

function TeaHouseCreateViewNew:initBtnDissolve()
    self._btnDissolve:setVisible(false)
end

function TeaHouseCreateViewNew:initNodeBase()
    self._btnSelectBase:setVisible(true)
end

function TeaHouseCreateViewNew:initNodePlayMode()
    self._btnSelectPlayMode:setVisible(false)
    self._nodePlayMode:setVisible(false)
end

function TeaHouseCreateViewNew:initCostTypePanel()

    self._checkBoxCostTypeAll:setVisible(false)
    self._checkBoxCostTypeLast:setVisible(false)
    ...
    self._checkBoxCostTypeLast:addEventListener(handler(self, self.onCheckBoxEventCostType))
...
function TeaHouseCreateViewNew:setEveryDayCostLimitEnabled(enable)
    if enable then
        ...
        self._textFieldEveryDayCostLimit:setText(TeaHouseCreateViewNew.KW_DEFAULT_EVERYDAY_COSTLIMIT)
        self._textEveryDayCostLimit:setString(TeaHouseCreateViewNew.KW_DEFAULT_EVERYDAY_COSTLIMIT)
        self._textEveryDayCostLimit:setColor(KW_TEXTFIELD_FONT_COLOR)
    else
...
--设置充值输入框是否可以输入
function TeaHouseCreateViewNew:setRechargeEnabled(enable)
    if enable then
...
--设置输入框文本据中显示
function TeaHouseCreateViewNew:setTextFieldAlignmentCenter(textField)
...
--关闭
function TeaHouseCreateViewNew:onBtnEventClose(send, eventType)
...
function TeaHouseCreateViewNew:onBtnEventBase(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:onBtnEventPlayMode(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
    local GamePlayReachConfig = require("lobby.Modules.GamePlayReach.Config")
    -- 玩法触达：点击固定玩法页签，消功能红点并弹标准窗
    local teahouseData = TeaHouse.manager.teahouseData
    XH.lobby:getModule("GamePlayReach"):onFeatureEntry(GamePlayReachConfig.LOCATION.TEA_SETUP_PLAYMODE, {
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.NEW_PLAY_TYPE_SET_MODE_PUSH)
end

--解散比赛场
function TeaHouseCreateViewNew:onBtnEventDissolve(send, eventType)
    
end

function TeaHouseCreateViewNew:onCheckBoxEventCostType(send, eventType)
    local SetupConfig = TeaHouse.manager.configManager.SetupConfig
    if not SetupConfig.IsOpenCostTypeMultiSelect then
        self._checkBoxCostTypeLast:setSelected(self._checkBoxCostTypeLast == send)
...
function TeaHouseCreateViewNew:throwDate()
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU then
    ...
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU then
        if self._checkBoxCostTypeAll:isSelected() == true  then 
    ...
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY then
        if self._checkBoxCostTypeAll:isSelected() == true  then 
...
function TeaHouseCreateViewNew:updateCostTypePanel()
    self._checkBoxCostTypeLast:setEnabled(true)
    ...
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU then
        self._textCostTypeAll:setString(TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR[TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU])

        if SetupConfig.IsLockGuanZhuAllCost then
            table.insert(tempLockCheckBoxs, self._checkBoxCostTypeAll)
            table.insert(tempLockCheckBoxs, self._checkBoxCostTypeLast)
        end
        ...
        if SetupConfig.IsOpenGuanZhuAllCost then
            ...
            table.insert(tempShowCheckBoxs, self._checkBoxCostTypeLast)
        end
    ...
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU then
        self._textCostTypeAll:setString(TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR[TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU])
    ...
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG then
        table.insert(tempShowCheckBoxs, self._checkBoxCostTypeAverage)
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI then
        self._textCostTypeAll:setString(TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR[TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI])
    ...
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY then
        self._textCostTypeAll:setString(TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR[TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY])
    ...
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU then
        self._textCostTypeAll:setString(TeaHouseCreateViewNew.KW_COSTMODE_GUANZHU_TEXT_STR[TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU])
...
function TeaHouseCreateViewNew:savePayTypeData(selectMode, payTypeTable, bCurrencyFirst)
...
function TeaHouseCreateViewNew:onBtnEventSure(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
    ...
    if everyDayCostTouchEnable then
        ...
        everyDayCostLimitNum = TeaHouseCreateViewNew.KW_DEFAULT_EVERYDAY_COSTLIMIT
    end
...
function TeaHouseCreateViewNew:JudgeDate(tmpData)
...
function TeaHouseCreateViewNew:onBtnEventSurePlayMode(send, eventType)
    
end

--取消
function TeaHouseCreateViewNew:onBtnEventCancel(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:onBtnEventAutoAddRoomCardWhat(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:selectShowNode(tag)
    self._selectTag = tag
    ...
    self._listPanel:setVisible(tag == self.KW_SELECT_SETUP_NODE_TAG.BASE)
    self._textAutoAddCardHistoryTips:setVisible(tag == self.KW_SELECT_SETUP_NODE_TAG.BASE)
end

--模式选择列表打开或关闭
function TeaHouseCreateViewNew:onCheckBoxEventModeSelect(send, eventType)
...
--关闭选择模式
function TeaHouseCreateViewNew:closeModeSelectBtns()
    self._panelModeSelectBtns:setVisible(false)
    self._checkBoxModeSelect:setSelected(false)
end

--领队模式 按钮点击
function TeaHouseCreateViewNew:onBtnEventModeGuanzhu(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU)
     TeaHouse.throwDataManager:throwDataClick(XH.ThrowDataDefine.Tea_34)
end

--预付模式 按钮点击
function TeaHouseCreateViewNew:onBtnEventModeYufu(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU)
     TeaHouse.throwDataManager:throwDataClick(XH.ThrowDataDefine.Tea_37)
end

--流通模式 按钮点击
function TeaHouseCreateViewNew:onBtnEventModeLiutong(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG)
end

--其他模式 按钮点击
function TeaHouseCreateViewNew:onBtnEventModeFangFei(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI)
end

function TeaHouseCreateViewNew:onBtnEventModeLobby(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY)
    TeaHouse.throwDataManager:throwDataClick(XH.ThrowDataDefine.Tea_40)
end

function TeaHouseCreateViewNew:onBtnEventModeShaoXingQuanzhu(send, eventType)
    self:selectMode(TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU)
end

function TeaHouseCreateViewNew:selectMode(mode)
    ...
    if TeaHouse.LOBBY_ID.NINGBO == TeaHouse.BridgeData.getLobbyID() or TeaHouse.LOBBY_ID.YUYAO == TeaHouse.BridgeData.getLobbyID() then
        if TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI == mode then
            local SetupConfig = TeaHouse.manager.configManager.SetupConfig
            self:addItem(false,MODE_PAYTYPE_NAME.FANGFEI,SetupConfig.IsOpenFangFeiRecomend)
        end
    end
    ...
    if mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU then
        self:setEveryDayCostLimitEnabled(true)
        self:setRechargeEnabled(true)
        self:setModeSelect(MODE_PAYTYPE_NAME.QUANZHUWIN)
    elseif mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU then
        self:setEveryDayCostLimitEnabled(false)
        self:setRechargeEnabled(true)
        self:setModeSelect(MODE_PAYTYPE_NAME.YUFU)
    elseif mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG then
        self:setEveryDayCostLimitEnabled(false)
        self:setRechargeEnabled(true)
        self:setModeSelect(MODE_PAYTYPE_NAME.LIUTONG)
    elseif mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI then
        self:setEveryDayCostLimitEnabled(false)
        self:setRechargeEnabled(false)
        self:setModeSelect(MODE_PAYTYPE_NAME.FANGFEI)
    elseif mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY then
        self:setEveryDayCostLimitEnabled(false)
        self:setRechargeEnabled(false)
        self:setModeSelect(MODE_PAYTYPE_NAME.LOBBY)
    elseif mode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.SHAOXINGQUANZHU then
        self:setEveryDayCostLimitEnabled(false)
        self:setRechargeEnabled(false)
        self:setModeSelect(MODE_PAYTYPE_NAME.SHAOXINGGUANZHU)
    end
    -- 扣玩家卡模式不显示自动补卡/缺卡提醒
    local SetupConfig = TeaHouse.manager.configManager.SetupConfig
...
function TeaHouseCreateViewNew:setModeSelect(itemName)
    for _,v in ipairs(self._modeItems) do
        if v.BtnName == itemName then
...
function TeaHouseCreateViewNew:updateRoomCardVisible()
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.FANGFEI or self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LOBBY then
...
function TeaHouseCreateViewNew:onUpdateSinglePlayMode(event)
    self:updateSinglePlayMode(event.msg.nPlayModeId)
end

function TeaHouseCreateViewNew:onUpdateAgentPropCnt(event)
    local agentPropCnt = TeaHouse.manager.teahouseSetup:getAgentPropCount()
    self._textTotalRoomCard:setString("个人账户可用房卡：" .. agentPropCnt)
end

function TeaHouseCreateViewNew:initNeedCostRoomCards()
...
--预付模式 前项收费
--领队模式 后项收费
function TeaHouseCreateViewNew:getPayModeType()
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU then
        return TeaHouse.TeaHouseProtocol.ReqSetPayType.PayModePype.TYPE_PAY_MODEL_FRON
    elseif self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.GUANZHU then
...
function TeaHouseCreateViewNew:getPayType()
    local result = {}
    local SetupConfig = TeaHouse.manager.configManager.SetupConfig
    if SetupConfig.IsFixedPayType then
        return result
    end
    if self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.LIUTONG then
        --平摊
    ...
    elseif  self._selectMode == TeaHouseCreateViewNew.KW_TEAHOUSE_MODE.YUFU then
        ...
        elseif self._checkBoxCostTypeLast:isVisible() and (self._checkBoxCostTypeLast:isSelected() or not self._checkBoxCostTypeLast:isEnabled()) then
            table.insert(result, TeaHouse.TeaHouseProtocol.ReqSetPayType.PayType.TYPE_TEA_OWNER)
        end
...
function TeaHouseCreateViewNew:initRemodelCreateViewExtraUI()
    if TeaHouse.BridgeData.isRemodelArea() then
...
function TeaHouseCreateViewNew:onBtnEventBuyRoomCardTip(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:onBtnEventChargeRoomCardTip(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:parseParams(data)
...
function TeaHouseCreateViewNew:filterModeRemodelTeaHouse(setupConfig)
    local config = clone(setupConfig)
    return config
end

function TeaHouseCreateViewNew:onBtnEventSureCreate(msg)
    self:onBtnEventSure(self._btnSure, ccui.TouchEventType.ended)
end

function TeaHouseCreateViewNew:onCheckBoxEventAutoAddCardSelect(send, eventType)
...
function TeaHouseCreateViewNew:onCheckBoxEventLackCardNotifySelect(send, eventType)
    if eventType == ccui.CheckBoxEventType.selected then
...
function TeaHouseCreateViewNew:onBtnEventLackCardWhat(send, eventType)
    TeaHouse.UITool.onDefaultButtonClicked(send, eventType)
    if eventType ~= ccui.TouchEventType.ended then
        return
    end
...
function TeaHouseCreateViewNew:onCheckBoxEventAutoNumSelect(send, eventType)
    if eventType == ccui.CheckBoxEventType.selected then
...
function TeaHouseCreateViewNew:getSelectedAutoNum(parentNode)
...
function TeaHouseCreateViewNew:updateAutoAddErrStr()
    local myCardNum
    if TeaHouse.BridgeData.isRemodelArea() or TeaHouse.BridgeData.isNotTeaAgentMode() then
...
function TeaHouseCreateViewNew:updateLackNotifyErrStr()
    local selectedNum = 0
...
function TeaHouseCreateViewNew:selectAutoGeer(node)
    if not node then
        return
    end
