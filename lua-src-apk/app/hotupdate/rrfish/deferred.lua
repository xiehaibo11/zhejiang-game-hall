...
--local PENDING = 0			--等待状态
local RESOLVING = 1			--
local REJECTING = 2			--
local RESOLVED = 3			--执行状态
local REJECTED = 4			--拒绝状态

local function finish(classdeferred, state)
	state = state or REJECTED
	for _, f in ipairs(classdeferred.queue) do
		if state == RESOLVED then
			f:resolve(classdeferred.value)
		else
			f:reject(classdeferred.value)
		end
	end
	classdeferred.state = state
end
...
local function promise(classdeferred, next, success, failure, nonpromisecb)
	if type(classdeferred) == 'table' and type(classdeferred.value) == 'table' and isfunction(next) then
		local called = false
		local ok, err = pcall(next, classdeferred.value, function(v)
			if called then return end
			called = true
			classdeferred.value = v
			success()
		end, function(v)
			if called then return end
			called = true
...
local function fire(classdeferred)
	local next
	if type(classdeferred.value) == 'table' then
		next = classdeferred.value.next
	end
	promise(classdeferred, next, function()
		classdeferred.state = RESOLVING
		fire(classdeferred)
	end, function()
		classdeferred.state = REJECTING
		fire(classdeferred)
	end, function()
		local ok
		local v
		if classdeferred.state == RESOLVING and isfunction(classdeferred.success) then
			ok, v = pcall(classdeferred.success, classdeferred.value)
		elseif classdeferred.state == REJECTING and isfunction(classdeferred.failure) then
			ok, v = pcall(classdeferred.failure, classdeferred.value)
			if ok then
				classdeferred.state = RESOLVING
			end
		end
		...
		if ok ~= nil then
			if ok then
				...
				classdeferred.value = v
				return finish(classdeferred)
			end
		end

		if classdeferred.value == classdeferred then
			classdeferred.value = pcall(error, 'resolving promise with itself')
			return finish(classdeferred)
		else
			promise(classdeferred, next, function()
				finish(classdeferred, RESOLVED)
			end, function(state)
				finish(classdeferred, state)
			end, function()
				finish(classdeferred, classdeferred.state == RESOLVING and RESOLVED)
			end)
		end
	end)
end

local function resolve(classdeferred, state, value)
	if classdeferred.state == 0 then
		classdeferred.value = value
		classdeferred.state = state
		fire(classdeferred)
	end
