...
-- local Sha256 = require("app.Tool.Sha256")
local Sha
local shaOkCount = 0
local requireShaOK, _ = pcall(function()
    Sha = require("app.Tool.Sha")
end)
...
local MAP_KEY = {
    [Encrypt.KEY.DEFAULT] = "JIAMIKEY12349876",
}

---------------------------------------------------------
--@brief AES加密解密函数
--@param 需要加解密的str
--@param 加密还是解密，true为加密
--@param 加密的key
--@return 加解密的结果
---------------------------------------------------------
function Encrypt.aesEncrypt(str, isEncrypt, key)
    key = key or Encrypt.KEY.DEFAULT
    local obj = un.Encryption:new()
    obj:setAesKey(MAP_KEY[key])
    if isEncrypt then
...
local function s232num(s, i)
	local n = 0
	for a = i, i + 3 do n = n * 256 + string.byte(s, a) end
	return n
end
...
function Encrypt.sha256(msg)
	-- if Sha256 and Sha256.sha256_hex then
    --     return Sha256.sha256_hex(msg)
	-- end
	if requireShaOK and Sha and Sha.sha256 and shaOkCount >= 0 then
		if shaOkCount > 0 then
			return Sha.sha256(msg)
		end
		local finalMsg
		local ok, _ = pcall(function()
			finalMsg = Sha.sha256(msg)
		end)
		if ok then
			shaOkCount = shaOkCount + 1
			return finalMsg
		else
			shaOkCount = shaOkCount - 1
		end
...
function Encrypt.sha256_binary(msg)
	return hex_to_binary(Encrypt.sha256(msg))
end

function Encrypt.hmac_sha256(key, text)
	...
	return Encrypt.sha256(key_xord_with_0x5c .. Encrypt.sha256_binary(key_xord_with_0x36 .. text))
end

function Encrypt.rfc3339TimeString()
    return os.date("%Y-%m-%dT%H:%M:%S+08:00",os.time())
end
