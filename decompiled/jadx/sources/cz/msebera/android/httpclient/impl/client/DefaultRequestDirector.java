...
public class DefaultRequestDirector implements RequestDirector {
    ...
    public HttpResponse execute(HttpHost httpHost, HttpRequest httpRequest, HttpContext httpContext) throws HttpException, IOException {
        ...
        this.virtualHost = (HttpHost) requestWrapperWrapRequest.getParams().getParameter(ClientPNames.VIRTUAL_HOST);
        ...
        boolean z = false;
        HttpHost httpHostExtractHost = httpHost;
            ...
            try {
                ...
                try {
                    ...
                    if (this.virtualHost != null) {
                        httpHostExtractHost = this.virtualHost;
                    } else {
                        URI uri = request.getURI();
                        if (uri.isAbsolute()) {
                            httpHostExtractHost = URIUtils.extractHost(uri);
                        }
                    }
                    if (httpHostExtractHost == null) {
                        httpHostExtractHost = route.getTargetHost();
                    }
                    request.resetHeaders();
                    rewriteRequestURI(request, route);
                    httpContext.setAttribute("http.target_host", httpHostExtractHost);
