...
public class FullScreenUtils {
    ...
    public void hideBar(Activity activity) {
        activity.getWindow().getDecorView().setSystemUiVisibility(activity.getWindow().getDecorView().getSystemUiVisibility() | 4614);
        NotchScreenUtils.getInstance().setOnApplyWindowInsetsListener(activity);
    }

    public int getBarSize(Activity activity) {
        return NotchScreenUtils.getInstance().getNotSize();
    }

    public int getBoomBarSize(Activity activity) {
        return DensityUtils.px2dp(activity, DensityUtils.getInstance().getNavigationBarHeight(activity));
    }
}
