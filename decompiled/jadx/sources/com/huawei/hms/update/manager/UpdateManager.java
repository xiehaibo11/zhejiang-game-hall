...
public class UpdateManager {
    ...
    public static boolean d(Context context) {
        String str;
        List<ResolveInfo> listQueryIntentServices = context.getPackageManager().queryIntentServices(new Intent("com.apptouch.intent.action.update_hms"), 0);
    ...
    public static Intent getStartUpdateIntent(Activity activity, UpdateBean updateBean) {
        ...
        Intent intentStartBridgeActivity = BridgeActivity.getIntentStartBridgeActivity(activity, p.a(iIntValue));
    ...
    public static Intent startUpdateIntent(Activity activity) {
        ...
        updateBean.setTypeList(arrayList);
        if (arrayList.size() > 1) {
            intentStartBridgeActivity = BridgeActivity.getIntentStartBridgeActivity(activity, p.a(((Integer) arrayList.get(1)).intValue()));
        } else {
            int iIntValue = -2;
            if (arrayList.size() > 0) {
                iIntValue = ((Integer) arrayList.get(0)).intValue();
            ...
            intentStartBridgeActivity = BridgeActivity.getIntentStartBridgeActivity(activity, p.a(iIntValue));
        }
        intentStartBridgeActivity.putExtra(BridgeActivity.EXTRA_DELEGATE_UPDATE_INFO, updateBean);
        return intentStartBridgeActivity;
    }
    ...
    public static void a(Context context, ArrayList<Integer> arrayList) {
        ...
        if (SystemUtils.isSystemApp(context.getApplicationContext(), PackageConstants.SERVICES_PACKAGE_APPMARKET) && b(context) && SystemUtils.isChinaROM()) {
            arrayList.add(0);
            arrayList.add(6);
        } else {
            c(context, arrayList);
    ...
    public static boolean c(Context context) {
        ...
        if (applicationInfo != null && applicationInfo.metaData != null && applicationInfo.metaData.containsKey("com.huawei.hms.client.service.name:base")) {
            ...
            if (!TextUtils.isEmpty(string) && string.split(Constants.COLON_SEPARATOR).length == 2) {
    ...
    public static Intent getStartUpdateIntent(Context context, UpdateBean updateBean) {
        ...
        updateBean.setTypeList(arrayList);
        Intent intentStartBridgeActivity = BridgeActivity.getIntentStartBridgeActivity(context, p.a(iIntValue));
        intentStartBridgeActivity.putExtra(BridgeActivity.EXTRA_DELEGATE_UPDATE_INFO, updateBean);
        return intentStartBridgeActivity;
    }
}
