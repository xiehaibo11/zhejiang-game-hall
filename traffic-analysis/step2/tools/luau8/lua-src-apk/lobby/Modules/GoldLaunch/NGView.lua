local GoldLaunchView = class("GoldLaunchView", NG.ViewBase)
...
function GoldLaunchView:initUI()
    self._KW_BTN_CLOSE:setVisible(false)
    if self._param.type == GoldLaunchConfig.Type.EXIT then
        NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_26, nil, { la_numid = GoldPlayerData.NumId })
    elseif self._param.type == GoldLaunchConfig.Type.AWARD then
        NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_24, nil, { la_numid = GoldPlayerData.NumId })
    end
...
function GoldLaunchView:flushPos(node, index, all)
    local size = self._KW_PANEL_LIST:getContentSize()
    local startPos
    -- 懒得调了，直接UI对应写死
    local posVec = {
        { 540 },
    ...
    if all == 1 then
        startPos = self._KW_PANEL_LIST:getContentSize().width / 2
    elseif all == 2 then
        startPos = 375
    elseif all == 3 then
        startPos = 245
...
function GoldLaunchView:onClickClose()
    NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_23, nil, { la_numid = GoldPlayerData.NumId })
    self:onClickCloseView()
end

-- 确认退出
function GoldLaunchView:onClickExit()
    NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_27, nil, { la_numid = GoldPlayerData.NumId })
...
function GoldLaunchView:onClickContinue()
    NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_28, nil, { la_numid = GoldPlayerData.NumId })
    self:onClickCloseView()
end

-- 领取奖励
function GoldLaunchView:onClickGet()
    NG.TipTool.showToast("奖励已成功发放~")
    NG.throwDataManager:throwData(NG.GoldThrowDataDefine.GOLD_QUICK_THROW_25, nil, { la_numid = GoldPlayerData.NumId })
    self:onClickCloseView()
end
