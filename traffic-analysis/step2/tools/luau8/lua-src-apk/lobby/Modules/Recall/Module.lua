...
function RecallModule:getReqConfig()
    return {
        ReqRecallPlayerList = { reqPath = "lobby.Req.Recall.ReqRecallPlayerList", callBack = self.onRespRecallPlayerList },
...
function RecallModule:shareWX()
    math.randomseed(tonumber(tostring(os.time()):reverse():sub(1,6)))
    local rand = math.random(3)
    local shareText = KW_SHARE_TEXT[rand] or {}
    local url = "http://act.anquanqipai.com/friend_recall/share?"
