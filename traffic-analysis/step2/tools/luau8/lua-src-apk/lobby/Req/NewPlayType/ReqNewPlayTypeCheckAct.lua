local ReqNewPlayTypeCheckAct = class("ReqNewPlayTypeCheckAct", XH.ReqHttp)
...
function ReqNewPlayTypeCheckAct:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeCheckAct.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_CHECK_ACT)
...
function ReqNewPlayTypeCheckAct:onHttpCallBack(eType, status, response)
    if status == 200 then
