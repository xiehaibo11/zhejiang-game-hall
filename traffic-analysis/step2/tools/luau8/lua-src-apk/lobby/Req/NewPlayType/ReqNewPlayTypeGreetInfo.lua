local ReqNewPlayTypeGreetInfo = class("ReqNewPlayTypeGreetInfo", XH.ReqHttp)
...
function ReqNewPlayTypeGreetInfo:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeGreetInfo.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_GREET_INFO)
...
function ReqNewPlayTypeGreetInfo:onHttpCallBack(eType, status, response)
    if status == 200 then
...
return ReqNewPlayTypeGreetInfo�