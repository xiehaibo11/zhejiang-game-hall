local ReqNewPlayTypeGetPrize = class("ReqNewPlayTypeGetPrize", XH.ReqHttp)
...
function ReqNewPlayTypeGetPrize:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeGetPrize.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_ACT_GET_PRIZE)
...
function ReqNewPlayTypeGetPrize:onHttpCallBack(eType, status, response)
    if status == 200 then
