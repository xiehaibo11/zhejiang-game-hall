local ReqNewPlayTypeGetAward = class("ReqNewPlayTypeGetAward", XH.ReqHttp)
...
function ReqNewPlayTypeGetAward:start(stageID, teaNumber, timeoutTime)
    if ReqNewPlayTypeGetAward.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "stage_id=" .. stageID)
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_STAGE_AWARD)
...
function ReqNewPlayTypeGetAward:onHttpCallBack(eType, status, response)
    if status == 200 then
...
return ReqNewPlayTypeGetAwardr