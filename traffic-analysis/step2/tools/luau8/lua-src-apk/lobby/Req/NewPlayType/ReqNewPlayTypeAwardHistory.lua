local ReqNewPlayTypeAwardHistory = class("ReqNewPlayTypeAwardHistory", XH.ReqHttp)
...
function ReqNewPlayTypeAwardHistory:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeAwardHistory.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_AWARD_HISTORY)
...
function ReqNewPlayTypeAwardHistory:onHttpCallBack(eType, status, response)
    if status == 200 then
...
return ReqNewPlayTypeAwardHistory