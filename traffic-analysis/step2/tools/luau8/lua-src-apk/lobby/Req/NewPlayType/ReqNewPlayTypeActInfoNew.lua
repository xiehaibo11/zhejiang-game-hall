local ReqNewPlayTypeActInfoNew = class("ReqNewPlayTypeActInfoNew", XH.ReqHttp)
...
function ReqNewPlayTypeActInfoNew:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeActInfoNew.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_ACT_INFO_NEW)
...
function ReqNewPlayTypeActInfoNew:onHttpCallBack(eType, status, response)
    if status == 200 then
...
return ReqNewPlayTypeActInfoNew