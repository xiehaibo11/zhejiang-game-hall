...
function Req:start(timeoutTime)
    ...
    self._timeoutTime = 30 --timeoutTime or XH.KW_CONFIG_LOGIC_TIME_OUT
    if self._timeoutTime ~= nil and self._timeoutTime > 0 then
        self._LogicScheduleScriptID = cc.Director:getInstance():getScheduler():scheduleScriptFunc(
                                          function()
                self:timeout()
            end, self._timeoutTime, false)
    end
