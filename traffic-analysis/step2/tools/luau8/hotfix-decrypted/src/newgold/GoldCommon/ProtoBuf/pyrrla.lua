...
service Pyrrla {
  rpc GetGameConfig (GameConfigRequest) returns (GameConfigReply);

  // 请求场次在线人数
  rpc ListOnlineCount(ListOnlineCountRequest) returns (ListOnlineCountReply);

  // 请求加入游戏
  rpc EnterGame(JoinDispatchQueueRequest) returns (JoinDispatchQueueReply);

  // 请求离开游戏
  rpc LeaveGame(LeaveDispatchQueueRequest) returns (LeaveDispatchQueueReply);

  // 匹配服通知金币服
  rpc EnterGameNotify(EnterGameNotifyRequest) returns (EnterGameNotifyReply);

  // 金币破产补助
  rpc GoldBroke(GoldBrokeRequest) returns (GoldBrokeReply);
}
