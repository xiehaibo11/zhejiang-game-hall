...
local UIMahHuArea3D = NG.GAME.gameClass("UIMahHuArea3D", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.ViewBase.UIMahHuAreaBase")

function UIMahHuArea3D:ctor(localSeat)
    self._config = {}
    UIMahHuArea3D.super.ctor(self, localSeat)

    self:setCameraMask(UIMahConfig3D.DeskCameraMask)
end

function UIMahHuArea3D:_getMahType()
...
function UIMahHuArea3D:_createMah(mahValue)
...
function UIMahHuArea3D:_getLayoutConfig()
    return UIMahConfig3D.HuAreaLayout[self._localSeat] or {}
end

function UIMahHuArea3D:_updateMahsPosition()
...
function UIMahHuArea3D:_getSingleLineMahsCount()
    return self._singleLineMahsCount or UIMahConfig3D.HuSingleLineMaxCount
end

function UIMahHuArea3D:setSingleLineMahsCount(mahsCount)
    self._singleLineMahsCount = mahsCount
end

function UIMahHuArea3D:setMahColor(colorFlag)
    self._config.mahColorFalg = colorFlag

    self:_updateMahColor()
end

function UIMahHuArea3D:_updateMahColor()
    for _, mah in pairs(self._allMahs) do
        mah:setMahColor(self._config.mahColorFalg)
    end
end

function UIMahHuArea3D:showLastOutMahCursor()
...
function UIMahHuArea3D:_addHu(HuMahValue, showAni)
    local mah = UIMahHuArea3D.super._addHu(self, HuMahValue)
    if mah and showAni then

        local params = { path = "NewGoldRes/Spine/MahXueLiu/", tex = "hupai.json", ske = "hupai.atlas", armatureName = "hupai", loop = false }
        local aniNode = display.playDargonBonesSpine(params)
        if aniNode then
            mah:addChild(aniNode)
            aniNode:setCameraMask(mah:getCameraMask())
            aniNode:setScale(0.1)
            aniNode:setPosition3D(cc.vec3(0, -0.2, 0.5))
            aniNode:setRotation3D(cc.vec3(90, 0, 0))

            aniNode:runAction(cc.Sequence:create(cc.DelayTime:create(0.55), cc.CallFunc:create(function()
