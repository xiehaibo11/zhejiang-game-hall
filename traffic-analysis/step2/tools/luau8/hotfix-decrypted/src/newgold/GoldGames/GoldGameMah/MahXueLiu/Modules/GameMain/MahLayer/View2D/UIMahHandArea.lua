...
local UIMahHandArea = NG.GAME.gameClass("UIMahHandArea2D", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.ViewBase.UIMahHandAreaBase")
...
function UIMahHandArea:ctor(localSeat, config)
    ...
    self._maxHandMahCount = NG.goldGame:getModule("GameMain.MahLayer"):getMaxHandCounts()
...
function UIMahHandArea:_getHandMahType(bStand, mahValue)
    if self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.BOTTOM then
        if mahValue == NG.GAME.GameDefine.MAH_VALUE.BACK and NG.GAME.roomTableData:getIsSeer() then
            return NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_BACKWARD
        end
        if mahValue == NG.GAME.GameDefine.MAH_VALUE.BACK and not bStand then
            return NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_VERTICAL
        end
    ...
    elseif self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.RIGHT then
        if mahValue == NG.GAME.GameDefine.MAH_VALUE.BACK and not bStand then
            return NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_HORIZONTAL
        end
...
function UIMahHandArea:_getHandMahScale()
    ...
    if self:_isCombShowArrowByMah() and self._localSeat ~= NG.GAME.GameDefine.LOCAL_SEAT.BOTTOM then
...
function UIMahHandArea:_isCombShowArrowByMah()
    return self:_getConfigValueByKey(UIMahConfig.MahSettingKey.LieType) == NG.GAME.GameDefine.COMB_LIE_TYPE.ARROW_BY_MAH
end
...
function UIMahHandArea:_isCombsAtLeft()
    ...
    if self:_getConfigValueByKey(UIMahConfig.MahSettingKey.PlaceType) == NG.GAME.GameDefine.HANDAREA_PLACE_TYPE.COMB_AT_LEFT then
        return true
    else
        local layoutConf = self:_getLayoutConfig()
...
function UIMahHandArea:_updateHandMahsPosition(bShowAction)
    ...
    for index, mah in ipairs(self._handMahs) do
        ...
        if bShowAction and NG.GAME.settingData:getInsertStyle() == 1 then
...
function UIMahHandArea:_getCombStartPos()
    ...
    if not self:_isCombsAtLeft() then
        ...
        elseif self._maxHandMahCount == NG.GAME.GameDefine.MAX_HAND_COUNT_TYPE.EIGHT then
...
function UIMahHandArea:_getMaxPlayerCount()
    return self._maxPlayerCount or NG.GAME.GameDefine.MAX_HAND_COUNT_TYPE.FOURTEEN
end
...
function UIMahHandArea:addTKongComb(combData)
    if combData.combFlag ~= NG.GAME.GameDefine.COMB_FLAG.TKONG
