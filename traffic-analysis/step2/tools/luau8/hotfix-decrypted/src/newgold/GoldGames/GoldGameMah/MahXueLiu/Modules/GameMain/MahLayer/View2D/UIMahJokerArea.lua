...
local UIMahJokerArea = NG.GAME.gameClass("UIMahJokerArea", NG.ViewBase)
...
function UIMahJokerArea:setJokerMahs(mahIDs)
    ...
    for index = 1,#mahIDs do
        self._jokerMahArray[index] = UIMah.new(NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TOUP, config)
        self._jokerMahArray[index]:setMahValue(mahIDs[index])
        self._jokerMahArray[index]:setName(index)
        if mahIDs[index] ~= NG.GAME.GameDefine.MAH_VALUE.BACK then
