...
local UIMahWallArea3D = NG.GAME.gameClass("UIMahWallArea3D", NG.ViewBase)
local MAH_SCALE = 0.78
...
function UIMahWallArea3D:_createMah(localSeat, mahCount, index, bAbove)
    ...
    local posZ = UIMahConfig3D.MahModelSize.Thick * MAH_SCALE / 2
    local addDirection = layoutConf.IsForwardAdd and 1 or -1
    local mahWidth = UIMahConfig3D.MahModelSize.Width * MAH_SCALE
    if layoutConf.IsAddX then
    ...
    if bAbove then
        posZ = UIMahConfig3D.MahModelSize.Thick * MAH_SCALE * 3/2
    end
...
function UIMahWallArea3D:setStartTakeData(startTakeData, openMahsPos)
    if not startTakeData or not startTakeData.startLocalSeat or not startTakeData.startIndex or startTakeData.startLocalSeat < 0 then
        return
    end
