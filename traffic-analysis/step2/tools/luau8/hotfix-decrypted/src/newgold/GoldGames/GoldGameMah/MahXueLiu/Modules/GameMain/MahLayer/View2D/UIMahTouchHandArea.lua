...
local UIMahTouchHandArea = NG.GAME.gameClass("UIMahTouchHandArea", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.View2D.UIMahHandArea")
...
function UIMahTouchHandArea:_isSingleClick(touchMah)
    if not NG.GAME.GameDefine then
        return false
    end
    if self:_getConfigValueByKey(UIMahConfig.MahSettingKey.PlayType) ~= NG.GAME.GameDefine.HANDMAHS_PLAY_TYPE.SINGLE_CLICK then
        return false
    end
