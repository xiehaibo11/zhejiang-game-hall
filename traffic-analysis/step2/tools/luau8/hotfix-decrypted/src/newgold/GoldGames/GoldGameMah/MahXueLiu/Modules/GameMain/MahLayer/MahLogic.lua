local MahLogic = NG.GAME.gameClass("MahLogic")
...
function MahLogic.checkIsWanNeng(mahValue)
    return mahValue == NG.GAME.GameDefine.NEW_CARD_VALUE.WANNENG_LAI
end

function MahLogic.getRealMahValue(mahValue, jokerValues, insteadValues)
    if mahValue == nil then
        return NG.GAME.GameDefine.MAH_VALUE.NONE
    end
...
local changeV = {[97] = 16, [98] = 32, [99] = 48, [100] = 64, [101] = 0, [102] = 15}
function MahLogic.toSortValue(mahValue)
    return changeV[mahValue] or mahValue
end
...
function MahLogic.sortMahValues(mahValues, jokerValues, insteadValues)
    ...
    local sortFunc = function (mahValue1, mahValue2)
        local isJoker1 = MahLogic.checkIsWanNeng(mahValue1)
        local isJoker2 = MahLogic.checkIsWanNeng(mahValue2)
        ...
        local realMahValue1 = MahLogic.toSortValue(MahLogic.getRealMahValue(mahValue1, jokerValues, insteadValues))
        local realMahValue2 = MahLogic.toSortValue(MahLogic.getRealMahValue(mahValue2, jokerValues, insteadValues))
...
function MahLogic.checkMahValueIsFlower(mahValue)
    -- if mahValue >= NG.GAME.GameDefine.MAH_VALUE.HUA_MEI and mahValue <= NG.GAME.GameDefine.MAH_VALUE.HUA_JIN_YUAN_BAO then
    --     return true
    -- end
    return false
end
...
function MahLogic.checkLocalSeat(localSeat)
    ...
    if localSeat == NG.GAME.GameDefine.LOCAL_SEAT.LEFT
    or localSeat == NG.GAME.GameDefine.LOCAL_SEAT.RIGHT
    or localSeat == NG.GAME.GameDefine.LOCAL_SEAT.TOP
    or localSeat == NG.GAME.GameDefine.LOCAL_SEAT.BOTTOM then
        return true
    end
    return false
end
...
function MahLogic.isNormalMahValue(mahValue)
    ...
    if (mahValue >= NG.GAME.GameDefine.MAH_VALUE.WAN_1 and mahValue <= NG.GAME.GameDefine.MAH_VALUE.WAN_9)
    or (mahValue >= NG.GAME.GameDefine.MAH_VALUE.TIAO_1 and mahValue <= NG.GAME.GameDefine.MAH_VALUE.TIAO_9)
    or (mahValue >= NG.GAME.GameDefine.MAH_VALUE.TONG_1 and mahValue <= NG.GAME.GameDefine.MAH_VALUE.TONG_9)
    or (mahValue >= NG.GAME.GameDefine.MAH_VALUE.FENG_DONG and mahValue <= NG.GAME.GameDefine.MAH_VALUE.FENG_BEI)
    or (mahValue >= NG.GAME.GameDefine.MAH_VALUE.JIAN_ZHONG and mahValue <= NG.GAME.GameDefine.MAH_VALUE.JIAN_BAI)
    or (mahValue >= NG.GAME.GameDefine.MAH_VALUE.HUA_MEI and mahValue <= NG.GAME.GameDefine.MAH_VALUE.HUA_JIN_YUAN_BAO) then
        return true
    end
    return false
end
...
function MahLogic.isShowFaceByMahType(mahType)
    ...
    if mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_FORWARD
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TOUP
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TODOWN
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TOLEFT
    or mahType == "default"
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TORIGHT then
        return true
    end
    return false
end
...
function MahLogic.isStandMah(mahType)
    ...
    if mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_FORWARD 
    or mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_BACKWARD
    or mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_TOLEFT
    or mahType == "default"
    or mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_TORIGHT then
        return true
    end
    return false
end
...
function MahLogic.isLeftOrRightStandMah(mahType)
    ...
    if mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_TOLEFT 
    or mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_TORIGHT then
        return true
    end
    return false
...
function MahLogic.isCanChangeThick(mahType)
    ...
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TODOWN
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TOLEFT
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TORIGHT
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_VERTICAL
    or mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_HORIZONTAL then
        return true
    end
    return false
end
...
function MahLogic.isCanChangeHeight(mahType)
    ...
    if mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_FORWARD then
        return true
    end
    return false
end
...
function MahLogic.checkIsMahBack(mahValue)
    ...
    return mahValue == NG.GAME.GameDefine.MAH_VALUE.BACK
end
...
function MahLogic.playGLActionAni(node, callback, maxScale) 
    local SpecialHuGL = NG.GAME.gameRequire("Modules.GameMain.BasicMahjong.GLProgram.SpecialHuGL")
...
function MahLogic.isFlowerMahValue(mahValue)
    ...
    if mahValue >= NG.GAME.GameDefine.MAH_VALUE.HUA_MEI and mahValue <= NG.GAME.GameDefine.MAH_VALUE.HUA_DONG then
        return true
    end
    return false
end
...
function MahLogic.isFlowerMahNumberLeft(mahValue)
    if MahLogic.isFlowerMahValue(mahValue) then
        if mahValue >= NG.GAME.GameDefine.MAH_VALUE.HUA_CHUN and mahValue <= NG.GAME.GameDefine.MAH_VALUE.HUA_DONG then
            return true
        end
    end
    return false
