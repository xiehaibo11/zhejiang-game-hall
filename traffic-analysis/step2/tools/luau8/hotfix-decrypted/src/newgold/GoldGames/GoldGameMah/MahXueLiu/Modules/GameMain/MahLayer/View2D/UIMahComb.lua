...
local UIMahComb = NG.GAME.gameClass("UIMahComb2D", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.ViewBase.UIMahCombBase")
...
function UIMahComb:_resetMahs()
    ...
    local inMahValue = self._combData.inMahValue or NG.GAME.GameDefine.MAH_VALUE.NONE
    for index, mahValue in ipairs(showMahValues) do
        ...
        elseif self:_isNeedShowArrowByMah() then
            -- if self._combData.combFlag == NG.GAME.GameDefine.COMB_FLAG.CHOW then
            --     isUsedByArrow = not bFindInMah and inMahValue == mahValue
            -- else
...
function UIMahComb:_isNeedShowArrowByMah()
    ...
    if self:getCombFlag() == NG.GAME.GameDefine.COMB_FLAG.CKONG then
        return false
    end
    return true
end
...
function UIMahComb:_sortChowMahValues(showMahValues)
    if self:getCombFlag() ~= NG.GAME.GameDefine.COMB_FLAG.CHOW then
        return showMahValues
    end
    if self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.TOP or self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.LEFT then
...
function UIMahComb:_isArrowByIconLieType()
    return self:_getConfigValueByKey(UIMahConfig.MahSettingKey.LieType) == NG.GAME.GameDefine.COMB_LIE_TYPE.ARROW_BY_ICON
end
...
function UIMahComb:_updateLayoutAndSize()
    if self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.BOTTOM or self._localSeat == NG.GAME.GameDefine.LOCAL_SEAT.TOP then
...
function UIMahComb:getShowMahValues()
    ...
    local bFindInMah = false
    if self:_isNeedShowArrowByMah() then
        if inMahValue ~= NG.GAME.GameDefine.MAH_VALUE.NONE then
