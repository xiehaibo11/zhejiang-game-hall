...
GameCreationView.resourceFilename = "creation_options_csb/xuezhandazhouMj.csb"
...
function GameCreationView:ctor(initConfigData)
    ...
    self.mMainGameRule = g_gameConstant.GAME_PLAY_RULE_SCMJ_XZDZ
end
        ...
        for i, v in pairs(wanfaItemTable) do
            if v == target then
                ...
                elseif i >= 4 and i <= 5 then
                    self:selectOneInSomesChoices(wanfaItemTable, i, 4, 2)
                elseif i >= 6 and i <= 7 then
                    self:selectOneInSomesChoices(wanfaItemTable, i, 6, 2)
                elseif i >= 8 and i <= 9 then
                    self:selectMostOneChoices(wanfaItemTable, i, 8, 2)
