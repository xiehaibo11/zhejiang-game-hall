local _M = {}
local nodeBase = require("newgold.MatchGames.PDK.LYPDK.common.baseLayer")

local PNG_POSTFIX = ".png"

local function getPath(name)
    return "game/lyPuKe/cardsUIs/"..name
end

local card_attr = {
    NAME = {[0] = "方块",[1] = "梅花",[2] = "红桃",[3] = "黑桃"},
    ...
    BG = {[1] = "bg_face1",[2] = "bg_back" , [4] = "bg_face1"},                                 -- 背景
    MASK = "mask.png",
    
    POS = { cv = cc.p(30,155),cs = cc.p(30,110),cb = cc.p(92,53)}
}
...
function _M.getCardInfoByData(data,kind)
    ...
    if kind ~= GameCMD.DRAW_BGCARD and kind ~= GameCMD.DRAW_FACECARD  or (kind == GameCMD.DRAW_FACECARD and data == nil)  then 
        assert(false,"PDKNewCard getCardInfoByData 错误：分析牌信息参数不正确~！")
    end 
    ...
    elseif kind == GameCMD.DRAW_FACECARD  then 
        ...
        local isNormal =  g_UserDataTable.gamePdkCardCfgIndex == 1

   
        if g_UserDataTable.gamePdkCardCfgIndex == 2 then 
            info.BG = getPath("bg_face2"..PNG_POSTFIX)
        elseif g_UserDataTable.gamePdkCardCfgIndex == 3 then 
            info.BG = getPath("bg_face3"..PNG_POSTFIX)
        end


        local pngFix = ""
        if g_UserDataTable.gamePdkCardCfgIndex == 3 then 
            pngFix = "-1"
        end


        info.CS = getPath(card_attr.CS[color].. pngFix.. PNG_POSTFIX)              -- 对应小花色图片

        info.CB = getPath(card_attr.CB[color].. pngFix.. PNG_POSTFIX)              -- 对应大花色图片

        info.CV = getPath(card_attr.CV[color]..value..  pngFix.. PNG_POSTFIX)       -- 对应牌值的颜色
...
function _M.getCardName(data)

    local value = data%16
    local color = math.modf(data/16)
    local name = card_attr.NAME[color] .. value
    return name
end
...
function _M.newCard(data,kind)
    ...
    function node:update(data,kind)
        ...
        elseif kind == GameCMD.DRAW_FACECARD then         -- 加载正面花色
            ...
            -- 牌值
            node.cv = newSprite(info.CV):move(card_attr.POS.cv):addTo(cardBg)
            -- 小花色
            node.cs = newSprite(info.CS):move(card_attr.POS.cs):addTo(cardBg)
            -- 大花色
            node.cb = newSprite(info.CB):move(card_attr.POS.cb):addTo(cardBg)
    ...
    function node:setShadowStatus(status)
        ...
        if node.cardBg then 
            local cardBg = node.cardBg
            if status then 
                cardBg:setColor( cc.c3b(192,184,164) )
            else
                cardBg:setColor( display.COLOR_WHITE )
            end
        end
    end
    ...
    -- 获取牌值
    function node:refreshCardFrame()
        -- return _data
        ...
        if not tolua.isnull(cardBg) then 
            cardBg:setSpriteFrame( info.BG )
        end

        if not tolua.isnull(cb) then 
            cb:setSpriteFrame( info.CB )
        end

        if not tolua.isnull(cs) then 
            cs:setSpriteFrame( info.CS )
        end

        if not tolua.isnull(cv) then 
            cv:setSpriteFrame( info.CV )
        end
...
function _M.newHandCardPanel(handData,model,isAddListen,onClick)
    ...
    if model == GameCMD.DRAW_FACECARD then 
        for i=1, num do 
            ...
            -- 获取牌的位置
            local pos = _M.getCardPos(1,i,size)
            -- 添加到节点
...
-- 将手牌全部重置
function _M.resetHandCardStatus(rootNode)
    
    if type(rootNode) ~= "userdata" then 
        return 
    end 

    local nodes = nodeBase:findChildWithName(rootNode,"HandCard")
...
function _M.setCardStatusByTipsData(rootNode,tipData)
    ...
    assert(nodes ~= nil," PDKNewCard setCardStatusByTipsData 错误：绘制提示时，参数1格式错误~！")
...
function _M.getTxtLabelByNum(num)
    ...
    local label = ccui.Text:create("", LYPDKResource.FontRes, 18):setAnchorPoint(cc.p(0.5, 0.5))
...
-- --大小王
--  LITTLE_KING((byte) 0x41),
--  BIG_KING((byte) 0x42),�T