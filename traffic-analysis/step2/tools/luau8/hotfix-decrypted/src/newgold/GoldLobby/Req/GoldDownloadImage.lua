local GoldDownloadImage = class("GoldDownloadImage", NG.Req)

local DEFAULT_DOWNLOAD_IMAGE_DIR = device.writablePath .. "tmpImageCachDir/"

function GoldDownloadImage:ctor()
    GoldDownloadImage.super.ctor(self)
    self._url = ""
    self._localUrl = ""
...
function GoldDownloadImage:setUrl(url, force, retryCount)
    if url and url ~= "" then
        if url ~= self._url then
            ...
            self._url = url
            local name = NG.StringTool.getImageNameByUrl(self._url)
            self._localUrl = self._dir .. name
            if force then
                self:ReqGoldDownloadImage(self._url)
            else
...
function GoldDownloadImage:ReqGoldDownloadImage(url)
...
--获取文件大小
function GoldDownloadImage:getFileSize(fileName)
    local fileSize = -1
