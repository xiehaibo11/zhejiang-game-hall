...
ActivityModule.showRed = {
    ...
    [ActivityModule.ActivityType.ActivitBuyu] = false,
}
...
function ActivityModule:getActsByLobbyID()
    --TODO 从web获取当前区的活动信息
    local webData = clone(XH.lobby:getModule("Lobby"):getWebTabsData())
    local tmpTabs = webData.page or {}
    for i=#tmpTabs, 1, -1 do
        if tmpTabs[i].icon_code and tmpTabs[i].icon_code == "LUCKYTASK" and not XH.lobby:getModule("LuckyTask"):isHaveTask() then
            table.remove(tmpTabs, i)
            break
        end
    end
    -- local hasAntiHack = false
    -- for i=#tmpTabs, 1, -1 do
    --     if tmpTabs[i].icon_code and tmpTabs[i].icon_code == "ANTI_HACK" then
    --         hasAntiHack = true
    --         break
    --     end
    -- end
    -- if not hasAntiHack then
    --     local antiHack = {}
    --     antiHack.icon_code = "ANTI_HACK"
    --     antiHack.icon_name = "外挂悬赏"
    --     antiHack.sort = 100
    --     table.insert(tmpTabs, antiHack)
    -- end
...
function ActivityModule:respAcceptPresenter(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        ...
        for _,v in pairs(self._fanzhaAwardInfo) do
