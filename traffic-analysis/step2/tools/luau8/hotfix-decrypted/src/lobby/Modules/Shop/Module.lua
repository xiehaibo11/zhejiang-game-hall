...
ShopModule.EVENT_SHOP_BUY_ROOMCARD = "EVENT_SHOP_BUY_ROOMCARD"
ShopModule.EVENT_PAY_CONSULT = "EVENT_PAY_CONSULT"
...
function ShopModule:getReqConfig()
    return {
        ...
        ReqDiamondProductsInfoEx = { reqPath = "lobby.Req.Shop.ReqProductList", callBack = self.respDiamondProductsInfoEx },
        ...
        ReqPayConsult = {reqPath = "lobby.Modules.Shop.ReqPayConsult", callBack = self.onPayConsultCallBack},
    }
end
...
function ShopModule:getProxyEvents()
    return {
        ...
        { module = XH.netEngine, eventName = "116_1905", callBack = "onRechargeNotify"},
    }
end

function ShopModule:onRechargeNotify()
    if not XH.isEmbeddedApp() then
        return
    end
    print("yuhang onRechargeNotify")
    XH.sdkManager:onIapPayResult(XH.ThirdDefine.IAP_PAYRESULT.PAYRESULT_SUCCESS, "")
end
...
-- 请求钻石商品信息
function ShopModule:reqDiamondProductsInfoEx()
    if (not self._ymnProduct or not next(self._ymnProduct)) and not self._requestYmn then
        self:startReq("ReqDiamondProductsInfoEx", XH.configerManager:getShopAppId(), XH.configerManager:getShopPackageAppId())
        self._requestYmn = true
    end
end

-- 钻石信息结果
function ShopModule:respDiamondProductsInfoEx(req, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        data = data or {}
        self._rmbPropsAdd = data
        for _, v in pairs(data) do
            self._ymnProduct = self._ymnProduct or {}
            self._ymnProduct[v.productId] = v
        end
    end
    self._requestYmn = false
end

function ShopModule:getYmnProduct()
    return self._ymnProduct
end

function ShopModule:getRmbPropsAdd()
    return self._rmbPropsAdd
end
...
function ShopModule:respDiamondProductsInfo(req, type, data)
    ...
    if type == XH.Req.TYPE.SUCCESS then
        ...
        if (not self._ymnProduct or not next(self._ymnProduct)) then
            self:respDiamondProductsInfoEx(req, type, data)
        end
    else
...
function ShopModule:newFliterDataByArea(data)
    ...
    if XH.isEmbeddedApp() then
        for _, v in pairs(data) do
            if string.find(tostring(v.productId), tostring(5068)) then
                props[#props + 1] = v
            end
        end
        return props
...
function ShopModule:reqBuyDiamond(productInfo, scriptName, gifttype, autoOrder, bHideSuccessTip, propID, targetPropID, extParam)
    ...
    XH.TipTool.showLoading()
    self:recordBuyParam(productInfo, scriptName, gifttype, autoOrder, bHideSuccessTip, propID, targetPropID, extParam)
    self._bHideBuySuccessTip = bHideSuccessTip
    local gameId = XH.areaData:getConfigGameID() 
    local create_type = 0
    local room_level,limit
    if extParam then
        if extParam.triggerGameID then
            gameId = extParam.triggerGameID
        end
        if extParam.createType then
            create_type = extParam.createType
        end
        room_level = extParam.room_level
        limit = extParam.limit
    end
    ...
    if XH.isEmbeddedApp() then
        ...
        extData.embed_app_id = param.appID
        extData.embed_shop = 1
        extData.plugin_gold = 1
    end
    if productInfo.pay_attach then
        for k, v in pairs(productInfo.pay_attach) do
            extData[k] = v
        end
    end
    local ext = json.encode(extData)
    if extParam and extParam.ext then
        ext = extParam.ext
    end
    local proForpey = {
        order_id = "",
    ...
    elseif device.platform == "android" then
        if productInfo.paySupportStr then
            if productInfo.paySupportStr == "wechatPay" then
                XH.sdkManager:callFunctionWithMap(XH.ThirdDefine.SDK_INTERFACE_NAMES_KEY.WXBUY, {["0"] = proForpey })
            else
                XH.sdkManager:callFunctionWithMap(XH.ThirdDefine.SDK_INTERFACE_NAMES_KEY.ZFBBUY, {["0"] = proForpey })
            end
        elseif XH.isEmbeddedApp() then
            XH.TipTool.hideLoading()
            XH.viewManager:openView("PayTypeView", nil, proForpey,nil, productInfo)
        elseif XH.ChannelTool.checkIsECarChannel() then --车载渠道，传H5道具参数
            XH.TipTool.hideLoading()
            XH.viewManager:openView("PayTypeView", nil, proForH5, nil, productInfo)
...
function ShopModule:respExchangeProduct(req, type, data)
    ...
    -- 兑换成功通知
    self:dispatchEvent({name = self.EVENT_SHOP_BUY_ROOMCARD, data = {result = type ==  XH.Req.TYPE.SUCCESS}})
end
...
function ShopModule:respBuyDiamond(event)
    ...
    if code == 200 then
        ...
        local GetBankruptKey = function(buyParam)
            if not buyParam then
                return
            end
            local data = buyParam.productInfo
        ...
        if GetBankruptKey(self._buyParam) == YGiftBankruptcyDefine.ACTLIST.BankruptcyV2 then
            if XH.lobby:getModule("YGiftBankruptcyV2"):isPayWithGift(self._buyParam) then
                isShowTip = false
            end
        else
        ...
        if XH.lobby:getModule("GoldNew"):isPayWithGift(self._buyParam) then
            isShowTip = false
        end
        if XH.lobby:getModule("UnlimitGoldAct"):isPayWithGift(self._buyParam) or XH.lobby:getModule("UnlimitBoxAct"):isPayWithGift(self._buyParam) then
            isShowTip = false
        end
        if XH.lobby:getModule("UnlimitedGift"):isPayWithGift(self._buyParam) then
            isShowTip = false
        end
        if XH.lobby:getModule("RecallNew"):isPayWithGift(self._buyParam) then
            isShowTip = false
        end
        if XH.lobby:getModule("MonthlyCard"):isPayWithGift(self._buyParam) then
            isShowTip = false
        end
        tipStr = "购买成功"
        XH.playerData:flushPlayerDrop()
        self:dispatchEvent( { name = self.EVNET_DIAMOND_BUY_RESULT, msg = {resultTag = self.BUY_DIAMOND_RESULT.SUCCESS} })
        if self._buyParam and self._buyParam.productInfo and self._buyParam.productInfo.id then
    ...
    if isShowTip then
        if code == 200 then
            self:showTipsWithPhoneBind(tipStr)
        elseif code == 202 and self._buyParam and next(self._buyParam) then
            -- 会员模块不处理重新支付逻辑
...
-- 购买商品
function ShopModule:reqBuyProduct(productType, selectIdx, gift_cfg, actPrice, showTips)
    local props = self:getShopItemsWithId(productType)
    ...
    if productInfo.multi_score_config and #productInfo.multi_score_config > 0 then
        self:reqExchangeProduct(productType, productInfo.id, productInfo.multi_score_config[1].type, productInfo.value, gift_cfg, showTips, nil, productInfo.prop_type, actPrice)
    else
        self:reqExchangeProduct(productType, productInfo.id, nil, nil, nil, showTips, nil, productInfo.prop_type, actPrice)
    end
...
function ShopModule:onReqSystemTimesCallBack(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        ...
        if timestamp ~= nil then
            ...
            local DEBUG = require("app.Config.GlobalConfig").IsDebug
            if DEBUG then
                if self._diffTime > 24 * 3600 then
                    XH.TipTool.showToast("来自测试环境的提示:本地时间差距大于1天，请注意")
                end
            end
        end
    end
...
function ShopModule:showTipsWithPhoneBind(tipStr)
    ...
    if XH.playerData:getPlayerPhone() == "" and getPhoneBindState == "" and phoneBindShopTimes == 0 and not XH.isEmbeddedApp() then
...
-- 请求指定标签下的商品
function ShopModule:reqShopProductByConfig(id, hideLoaing, callback)
    if not hideLoaing then
        XH.TipTool.showLoading()
    end
    self._hasReqItems = id
    self._reqShopProductByConfigCallback = callback
...
function ShopModule:onReqShopProductByConfigCallBack(req, type, data)
    if type == XH.Req.TYPE.SUCCESS then
            ...
            if specialTag == ShopConfig.SpecialTag.SXVIP then
                ...
                for merge_key, goodsList in pairs(mergedGoodTable) do
                    ...
                    for _, goods in ipairs(goodsList) do
                        ...
                        if goods.tags and goods.tags[1] and goods.tags[1].key then --根据标签记录商品
        ...
        if self._reqShopProductByConfigCallback then
            self._reqShopProductByConfigCallback(data)
        end
        self._reqShopProductByConfigCallback = nil
...
function ShopModule:reqPayConsult(totalAmount)
    self._payOperationInfo = nil
    self._preConsultId = nil

    if device.platform == "android" and XH.sdkManager:isSupportFunctionAndroid("alp_open_iap") == "true" then
        -- 2.0支付
        local phone = XH.playerData:getRealPhone()
        if phone == nil or phone == "" then
            return
        end
        -- 请求接口
        self:startReq("ReqPayConsult", phone, totalAmount)
    end
end

function ShopModule:onPayConsultCallBack(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        self._payOperationInfo = nil
        local operationInfo = nil
        if data and data.channelInfoList and data.channelInfoList[1] then
            local channelInfo = data.channelInfoList[1]
            if channelInfo.operationList and channelInfo.operationList[1] and channelInfo.operationList[1] then
                operationInfo = channelInfo.operationList[1]
                if operationInfo.viewData and operationInfo.viewData.payOperationInfo then
                    self._payOperationInfo = operationInfo.viewData.payOperationInfo
                end
            end
        end
        self._preConsultId = data.preConsultId
        if self._payOperationInfo and self._preConsultId and operationInfo then
            self:dispatchEvent({name = self.EVENT_PAY_CONSULT, msg = {payConsultInfo = operationInfo.viewData.operationTip}})
        end
    end
end
...
function ShopModule:recordBuyParam(productInfo, scriptName, gifttype, autoOrder, bHideSuccessTip, propID, targetPropID, extParam)
    self._buyParam = { }
    ...
    self._buyParam.extParam = extParam
end
...
function ShopModule:addPayOperationInfo(data)
    local jsonData = json.decode(data)
    if self._payOperationInfo and self._preConsultId then
        jsonData.payOperationInfo = self._payOperationInfo
        jsonData.preConsultId = self._preConsultId
    end
...
function ShopModule:onPayTalkingDataSDK(orderId)
    ...
    if self._buyParam == nil or self._buyParam.productInfo == nil then -- 确保商品存在不报错
        return
    end
    local price
    if device.platform == "ios" then 
