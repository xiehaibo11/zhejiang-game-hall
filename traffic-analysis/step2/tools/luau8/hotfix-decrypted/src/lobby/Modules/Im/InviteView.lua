...
function IMInviteView:onTouchEventClose(send,eventType)
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.hyxt24052711, self:getThrowData("关闭"))
    self:close()
end
...
function IMInviteView:onTouchEventRefuse(send,eventType)
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.hyxt24052711, self:getThrowData("拒绝"))
...
function IMInviteView:onTouchEventWaite(send,eventType)
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.hyxt24052711, self:getThrowData("等我一会"))
...
function IMInviteView:onTouchEventAgree(send,eventType)
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.hyxt24052711, self:getThrowData("同意"))

    self:close()
end

function IMInviteView:getThrowData(strType)
    local data = {
        block_item_id = strType,
        block_label = "大厅-牌友",
        item_id = XH.areaData:getAreaID() .. "_" .. (self._info.invitePlayer or ""),
        game_id = XH.areaData:getAreaID() .. "_" .. (self._info.gameid or 0),
        room_id = self._info.roomid,
        players = self._info.charis
    }
    return data
end
