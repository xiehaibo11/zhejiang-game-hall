...
function IMMessageView:onTouchEventAgreeInvite(data, send, eventType)
    ...
    local posData = {
        block_item_id = "同意",
        block_label = "大厅-牌友",
        item_id = XH.areaData:getAreaID() .. "_" .. (data.fromNumid or ""),
        game_id = XH.areaData:getAreaID() .. "_" .. (self._joinInfo.gameid or 0),
        room_id = data.roomid,
        players = data.charis
    }
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.hyxt24052711, posData)
end
