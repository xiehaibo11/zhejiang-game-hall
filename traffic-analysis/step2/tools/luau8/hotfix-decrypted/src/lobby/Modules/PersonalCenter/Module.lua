...
function PersonalCenterModule:getVipLogInfo(page, page_size, needNew, pageType)
    if not self._vipLogInfo or needNew == true then
        self:reqVipLogInfo(page, page_size, pageType)
    else
        return self._vipLogInfo
    end
end

function PersonalCenterModule:reqVipLogInfo(page, page_size, pageType)
    self:startReq("ReqVipLogInfo", 10, page, page_size, pageType)
end
