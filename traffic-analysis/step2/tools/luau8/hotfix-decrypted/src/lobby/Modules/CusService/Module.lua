...
function CusServiceModule:getTempServiceUrl()
    ...
    if url then
        ...
        local nAvatar = ""
        local nChannelId = XH.SysTool:getChannelID()
        local deviceName = XH.SysTool:getChannelID(un.Device.getDeviceName())
        local appVersion = XH.SysTool:GetBundleVersion()
...
function CusServiceModule:oponCusServerView(isTempNumid,nSourceID)
    if self._ConfigJson or self:readConfig() then
        -- ios这个版本客服系统无法打开
        if device.platform == "ios" and (XH.SysTool:GetBundleVersion() == "1.3.46" or XH.SysTool:GetBundleVersion() == "1.3.45") then
            XH.sdkManager:openYmnUrl(self:getCusServerUrl(isTempNumid, nSourceID))
        elseif self._ConfigJson.externalLinkAredIds and not self._ConfigJson.AllExternalLink 
...
function CusServiceModule:getCusServerUrl(isTempNumid, nSource)
    --清除未读消息
    local serviceUrl
    self:reqCusServiceClearUnreadMsg()
    if nSource then
