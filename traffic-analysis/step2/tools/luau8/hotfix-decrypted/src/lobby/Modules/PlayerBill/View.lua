...
function PlayerBillView:getBindingInfo()
    return {
        ...
        ["_KW_BTN_ALLWIN"] = { varName = "_btnAllWin", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventAllWin" },
        ["_KW_BTN_ALLWIN_GOLD"] = { varName = "_btnAllWinGold", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventAllWin" },
        ...
        ["_KW_BTN_BILL"] = { varName = "_btnBill", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventShowBill" },
        ["_KW_BTN_GOLD_BILL"] = { varName = "_btnGoldBill", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventShowGoldBill" },
        ["_KW_PANEL_LIST_GOLD"] = { varName = "_panelBillListGold" },
        ["_KW_PANEL_GOLD"] = { varName = "_panelGold" },
        ["_KW_TEXT_DATE_GOLD"] = { varName = "_textDateGold" },
        ["_KW_BTN_SHOW_DATE_GOLD"] = { varName = "_btnShowDateGold", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventShowDateGold" },
        ["_KW_BTN_HIDE_DATE_GOLD"] = { varName = "_btnHideDateGold", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventHideDateGold" },
        ["_KW_IMG_DATE_SELECT_GOLD"] = { varName = "_imgDateSelectGold" },
        ["_KW_TEXT_NOTICE_NO_BILL_GOLD"] = { varName = "_textNoticeNoBillGold" },
        ["_KW_PANEL_OPEN_VIP"] = { varName = "_panelOpenVip" },
        ["_KW_BUTTON_OPEN_VIP"] = { varName = "_btnOpenVip", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventOpenVip" },
        ["_KW_BTN_DATE_SELECT_GOLD_1"] = { varName = "_btnDateSelectGold_1", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_2"] = { varName = "_btnDateSelectGold_2", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_3"] = { varName = "_btnDateSelectGold_3", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_4"] = { varName = "_btnDateSelectGold_4", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_5"] = { varName = "_btnDateSelectGold_5", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_6"] = { varName = "_btnDateSelectGold_6", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
        ["_KW_BTN_DATE_SELECT_GOLD_7"] = { varName = "_btnDateSelectGold_7", type = XH.UI_TYPE.BUTTON, onTouchEnded = "onBtnEventDateSelectGold" },
    }
end
...
function PlayerBillView:ctor(param)
    ...
    self._tab =  param.tab or cc.UserDefault:getInstance():getStringForKey("btn_tab_bill", XH.PlayerBillTab.battle)
    self:initData()
    self:initView()
...
function PlayerBillView:initGoldBill()
    local showGoldTab = false
    if self._tab == XH.PlayerBillTab.gold then
        showGoldTab = true
    end
    self._btnBill:setBright(showGoldTab)
    self._btnGoldBill:setBright(not showGoldTab)
    self._btnBill:setTouchEnabled(showGoldTab)
    self._btnGoldBill:setTouchEnabled(not showGoldTab)
    self._panelAdapt:setVisible(not showGoldTab)
    self._panelGold:setVisible(showGoldTab)

    if XH.lobby:getModule("Sxvip"):isExpire() then
        self._panelOpenVip:setVisible(true)
        self._textNoticeNoBillGold:setVisible(false)
    else
        self._panelOpenVip:setVisible(false)
    end
end

function PlayerBillView:onBtnEventOpenVip()
    XH.viewManager:openView("SxvipPrivilegeView", nil, SxVipConfig.ThrowData.source.goldRecord)
    XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.hy24090628)
end

function PlayerBillView:onRemainder(event)
    local data = event.data
    if data.afterBuy then
        self._panelOpenVip:setVisible(false)
        XH.lobby:getModule("PlayerBill"):reqGoldBill(self._curBillTimeIndexGold)
    end
end

function PlayerBillView:onBtnEventShowBill()
    self._btnBill:setBright(false)
    self._btnGoldBill:setBright(true)
    self._btnBill:setTouchEnabled(false)
    self._btnGoldBill:setTouchEnabled(true)
    self._panelAdapt:setVisible(true)
    self._panelGold:setVisible(false)

    local module = XH.lobby:getModule("PlayerBill")
    module:readPlayerBillInfo(self._curBillTimeIndex)
    cc.UserDefault:getInstance():setStringForKey("btn_tab_bill", XH.PlayerBillTab.battle)
end

function PlayerBillView:onBtnEventShowGoldBill()
    self._btnBill:setBright(true)
    self._btnGoldBill:setBright(false)
    self._btnBill:setTouchEnabled(true)
    self._btnGoldBill:setTouchEnabled(false)
    self._panelAdapt:setVisible(false)
    self._panelGold:setVisible(true)
    XH.lobby:getModule("PlayerBill"):reqGoldBill(self._curBillTimeIndexGold)
    cc.UserDefault:getInstance():setStringForKey("btn_tab_bill", XH.PlayerBillTab.gold)

    XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.hy24090627)
end

function PlayerBillView:onBtnEventShowDateGold(send, eventType)
    self:showDateSelectGold()
end

function PlayerBillView:onBtnEventHideDateGold(send, eventType)
    self:closeDateSelectGold()
end

function PlayerBillView:closeDateSelectGold()
    self._imgDateSelectGold:setVisible(false)
    self._btnShowDateGold:setVisible(true)
    self._btnHideDateGold:setVisible(false)
end

function PlayerBillView:showDateSelectGold()
    self._imgDateSelectGold:setVisible(true)
    self._btnShowDateGold:setVisible(false)
    self._btnHideDateGold:setVisible(true)
end

function PlayerBillView:onBtnEventDateSelectGold(send, eventType)
    self:closeDateSelectGold()
    local index = send:getTag()
    self:selectDateGold(index)
end

function PlayerBillView:selectDateGold(index)
    if index == self._curBillTimeIndexGold then
        return
    end
    local dateStr = self["_btnDateSelectGold_" .. index]:getTitleText()
    self._textDateGold:setString(dateStr)
    self._curBillTimeIndexGold = index
    XH.lobby:getModule("PlayerBill"):reqGoldBill(self._curBillTimeIndexGold)
end

--刷新战绩总列表
function PlayerBillView:onUpdateTotalListGold(event)
    self._billInfoListGold = clone(XH.lobby:getModule("PlayerBill"):getGoldBillInfo())
    self:refreshTableViewGold()
    self._textNoticeNoBillGold:setVisible(#self._billInfoListGold == 0)
end

function PlayerBillView:refreshTableViewGold(isCleanup)
    if self._tvInfoListGold ~= nil and isCleanup then
        self._tvInfoListGold:removeFromParent()
        self._tvInfoListGold = nil
    end
    if self._tvInfoListGold == nil then
        self._tvInfoListGold = cc.TableView:create(self._panelBillListGold:getContentSize())
        self._tvInfoListGold:setDirection(cc.SCROLLVIEW_DIRECTION_VERTICAL)
        self._tvInfoListGold:setVerticalFillOrder(cc.TABLEVIEW_FILL_TOPDOWN)
        self._tvInfoListGold:setIgnoreAnchorPointForPosition(false)
        self._tvInfoListGold:setAnchorPoint(cc.p(0, 0))
        self._tvInfoListGold:setPosition(cc.p(0, 0))
        self._panelBillListGold:addChild(self._tvInfoListGold)
        self._tvInfoListGold:registerScriptHandler(handler(self, self.onTableCellTouchedGold), cc.TABLECELL_TOUCHED)
        self._tvInfoListGold:registerScriptHandler(handler(self, self.onCellSizeForTableGold), cc.TABLECELL_SIZE_FOR_INDEX)
        self._tvInfoListGold:registerScriptHandler(handler(self, self.onTableCellAtIndexGold), cc.TABLECELL_SIZE_AT_INDEX)
        self._tvInfoListGold:registerScriptHandler(handler(self, self.onNumberOfCellsInTableViewGold), cc.NUMBER_OF_CELLS_IN_TABLEVIEW)
    end
    if self._tvInfoListGold ~= nil then
        self._tvInfoListGold:reloadData()
    end
end

function PlayerBillView:onTableCellTouchedGold(tableView, cell)
end

function PlayerBillView:onCellSizeForTableGold(tableView, index)
    local size = self:getListPanel():getContentSize()
    return size.width, 220
end

function PlayerBillView:onTableCellAtIndexGold(tableView, index)
    local cell = tableView:dequeueCell()
    if not cell then
        cell = cc.TableViewCell:new()
        local infoItem = require("lobby.Modules.PlayerBill.GoldListItem"):new()
    ...
    if infoItem ~= nil then
        infoItem:updateInfo(self._billInfoListGold[index + 1], index + 1)
    end
    return cell
end

function PlayerBillView:onNumberOfCellsInTableViewGold(tableView)
    return #self._billInfoListGold
end
...
function PlayerBillView:adaptForLiuHai()
    ...
    self._panelGold:setContentSize(cc.size(nowSize.width - notchWidth*2, nowSize.height))
    ccui.Helper:doLayout(self._panelGold)
end
...
function PlayerBillView:getProxyEvents()
    ...
    return {
        ...
        { module = playerBillModule, eventKeyName = playerBillModule.EVENT_HALL_BILL_UPDATE_TOTALLIST_GOLD,  callBack = "onUpdateTotalListGold" },
...
function PlayerBillView:initDateSelect()
    ...
    for i = 1, self.MaxDateSelectCount do
        ...
        if self["_btnDateSelectGold_"..i] then
            self["_btnDateSelectGold_"..i]:setTag(i)
            self["_btnDateSelectGold_"..i]:setPositionY(20*(showCount-i+1)+40*(showCount-i+1/2))
            if i > showCount then
                self["_btnDateSelectGold_"..i]:setVisible(false)
            end
        end
    end
end
...
function PlayerBillView:onUpdateCurTime(event)
    ...
    self._textDateGold:setString(curBillDateStr)

    local showCount = XH.lobby:getModule("PlayerBill").SHOW_BILL_DAY
    self._curBillTimeIndex = showCount
    self._curBillTimeIndexGold = showCount
    ...
    for i = 1, showCount do
        ...
        if self["_btnDateSelectGold_"..i] then
            self["_btnDateSelectGold_"..i]:setTitleText(os.date("%Y-%m-%d", curTime - 86400 * (showCount - i)))
        end
    end
end
...
-- 总优胜值回调
function PlayerBillView:onBtnEventAllWin(send, eventType)
   local SxvipConfig = require("lobby.Modules.Sxvip.Config")
   if send == self._btnAllWin then
      XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.hy25052602, {block_item_id = self.hasFinger and "有特效" or "无特效", page_item_id = "对局战绩"})
      XH.viewManager:openView("SxvipView", nil, {defalutMenu = SxvipConfig.MENU_DEFINE.STAITISTICS})
   else
      XH.lobby:getModule("Sxvip.ThrowData"):throwData(XH.NewThrowDataDefine.hy25052602, {block_item_id = self.hasFinger and "有特效" or "无特效", page_item_id = "金币场战绩"})
...
-- 展示手指动画
function PlayerBillView:showSxvipFinger()
    ...
    if XH.lobby:getModule("Sxvip"):isExpire() and date ~= XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_SXVIPS_SHOW_FINNER_DATE, 0) then
        ...
        local aniNodeGold = self:createFingerAniNode()
        if aniNodeGold then
            aniNodeGold:setPosition(100, 10)
            self._btnAllWinGold:addChild(aniNodeGold)
        end
...
function PlayerBillView:createFingerAniNode()
