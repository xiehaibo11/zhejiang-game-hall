...
local scenePosQB = scenePos
scenePosQB.game_end = {posX = 0, posY = 100, showIcon = "all", direction = "y"}
        ...
        if showData then
            for scene, iconInfo in pairs(showData) do
                if showType == scene and iconInfo.status == 1 and next(iconInfo.game_sort) ~= nil then
                    if self._gameid == 42038 then
                        self:setPanelBGPos(scenePosQB[showType].posX, scenePosQB[showType].posY)
                    else
