...
function ReqGoldFirstPayGift:start(actId, lb_id, day)
    if ReqGoldFirstPayGift.super.start(self) == false then
        return
    end
    local reqData = {act_id = actId, lb_id = lb_id, ask_id = self._askId}
    ...
    XH.httpManager50:RequestPost(XH.HttpDefine.HTTP_ID_GOLD_PEGP_GETAWARD_REQUEST, UrlConf.GOLD_PEGP_GETAWARD_REQUEST, jsonStr,
...
function ReqGoldFirstPayGift:onHttpCallBack(httpID, status, response)
    if not httpID or httpID ~= XH.HttpDefine.HTTP_ID_GOLD_PEGP_GETAWARD_REQUEST then
        return
    end
