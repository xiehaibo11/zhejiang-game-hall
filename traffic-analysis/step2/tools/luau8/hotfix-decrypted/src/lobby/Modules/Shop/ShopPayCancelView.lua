...
function ShopPayCancelView:onTouchEvenRePay(send, event)
    if self._data and next(self._data) then
        XH.lobby:getModule("Shop"):reqBuyDiamond(self._data.productInfo, self._data.scriptName, self._data.gifttype, self._data.autoOrder, self._data.bHideSuccessTip, self._data.propID, self._data.targetPropID, self._data.extParam)
    end
    pcall(function()
        XH.lobby:getModule("ReviveGift"):rePay()
    end)
