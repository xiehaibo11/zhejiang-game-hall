    ...
    if ctype == XH.Req.TYPE.SUCCESS then
        ...
        if actData and actData.award and #actData.award > 0 then
            ...
            for i = 1, #actData.award do
                ...
                local propCnt = actData.award[i].propCount
                if self._awards[i] then
                    if self._awards[i].propId == XH.areaData:getPropLimitedTimeRoomCardID() then
...
function IdentityCheckModule:reqDefendIdentity(strName, strIDNum, isAutoBindPhone, isSupplement)
    XH.TipTool.showLoading()
    self._isAutoBindPhone = isAutoBindPhone or false
    self._isSupplement = isSupplement or false
...
function IdentityCheckModule:setIsSdk()
    self._isSdk = true
end

function IdentityCheckModule:getIsSdk()
    return self._isSdk or false
end

function IdentityCheckModule:flushGoldCoin()
    local gameID = XH.areaData:getCommonGoldGameID()
    local srsGroupID = XH.areaData:getSrsGroupID()
    self:startReq("ReqPlayerGoldCoin", gameID, srsGroupID, 10)
end

function IdentityCheckModule:onTranPlayerGoldCallBack(cell, type, data)
    if type == XH.Req.TYPE.SUCCESS then
        if data.purse then
...
function IdentityCheckModule:postIdentityData(state)
    if XH.lobby and XH.lobby:getModule("UserPath") then
        local eventData = {
            duration = os.time() - (self._startTime or 0),
            block_label = self._isSdk and "支付宝" or "手动",
            customize = {
                choice = XH.lobby:getModule("UserPath"):firstStart(),
                state = state or "未知"
            }
        }
        XH.lobby:getModule("UserPath"):throwDataRepeat(XH.NewThrowDataDefine.xyh24081406, eventData, true, self._isSupplement)
    end
end

function IdentityCheckModule:postPhoneBindData(state)
    if XH.lobby and XH.lobby:getModule("UserPath") then
        local eventData = {
            duration = os.time() - (self._startTime or 0),
            block_label = self._isSdk and "支付宝" or "手动",
        ...
        XH.lobby:getModule("UserPath"):throwDataRepeat(XH.NewThrowDataDefine.xyh25011002, eventData, true)
    end
end
