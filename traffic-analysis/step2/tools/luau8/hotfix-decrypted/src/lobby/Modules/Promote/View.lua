...
function PromoteView:ctor(isAdmin, isOwner, teaNumber, owner_numid, worldPos)
    ...
    if lobbyJsonData.promoteImg then
        self._KW_IMG_PROMOTE:removeAllChildren()
		local size = self._KW_IMG_PROMOTE:getContentSize()
		local headImg = XH.RemoteImage.new()
			:setPosition(size.width / 2, size.height / 2)
			:ignoreContentAdaptWithSize(false)
			:setContentSize(size.width, size.height)
			:addTo(self._KW_IMG_PROMOTE)
		headImg:setUrl(lobbyJsonData.promoteImg,true)
    end
    local title1 = PromoteConfig[XH.areaData:getLobbyID()].imgTitle1
    if title1 and self._title1 then
        self._title1:loadTexture(title1, ccui.TextureResType.localType)
        self._title1:ignoreContentAdaptWithSize(true)
    end
    local titleUser1 = PromoteConfig[XH.areaData:getLobbyID()].imgTitleUser1
    if titleUser1 and self._titleUser1 then
        self._titleUser1:loadTexture(titleUser1, ccui.TextureResType.localType)
        self._titleUser1:ignoreContentAdaptWithSize(true)
    end
...
function PromoteView:getBindingInfo()
    return {
        ...
        ["_title1"] = {varName = "_title1"},
        ["_titleUser1"] = {varName = "_titleUser1"},
        -- rule
        ...
        ["_LABLE_OWNER_RIGHT_REWARD_2"] = {varName = "_LABLE_OWNER_RIGHT_REWARD_2"},
        -- owner
        ...
        ["_KW_BTN_USER_RIGHT_INVITE"] = {varName = "KW_BTN_USER_RIGHT_INVITE", onTouchEnded = "onClickManagerInvite"},
        ["_KW_IMG_PROMOTE"] = {varName = "_KW_IMG_PROMOTE"},
    }
end
...
function PromoteView:initUI()
    ...
    self:initTextByConfig("userTitle1", self.LABLE_USER_TITLE_1)
    self:initTextByConfig("userTitle2", self.LABLE_USER_TITLE_2)
    self.KW_BTN_INTRO:setVisible(false)
    local isFree = false
    ...
    if PromoteConfig[XH.areaData:getLobbyID()] ~= nil and PromoteConfig[XH.areaData:getLobbyID()].IGNORECREATE then
        self.KW_BTN_USER_LEFT_INVITE:setVisible(false)
        self.KW_BTN_USER_RIGHT_INVITE:setVisible(false)
    end
    ...
    if type == PromoteDefine.TASK_TYPE.USER then
        ...
        if type == PromoteDefine.TASK_TYPE.A then
            self:updateRightA(baseInfo, taskInfo)
        elseif type == PromoteDefine.TASK_TYPE.C then
...
function PromoteView:updateLeftA(baseInfo, taskInfo)
    ...
    local text = string.gsub(PromoteConfig[XH.areaData:getLobbyID()]["teaView_ownerLeftTitle_A"], "y", baseInfo.owner_first_task_times)
    text = string.gsub(text,"z",taskInfo.first_task_award)
    self.LABLE_OWNER_LEFT_TITLE:setString(text)
    local repText = string.gsub(PromoteConfig[XH.areaData:getLobbyID()]["teaView_ownerLeftNum_A"],"z",taskInfo.first_task_award)
    self.LABLE_OWNER_LEFT_NUM:setString(repText)
...
function PromoteView:updateRightA(baseInfo, taskInfo)
    local hasMode = XH.lobby:getModule("Promote"):hasActPlayModes()
    local isOpenAct = XH.lobby:getModule("Promote"):isOpenAct()
    local resultText = string.gsub(PromoteConfig[XH.areaData:getLobbyID()]["teaView_ownerRightTitle_A"], "z", taskInfo.task.task_award)
    self.LABEL_OWNER_RIGHT_TITLE:setString(resultText)
    self.LABEL_OWNER_RIGHT_PROGRESS:setString("今日场次：" .. taskInfo.task.round .. "场")
    self.LABLE_OWNER_RIGHT_REWARD:setString("明日可领房卡：" .. taskInfo.task.round * taskInfo.task.task_award .. "张")
    self._LABLE_OWNER_RIGHT_REWARD_2:setString("累计可领房卡：" .. taskInfo.award.left_award .. "张")
    if self._isOwner then
        ...
        self.KW_BTN_OWNER_RIGHT_CREATE:setVisible(isOpenAct)
...
function PromoteView:updateRightC(baseInfo, taskInfo)
    ...
    if self.LABEL_OWNER_RIGHT_TITLE:getChildrenCount() == 0 then
        local textStr = "<font face='cocosStudio/Common/Font/fangzhengcuyuan.TTF' size='32' color='#EAB8B5'>" .. gameName .. "2/3/4人带" .. ruleName .. "玩法，比赛场每日对局达 <font color='#FFDE00'>" .. taskInfo.task_config.task_target .. "场</font> ，即可获得<font color='#FFDE00'>" .. taskInfo.task_config.task_award .. "房卡</font>奖励</font>"
...
function PromoteView:onClickIntro()
    ...
    local intro = time .. PromoteConfig[XH.areaData:getLobbyID()].ActUserRule
    if self._isAdmin or self._isOwner then
        intro = time .. PromoteConfig[XH.areaData:getLobbyID()].ActOwnerRule
    end
...
function PromoteView:noticeCreateTable()
    local ruleName = PromoteConfig[XH.areaData:getLobbyID()].RULE_NAME or '缺一色'
    XH.viewManager:openView("PromoteTipView", nil, PromoteDefine.TIP_TYPE.PLAYER_NOTICE, self._teaNumber, "当前比赛场无"..ruleName.."牌桌，可通知领队创建牌桌")
end
