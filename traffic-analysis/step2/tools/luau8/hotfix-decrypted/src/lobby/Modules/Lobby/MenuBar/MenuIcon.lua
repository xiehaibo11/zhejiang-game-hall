...
function LobbyMenuIcon.createIcon(iconID, bSmallIconBar)
    ...
    local param = {
        ...
        bShowWithSpineAndIcon = iconConf.SpineInfo ~= nil and iconConf.IconResName ~= nil,
...
function LobbyMenuIcon:ctor(info)
    ...
    if XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_ACT_ICON_CLICKED, false, {key = info.iconID}) then
        ...
        if info.bShowWithSpineAndIcon then
            self:initSpineAndIcon(info)
        end
    end
    self:setAnchorPoint(display.CENTER)
end

function LobbyMenuIcon:initTitle(ex)
    if not ex or not ex.Title or ex.Ttile == "" then
        return
    end
    if not self._txtTitle then
        if ex.Size then
            self:setContentSize(ex.Size)
        end
        local size = self:getContentSize()
        self._txtTitle = ccui.Text:create()
            :setFontName("res/cocosStudio/Common/Font/fangzhengcuyuan.TTF")
            :setFontSize(40)
            :setColor(cc.c3b(232, 200, 181))
            :setAnchorPoint(cc.p(0.5, 0.5))
            :setPosition(cc.p(size.width/2, size.height/2))
            :setString(ex.Title)
            :setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
            :setTextVerticalAlignment(cc.TEXT_ALIGNMENT_RIGHT)
            :setOpacity(255)
        self:addChild(self._txtTitle)
    end
end
...
function LobbyMenuIcon:initImageViewIcon(iconResPath, iconResName, iconScale)
    ...
    if not self._imgIcon then
        ...
        self._imgIcon:setCascadeOpacityEnabled(true)
...
function LobbyMenuIcon:showSpineAni(aniInfo)
    ...
    if aniInfo.RemoteKey and XH.RemoteSpineManager then
        ...
        anchor:setPosition(cc.p(size.width / 2 + (self._info.OffSetX or 0), size.height / 2 + (self._info.OffSetY or 0)))
        self:addChild(anchor)
        -- 对齐旧逻辑：SkeletonAnimation create scale（默认 0.6）+ IconAniScale 挂在父节点上
        local createScale = self._info.iconScaleSpine or 0.6
        local nodeScale = self._info.iconAniScale
        if nodeScale then
        ...
        if fb then
            -- SpineManager:playAni 无 create scale，用节点 setScale 近似
    ...
    if not aniInfo.JsonPath or not aniInfo.AtlasPath or not aniInfo.ArmatureName then
        return
    end
    ...
    local params = {path = "", tex = aniInfo.JsonPath, ske = aniInfo.AtlasPath, armatureName = aniInfo.ArmatureName, scale = self._info.iconScaleSpine or 0.6}
    local size = self:getContentSize()
    local armature = display.playDargonBonesSpine(params)
    if armature then
        armature:setVisible(false)
        armature:runAction(cc.Sequence:create(cc.DelayTime:create(1 / 30), cc.Show:create()))
...
function LobbyMenuIcon:initRedPoint()
    ...
    -- 防止lobby.plist被释放
    cc.SpriteFrameCache:getInstance():addSpriteFrames("cocosStudio/hall/Image/lobbyNew/_Plist.plist")
    local size = self:getContentSize()
    ...
    iconSprite:setSpriteFrame(self._info.RedPointRes or LobbyConfig.RedPointResName)
...
function LobbyMenuIcon:initProgressView(progressInfo)
    ...
    if progressInfo.progressSize then
        ...
        sprite:setContentSize(progressInfo.progressSize)
    end
...
function LobbyMenuIcon:initSpineAndIcon(info)
    self:initImageViewIcon(info.iconResPath, info.iconResName, info.iconScale)
    if info.iconSize then
        self:setContentSize(info.iconSize)
    end
    self:showSpineAni(info.spineInfo)
end
...
function LobbyMenuIcon:flushBottomTextImg()
    ...
    local imgWidth = textSize.width + 40
    self._tipTextImg:setContentSize(cc.size(imgWidth, orgImgSize.height))
    self._tipText:setPositionX(imgWidth/2)
    self._tipTextArrowImg:setPosition(cc.p(self._tipTextImg:getContentSize().width/2 , 8))
    self._tipTextArrowImg:setVisible(true)
end
...
function LobbyMenuIcon:initTipText()
    ...
    if textConf.PlistName then
        cc.SpriteFrameCache:getInstance():addSpriteFrames(textConf.PlistName)
    end
    local tipTextImg = ccui.ImageView:create()
    tipTextImg:loadTexture(textConf.BgResName, ccui.TextureResType.plistType)
    tipTextImg:setCascadeOpacityEnabled(true)
    ...
    if textConf.BgArrowResName then
        local imgArrow = ccui.ImageView:create()
        imgArrow:loadTexture(textConf.BgArrowResName, ccui.TextureResType.plistType)
        imgArrow:setCascadeOpacityEnabled(true)
        imgArrow:setAnchorPoint(0.5, 0.5)
        tipTextImg:addChild(imgArrow)
        tipTextImg:setCascadeColorEnabled(true)
        self._tipTextArrowImg = imgArrow
    end
...
function LobbyMenuIcon:flushBottomTipText()
    if self._tipTextImg then
        local size = self:getContentSize()
        self._tipTextImg:setPosition(cc.p(size.width/2 , size.height/2 + 38))
        self._tipTextImg:setContentSize(cc.size(0, 72))
        self._tipTextArrowImg:setVisible(false)
        self._tipTextArrowImg:setRotation(180)
    end
end
...
function LobbyMenuIcon:initTipNews(pic, pos)
    if self._tipNewsImg then
        return
    end
    local tipNewsImg = ccui.ImageView:create()
    tipNewsImg:loadTexture(pic or LobbyConfig.TipNewsConfig.ResName, ccui.TextureResType.plistType)
    tipNewsImg:setAnchorPoint(LobbyConfig.TipNewsConfig.AnchorPoint)
    tipNewsImg:setPosition(pos or self:getNewsAndNewAwardPos())
    self:addChild(tipNewsImg)
    self._tipNewsImg = tipNewsImg
end

function LobbyMenuIcon:showTipNews(bShow, pic, pos)
    if bShow == nil then
        bShow = true
    end
    self:initTipNews(pic, pos)
