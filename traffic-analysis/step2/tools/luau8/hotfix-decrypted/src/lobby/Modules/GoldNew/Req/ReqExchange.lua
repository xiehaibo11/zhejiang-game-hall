...
function ReqExchange:start(areaId, windMsgId, numId, uid, gameId, productId, productType, timeoutTime, productTypeEx, phoneNum, richType, propValue,multiCfgId,gift_cfg,paramExTable, subItemType)
    ...
    if phoneNum then
        local urlParam = "areaid=%d&gameid=%d&id=%d&numid=%s&phone=%s&prop_type=%d&sid=%d&timestamp=%s&uid=%s"
        data = string.format(urlParam, areaId, gameId, productId, numId, phoneNum, propTypeEx, windMsgId, timestamp, uid)
    else
        local urlParam = "areaid=%d&gameid=%d&id=%d&numid=%s&prop_type=%d&sid=%d&timestamp=%s&uid=%s"
        data = string.format(urlParam, areaId, gameId, productId, numId, propTypeEx, windMsgId, timestamp, uid)
    end
    print(data)
    XH.httpManager50:RequestGet(HttpDefine.URL_STORE_EXCHANGE_PRODUCT, UrlConf.URL_EXCHANGE_EX .. "?" .. data, 4, handler(self, self.onHttpResult),nil,true)
end
...
function ReqExchange:reqSessionID(timeout)
    ...
    local ReqSessionID = XH.Bridge:require("app.Req.ReqSessionID")
