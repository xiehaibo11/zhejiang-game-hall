...
function ShopView:flushTopInfo()
    self._textNumGold:setText(XH.playerData:getStrSR())
...
function ShopView:onTouchEventOpenBagLayer(send, event)
    ...
    self:hide()
    XH.Bridge.viewManager:openView("GoldHallBagView", nil, {openType = "Lobby"})
...
function ShopView:close()
    ...
    XH.lobby:getModule("GoldNew"):closeFullView("shop", true)
end
...
function ShopView:startShowUI()
    ...
    if shopTags and #shopTags > 0 then
        ...
        if self._defaultShopType and self._defaultShopType == ShopConfig.SpecialTag.DIAMOND then
            ...
            XH.lobby:getModule("Shop"):reqShopProductByConfig(self._displaySecondType, true)
        end
