...
function PlayerBillInputView:getBindingInfo()
    return {
        ...
        ["_KW_INPUT_RECORD"] = { varName = "_inputRecord" },
...
function PlayerBillInputView:ctor()
    ...
    self._defaultText = ""
    self._inputText = ""
    self._is50PlayBack = cc.UserDefault:getInstance():getBoolForKey("KW_TEST_OPEN_GOLD50_PLAYBACK", false)
    self:initUI()
end
...
function PlayerBillInputView:initUI()
    ...
    if self._textRecord then
        ...
        self._textRecord:setVisible(not self._is50PlayBack)
    end
    if self._inputRecord then
        self._inputRecord:setVisible(self._is50PlayBack)
    end
end
...
function PlayerBillInputView:refreshNums()
    ...
    if self._inputRecord then
        if #self._inputText > 0 then
            self._inputRecord:setText(self._inputText)
        else
            self._inputRecord:setText(self._defaultText)
        end
    end
end

function PlayerBillInputView:onRecordClicked(send, eventType)
    -- 判断是否是金币回放，打开小黑块并且输入长度大于6的时候，就是金币回放
    if self._inputRecord:isVisible() then
        self._inputText = self._inputRecord:getText()
    end
    local is50 = self._is50PlayBack and string.len(self._inputText) > 8
    XH.lobby:getModule("PlayerBill"):startPlayBack(self._inputText, is50)
