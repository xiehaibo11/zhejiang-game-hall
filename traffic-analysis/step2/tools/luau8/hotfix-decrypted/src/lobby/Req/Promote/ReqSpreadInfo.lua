local ReqSpreadInfo = class("ReqSpreadInfo", XH.ReqHttp)
local UrlConf = require("app.Config.UrlConf")
local PromoteConfig = require("lobby.Modules.Promote.Config")

function ReqSpreadInfo:start(timeoutTime)
    if ReqSpreadInfo.super.start(self, timeoutTime) == false then
        return
    end
...
function ReqSpreadInfo:onHttpCallBack(eType, status, response)
    if status == 200 then
