...
function View:initUI()
    ...
    -- 关闭按钮
    self._closeBtn = tool.findNodeByName(self._csbRoot, "CLOSE")
    self._closeBtn:addTouchEventListener(handler(self, self.onClickClose))
    if self._versionInfo.updateType == UPDATE_TYPE.Force then
        ...
        if tip.param then
            ...
            for i = 1, #param do
                if param[i] == "fileSize" then
                    print(self._versionInfo[param[i]])
                    print(self._versionInfo.fileSize)
                    data[i] = self._versionInfo[param[i]] / 1024
                else
