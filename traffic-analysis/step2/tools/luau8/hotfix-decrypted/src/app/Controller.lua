...
function Controller:onEnterForeground()
    ...
    elseif cc.PLATFORM_OS_ANDROID == targetPlatform then
        if display.getRunningScene():getTag() == XH.SCENE_TAG.LOGIN then
            if XH.login then
                XH.login:getModule("Login"):reqGetTokenByTempUUID()
            end
            -- 登录界面不请求剪切板
            return
        end
    ...
    local module =  XH.lobby:getModule("Shop")
    if module then
        module:reqSystemTimes()
    end
    -- 从十三张出来到前台刷新下金币
    if XH and XH.needFlushProp then
        XH.playerData:flushPlayerDrop()
        XH.playerData:flushGoldCoin()
        XH.needFlushProp = false
    end
end
...
function Controller:enterLogin(isShowYouKe,isUserChangeArea)
    ...
    if XH.isEmbeddedApp() then
        ...
        XH.loginData:setLobbyID(param.lobbyID)
    end
    ...
    XH.lobby:getModule("BindPhone"):resetData()
