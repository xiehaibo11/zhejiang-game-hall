...
function hotfixListener:onFailed(key, error, msg, data)
	...
	elseif error == un.const.HotFixError.DOWNLOAD_VERSION_FILE_FAILED then
		if ResChecker._scene.setRetryFunc then
			ResChecker._scene:setRetryFunc(self.retryLobby)
		end
		ResChecker._scene:hotUpdateFaile("加载游戏失败，请前往系统设置，检查网络后重启游戏。", true, key, true)
    else
...
function hotfixListener:retryLobby()
	ResChecker._startHotFix()
end
...
-- @param fileSize      文件大小
function hotfixListener:onDownloadInfo(fileCount, fileSize)
	if ResChecker._scene then
		ResChecker._scene._updateCount = fileSize
	end
end
