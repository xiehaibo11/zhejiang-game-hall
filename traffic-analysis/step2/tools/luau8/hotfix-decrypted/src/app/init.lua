...
XH.isEmbeddedApp = function()
    if device.platform ~= "android" then
        return false
    end
    require("lobby.Modules.Plugin.Module")
    if device.platform == "windows" then
        XH.PluginModule.initAreaConfig()
        return true
    end
    if XH.bEmbeddedApp ~= nil then
        return XH.bEmbeddedApp
    end
    ...
    if ok then
        XH.bEmbeddedApp = ret == "1"
    else
        XH.bEmbeddedApp = false
    end
    if XH.bEmbeddedApp then
        XH.PluginModule.initAreaConfig()
    end
    return XH.bEmbeddedApp
end
...
local initBase = function ()
    ...
    XH.ReqProtobuf50 = require("app.Base.Req.ReqProtobuf50")
    XH.Protobuf = require("app.Protobuf.Protobuf")
end
...
local initTool = function ()
    ...
    XH.TeachingTool = require("app.Tool.TeachingTool")
end
...
local initUI = function ()
    ...
    XH.UIPackageView = require("app.Base.UI.UIPackageView")
end
...
local initProtobuf = function ()
    XH.PyrrlaProtobuf = require("app.Protobuf.ProtoFile.PyrrlaProtobuf")
    XH.RheaProtobuf = require("app.Protobuf.ProtoFile.RheaProtobuf")
    XH.NyxProtobuf = require("app.Protobuf.ProtoFile.NyxProtobuf")
end
...
local initManager = function ()
    ...
    XH.SpineManager = require("app.Manager.SpineManager")
    XH.RemoteSpineManager = require("app.Manager.RemoteSpineManager")
    XH.VideoManager = require("app.Manager.VideoManager")
end
...
require("lobby.Modules.UITracking.HookingTouchEvent")
require("lobby.Modules.GoldNew.Bridge")
XH.Creator2Interface = require("lobby.Modules.LianYun.Creator2Interface")

XH.goldLobbyRootPath = "lobby"
X 