...
function TeaHouseManager:onReciveMatchLinkMsg(XYID, buff, len)
    if XYID == XH.MatchLinkProtocol.RespInvite.XY_ID then
        ...
        -- 50金币场没位置信息，这边收到邀请会直接显示
        local isGoldRoom = CF and CF.roomData and CF.roomData:isGoldRoom() and CF.isInGameScene()
        if ((position.gameAppID and position.gameAppID ~= 0) or isGoldRoom) and XH.gameManager and not XH.lobby:getModule("Im"):getData():getIsSeer() then
            return
        end
    ...
    if respInvite.ucType == XH.MatchLinkProtocol.RespInvite.TYPE.TYPE_PROMOTE then
        ...
        if info.type == PromoteDefine.TIP_TYPE.PLAYER_NOTICE then
            ...
            if XH.SceneManager:getInstance():getTopSceneTag() ~= XH.SCENE_TAG.GAME then
                ...
                local str = "比赛场（" .. info.teaName .. "）玩家请求添加"..ruleName.."牌桌，是否稍后前往添加"
...
function TeaHouseManager:onRespInvite(XYID, buff, len)
    ...
    if respInvite.ucType == XH.MatchLinkProtocol.RespInvite.TYPE.TYPE_SEND then
        ...
        local agreeFunc = function()
            ...
            elseif XH.gameManager and XH.lobby:getModule("Im"):getData():getIsSeer() then
                XH.gameManager:leaveGame()
            elseif XH.gameManager and CF and XH.gameManager:isPlayBack() then
                XH.gameManager:leaveGame()
            end
            ...
            local data = {
                block_item_id = "确定",
                block_label = "比赛场-成员.最近对局",
                item_id = XH.areaData:getAreaID() .. "_" ..  respInvite.nFromNumid,
                game_id = XH.areaData:getAreaID() .. "_" .. (reciveInfo.gameID or ""),
                room_id = reciveInfo.boxRoomID or "",
                players = reciveInfo.chairs or 0
            }
    ...
    elseif respInvite.ucType == XH.MatchLinkProtocol.RespInvite.TYPE.TYPE_LEVEL_UP then
        local info = {}
        info.tip = "比赛场桌子已满，绑定手机后可以升级至高级比赛场增加桌子数量，是否绑定？"
        info.type = 0
        info.funcOK  = function()
            if self:isInTeaHouse() then
    ...
    elseif respInvite.ucType == XH.MatchLinkProtocol.RespInvite.TYPE.TYPE_RECALL_NEW then
        local reciveInfo = json.decode(respInvite.szMsg)
        local showTip = "玩家【" .. reciveInfo.nick .. "】邀请您到比赛场【" .. reciveInfo.teaName .. "(" .. reciveInfo.nPasswd  ..  ")】打一局"
        XH.TipTool.showTip({
			type = XH.TIP_LAYER_TYPE.OK,
            funcOK = function()
                XH.teaHouseManager:joinTeaHouse(reciveInfo.nPasswd)
            end
        }, showTip)
    end
end
...
function TeaHouseManager:reJoinTeaHouse(param)
    if not param then
        return
    end
    if XH.areaData:isSupportFriendsV2() and param.tableInfo then
        XH.lobby:getModule("Im"):reqWillJoinTable(param.tableInfo,true)
    else
        require("teahouse.init")
        if TeaHouse and TeaHouse.init and TeaHouse.reJoinTeaHouse then
            TeaHouse.init()
            TeaHouse.reJoinTeaHouse(param)
        end
    end
end
...
function TeaHouseManager:reqNotice(fromNumID, toNumID, msg, ucType)
    if not XH.areaData:getAreaID()  then
        return
    end
    local srsGroupID = XH.areaData:getSrsGroupID()
    ...
    reqInvite.nToAreaid = XH.areaData:getAreaID()
    reqInvite.nToNumid = toNumID
    reqInvite.ucType = ucType
    reqInvite.szMsg = msg
    XH.netEngine:sendProtocol(reqInvite, XH.MatchLinkProtocol.ReqInvite.processid, XH.areaData:getMatchLinkAppID() or 0, srsGroupID)
    TeaHouse.TipTool.showToast("通知已发送")
end

return TeaHouseManager�X