...
function MessageBoxView:ctor(param)
    ...
    if param.bLeft then
        ...
        self.KWA_MSG_LABEL:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT)
    else
        if self._KWA_MSG_LABEL_LEFT then
    ...
    if param.richText then
        local richText = ccui.RichText:createWithXML(param.msg, {})
        richText:setAnchorPoint(cc.p(0.5, 0.5))
        richText:ignoreContentAdaptWithSize(false)
        richText:setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
        local contentSize = self.KWA_MSG_LABEL:getContentSize()
        richText:setContentSize(contentSize)
        richText:setPosition(cc.p(contentSize.width/2, contentSize.height/2))
        self.KWA_MSG_LABEL:addChild(richText)
        self.KWA_MSG_LABEL:setString("")
    end

    self.KWA_BTND_OK_1:setScale9Enabled(false)
    self.KWA_BTND_OK_1:ignoreContentAdaptWithSize(true)
    self.KWA_BTND_OK_2:setScale9Enabled(false)
    self.KWA_BTND_OK_2:ignoreContentAdaptWithSize(true)
    self.KWA_BTND_CANCEL_2:setScale9Enabled(false)
    self.KWA_BTND_CANCEL_2:ignoreContentAdaptWithSize(true)
...
function MessageBoxView:initUI(msgString)
    ...
    if self.currentType == XH.TIP_LAYER_TYPE.CLOSE_ONLY then
        self.KWA_PANEL_BUTTON_LIST_1:setVisible(false)
        self.KWA_PANEL_BUTTON_LIST_2:setVisible(false)
...
function MessageBoxView:updateButtonOK1Image(imageUrl)
    self.KWA_BTND_OK_1:loadTextures(imageUrl, imageUrl, imageUrl, ccui.TextureResType.plistType)

end

function MessageBoxView:updateButtonOK2Image(imageUrl)
    self.KWA_BTND_OK_2:loadTextures(imageUrl, imageUrl, imageUrl, ccui.TextureResType.plistType)

end

function MessageBoxView:updateButtonCancelImage(imageUrl)
    self.KWA_BTND_CANCEL_2:loadTextures(imageUrl, imageUrl, imageUrl, ccui.TextureResType.plistType)
end

function MessageBoxView:updateButtonCanel2Image(imageUrl)
    self.KWA_BTND_CANCEL_2:loadTextures(imageUrl, imageUrl, imageUrl, ccui.TextureResType.plistType)
end

function MessageBoxView:setEnableBGTouch(enabled)
    self.bEnableBGTouch = enabled
end

function MessageBoxView:getCloseBtn()
    return self.KWA_BTND_CLOSE
end

return MessageBoxViewj