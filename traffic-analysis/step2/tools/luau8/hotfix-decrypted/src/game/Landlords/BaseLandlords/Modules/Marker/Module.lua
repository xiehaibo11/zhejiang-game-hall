local MarkerModule = CF.gameClass("MarkerModule", "game.BaseCardGame.Modules.Marker.Module")

function MarkerModule:getSubXYDealList()
    local xyTable = MarkerModule.super.getSubXYDealList(self)
    xyTable[#xyTable + 1] = {callback = handler(self, self.onMsgPlayCard), msgClass = CF.GameProtocol.msgPlayCard}
    return xyTable
end
