...
RoomCenterInfoView.KW_TABLE_BG_FILE_NAME = {
    ...
    [2] = "", -- 霞光胜境，动画里带了
    [3] = "res/tex/image/background_3.jpg",
    [4] = "res/tex/image/background_2.jpg",
    [5] = "res/tex/image/background_1.jpg",
}

local TABLE_BG_STYPE = { THREE_D = 1, TWO_D = 2,}

function RoomCenterInfoView:getProxyEvents()
    local proxyEvents = RoomCenterInfoView.super.getProxyEvents(self)
    proxyEvents[#proxyEvents + 1] = { module = CF.settingData, eventKeyName = "EVENT_LANDLORDS_TABLE_STYLE_CHANGED", callBack = "onTableBgChanged" }
    return proxyEvents
end
...
function RoomCenterInfoView:getTableBgFileStr()
    local tableStyle = CF.settingData:getLandlordsTableStyle()
    if CF.getLobbyModule("Sxvip"):isExpire() and CF.settingData:isVipTable(tableStyle) then
        tableStyle = 1
        CF.settingData:setLandlordsTableStyle(1)
    end
...
function RoomCenterInfoView:updateTableBgProp()
    ...
    if next(tableBgData) then
        ...
        if CF.settingData:getLandlordsTableStyle() == TABLE_BG_STYPE.THREE_D then
