...
function PlayerHeadNode:getBindingInfo()
    ...
    bindInfo["_KW_ING_DOUBLE"] = { varName = "_doubleFlagSp" }
...
function PlayerHeadNode:getProxyEvents()
    ...
    proxyEvents[#proxyEvents + 1] = { module = CF.game:getModule("PropUse"), eventKeyName = "EVENT_UPDATE_DOUBLE_FLAG", callBack = "updateDoubleFlag" }
    return proxyEvents
end
...
function PlayerHeadNode:initPlayerFlag()
    ...
    local doubleFlag = gameData:getDoubleData(self._seatId)
    local mddFlag = gameData:getMddData(self._seatId)
    if doubleFlag ~= nil and doubleFlag == true then
        self._doubleFlagSp:setVisible(true)
    else
        self._doubleFlagSp:setVisible(false)
    end
    if mddFlag ~= nil then
...
function PlayerHeadNode:updateDoubleFlag(event)
    local msg = event.msg
    local gameData = CF.game:getModule("GameLayer"):getGameData()
    if msg.bFlag[self._seatId] == true then
        self._doubleFlagSp:setVisible(true)
        gameData:setDoubleData(self._seatId, true)
    else
        self._doubleFlagSp:setVisible(false)
        gameData:setDoubleData(self._seatId, false)
    end
end
