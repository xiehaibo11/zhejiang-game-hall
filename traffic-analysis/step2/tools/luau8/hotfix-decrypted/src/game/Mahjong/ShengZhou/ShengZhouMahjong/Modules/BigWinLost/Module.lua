...
function BigWinLostModule:onMsgTotalResult(msgData)
    ...
    for seat = 0, CF.roomData:getMaxPlayer() - 1 do
        ...
        if player then
            ...
            local refannum = {}
			for k,v in pairs(WinLostFanConfig.KW_WINLOST_FAN_NUMS) do
				refannum[v] = k
			end

            if msgData.maxFanCount[seat] > 0 then
                tmpStrType[#tmpStrType + 1] = "最大番 " .. WinLostFanConfig.KW_WINLOST_FAN_TYPE[refannum[msgData.maxFanNum[seat]]] or ""
