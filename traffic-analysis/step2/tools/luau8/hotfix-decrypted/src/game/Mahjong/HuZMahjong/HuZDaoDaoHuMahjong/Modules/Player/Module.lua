...
function PlayerModule:getSubXYDealList()
    ...
    subXYDealList[#subXYDealList + 1] = {callback = handler(self,self.onMsgRequestIsPlayerIPLimit),msgClass = CF.GameProtocol.msgRequestIsPlayerIPLimit}
...
function PlayerModule:onMsgRespondIsPlayerIPLimit(msgData)
    ...
    if isIPLimit == true and CF.roomData:isIpLimit() then
        self:dealGPSLimitTip()
    end
end

function PlayerModule:onMsgRequestIsPlayerIPLimit(msgData)
    CF.msgManager:sendRequestIsPlayerIPLimit()
end
