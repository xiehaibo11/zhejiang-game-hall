local EarlyStartView = CF.gameClass("EarlyStartView", CF.ViewBase)
...
EarlyStartView.ClsName = "EarlyStartView"
EarlyStartView.ObjName = "EarlyStartView"

local statusChinese = {"待确认...", "同意", "拒绝", "同意"}
local statusColor = {cc.c3b(134, 134, 134), cc.c3b(9, 168, 1), cc.c3b(242, 51, 51), cc.c3b(9, 168, 1),}

function EarlyStartView:getCSBPath()
    return "cocosStudio/MahjongNew/GameLayer/CSB/TaiZhou/EarlyStart.csb"
end

function EarlyStartView:getBindingInfo()
    return {
...
function EarlyStartView:ctor(param)
    param = param or {}
    EarlyStartView.super.ctor(self, param)
...
function EarlyStartView:getProxyEvents()
    return {
        {module = CF.game:getModule("EarlyStart"), eventKeyName = "EVENT_UPDATE_EARLY_BEGIN_UI", callBack = "onUpdateEarlyQuickStartUI"},
    }
end

function EarlyStartView:createChildren()
...
function EarlyStartView:calPlayerInfo(seat, playerCount)
...
function EarlyStartView:changeToWaiteUI(isChange)
...
function EarlyStartView:setColockTime(tm)
    tm = tm or 0
...
function EarlyStartView:startclock(tm)
    tm = tm or 0
...
function EarlyStartView:updateImg(node, url)
    if not node then
        return
    end
...
function EarlyStartView:onUpdateEarlyQuickStartUI(event)
    local seat = event.msg.seat
    local status = event.msg.status or EarlyStartView.STATUS.DEFAULT
    ...
    if seat  then
        if status == EarlyStartView.STATUS.REQUEST then
...
function EarlyStartView:showSelf(zorder)
	zorder = zorder or CF.ZORDER.DIALOG
	local runningScene = display.getRunningScene()
    if runningScene then
        self:setName(EarlyStartView.ObjName)
        runningScene:addChild(self, zorder)
	end
	return self
end

function EarlyStartView:onBtnClose(send, eventType)
    CF.game:getModule("Dismiss"):sendRespondDismiss(false)
    self:removeSelf()
end

function EarlyStartView:onBtnAgreeEvent(send, eventType)
   CF.msgManager:sendGameStart()
   CF.msgManager:sendReqAdPlayerAgree(true)
   self:changeToWaiteUI(true)
end

function EarlyStartView:onBtnRefuseEvent(send, eventType)
