...
local UIMahTouchHandArea = CF.gameRequire("Modules.GameLayer.View2D.UIMahTouchHandArea")
local UIMahOutArea = CF.gameRequire("Modules.GameLayer.View2D.UIMahOutArea")
local UIMahPlayerOutArea = CF.gameRequire("Modules.GameLayer.View2D.UIMahPlayerOutArea")
...
local UIMahConfigDeadArea = CF.gameRequire("Modules.GameLayer.Config.UIMahConfigDeadArea")
local UIMahDeadAreaLayer = CF.gameRequire("Modules.GameLayer.View2D.UIMahDeadAreaLayer")

local UIMahLayer = CF.gameClass("UIMahLayer2D", "game.Mahjong.BasicMahjong.Modules.GameLayer.ViewBase.UIMahLayerBase")
...
function UIMahLayer:getProxyEvents()
    ...
    proxyEvents[#proxyEvents + 1] = {module = CF.roomData, eventKeyName = "EVENT_CARD_HOLDER_CHANGED", callBack = "onCardHolderChanged"}
    proxyEvents[#proxyEvents + 1] = {module = CF.settingData, eventKeyName = "EVENT_CLEAR_MODEL", callBack = "onEventClearModel"}
    proxyEvents[#proxyEvents + 1] = {module = CF.settingData, eventKeyName = "EVENT_MAH_OUT_CARD_STYLE_CHANGED", callBack = "onEventOutCardStyleChanged"}
    proxyEvents[#proxyEvents + 1] = {module = CF.settingData, eventKeyName = "EVENT_MAHJONG_CONFIG", callBack = "onEventFlushAllCardsConfig"}
    proxyEvents[#proxyEvents + 1] = {module = CF.settingData, eventKeyName = "EVENT_MAH_TABLE_OUTMAH_TYPE", callBack = "onEventOutTableMahType"}
    proxyEvents[#proxyEvents + 1] = {module = CF.game:getModule("WinLost"), eventKeyName = "EVENT_HIDE_SHOWDEAD_BTN", callBack = "onEventHideShowDeadBtn"}
    proxyEvents[#proxyEvents + 1] = {module = CF.game:getModule("ChangeCard"), eventKeyName = "EVENT_UPDATE_SURPLUS_MAH", callBack = "updateSurPlusMah"}
    proxyEvents[#proxyEvents + 1] = { module = XH.lobby:getModule("Lobby"), eventKeyName = "EVENT_WIN_SIZE_CHANGE", callBack = "onWinSizeChange" }
    return proxyEvents
end
...
function UIMahLayer:ctor(param, rootNode)
    ...
    CF.UITool.adaptForLiuHai({self._posDead_2,self._deadPanel})
end

function UIMahLayer:onWinSizeChange()
...
function UIMahLayer:setOpenMahs(openMahs)
    ...
    if self._openMahs.setShowFlowerMah then
        self._openMahs:setShowFlowerMah(self:getGameData():getOpenMahToFlower())
    end
...
function UIMahLayer:onEventOutTableMahType(event)
    if event.data then
        ...
        local fourDirVisibleFunc = function(isFourDir)
            for lSeat = 1 , CF.roomData:getMaxPlayer() do
        ...
        if event.data == CF.GameDefine.OUTMAHS_PUT_TYPE.FOUR_DIRECTION then
            for seat = 0 , CF.roomData:getChairs() do
                local event1 = {data = {localSeat = CF.roomData:seatToLocal(seat)}}
...
function UIMahLayer:onTouchEventShowDeadMah(send,eventType)
    ...
    CF.throwDataManager:throwDataClick(CF.ThrowDataDefine.TABLE_30)
end
...
function UIMahLayer:onEventShowOutMahAction(event)
    ...
    -- 旁观玩家显示大牌
    if CF.roomData and CF.roomData:getIsSeer() and CF.settingData:getOutStyle() == 2 then
        self:showOutMahToBig(localSeat, event.data.mah)
        return
    end
    if localSeat ~= CF.GameDefine.LOCAL_SEAT.BOTTOM and CF.settingData:getOutStyle() == 2 then
        self:showOutMahToBig(localSeat, event.data.mah)
        return
    end
    ...
    -- 复制牌桌要显示的牌
    local config = CF.game:getModule("GameLayer"):getMahAllConfig(nil, true)
    local cloneNode = UIMah.new(CF.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TOUP, config)
    ...
    -- 起飞
    if CF.settingData:getOutMoveStyle() == 1 then
...
function UIMahLayer:onEventOutCardStyleChanged(event)
    local config = {}
    config.handMahsPlayType = CF.settingData:getMahOutCardStyle()
...
function UIMahLayer:updateSurPlusMah()
    if self._surPlusMahBackMah then
        self._surPlusMahBackMah:_onSetConfig({
            [UIMahConfig.MahSettingKey.BackColorType] = 1,
            [UIMahConfig.MahSettingKey.LightType] = 1,
            [UIMahConfig.MahSettingKey.BackPatternType] = 1,
            [UIMahConfig.MahSettingKey.ShapeType] = 1,
        })
    end
end
...
function UIMahLayer:_getLastOutMahCursor()
    if not self._cursorImg then
        ...
        CF.UITool.playJsonAnimationByName(self._cursorImg,"gb_ani","res/animation/Mahjong/Base/gb_ani/gb_ani.ExportJson",1, "Animation1", cc.p(0,0))
...
function UIMahLayer:_getHandAreaRootNode(localSeat)
    ...
    if localSeat == CF.GameDefine.LOCAL_SEAT.RIGHT and self._maxHandMahCount == CF.GameDefine.MAX_HAND_COUNT_TYPE.SEVENTEEN then
...
function UIMahLayer:_updateBottomFlowerAreaPosition()
    local flowerArea = self._flowerAreas[CF.GameDefine.LOCAL_SEAT.BOTTOM]
...
function UIMahLayer:_getPlayerOutAreaMaxLineCount()
    ...
    if CF.configData:getHaveFlower() then
        return 2
    end
    return 3
end
...
function UIMahLayer:onShowDeadAreaMahBtn()
    if self:isShowDeadMahButton(CF.roomData:getGameID(),CF.roomData:getChairs(),nil) == false then return end 
    if self._showDeadMahButton and self._showDeadMahButton:isVisible() == true  then 
        return 
    else
        self._showDeadMahButton:setVisible(true)
        local deadBtnPos = self:getShowDeadButtonPos(CF.roomData:getChairs())
