...
function BigWinLostModule:onMsgTotalResult(msgData)
    ...
    for seat = 0, CF.roomData:getMaxPlayer() - 1 do
        ...
        if player then
            ...
            local tmpStrType = {"最高 " .. msgData.maxScoreOfTotal[seat] .. " 次", "累计 " .. msgData.m_iFlowerNum[seat] .. " 花",
                                "自摸 " .. msgData.maxWinByOwnNum[seat] .. " 次","接炮 " .. msgData.maxJiePaoNum[seat] .. " 次"}
...
function BigWinLostModule:calcBigWinnerSeat(totalScore)
    ...
    for i = 0, #totalScore do
        if maxScore < totalScore[i] then
            maxScore = totalScore[i]
            isOnlyBigWinner = true
        elseif maxScore == totalScore[i] then
            isOnlyBigWinner = false
        end
    end
    if not isOnlyBigWinner then
        bigWinnerSeat = CF.roomData:getMaxPlayer() + 1
    end
