...
function SpecialAniLayer:getProxyEvents()
    return {
        ...
        {module = CF.game:getModule("GameLayer"), eventKeyName = "EVENT_SHOW_WIN_ANI", callBack = "onShowPlayHuAni"},
    }
end
...
function SpecialAniLayer:onShowPlayHuAni(event)
    local msg = event.msg
    local WinLostFanConfig = CF.gameRequire("Config.WinLostFanConfig")
    local aniName = WinLostFanConfig.FANID_TO_ANI[msg.fanID]
    ...
    if playAniNode then
        ...
        playAniNode:removeAllChildren()
        playAniNode:setVisible(true)
        local aniPath = "res/animation/Mahjong/HuZhou/pxtx_ani_huzhou/pxtx_ani_huzhou.ExportJson"
        CF.UITool.playJsonAnimationByName(playAniNode, "pxtx_ani_huzhou", aniPath, 0, aniName, nil, nil, function()
            playAniNode:setVisible(false)
        end)
    end
end
