...
function PlayerView:getProxyEvents()
    ...
    proxyEvents[#proxyEvents + 1] = { module = CF.game:getModule("EarlyStart"), eventKeyName = "EVENT_DYNAMIC_CHANGE_SEAT", callBack = "dynamicChangeSeat" }
    return proxyEvents
end
...
function PlayerView:getHeadPosByLocalSeat(localSeat)
    ...
    if localSeat == 3 then
        return cc.p(self["_headPos"..localSeat]:getPositionX()-20,self["_headPos"..localSeat]:getPositionY())
    else
