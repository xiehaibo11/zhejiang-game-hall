...
function RoomInfoView:onGameRuleChanged(event)
    ...
    if string.find(strGameRule, "AAA") then
        CardEnum.S_BSPECFCARD3A = true
    else
        CardEnum.S_BSPECFCARD3A = false
    end
    if string.find(strGameRule, "必须管") then
        CF.roomData:setIsMustOut(true)
    end
    if string.find(strGameRule, "逢跑必压") or string.find(strGameRule, "报单必大") then
        CF.roomData:setIsMustControl(true)
    end
end
