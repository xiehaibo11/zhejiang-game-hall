...
local SaveKeyTab = {
    ...
    LanguageStyle = 'SHUTCARD_LAUGUAGE_STYLE', --方言开关
    AutoRaiseStyle = 'SHUTCARD_AUTORAISE_STYLE', --自动提牌
}
...
SettingData.EVENT_AUTORAISE_STYLE_CHANGED = 'EVENT_AUTORAISE_STYLE_CHANGED'
...
--设置自动提牌
function SettingData:setAutoRaise(isNormal)
    if isNormal ~= nil then
        self._settings[SaveKeyTab.AutoRaiseStyle] = isNormal
        local saveKey = self:getFullSaveKey(SaveKeyTab.AutoRaiseStyle)
        userDefault:setBoolForKey(saveKey, isNormal)
        self:dispatch(self.EVENT_AUTORAISE_STYLE_CHANGED, isNormal)
    end
end

--获得是否是自动提牌
function SettingData:getAutoRaise()
    local style = self._settings[SaveKeyTab.AutoRaiseStyle]
