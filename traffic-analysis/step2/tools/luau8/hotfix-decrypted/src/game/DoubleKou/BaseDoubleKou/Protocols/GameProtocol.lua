...
GameProtocol.msgWinLost = {
    ...
    nDoubleFlag = {},
    nDoubleAdd = {},
    sNewXianShu = {},  -- 八王模式最大16线

    someCards = 0,              -- 一些扩展卡的状态
    returnValue = 0,            -- 补偿值
    nIncValues = {},  -- 玩家最终的加倍结果
    ...
    bistream = function(self, buff, bufflen)
        ...
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.bTrust[i] = bis:readBool()
                self.bDouble[i] = bis:readBool()
            end
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.nContract[i] = bis:readInt32()
            end
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
        ...
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.bPoChan[i] = bis:readInt32()
            end
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.nDoubleFlag[i] = bis:readInt32()
            end
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.nDoubleAdd[i] = bis:readInt32()
            end
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                self.sNewXianShu[i] = {}
                for j = 0, 16 do
                    self.sNewXianShu[i][j] = bis:readInt16()
                end
            end
        end
        if bis:getAvailableSize() > 0 then
            self.someCards = bis:readInt32()
        end
        if bis:getAvailableSize() > 0 then
            self.returnValue = bis:readInt64()
        end
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                local nIncValues = bis:readFloat()
                self.nIncValues[i] = math.floor(nIncValues*1000)/1000
            end
        end
        return self
    end
...
GameProtocol.msgSpecialBombGong = {
    XY_ID = 1470,
    ...
    bistream = function (self,buff, bufflen)
        ...
        self.nCardsType = bis:readInt32()
        self.nCardsGong = bis:readInt32()
        return self
    end 
}
...
GameProtocol.msgNoOutGong = {
    ...
    --    bostream = function (self)
    --        local bos = un.network.IStream.new()
    --        bos:writeInt16(self.sEndType)
    --        for i = 0, CF.roomData:getMaxPlayer() - 1 do
    --            bos:writeInt16(self.sNoOutGong[i])
    --        end
    --        for i = 0, CF.roomData:getMaxPlayer() - 1 do
    --            bos:writeInt16(self.sCaiXiang[i])
    --        end
    --        return bos
    --    end,
        ...
        if bis:getAvailableSize() > 0 then
            for i = 0, maxPlayerCount - 1 do
                ...
                for j = 1, self.ucCount[i] do
                    self.ucCards[i][j] = bis:readUInt8()
                end
            end
        end
        return self
...
-- 千变双扣用到，对局时长小于一定时间时候，会下发本局游戏不计分的提示
GameProtocol.msgChatMsg = {
    XY_ID = 1446,
    chatMsg = "",
    new = function(self, o)
        o = o or {}
    ...
    bostream = function(self)
        local bos = un.network.IStream.new()
        bos:writeString(self.chatMsg)
        return bos
    end,
    bistream = function(self, buff, bufflen)
        local bis = un.network.OStream.new(buff)

        self.chatMsg = bis:readString()
        return self
    end
}
