...
function RoomInfoView:getProxyEvents()
    ...
    proxyEvents[#proxyEvents + 1] = { module = CF.game:getModule("GameLayer")._gameData, eventKeyName = "EVENT_GAME_MUT_CHANGED", callBack = "onGameMutChanged" }
    return proxyEvents
end
...
function RoomInfoView:onGameMutChanged(event)
    ...
    if msg.fMut and msg.fMut > 0 then 
        winMut = msg.fMut
        lostMut = msg.fMut
    end 
    ...
    if CF.roomData:isNewUI() then
        self:setOneInfoNodeValue("win", "x" .. winMut)
        self:setOneInfoNodeValue("lost", "x" .. lostMut)
    end

    if msg.isPlayWin then
        local winText = nil
        if CF.roomData:isNewUI() then
            winText = self:getInfoNodeByName("win", "x" .. winMut)._valueText
            CF.game:getModule("RoomInfo"):playAnimation()
        else
            winText = self:getInfoNodeByName("winLost")._valueText2
        end
        if winText then 
