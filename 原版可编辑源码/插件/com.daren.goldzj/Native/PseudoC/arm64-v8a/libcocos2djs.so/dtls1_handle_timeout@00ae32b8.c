...

{
  ...
  lVar7 = *(long *)(p_Var6 + 0x1e8);
                    /* try { // try from 00ae32f8 to 00be32ff has its CatchHandler @ 00ae3624 */
  if (local_30.tv_sec <= lVar7) {
                    /* try { // try from 00ae3300 to 00be34a7 has its CatchHandler @ 00ae2754 */
    if (lVar7 - local_30.tv_sec == 0) {
      lVar9 = *(long *)(p_Var6 + 0x1f0);
      if (lVar9 <= local_30.tv_usec) goto LAB_00ae3360;
    }
    else {
      lVar9 = *(long *)(p_Var6 + 0x1f0);
    }
...
LAB_00ae3360:
  ...
  *(int *)(p_Var6 + 0x1d8) = iVar8;
  ...
  p_Var6 = param_1->msg_callback;
  *(ulong *)(p_Var6 + 0x1e8) = *(long *)(p_Var6 + 0x1e8) + (ulong)*(ushort *)(p_Var6 + 0x1f8);
                    /* try { // try from 00ae34a8 to 00be34af has its CatchHandler @ 00ae35e8 */
  pBVar4 = SSL_get_rbio(param_1);
                    /* try { // try from 00ae34b0 to 00be356b has its CatchHandler @ 00ae2754 */
  BIO_ctrl(pBVar4,0x2d,0,param_1->msg_callback + 0x1e8);
