
/* v8::internal::wasm::Result<std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> >,
   std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> > > > >
   v8::internal::wasm::Decoder::toResult<std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> >,
   std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> > > >,
   std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> >,
   std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> > > >
   >(std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> >,
   std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,
   std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry> > > >&&) */

void v8::internal::wasm::Decoder::
     toResult<std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry>>,std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry>>>>,std::__ndk1::vector<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry>>,std::__ndk1::allocator<std::__ndk1::vector<v8::internal::wasm::AsmJsOffsetEntry,std::__ndk1::allocator<v8::internal::wasm::AsmJsOffsetEntry>>>>>
               (vector *param_1)

{
  undefined4 uVar1;
  vector vVar2;
  long lVar3;
  undefined7 *puVar4;
  undefined8 *in_x1;
  undefined8 *in_x8;
  undefined8 uVar5;
  ulong __n;
  void *__src;
  ulong uVar6;
  undefined8 uVar7;
  undefined8 uVar8;
  byte local_80;
  undefined7 uStack_7f;
  undefined1 uStack_78;
  undefined7 uStack_77;
  undefined7 *local_70;
  undefined7 local_68;
  undefined1 uStack_61;
  undefined7 uStack_60;
  long local_58;
  
  lVar3 = tpidr_el0;
  local_58 = *(long *)(lVar3 + 0x28);
  vVar2 = param_1[0x30];
  if (((byte)vVar2 & 1) == 0) {
    if ((byte)vVar2 >> 1 == 0) {
LAB_015eeca8:
      uVar8 = in_x1[1];
      uVar7 = *in_x1;
      uVar5 = in_x1[2];
      in_x1[1] = 0;
      in_x1[2] = 0;
      *in_x1 = 0;
      *(undefined4 *)(in_x8 + 3) = 0;
      *(undefined1 *)(in_x8 + 4) = 0;
      *(undefined8 *)((long)in_x8 + 0x21) = 0;
      in_x8[5] = 0;
      in_x8[6] = 0;
      in_x8[1] = uVar8;
      *in_x8 = uVar7;
      in_x8[2] = uVar5;
      goto LAB_015eed74;
    }
  }
  else if (*(long *)(param_1 + 0x38) == 0) goto LAB_015eeca8;
  uVar1 = *(undefined4 *)(param_1 + 0x28);
  uStack_7f = 0;
  uStack_78 = 0;
  uStack_77 = 0;
  local_70 = (undefined7 *)0x0;
  if (((byte)vVar2 & 1) == 0) {
    local_70 = *(undefined7 **)(param_1 + 0x40);
    uVar5 = *(undefined8 *)(param_1 + 0x30);
    uStack_78 = (undefined1)*(undefined8 *)(param_1 + 0x38);
    uStack_77 = (undefined7)((ulong)*(undefined8 *)(param_1 + 0x38) >> 8);
    local_80 = (byte)uVar5;
    uStack_7f = (undefined7)((ulong)uVar5 >> 8);
  }
  else {
    __n = *(ulong *)(param_1 + 0x38);
    if (0xffffffffffffffef < __n) {
                    /* WARNING: Subroutine does not return */
      abort();
    }
    __src = *(void **)(param_1 + 0x40);
    if (__n < 0x17) {
      puVar4 = &uStack_7f;
      local_80 = (byte)((int)__n << 1);
      if (__n != 0) goto LAB_015eed20;
    }
    else {
      uVar6 = __n + 0x10 & 0xfffffffffffffff0;
      puVar4 = operator_new(uVar6);
      local_80 = (byte)uVar6 | 1;
      uStack_7f = (undefined7)(uVar6 >> 8);
      uStack_78 = (undefined1)__n;
      uStack_77 = (undefined7)(__n >> 8);
      local_70 = puVar4;
LAB_015eed20:
      memcpy(puVar4,__src,__n);
    }
    *(undefined1 *)((long)puVar4 + __n) = 0;
  }
  puVar4 = local_70;
  uVar5 = CONCAT71(uStack_77,uStack_78);
  *(undefined4 *)(in_x8 + 3) = uVar1;
  local_68 = uStack_7f;
  uStack_61 = uStack_78;
  uStack_60 = uStack_77;
  uVar7 = CONCAT17(uStack_78,uStack_7f);
  uStack_7f = 0;
  uStack_78 = 0;
  uStack_77 = 0;
  local_70 = (undefined7 *)0x0;
  *in_x8 = 0;
  in_x8[1] = 0;
  in_x8[2] = 0;
  *(byte *)(in_x8 + 4) = local_80;
  in_x8[5] = uVar5;
  in_x8[6] = puVar4;
  *(undefined8 *)((long)in_x8 + 0x21) = uVar7;
LAB_015eed74:
  if (*(long *)(lVar3 + 0x28) != local_58) {
                    /* WARNING: Subroutine does not return */
    __stack_chk_fail();
  }
  return;
}

