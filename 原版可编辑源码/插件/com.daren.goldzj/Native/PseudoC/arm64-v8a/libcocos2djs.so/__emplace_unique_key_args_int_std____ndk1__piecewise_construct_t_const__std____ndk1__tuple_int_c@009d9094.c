
/* std::__ndk1::pair<std::__ndk1::__hash_iterator<std::__ndk1::__hash_node<std::__ndk1::__hash_value_type<int,
   cocos2d::VideoPlayer*>, void*>*>, bool>
   std::__ndk1::__hash_table<std::__ndk1::__hash_value_type<int, cocos2d::VideoPlayer*>,
   std::__ndk1::__unordered_map_hasher<int, std::__ndk1::__hash_value_type<int,
   cocos2d::VideoPlayer*>, std::__ndk1::hash<int>, true>, std::__ndk1::__unordered_map_equal<int,
   std::__ndk1::__hash_value_type<int, cocos2d::VideoPlayer*>, std::__ndk1::equal_to<int>, true>,
   std::__ndk1::allocator<std::__ndk1::__hash_value_type<int, cocos2d::VideoPlayer*> >
   >::__emplace_unique_key_args<int, std::__ndk1::piecewise_construct_t const&,
   std::__ndk1::tuple<int const&>, std::__ndk1::tuple<> >(int const&,
   std::__ndk1::piecewise_construct_t const&, std::__ndk1::tuple<int const&>&&,
   std::__ndk1::tuple<>&&) */

undefined1  [16]
std::__ndk1::
__hash_table<std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::__unordered_map_hasher<int,std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::hash<int>,true>,std::__ndk1::__unordered_map_equal<int,std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::equal_to<int>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>>>
::
__emplace_unique_key_args<int,std::__ndk1::piecewise_construct_t_const&,std::__ndk1::tuple<int_const&>,std::__ndk1::tuple<>>
          (int *param_1,piecewise_construct_t *param_2,tuple *param_3,tuple *param_4)

{
  undefined4 uVar1;
  ulong uVar2;
  undefined8 uVar3;
  ulong uVar4;
  long *plVar5;
  long lVar6;
  ulong uVar7;
  long *plVar8;
  ulong uVar9;
  ulong uVar10;
  ulong unaff_x24;
  undefined1 auVar11 [16];
  
  uVar9 = *(ulong *)(param_1 + 2);
  uVar10 = (ulong)*(int *)param_2;
  if (uVar9 != 0) {
    uVar4 = uVar9 - 1;
    if ((uVar4 & uVar9) == 0) {
      unaff_x24 = uVar4 & uVar10;
    }
    else {
      unaff_x24 = uVar10;
      if (uVar9 <= uVar10) {
        uVar7 = 0;
        if (uVar9 != 0) {
          uVar7 = uVar10 / uVar9;
        }
        unaff_x24 = uVar10 - uVar7 * uVar9;
      }
    }
    plVar8 = *(long **)(*(long *)param_1 + unaff_x24 * 8);
    if (plVar8 != (long *)0x0) {
      do {
        plVar8 = (long *)*plVar8;
        if (plVar8 == (long *)0x0) goto LAB_009d9148;
        uVar7 = plVar8[1];
        if (uVar7 != uVar10) {
          if ((uVar4 & uVar9) == 0) {
            uVar7 = uVar7 & uVar4;
          }
          else if (uVar9 <= uVar7) {
            uVar2 = 0;
            if (uVar9 != 0) {
              uVar2 = uVar7 / uVar9;
            }
            if (uVar7 - uVar2 * uVar9 == unaff_x24) goto LAB_009d9134;
            goto LAB_009d9148;
          }
          if (uVar7 != unaff_x24) goto LAB_009d9148;
        }
LAB_009d9134:
      } while ((int)plVar8[2] != *(int *)param_2);
      uVar3 = 0;
      goto LAB_009d9270;
    }
  }
LAB_009d9148:
  plVar8 = operator_new(0x20);
  uVar1 = **(undefined4 **)param_4;
  plVar8[3] = 0;
                    /* try { // try from 009d9160 to 00ad919f has its CatchHandler @ 009d9160
                       catch() { ... } // from try @ 009d9160 with catch @ 009d9160
                       catch() { ... } // from try @ 009d9234 with catch @ 009d9160
                       catch() { ... } // from try @ 009d9448 with catch @ 009d9160 */
  *plVar8 = 0;
  plVar8[1] = uVar10;
  *(undefined4 *)(plVar8 + 2) = uVar1;
  if ((uVar9 == 0) || ((float)param_1[8] * (float)uVar9 < (float)(*(long *)(param_1 + 6) + 1))) {
    if (uVar9 < 3) {
      uVar4 = 1;
    }
    else {
                    /* try { // try from 009d91a0 to 00ad91cf has its CatchHandler @ 009d94c0 */
      uVar4 = (ulong)((uVar9 - 1 & uVar9) != 0);
    }
    uVar4 = uVar4 | uVar9 << 1;
    uVar9 = (ulong)((float)(*(long *)(param_1 + 6) + 1) / (float)param_1[8]);
    if (uVar9 <= uVar4) {
      uVar9 = uVar4;
    }
    rehash((__hash_table<std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::__unordered_map_hasher<int,std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::hash<int>,true>,std::__ndk1::__unordered_map_equal<int,std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>,std::__ndk1::equal_to<int>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<int,cocos2d::VideoPlayer*>>>
            *)param_1,uVar9);
    uVar9 = *(ulong *)(param_1 + 2);
    if ((uVar9 - 1 & uVar9) == 0) {
      unaff_x24 = uVar9 - 1 & uVar10;
    }
    else {
                    /* try { // try from 009d91d8 to 00ad91ef has its CatchHandler @ 009d94b8 */
      unaff_x24 = uVar10;
      if (uVar9 <= uVar10) {
        uVar4 = 0;
        if (uVar9 != 0) {
          uVar4 = uVar10 / uVar9;
        }
        unaff_x24 = uVar10 - uVar4 * uVar9;
      }
    }
  }
                    /* try { // try from 009d91f8 to 00ad91ff has its CatchHandler @ 009d94bc */
  lVar6 = *(long *)param_1;
  plVar5 = *(long **)(lVar6 + unaff_x24 * 8);
                    /* try { // try from 009d9200 to 00ad9233 has its CatchHandler @ 009d94c8 */
  if (plVar5 == (long *)0x0) {
    plVar5 = (long *)(param_1 + 4);
    *plVar8 = *plVar5;
    *plVar5 = (long)plVar8;
    *(long **)(lVar6 + unaff_x24 * 8) = plVar5;
    if (*plVar8 != 0) {
      uVar10 = *(ulong *)(*plVar8 + 8);
                    /* try { // try from 009d9234 to 00ad9283 has its CatchHandler @ 009d9160 */
      if ((uVar9 - 1 & uVar9) == 0) {
        uVar10 = uVar10 & uVar9 - 1;
      }
      else if (uVar9 <= uVar10) {
        uVar4 = 0;
        if (uVar9 != 0) {
          uVar4 = uVar10 / uVar9;
        }
        uVar10 = uVar10 - uVar4 * uVar9;
      }
      plVar5 = (long *)(*(long *)param_1 + uVar10 * 8);
      goto LAB_009d925c;
    }
  }
  else {
    *plVar8 = *plVar5;
LAB_009d925c:
    *plVar5 = (long)plVar8;
  }
  uVar3 = 1;
  *(long *)(param_1 + 6) = *(long *)(param_1 + 6) + 1;
LAB_009d9270:
  auVar11._8_8_ = uVar3;
  auVar11._0_8_ = plVar8;
                    /* try { // try from 009d9284 to 00ad9447 has its CatchHandler @ 009d94c8 */
  return auVar11;
}

