...

{
  ...
  if (ctx == (X509_STORE_CTX *)0x0) {
                    /* try { // try from 00ac1784 to 00bc17a3 has its CatchHandler @ 00ac1a74 */
    ERR_put_error(0x14,0xcf,0x41,"ssl/ssl_cert.c",0x189);
    return 0;
  }
                    /* try { // try from 00ac1664 to 00bc166b has its CatchHandler @ 00ac1a50 */
  x509 = (X509 *)OPENSSL_sk_value(param_2,0);
                    /* try { // try from 00ac166c to 00bc1783 has its CatchHandler @ 00ac160c */
  iVar1 = X509_STORE_CTX_init(ctx,store,x509,param_2);
  if (iVar1 == 0) {
                    /* try { // try from 00ac17a4 to 00bc18af has its CatchHandler @ 00ac160c */
    ...
    iVar3 = CRYPTO_THREAD_run_once(&DAT_01d38ea0,FUN_00ac0824);
    iVar1 = -1;
    if ((iVar3 != 0) && (DAT_01d38ea4 != 0)) {
      iVar1 = DAT_01d1e1b0;
    }
