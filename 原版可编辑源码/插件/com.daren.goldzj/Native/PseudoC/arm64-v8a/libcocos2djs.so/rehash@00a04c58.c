...
/* std::__ndk1::__hash_table<std::__ndk1::__hash_value_type<int, std::__ndk1::function<void
   ...
   const&, std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> > const&, int, std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> > const&)> > > >::rehash(unsigned
   long) */
...

{
  ...
  ulong uVar2;
                    ...
                    /* try { // try from 00a04c70 to 00b04c77 has its CatchHandler @ 00a04c80 */
  if (param_1 - 1 == 0) {
    param_1 = 2;
  }
  else {
                    /* catch(type#1 @ 00000000) { ... } // from try @ 00a04c70 with catch @ 00a04c80
                        */
...
LAB_00a04ca8:
  ...
  if (param_1 < uVar2) {
    ...
    if (uVar1 < uVar2) goto LAB_00a04ca8;
  }
  return;
}
