
/* std::__ndk1::pair<std::__ndk1::__hash_iterator<std::__ndk1::__hash_node<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, cocos2d::PcmData>, void*>*>,
   bool> std::__ndk1::__hash_table<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, cocos2d::PcmData>,
   std::__ndk1::__unordered_map_hasher<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >,
   std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> >, cocos2d::PcmData>,
   std::__ndk1::hash<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> > >, true>,
   std::__ndk1::__unordered_map_equal<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >,
   std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> >, cocos2d::PcmData>,
   std::__ndk1::equal_to<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> > >, true>,
   std::__ndk1::allocator<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, cocos2d::PcmData> >
   >::__emplace_unique_key_args<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> >, std::__ndk1::pair<std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, cocos2d::PcmData>
   >(std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
   const&, std::__ndk1::pair<std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>,
   std::__ndk1::allocator<char> >, cocos2d::PcmData>&&) */

long * __thiscall
std::__ndk1::
__hash_table<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::__unordered_map_hasher<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::hash<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::__unordered_map_equal<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::equal_to<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>>>
::
__emplace_unique_key_args<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::pair<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>>
          (__hash_table<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::__unordered_map_hasher<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::hash<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::__unordered_map_equal<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::equal_to<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>>>
           *this,basic_string *param_1,pair *param_2)

{
  size_t sVar1;
  basic_string *pbVar2;
  byte bVar3;
  ulong uVar4;
  long lVar5;
  size_t __n;
  uint uVar6;
  ulong uVar7;
  void *__s1;
  undefined8 uVar8;
  undefined8 *puVar9;
  ulong uVar10;
  long *plVar11;
  __hash_table<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::__unordered_map_hasher<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::hash<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::__unordered_map_equal<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::equal_to<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>>>
  *p_Var12;
  long lVar13;
  long *plVar14;
  ulong uVar15;
  ulong unaff_x27;
  ulong uVar16;
  long lVar17;
  __murmur2_or_cityhash<unsigned_long,64ul> a_Stack_70 [8];
  long local_68;
  
  lVar5 = tpidr_el0;
  local_68 = *(long *)(lVar5 + 0x28);
                    /* try { // try from 00a772c8 to 00b772df has its CatchHandler @ 00a77564 */
  uVar7 = *(ulong *)(param_1 + 8);
  pbVar2 = *(basic_string **)(param_1 + 0x10);
  if (((byte)*param_1 & 1) == 0) {
    pbVar2 = param_1 + 1;
    uVar7 = (ulong)((byte)*param_1 >> 1);
  }
  uVar7 = __murmur2_or_cityhash<unsigned_long,64ul>::operator()(a_Stack_70,pbVar2,uVar7);
  uVar15 = *(ulong *)(this + 8);
                    /* try { // try from 00a772e8 to 00b772f3 has its CatchHandler @ 00a77560 */
  if (uVar15 != 0) {
    uVar16 = uVar15 - 1;
                    /* try { // try from 00a772f4 to 00b772ff has its CatchHandler @ 00a7755c */
    if ((uVar16 & uVar15) == 0) {
      unaff_x27 = uVar16 & uVar7;
    }
    else {
      unaff_x27 = uVar7;
                    /* try { // try from 00a77300 to 00b77317 has its CatchHandler @ 00a77574 */
      if (uVar15 <= uVar7) {
        uVar10 = 0;
        if (uVar15 != 0) {
          uVar10 = uVar7 / uVar15;
        }
        unaff_x27 = uVar7 - uVar10 * uVar15;
      }
    }
    puVar9 = *(undefined8 **)(*(long *)this + unaff_x27 * 8);
    if ((puVar9 != (undefined8 *)0x0) && (plVar14 = (long *)*puVar9, plVar14 != (long *)0x0)) {
                    /* try { // try from 00a77334 to 00b7733b has its CatchHandler @ 00a77544 */
      __n = (ulong)((byte)*param_1 >> 1);
      pbVar2 = param_1 + 1;
                    /* try { // try from 00a77348 to 00b7736b has its CatchHandler @ 00a77540 */
      if (((byte)*param_1 & 1) != 0) {
        __n = *(size_t *)(param_1 + 8);
        pbVar2 = *(basic_string **)(param_1 + 0x10);
      }
      do {
        uVar10 = plVar14[1];
        if (uVar10 != uVar7) {
          if ((uVar16 & uVar15) == 0) {
            uVar10 = uVar10 & uVar16;
          }
          else if (uVar15 <= uVar10) {
            uVar4 = 0;
            if (uVar15 != 0) {
              uVar4 = uVar10 / uVar15;
            }
            if (uVar10 - uVar4 * uVar15 == unaff_x27) goto LAB_00a77388;
            break;
          }
                    /* try { // try from 00a77380 to 00b773e7 has its CatchHandler @ 00a77574 */
          if (uVar10 != unaff_x27) break;
        }
LAB_00a77388:
        bVar3 = *(byte *)(plVar14 + 2);
        sVar1 = (ulong)(bVar3 >> 1);
        if ((bVar3 & 1) != 0) {
          sVar1 = plVar14[3];
        }
        if (sVar1 == __n) {
          __s1 = (void *)plVar14[4];
          if ((bVar3 & 1) == 0) {
            __s1 = (void *)((long)plVar14 + 0x11);
          }
          if ((bVar3 & 1) == 0) {
            if (__n != 0) {
              uVar10 = 0;
              while (*(basic_string *)((long)plVar14 + uVar10 + 0x11) == pbVar2[uVar10]) {
                uVar10 = uVar10 + 1;
                if (bVar3 >> 1 == uVar10) goto LAB_00a774b8;
              }
              goto LAB_00a773f8;
            }
          }
          else if (__n != 0) {
                    /* try { // try from 00a773ec to 00b7742f has its CatchHandler @ 00a77544 */
            uVar6 = memcmp(__s1,pbVar2,__n);
            __s1 = (void *)(ulong)uVar6;
            if (uVar6 != 0) goto LAB_00a773f8;
          }
LAB_00a774b8:
          uVar8 = 0;
          goto LAB_00a7754c;
        }
LAB_00a773f8:
        plVar14 = (long *)*plVar14;
      } while (plVar14 != (long *)0x0);
    }
  }
  plVar14 = operator_new(0x58);
  lVar17 = *(long *)(param_2 + 8);
  lVar13 = *(long *)param_2;
  plVar14[4] = *(long *)(param_2 + 0x10);
  plVar14[3] = lVar17;
  plVar14[2] = lVar13;
  *(undefined8 *)(param_2 + 8) = 0;
  *(undefined8 *)(param_2 + 0x10) = 0;
  *(undefined8 *)param_2 = 0;
  __s1 = (void *)cocos2d::PcmData::PcmData((PcmData *)(plVar14 + 5),(PcmData *)(param_2 + 0x18));
                    /* try { // try from 00a77434 to 00b77477 has its CatchHandler @ 00a77540 */
  *plVar14 = 0;
  plVar14[1] = uVar7;
  if ((uVar15 == 0) ||
     (*(float *)(this + 0x20) * (float)uVar15 < (float)(*(long *)(this + 0x18) + 1))) {
    if (uVar15 < 3) {
      uVar16 = 1;
    }
    else {
      uVar16 = (ulong)((uVar15 - 1 & uVar15) != 0);
    }
    uVar16 = uVar16 | uVar15 << 1;
    uVar15 = (ulong)((float)(*(long *)(this + 0x18) + 1) / *(float *)(this + 0x20));
    if (uVar15 <= uVar16) {
      uVar15 = uVar16;
    }
                    /* try { // try from 00a77490 to 00b774cb has its CatchHandler @ 00a77574 */
    __s1 = (void *)rehash(this,uVar15);
    uVar15 = *(ulong *)(this + 8);
    if ((uVar15 - 1 & uVar15) == 0) {
      unaff_x27 = uVar15 - 1 & uVar7;
    }
    else {
      unaff_x27 = uVar7;
      if (uVar15 <= uVar7) {
                    /* try { // try from 00a774cc to 00b775cb has its CatchHandler @ 00a77274 */
        uVar16 = 0;
        if (uVar15 != 0) {
          uVar16 = uVar7 / uVar15;
        }
        unaff_x27 = uVar7 - uVar16 * uVar15;
      }
    }
  }
  lVar13 = *(long *)this;
  plVar11 = *(long **)(lVar13 + unaff_x27 * 8);
  if (plVar11 == (long *)0x0) {
    p_Var12 = this + 0x10;
    *plVar14 = *(long *)p_Var12;
    *(long **)p_Var12 = plVar14;
    *(__hash_table<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::__unordered_map_hasher<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::hash<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::__unordered_map_equal<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>,std::__ndk1::equal_to<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>>,true>,std::__ndk1::allocator<std::__ndk1::__hash_value_type<std::__ndk1::basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>,cocos2d::PcmData>>>
      **)(lVar13 + unaff_x27 * 8) = p_Var12;
    if (*plVar14 == 0) goto LAB_00a7753c;
    uVar7 = *(ulong *)(*plVar14 + 8);
    if ((uVar15 - 1 & uVar15) == 0) {
      uVar7 = uVar7 & uVar15 - 1;
    }
    else if (uVar15 <= uVar7) {
      uVar16 = 0;
      if (uVar15 != 0) {
        uVar16 = uVar7 / uVar15;
      }
      uVar7 = uVar7 - uVar16 * uVar15;
    }
    plVar11 = (long *)(*(long *)this + uVar7 * 8);
  }
  else {
    *plVar14 = *plVar11;
  }
  *plVar11 = (long)plVar14;
LAB_00a7753c:
                    /* catch() { ... } // from try @ 00a77348 with catch @ 00a77540
                       catch() { ... } // from try @ 00a77434 with catch @ 00a77540 */
  uVar8 = 1;
                    /* catch() { ... } // from try @ 00a77334 with catch @ 00a77544
                       catch() { ... } // from try @ 00a773ec with catch @ 00a77544 */
  *(long *)(this + 0x18) = *(long *)(this + 0x18) + 1;
LAB_00a7754c:
  if (*(long *)(lVar5 + 0x28) != local_68) {
                    /* WARNING: Subroutine does not return */
    __stack_chk_fail(__s1,uVar8);
  }
                    /* catch() { ... } // from try @ 00a772f4 with catch @ 00a7755c */
                    /* catch() { ... } // from try @ 00a772e8 with catch @ 00a77560 */
                    /* catch() { ... } // from try @ 00a772c8 with catch @ 00a77564 */
                    /* catch() { ... } // from try @ 00a77300 with catch @ 00a77574
                       catch() { ... } // from try @ 00a77380 with catch @ 00a77574
                       catch() { ... } // from try @ 00a77490 with catch @ 00a77574 */
  return plVar14;
}

