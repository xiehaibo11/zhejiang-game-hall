...

{
  ...
  if (in == (uchar *)0x0) {
    ...
    return;
                    ...
                    /* try { // try from 00b000a4 to 00c000af has its CatchHandler @ 00b00324 */
  if (siz == 0) {
    return;
  }
                    /* try { // try from 00b000b0 to 00c0036f has its CatchHandler @ 00affec4 */
  puVar4 = out + (siz - 1);
  if (0x1f < siz) {
    uVar6 = siz & 0x1f;
    lVar5 = siz - uVar6;
    if (lVar5 == 0) goto LAB_00b00160;
  ...
  do {
    ...
    *puVar4 = *in;
    ...
    in = in + 1;
                    ...
                    /* WARNING: Read-only address (ram,0x018a88e0) is written */
  return;
}
