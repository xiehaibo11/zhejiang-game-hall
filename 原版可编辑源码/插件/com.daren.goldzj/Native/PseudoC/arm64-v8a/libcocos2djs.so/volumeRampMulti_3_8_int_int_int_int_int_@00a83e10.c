
/* void cocos2d::volumeRampMulti<3, 8, int, int, int, int, int>(int*, unsigned long, int const*,
   int*, int*, int const*, int*, int) */

void cocos2d::volumeRampMulti<3,8,int,int,int,int,int>
               (int *param_1,ulong param_2,int *param_3,int *param_4,int *param_5,int *param_6,
               int *param_7,int param_8)

{
  int *piVar1;
  int iVar2;
  int iVar3;
  int iVar4;
  int iVar5;
  int iVar6;
  int iVar7;
  int iVar8;
  int iVar9;
  
  if (param_4 == (int *)0x0) {
    iVar9 = *param_5;
    do {
                    /* try { // try from 00a83f5c to 00b83f73 has its CatchHandler @ 00a84188 */
      param_2 = param_2 - 1;
      *param_1 = *param_1 + (iVar9 >> 0x10) * (*param_3 >> 0xc);
                    /* try { // try from 00a83f7c to 00b83f87 has its CatchHandler @ 00a84184 */
                    /* try { // try from 00a83f88 to 00b83f93 has its CatchHandler @ 00a84180 */
      param_1[1] = param_1[1] + (*param_5 >> 0x10) * (param_3[1] >> 0xc);
                    /* try { // try from 00a83f94 to 00b83fab has its CatchHandler @ 00a84198 */
      param_1[2] = param_1[2] + (*param_5 >> 0x10) * (param_3[2] >> 0xc);
      param_1[3] = param_1[3] + (*param_5 >> 0x10) * (param_3[3] >> 0xc);
                    /* try { // try from 00a83fc8 to 00b83fdf has its CatchHandler @ 00a84168 */
      param_1[4] = param_1[4] + (*param_5 >> 0x10) * (param_3[4] >> 0xc);
      param_1[5] = param_1[5] + (*param_5 >> 0x10) * (param_3[5] >> 0xc);
                    /* try { // try from 00a83ffc to 00b8409f has its CatchHandler @ 00a84198 */
      param_1[6] = param_1[6] + (*param_5 >> 0x10) * (param_3[6] >> 0xc);
      piVar1 = param_3 + 7;
      param_3 = param_3 + 8;
      param_1[7] = param_1[7] + (*param_5 >> 0x10) * (*piVar1 >> 0xc);
      param_1 = param_1 + 8;
      iVar9 = *param_5 + *param_6;
      *param_5 = iVar9;
    } while (param_2 != 0);
  }
  else {
    do {
      iVar2 = *param_3;
                    /* try { // try from 00a83e24 to 00b83f07 has its CatchHandler @ 00a83c60 */
      *param_1 = *param_1 + (*param_5 >> 0x10) * (iVar2 >> 0xc);
      iVar3 = param_3[1];
      param_1[1] = param_1[1] + (*param_5 >> 0x10) * (iVar3 >> 0xc);
      iVar4 = param_3[2];
      param_1[2] = param_1[2] + (*param_5 >> 0x10) * (iVar4 >> 0xc);
      iVar5 = param_3[3];
      param_1[3] = param_1[3] + (*param_5 >> 0x10) * (iVar5 >> 0xc);
      iVar6 = param_3[4];
                    /* catch() { ... } // from try @ 00a83ce0 with catch @ 00a83e98 */
                    /* catch() { ... } // from try @ 00a83cd4 with catch @ 00a83e9c */
                    /* catch() { ... } // from try @ 00a83cb4 with catch @ 00a83ea0 */
      param_1[4] = param_1[4] + (*param_5 >> 0x10) * (iVar6 >> 0xc);
      iVar7 = param_3[5];
                    /* catch() { ... } // from try @ 00a83cec with catch @ 00a83eb0 */
      param_1[5] = param_1[5] + (*param_5 >> 0x10) * (iVar7 >> 0xc);
      iVar8 = param_3[6];
      param_1[6] = param_1[6] + (*param_5 >> 0x10) * (iVar8 >> 0xc);
      iVar9 = param_3[7];
      param_3 = param_3 + 8;
      param_1[7] = param_1[7] + (*param_5 >> 0x10) * (iVar9 >> 0xc);
                    /* try { // try from 00a83f08 to 00b83f5b has its CatchHandler @ 00a83f08
                       catch() { ... } // from try @ 00a83f08 with catch @ 00a83f08
                       catch() { ... } // from try @ 00a84154 with catch @ 00a83f08 */
      iVar9 = iVar9 + iVar8 + iVar7 + iVar6 + iVar5 + iVar4 + iVar3 + iVar2;
      *param_5 = *param_5 + *param_6;
      iVar2 = iVar9 + 7;
      if (-1 < iVar9) {
        iVar2 = iVar9;
      }
      *param_4 = *param_4 + (*param_7 >> 0x10) * (iVar2 >> 0xf);
      param_2 = param_2 - 1;
      param_1 = param_1 + 8;
      *param_7 = *param_7 + param_8;
      param_4 = param_4 + 1;
    } while (param_2 != 0);
  }
  return;
}

