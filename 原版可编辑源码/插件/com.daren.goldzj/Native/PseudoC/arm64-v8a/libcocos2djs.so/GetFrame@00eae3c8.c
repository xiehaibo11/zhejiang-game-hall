
/* v8::StackTrace::GetFrame(v8::Isolate*, unsigned int) const */

ulong * __thiscall v8::StackTrace::GetFrame(StackTrace *this,Isolate *param_1,uint param_2)

{
  Isolate *pIVar1;
  Isolate *pIVar2;
  ulong *puVar3;
  undefined4 uVar4;
  ulong *puVar5;
  Isolate *pIVar6;
  long lVar7;
  ulong uVar8;
  Isolate *local_60;
  undefined8 local_58;
  long lStack_50;
  ulong *local_48;
  
  uVar4 = *(undefined4 *)(param_1 + 0x2c60);
  pIVar1 = param_1 + 0x95a0;
  *(undefined4 *)(param_1 + 0x2c60) = 5;
  local_48 = *(ulong **)pIVar1;
  uVar8 = *(ulong *)(param_1 + 0xa8);
  if (local_48 == *(ulong **)(param_1 + 0x95a8)) {
    local_48 = (ulong *)internal::HandleScope::Extend((Isolate *)param_1);
  }
  *(ulong **)pIVar1 = local_48 + 1;
  *local_48 = uVar8;
  HandleScope::Initialize((HandleScope *)&local_60,param_1);
  uVar8 = *(ulong *)this & 0xffffffff00000000 |
          (ulong)*(uint *)(*(ulong *)this + (long)(int)(param_2 << 2) + 7);
  if (*(CanonicalHandleScope **)(param_1 + 0x95b8) == (CanonicalHandleScope *)0x0) {
    pIVar6 = *(Isolate **)pIVar1;
    if (pIVar6 == *(Isolate **)(param_1 + 0x95a8)) {
      pIVar6 = (Isolate *)internal::HandleScope::Extend((Isolate *)param_1);
    }
    *(Isolate **)pIVar1 = pIVar6 + 8;
    *(ulong *)pIVar6 = uVar8;
  }
  else {
    pIVar6 = (Isolate *)
             internal::CanonicalHandleScope::Lookup
                       (*(CanonicalHandleScope **)(param_1 + 0x95b8),uVar8);
  }
  puVar5 = local_48;
  if (((*local_48 & 1) != 0) && ((int)*local_48 == *(int *)(local_60 + 0xa8))) {
LAB_00eae4e8:
    pIVar2 = local_60 + 0xa0;
    if (pIVar6 != (Isolate *)0x0) {
      pIVar2 = pIVar6;
    }
    puVar3 = (ulong *)0x0;
    if (pIVar6 != (Isolate *)0x0) {
      puVar3 = puVar5;
    }
    *puVar5 = *(ulong *)pIVar2;
    *(undefined8 *)(local_60 + 0x95a0) = local_58;
    *(int *)(local_60 + 0x95b0) = *(int *)(local_60 + 0x95b0) + -1;
    if (*(long *)(local_60 + 0x95a8) != lStack_50) {
      *(long *)(local_60 + 0x95a8) = lStack_50;
      internal::HandleScope::DeleteExtensions(local_60);
    }
    *(undefined4 *)(param_1 + 0x2c60) = uVar4;
    return puVar3;
  }
  lVar7 = base::Thread::GetThreadLocal(internal::Isolate::isolate_key_);
  if (lVar7 != 0) {
    if (*(code **)(lVar7 + 0xb738) != (code *)0x0) {
      (**(code **)(lVar7 + 0xb738))("EscapableHandleScope::Escape","Escape value set twice");
      *(undefined1 *)(lVar7 + 0xb6b9) = 1;
      goto LAB_00eae4e8;
    }
  }
  base::OS::PrintError
            ("\n#\n# Fatal error in %s\n# %s\n#\n\n","EscapableHandleScope::Escape",
             "Escape value set twice");
                    /* WARNING: Subroutine does not return */
  base::OS::Abort();
}

