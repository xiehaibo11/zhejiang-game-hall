...
/* JSB_glUniform1ivRegistry(v8::FunctionCallbackInfo<v8::Value> const&) */

void JSB_glUniform1ivRegistry(FunctionCallbackInfo *param_1)
  ...
  if ((plVar7[1] - *plVar7 & 0xffffffff0U) == 0x20) {
    ...
    if (bVar1) {
      __android_log_print(6,"jswrapper","jsb: ERROR: File %s: Line: %d, Function: %s\n",
                          ...
                          ,0x933,"JSB_glUniform1iv");
      __android_log_print(6,"jswrapper",&DAT_0190552e);
    }
    else {
      glUniform1iv(local_7c,local_48 & 0xffffffff,local_50);
    }
    if (local_40 != '\0') {
      free(local_50);
    }
    if (!bVar1) goto LAB_008c4eac;
  }
  else {
    __android_log_print(6,"jswrapper","jsb: ERROR: File %s: Line: %d, Function: %s\n",
                        "F:/darenneiqian/frameworks/cocos2d-x/cocos/scripting/js-bindings/manual/jsb_opengl_manual.cpp"
                        ,0x92c,"JSB_glUniform1iv");
