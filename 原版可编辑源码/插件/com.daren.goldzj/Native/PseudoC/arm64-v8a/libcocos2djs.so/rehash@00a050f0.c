...
/* std::__ndk1::__hash_table<std::__ndk1::__hash_value_type<int, std::__ndk1::function<void
   ...
   (std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
   const&, float)> > > >::rehash(unsigned long) */
...

{
  ...
  if (param_1 - 1 == 0) {
                    /* try { // try from 00a0510c to 00b0510f has its CatchHandler @ 00a0511c */
    param_1 = 2;
                    /* try { // try from 00a05110 to 00b0512f has its CatchHandler @ 00a050e0 */
  }
  else {
                    /* catch() { ... } // from try @ 00a0510c with catch @ 00a0511c */
  ...
  if (param_1 < uVar2) {
    ...
    if (uVar2 < 3 || (uVar2 - 1 & uVar2) != 0) {
      ...
      uVar1 = 1L << ((ulong)(uint)-(int)LZCOUNT(uVar1 - 1) & 0x3f);
                    /* catch(type#1 @ 00000000) { ... } // from try @ 00a0521c with catch @ 00a051a4
                       catch(type#1 @ 00000000) { ... } // from try @ 00a05254 with catch @ 00a051a4
                        */
    }
    if (uVar1 <= param_1) {
      uVar1 = param_1;
    }
    if (uVar1 < uVar2) goto LAB_00a05140;
  }
  return;
}
