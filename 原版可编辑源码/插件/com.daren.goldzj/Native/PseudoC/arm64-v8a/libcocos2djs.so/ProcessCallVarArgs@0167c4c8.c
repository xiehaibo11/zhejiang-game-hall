
/* v8::internal::compiler::BytecodeGraphBuilder::ProcessCallVarArgs(v8::internal::ConvertReceiverMode,
   v8::internal::compiler::Node*, v8::internal::interpreter::Register, int) */

undefined8 * __thiscall
v8::internal::compiler::BytecodeGraphBuilder::ProcessCallVarArgs
          (BytecodeGraphBuilder *this,int param_2,undefined8 param_3,int param_4,uint param_5)

{
  undefined8 uVar1;
  Zone *this_00;
  undefined8 *puVar2;
  ulong uVar3;
  undefined8 *puVar4;
  
  if (param_2 == 0) {
    uVar1 = JSGraph::UndefinedConstant(*(JSGraph **)(this + 0x10));
  }
  else {
    uVar1 = Environment::LookupRegister(*(Environment **)(this + 0xa8),param_4);
    param_4 = param_4 + 1;
  }
  this_00 = *(Zone **)(this + 8);
  uVar3 = -(ulong)(param_5 + 2 >> 0x1f) & 0xfffffff800000000 | (ulong)(param_5 + 2) << 3;
  puVar2 = *(undefined8 **)(this_00 + 0x10);
  if ((ulong)(*(long *)(this_00 + 0x18) - (long)puVar2) < uVar3) {
    puVar2 = (undefined8 *)Zone::NewExpand(this_00,uVar3);
  }
  else {
    *(ulong *)(this_00 + 0x10) = (long)puVar2 + uVar3;
  }
  *puVar2 = param_3;
  puVar2[1] = uVar1;
  if (0 < (int)param_5) {
    uVar3 = (ulong)param_5;
    puVar4 = puVar2 + 2;
    do {
      uVar1 = Environment::LookupRegister(*(Environment **)(this + 0xa8),param_4);
      *puVar4 = uVar1;
      uVar3 = uVar3 - 1;
      param_4 = param_4 + 1;
      puVar4 = puVar4 + 1;
    } while (uVar3 != 0);
  }
  return puVar2;
}

