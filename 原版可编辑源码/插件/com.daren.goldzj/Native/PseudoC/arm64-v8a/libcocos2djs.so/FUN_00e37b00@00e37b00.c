
undefined8 FUN_00e37b00(long param_1,undefined8 *param_2)

{
  undefined8 uVar1;
  undefined8 uVar2;
  undefined8 uVar3;
  undefined8 uVar4;
  undefined8 uVar5;
  undefined8 uVar6;
  undefined8 uVar7;
  
  uVar1 = *(undefined8 *)(param_1 + 0x138);
  uVar3 = *(undefined8 *)(param_1 + 0x150);
  uVar2 = *(undefined8 *)(param_1 + 0x148);
  param_2[1] = *(undefined8 *)(param_1 + 0x140);
  *param_2 = uVar1;
  param_2[3] = uVar3;
  param_2[2] = uVar2;
  uVar2 = *(undefined8 *)(param_1 + 0x160);
  uVar1 = *(undefined8 *)(param_1 + 0x158);
  uVar3 = *(undefined8 *)(param_1 + 0x178);
  uVar5 = *(undefined8 *)(param_1 + 400);
  uVar4 = *(undefined8 *)(param_1 + 0x188);
  uVar7 = *(undefined8 *)(param_1 + 0x170);
  uVar6 = *(undefined8 *)(param_1 + 0x168);
  param_2[9] = *(undefined8 *)(param_1 + 0x180);
  param_2[8] = uVar3;
  param_2[0xb] = uVar5;
  param_2[10] = uVar4;
  param_2[5] = uVar2;
  param_2[4] = uVar1;
  param_2[7] = uVar7;
  param_2[6] = uVar6;
  uVar2 = *(undefined8 *)(param_1 + 0x1a0);
  uVar1 = *(undefined8 *)(param_1 + 0x198);
  uVar3 = *(undefined8 *)(param_1 + 0x1b8);
  uVar5 = *(undefined8 *)(param_1 + 0x1d0);
  uVar4 = *(undefined8 *)(param_1 + 0x1c8);
  uVar7 = *(undefined8 *)(param_1 + 0x1b0);
  uVar6 = *(undefined8 *)(param_1 + 0x1a8);
  param_2[0x11] = *(undefined8 *)(param_1 + 0x1c0);
  param_2[0x10] = uVar3;
  param_2[0x13] = uVar5;
  param_2[0x12] = uVar4;
  param_2[0xd] = uVar2;
  param_2[0xc] = uVar1;
  param_2[0xf] = uVar7;
  param_2[0xe] = uVar6;
  uVar1 = *(undefined8 *)(param_1 + 0x1f8);
  uVar3 = *(undefined8 *)(param_1 + 0x210);
  uVar2 = *(undefined8 *)(param_1 + 0x208);
  uVar5 = *(undefined8 *)(param_1 + 0x1e0);
  uVar4 = *(undefined8 *)(param_1 + 0x1d8);
  uVar7 = *(undefined8 *)(param_1 + 0x1f0);
  uVar6 = *(undefined8 *)(param_1 + 0x1e8);
  param_2[0x19] = *(undefined8 *)(param_1 + 0x200);
  param_2[0x18] = uVar1;
  param_2[0x1b] = uVar3;
  param_2[0x1a] = uVar2;
  param_2[0x15] = uVar5;
  param_2[0x14] = uVar4;
  param_2[0x17] = uVar7;
  param_2[0x16] = uVar6;
  return 0;
}

