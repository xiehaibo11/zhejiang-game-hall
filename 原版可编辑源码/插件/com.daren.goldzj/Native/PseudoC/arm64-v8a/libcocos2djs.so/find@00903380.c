...
/* se::NonRefNativePtrCreatedByCtorMap::find(void*) */

long se::NonRefNativePtrCreatedByCtorMap::find(void *param_1)

{
  ...
  uVar3 = __nonRefNativeObjectCreatedByCtorMap[1];
  if (uVar3 == 0) {
    return 0;
  }
  ...
  plVar2 = *(long **)(*__nonRefNativeObjectCreatedByCtorMap + uVar6 * 8);
  if (plVar2 != (long *)0x0) {
    do {
      while( true ) {
        while( true ) {
