
void FUN_01438a60(undefined8 param_1,undefined8 param_2,undefined8 param_3)

{
  uint *puVar1;
  undefined8 *puVar2;
  long lVar3;
  ulong uVar4;
  byte bVar5;
  char cVar6;
  undefined4 uVar7;
  long lVar8;
  undefined4 uVar9;
  ulong uVar10;
  undefined8 uVar11;
  long lVar12;
  ulong uVar13;
  int iVar14;
  int iVar15;
  long unaff_x19;
  long unaff_x20;
  long unaff_x26;
  long unaff_x29;
  
  *(int *)(unaff_x29 + -0x20) = (int)unaff_x19 * 2;
  lVar12 = *(long *)(unaff_x29 + -8);
  bVar5 = *(byte *)(unaff_x20 + unaff_x19 + 1);
  cVar6 = *(char *)(unaff_x20 + unaff_x19 + 2);
  iVar14 = (int)cVar6;
  if (0xb < bVar5) {
    iVar15 = (int)cVar6;
    if (0x11 < bVar5) {
      if (0x14 < bVar5) {
        if (0x16 < bVar5) {
          if (bVar5 == 0x17) {
            lVar12 = thunk_FUN_0134a790(*(undefined8 *)(unaff_x29 + (long)iVar14 * 8));
            goto LAB_014390c8;
          }
          if (bVar5 == 0x18) {
            lVar12 = FUN_013492a0(*(undefined8 *)(unaff_x29 + (long)iVar14 * 8));
            goto LAB_014390c8;
          }
        }
        if (bVar5 == 0x15) {
          if ((*(ulong *)(unaff_x29 + (long)iVar14 * 8) & 1) == 0) {
            lVar12 = *(long *)(unaff_x26 + 0xb8);
          }
          else {
            lVar12 = *(long *)(unaff_x26 + 0xc0);
          }
          goto LAB_014390c8;
        }
        if (bVar5 == 0x16) {
          lVar12 = FUN_013e8ae0(*(undefined8 *)(unaff_x29 + (long)iVar14 * 8));
          goto LAB_014390c8;
        }
      }
      if (bVar5 == 0x12) {
        puVar2 = (undefined8 *)(unaff_x29 + (long)iVar15 * 8);
        lVar12 = FUN_01369d20(*puVar2,puVar2[-1]);
        goto LAB_014390c8;
      }
      if (bVar5 == 0x13) {
        uVar10 = *(ulong *)(unaff_x29 + (long)iVar15 * 8);
        if ((uVar10 & 1) == 0) {
          lVar12 = *(long *)(unaff_x26 + 0xc0);
        }
        else if (*(short *)(unaff_x26 + (ulong)*(uint *)(uVar10 - 1) + 7) == 0x423) {
          lVar12 = *(long *)(unaff_x26 + 0xb8);
        }
        else {
          lVar12 = *(long *)(unaff_x26 + 0xc0);
        }
        goto LAB_014390c8;
      }
      if (bVar5 == 0x14) {
        uVar10 = *(ulong *)(unaff_x29 + (long)iVar15 * 8);
        if ((uVar10 & 1) == 0) {
          lVar12 = *(long *)(unaff_x26 + 0xc0);
        }
        else if (*(ushort *)(unaff_x26 + (ulong)*(uint *)(uVar10 - 1) + 7) < 0xa9) {
          lVar12 = *(long *)(unaff_x26 + 0xc0);
        }
        else {
          lVar12 = *(long *)(unaff_x26 + 0xb8);
        }
        goto LAB_014390c8;
      }
    }
    if (0xe < bVar5) {
      if (bVar5 == 0xf) {
        puVar2 = (undefined8 *)(unaff_x29 + (long)iVar15 * 8);
        lVar12 = FUN_0136ac20(*puVar2,puVar2[-1]);
        goto LAB_014390c8;
      }
      if (bVar5 == 0x10) {
        puVar2 = (undefined8 *)(unaff_x29 + (long)iVar15 * 8);
        lVar12 = FUN_0137aaa0(*puVar2,puVar2[-1]);
        goto LAB_014390c8;
      }
      if (bVar5 == 0x11) {
        uVar10 = *(ulong *)(unaff_x29 + (long)iVar15 * 8);
        if (((uVar10 & 1) == 0) ||
           (*(ushort *)(unaff_x26 + (ulong)*(uint *)(uVar10 - 1) + 7) < 0xa9)) {
                    /* WARNING: Subroutine does not return */
          FUN_013be7e0(0,*(undefined8 *)(unaff_x26 + 0x20d8));
        }
        uVar7 = FUN_013c0ba0(uVar10,*(undefined8 *)(unaff_x26 + 0x848));
        uVar13 = **(ulong **)(unaff_x26 + 0x1428);
        lVar3 = unaff_x26 +
                (ulong)*(uint *)(unaff_x26 +
                                 (ulong)*(uint *)(unaff_x26 + (ulong)*(uint *)(lVar12 + -1) + 0x13)
                                + 0x37);
        lVar8 = (ulong)*(byte *)(lVar3 + 3) * 4;
        uVar4 = uVar13 + lVar8;
        if (uVar4 < **(ulong **)(unaff_x26 + 0x1430)) {
          lVar12 = uVar13 + 1;
          **(ulong **)(unaff_x26 + 0x1428) = uVar4;
        }
        else {
          lVar12 = FUN_01348560();
        }
        *(int *)(lVar12 + -1) = (int)lVar3;
        lVar8 = lVar8 + -1;
        uVar9 = (undefined4)*(undefined8 *)(unaff_x26 + 0x168);
        *(undefined4 *)(lVar12 + 3) = uVar9;
        *(undefined4 *)(lVar12 + 7) = uVar9;
        if (lVar8 != 0xb) {
          uVar11 = *(undefined8 *)(unaff_x26 + 0xa0);
          do {
            lVar8 = lVar8 + -4;
            *(int *)(lVar12 + lVar8) = (int)uVar11;
          } while (lVar8 != 0xb);
        }
        *(int *)(lVar12 + 0xb) = (int)uVar10;
        *(undefined4 *)(lVar12 + 0xf) = uVar7;
        goto LAB_014390c8;
      }
    }
    if (bVar5 == 0xc) {
      *(undefined4 *)(*(long *)(unaff_x29 + (long)iVar14 * 8) + 0x1f) = 0xfffffffe;
      lVar12 = *(long *)(unaff_x26 + 0xa0);
      goto LAB_014390c8;
    }
    if (bVar5 == 0xd) {
      puVar1 = (uint *)(lVar12 + -1);
      for (; *(int *)(lVar12 + -1) !=
             *(int *)(unaff_x26 + (ulong)*(uint *)(unaff_x26 + (ulong)*puVar1 + 0x13) + 0x1ab);
          lVar12 = unaff_x26 + (ulong)*(uint *)(lVar12 + 0xb)) {
      }
      lVar12 = unaff_x26 + (ulong)*(uint *)(unaff_x26 + (ulong)*(uint *)(lVar12 + 0xf) + 0x2b);
      if ((int)lVar12 == (int)*(undefined8 *)(unaff_x26 + 0xa8)) {
                    /* WARNING: Subroutine does not return */
        FUN_013be7e0(0,*(undefined8 *)(unaff_x26 + 0x2138));
      }
      goto LAB_014390c8;
    }
    if (bVar5 == 0xe) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      FUN_01347360(*(byte *)(unaff_x20 + unaff_x19 + 3) - 2,*puVar2,puVar2 + -1);
      return;
    }
  }
  if (5 < bVar5) {
    if (8 < bVar5) {
      if (bVar5 == 9) {
        puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
        lVar12 = FUN_013babe0(*puVar2,puVar2[-1],puVar2[-2]);
        goto LAB_014390c8;
      }
      if (bVar5 == 10) {
        puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
        lVar12 = FUN_0137aba0(*puVar2,puVar2[-1]);
        goto LAB_014390c8;
      }
      if (bVar5 == 0xb) {
        lVar12 = unaff_x26 + (ulong)*(uint *)(*(long *)(unaff_x29 + (long)iVar14 * 8) + 0x1b);
        goto LAB_014390c8;
      }
    }
    if (bVar5 == 6) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_013bc980(*puVar2,puVar2[-1]);
      goto LAB_014390c8;
    }
    if (bVar5 == 7) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_013bab20(*puVar2,puVar2[-1]);
      goto LAB_014390c8;
    }
    if (bVar5 == 8) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_013ba980(*puVar2,puVar2[-1],puVar2[-2]);
      goto LAB_014390c8;
    }
  }
  if (2 < bVar5) {
    if (bVar5 == 3) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_01377620(*puVar2,puVar2[-1],puVar2[-2]);
      goto LAB_014390c8;
    }
    if (bVar5 == 4) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_013776e0(*puVar2,puVar2[-1],puVar2[-2]);
      goto LAB_014390c8;
    }
    if (bVar5 == 5) {
      puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
      lVar12 = FUN_013bc480(*puVar2,puVar2[-1]);
      goto LAB_014390c8;
    }
  }
  if (bVar5 == 0) {
    puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
    lVar12 = FUN_013777c0(*puVar2,puVar2[-1]);
  }
  else if (bVar5 == 1) {
    puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
    lVar12 = FUN_01377ce0(*puVar2,puVar2[-1]);
  }
  else {
    if (bVar5 != 2) {
                    /* WARNING: Subroutine does not return */
      FUN_013be7e0(1,*(undefined8 *)(unaff_x26 + 0x2630),param_3,unaff_x29,
                   *(undefined8 *)(unaff_x29 + -8));
    }
    puVar2 = (undefined8 *)(unaff_x29 + (long)iVar14 * 8);
    lVar12 = FUN_01377340(*puVar2,puVar2[-1]);
  }
LAB_014390c8:
                    /* WARNING: Could not recover jumptable at 0x014390f8. Too many branches */
                    /* WARNING: Treating indirect jump as call */
  (**(code **)(*(long *)(unaff_x26 + 0x13e0) +
              (ulong)*(byte *)(*(long *)(unaff_x29 + -0x18) +
                              ((long)((ulong)*(uint *)(unaff_x29 + -0x20) << 0x20) >> 0x21) + 4) * 8
              ))(lVar12);
  return;
}

