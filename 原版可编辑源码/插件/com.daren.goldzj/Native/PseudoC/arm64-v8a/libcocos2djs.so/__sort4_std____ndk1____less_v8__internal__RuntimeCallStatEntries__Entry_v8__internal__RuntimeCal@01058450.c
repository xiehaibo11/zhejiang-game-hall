
/* unsigned int
   std::__ndk1::__sort4<std::__ndk1::__less<v8::internal::RuntimeCallStatEntries::Entry,
   v8::internal::RuntimeCallStatEntries::Entry>&,
   std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>
   > 
   >(std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>
   >, 
   std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>
   >, 
   std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>
   >, 
   std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>
   >, std::__ndk1::__less<v8::internal::RuntimeCallStatEntries::Entry,
   v8::internal::RuntimeCallStatEntries::Entry>&) */

uint std::__ndk1::
     __sort4<std::__ndk1::__less<v8::internal::RuntimeCallStatEntries::Entry,v8::internal::RuntimeCallStatEntries::Entry>&,std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>>>
               (undefined8 param_1,long param_2,undefined8 param_3,long param_4,undefined8 param_5,
               long param_6,undefined8 param_7,long param_8)

{
  uint uVar1;
  uint uVar2;
  undefined8 uVar3;
  undefined8 uVar4;
  undefined8 *puVar5;
  undefined8 *puVar6;
  undefined8 uVar7;
  undefined8 uVar8;
  undefined8 uVar9;
  undefined8 uVar10;
  undefined8 uVar11;
  undefined8 uVar12;
  undefined8 uVar13;
  
  uVar1 = __sort3<std::__ndk1::__less<v8::internal::RuntimeCallStatEntries::Entry,v8::internal::RuntimeCallStatEntries::Entry>&,std::__ndk1::reverse_iterator<std::__ndk1::__wrap_iter<v8::internal::RuntimeCallStatEntries::Entry*>>>
                    ();
  if ((*(long *)(param_8 + -0x20) < *(long *)(param_6 + -0x20)) ||
     ((uVar2 = uVar1, *(long *)(param_8 + -0x20) <= *(long *)(param_6 + -0x20) &&
      (*(ulong *)(param_8 + -0x18) < *(ulong *)(param_6 + -0x18))))) {
    puVar5 = (undefined8 *)(param_6 + -0x28);
    uVar3 = *(undefined8 *)(param_6 + -8);
    uVar12 = *(undefined8 *)(param_6 + -0x20);
    uVar10 = *puVar5;
    uVar9 = *(undefined8 *)(param_6 + -0x10);
    uVar7 = *(undefined8 *)(param_6 + -0x18);
    uVar8 = *(undefined8 *)(param_8 + -0x28);
    uVar13 = *(undefined8 *)(param_8 + -0x10);
    uVar11 = *(undefined8 *)(param_8 + -0x18);
    uVar4 = *(undefined8 *)(param_8 + -8);
    *(undefined8 *)(param_6 + -0x20) = *(undefined8 *)(param_8 + -0x20);
    *puVar5 = uVar8;
    *(undefined8 *)(param_6 + -0x10) = uVar13;
    *(undefined8 *)(param_6 + -0x18) = uVar11;
    *(undefined8 *)(param_6 + -8) = uVar4;
    *(undefined8 *)(param_8 + -0x20) = uVar12;
    *(undefined8 *)(param_8 + -0x28) = uVar10;
    *(undefined8 *)(param_8 + -0x10) = uVar9;
    *(undefined8 *)(param_8 + -0x18) = uVar7;
    *(undefined8 *)(param_8 + -8) = uVar3;
    if ((*(long *)(param_6 + -0x20) < *(long *)(param_4 + -0x20)) ||
       ((uVar2 = uVar1 + 1, *(long *)(param_6 + -0x20) <= *(long *)(param_4 + -0x20) &&
        (*(ulong *)(param_6 + -0x18) < *(ulong *)(param_4 + -0x18))))) {
      puVar6 = (undefined8 *)(param_4 + -0x28);
      uVar3 = *(undefined8 *)(param_4 + -8);
      uVar12 = *(undefined8 *)(param_4 + -0x20);
      uVar10 = *puVar6;
      uVar9 = *(undefined8 *)(param_4 + -0x10);
      uVar7 = *(undefined8 *)(param_4 + -0x18);
      uVar8 = *puVar5;
      uVar13 = *(undefined8 *)(param_6 + -0x10);
      uVar11 = *(undefined8 *)(param_6 + -0x18);
      uVar4 = *(undefined8 *)(param_6 + -8);
      *(undefined8 *)(param_4 + -0x20) = *(undefined8 *)(param_6 + -0x20);
      *puVar6 = uVar8;
      *(undefined8 *)(param_4 + -0x10) = uVar13;
      *(undefined8 *)(param_4 + -0x18) = uVar11;
      *(undefined8 *)(param_4 + -8) = uVar4;
      *(undefined8 *)(param_6 + -0x20) = uVar12;
      *puVar5 = uVar10;
      *(undefined8 *)(param_6 + -0x10) = uVar9;
      *(undefined8 *)(param_6 + -0x18) = uVar7;
      *(undefined8 *)(param_6 + -8) = uVar3;
      if ((*(long *)(param_4 + -0x20) < *(long *)(param_2 + -0x20)) ||
         ((uVar2 = uVar1 + 2, *(long *)(param_4 + -0x20) <= *(long *)(param_2 + -0x20) &&
          (*(ulong *)(param_4 + -0x18) < *(ulong *)(param_2 + -0x18))))) {
        uVar3 = *(undefined8 *)(param_2 + -8);
        uVar12 = *(undefined8 *)(param_2 + -0x20);
        uVar10 = *(undefined8 *)(param_2 + -0x28);
        uVar9 = *(undefined8 *)(param_2 + -0x10);
        uVar7 = *(undefined8 *)(param_2 + -0x18);
        uVar8 = *puVar6;
        uVar13 = *(undefined8 *)(param_4 + -0x10);
        uVar11 = *(undefined8 *)(param_4 + -0x18);
        uVar4 = *(undefined8 *)(param_4 + -8);
        *(undefined8 *)(param_2 + -0x20) = *(undefined8 *)(param_4 + -0x20);
        *(undefined8 *)(param_2 + -0x28) = uVar8;
        *(undefined8 *)(param_2 + -0x10) = uVar13;
        *(undefined8 *)(param_2 + -0x18) = uVar11;
        *(undefined8 *)(param_2 + -8) = uVar4;
        *(undefined8 *)(param_4 + -8) = uVar3;
        *(undefined8 *)(param_4 + -0x20) = uVar12;
        *puVar6 = uVar10;
        *(undefined8 *)(param_4 + -0x10) = uVar9;
        *(undefined8 *)(param_4 + -0x18) = uVar7;
        uVar2 = uVar1 + 3;
      }
    }
  }
  return uVar2;
}

