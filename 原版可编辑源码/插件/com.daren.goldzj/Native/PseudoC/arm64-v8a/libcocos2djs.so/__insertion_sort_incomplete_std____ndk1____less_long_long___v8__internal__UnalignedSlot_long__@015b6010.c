
/* bool std::__ndk1::__insertion_sort_incomplete<std::__ndk1::__less<long, long>&,
   v8::internal::UnalignedSlot<long> >(v8::internal::UnalignedSlot<long>,
   v8::internal::UnalignedSlot<long>, std::__ndk1::__less<long, long>&) */

bool std::__ndk1::
     __insertion_sort_incomplete<std::__ndk1::__less<long,long>&,v8::internal::UnalignedSlot<long>>
               (long *param_1,long *param_2,undefined8 param_3)

{
  long lVar1;
  long *plVar2;
  int iVar3;
  long lVar4;
  long lVar5;
  long *plVar6;
  long lVar7;
  long lVar8;
  long *plVar9;
  
  switch((ulong)(long)((int)param_2 - (int)param_1) >> 3 & 0xffffffff) {
  case 0:
  case 1:
    break;
  case 2:
    lVar4 = *param_1;
    if (param_2[-1] < lVar4) {
      *param_1 = param_2[-1];
      param_2[-1] = lVar4;
      return true;
    }
    break;
  case 3:
    __sort3<std::__ndk1::__less<long,long>&,v8::internal::UnalignedSlot<long>>
              (param_1,param_1 + 1,param_2 + -1,param_3);
    return true;
  case 4:
    __sort4<std::__ndk1::__less<long,long>&,v8::internal::UnalignedSlot<long>>
              (param_1,param_1 + 1,param_1 + 2,param_2 + -1,param_3);
    return true;
  case 5:
    FUN_015b5f50(param_1,param_1 + 1,param_1 + 2,param_1 + 3,param_2 + -1);
    break;
  default:
    __sort3<std::__ndk1::__less<long,long>&,v8::internal::UnalignedSlot<long>>
              (param_1,param_1 + 1,param_1 + 2,param_3);
    if (param_1 + 3 != param_2) {
      iVar3 = 0;
      lVar4 = 0x10;
      plVar6 = param_1 + 3;
      plVar9 = param_1 + 2;
      do {
        plVar2 = plVar6;
        lVar5 = *plVar2;
        lVar7 = *plVar9;
        lVar1 = lVar4;
        if (lVar5 < lVar7) {
          do {
            lVar8 = lVar1;
            *(long *)((long)param_1 + lVar8 + 8) = lVar7;
            plVar6 = param_1;
            if (lVar8 == 0) goto LAB_015b60b4;
            lVar7 = *(long *)((long)param_1 + lVar8 + -8);
            lVar1 = lVar8 + -8;
          } while (lVar5 < lVar7);
          plVar6 = (long *)((long)param_1 + lVar8);
LAB_015b60b4:
          iVar3 = iVar3 + 1;
          *plVar6 = lVar5;
          if (iVar3 == 8) {
            return plVar2 + 1 == param_2;
          }
        }
        lVar4 = lVar4 + 8;
        plVar6 = plVar2 + 1;
        plVar9 = plVar2;
      } while (plVar2 + 1 != param_2);
    }
  }
  return true;
}

