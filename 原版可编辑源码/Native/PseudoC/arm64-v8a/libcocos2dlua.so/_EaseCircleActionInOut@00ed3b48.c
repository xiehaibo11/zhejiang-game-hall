
/* non-virtual thunk to cocos2d::EaseCircleActionInOut::~EaseCircleActionInOut() */

void __thiscall cocos2d::EaseCircleActionInOut::~EaseCircleActionInOut(EaseCircleActionInOut *this)

{
  *(undefined ***)(this + -0x28) = &PTR__ActionEase_016f5ab0;
  *(undefined ***)this = &PTR_clone_016f5b18;
  if (*(Ref **)(this + 0x30) != (Ref *)0x0) {
                    /* try { // try from 00ed3b7c to 00fd3ca7 has its CatchHandler @ 00ed3b7c
                       catch() { ... } // from try @ 00ed3b7c with catch @ 00ed3b7c
                       catch() { ... } // from try @ 00ed4080 with catch @ 00ed3b7c */
    Ref::release(*(Ref **)(this + 0x30));
  }
  Action::~Action((Action *)(this + -0x28));
  return;
}

