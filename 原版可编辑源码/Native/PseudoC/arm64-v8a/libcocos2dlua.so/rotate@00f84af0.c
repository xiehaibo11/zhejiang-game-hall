
/* cocos2d::Mat4::rotate(cocos2d::Quaternion const&, cocos2d::Mat4*) const */

void __thiscall cocos2d::Mat4::rotate(Mat4 *this,Quaternion *param_1,Mat4 *param_2)

{
  long lVar1;
  float fVar2;
  float fVar3;
  float fVar4;
  float fVar5;
  float fVar6;
  float fVar7;
  float local_68;
  float local_64;
  float fStack_60;
  undefined4 local_5c;
  float local_58;
  float fStack_54;
  float local_50;
  undefined4 local_4c;
  float local_48;
  float fStack_44;
  float local_40;
  undefined8 local_3c;
  undefined8 local_34;
  undefined4 local_2c;
  long local_28;
  
  lVar1 = tpidr_el0;
  local_28 = *(long *)(lVar1 + 0x28);
  fVar2 = *(float *)param_1;
  fVar3 = *(float *)(param_1 + 4);
  fVar4 = *(float *)(param_1 + 8);
  fVar5 = *(float *)(param_1 + 0xc);
  local_5c = 0;
  fVar6 = fVar3 + fVar3;
  fVar7 = fVar4 + fVar4;
  fStack_44 = (fVar2 + fVar2) * fVar5;
  local_64 = fVar2 * fVar6 + fVar5 * fVar7;
  local_58 = fVar2 * fVar6 - fVar5 * fVar7;
  local_40 = 1.0 - fVar2 * (fVar2 + fVar2);
  fStack_60 = fVar2 * fVar7 - fVar6 * fVar5;
  local_48 = fVar2 * fVar7 + fVar6 * fVar5;
  local_50 = fVar3 * fVar7 + fStack_44;
  fStack_44 = fVar3 * fVar7 - fStack_44;
  local_68 = (1.0 - fVar3 * fVar6) - fVar4 * fVar7;
  fStack_54 = local_40 - fVar4 * fVar7;
  local_40 = local_40 - fVar3 * fVar6;
  local_4c = 0;
  local_34 = 0;
  local_3c = 0;
  local_2c = 0x3f800000;
  MathUtil::multiplyMatrix((float *)this,&local_68,(float *)param_2);
  if (*(long *)(lVar1 + 0x28) == local_28) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}

