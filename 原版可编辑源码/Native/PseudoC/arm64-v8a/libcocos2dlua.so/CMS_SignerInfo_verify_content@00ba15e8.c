...
LAB_00ba17d8:
    ...
    if (iVar1 < 1) {
                    /* try { // try from 00ba16fc to 00ca1703 has its CatchHandler @ 00ba1778 */
                    /* try { // try from 00ba1704 to 00ca1727 has its CatchHandler @ 00ba1540 */
      iVar1 = 0x93;
      iVar2 = 0x324;
LAB_00ba17d4:
...
LAB_00ba16f0:
    ...
    p2 = EVP_MD_CTX_md(ctx);
                    /* try { // try from 00ba1728 to 00ca1737 has its CatchHandler @ 00ba1788 */
    ctx_00 = EVP_PKEY_CTX_new(*(EVP_PKEY **)(si + 0x40),(ENGINE *)0x0);
                    /* try { // try from 00ba1738 to 00ca1767 has its CatchHandler @ 00ba1540 */
    if ((ctx_00 != (EVP_PKEY_CTX *)0x0) && (iVar1 = EVP_PKEY_verify_init(ctx_00), 0 < iVar1)) {
      iVar2 = -1;
      iVar1 = EVP_PKEY_CTX_ctrl(ctx_00,-1,0xf8,1,0,p2);
                    /* try { // try from 00ba1768 to 00ca1777 has its CatchHandler @ 00ba1778 */
      if (iVar1 < 1) goto LAB_00ba17e0;
      *(EVP_PKEY_CTX **)(si + 0x50) = ctx_00;
                    /* catch() { ... } // from try @ 00ba16fc with catch @ 00ba1778
                       catch() { ... } // from try @ 00ba1768 with catch @ 00ba1778
                       try { // try from 00ba1778 to 00ca17c3 has its CatchHandler @ 00ba1540 */
...
LAB_00ba1820:
        ...
        if (0 < iVar2) goto LAB_00ba17e0;
        ERR_put_error(0x2e,0x9a,0x9e,"crypto/cms/cms_sd.c",0x347);
        goto LAB_00ba16f0;
      }
                    /* catch() { ... } // from try @ 00ba1728 with catch @ 00ba1788 */
