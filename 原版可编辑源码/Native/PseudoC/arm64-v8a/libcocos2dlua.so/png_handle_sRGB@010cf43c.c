...

{
  ...
  if ((*(uint *)(param_1 + 0x164) & 6) == 0) {
    if (param_3 == 1) {
                    /* try { // try from 010cf488 to 011cf48f has its CatchHandler @ 010cf578 */
      png_read_data(param_1,local_24,1);
                    /* try { // try from 010cf494 to 011cf4fb has its CatchHandler @ 010cf57c */
  ...
  png_chunk_benign_error(param_1,pcVar3);
                    /* try { // try from 010cf4fc to 011cf597 has its CatchHandler @ 010cf320 */
  return;
}
