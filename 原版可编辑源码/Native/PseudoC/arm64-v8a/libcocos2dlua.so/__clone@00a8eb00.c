
/* std::__ndk1::__function::__func<std::__ndk1::__bind<void
   (fairygui::GTree::*)(fairygui::EventContext*), fairygui::GTree*,
   std::__ndk1::placeholders::__ph<1> const&>, std::__ndk1::allocator<std::__ndk1::__bind<void
   (fairygui::GTree::*)(fairygui::EventContext*), fairygui::GTree*,
   std::__ndk1::placeholders::__ph<1> const&> >, void
   (fairygui::EventContext*)>::__clone(std::__ndk1::__function::__base<void
   (fairygui::EventContext*)>*) const */

void __thiscall
std::__ndk1::__function::
__func<std::__ndk1::__bind<void(fairygui::GTree::*)(fairygui::EventContext*),fairygui::GTree*,std::__ndk1::placeholders::__ph<1>const&>,std::__ndk1::allocator<std::__ndk1::__bind<void(fairygui::GTree::*)(fairygui::EventContext*),fairygui::GTree*,std::__ndk1::placeholders::__ph<1>const&>>,void(fairygui::EventContext*)>
::__clone(__func<std::__ndk1::__bind<void(fairygui::GTree::*)(fairygui::EventContext*),fairygui::GTree*,std::__ndk1::placeholders::__ph<1>const&>,std::__ndk1::allocator<std::__ndk1::__bind<void(fairygui::GTree::*)(fairygui::EventContext*),fairygui::GTree*,std::__ndk1::placeholders::__ph<1>const&>>,void(fairygui::EventContext*)>
          *this,__base *param_1)

{
  undefined8 uVar1;
  undefined8 uVar2;
  
  *(undefined ***)param_1 = &PTR_FUN_016a70f8;
                    /* try { // try from 00a8eb10 to 00b8ebab has its CatchHandler @ 00a8e570 */
  uVar2 = *(undefined8 *)(this + 0x10);
  uVar1 = *(undefined8 *)(this + 8);
  *(undefined8 *)(param_1 + 0x18) = *(undefined8 *)(this + 0x18);
  *(undefined8 *)(param_1 + 0x10) = uVar2;
  *(undefined8 *)(param_1 + 8) = uVar1;
  return;
}

