
/* std::__ndk1::time_get<char, std::__ndk1::istreambuf_iterator<char, std::__ndk1::char_traits<char>
   > >::do_get_monthname(std::__ndk1::istreambuf_iterator<char, std::__ndk1::char_traits<char> >,
   std::__ndk1::istreambuf_iterator<char, std::__ndk1::char_traits<char> >, std::__ndk1::ios_base&,
   unsigned int&, tm*) const */

void __thiscall
std::__ndk1::time_get<char,std::__ndk1::istreambuf_iterator<char,std::__ndk1::char_traits<char>>>::
do_get_monthname(time_get<char,std::__ndk1::istreambuf_iterator<char,std::__ndk1::char_traits<char>>>
                 *this,undefined8 param_2,undefined8 param_3,undefined8 param_4,undefined8 param_5,
                long param_6)

{
  long lVar1;
  long lVar2;
  long lVar3;
  __shared_count *local_90;
  undefined8 local_88;
  undefined8 *local_80;
  code *local_78;
  undefined8 uStack_70;
  undefined8 **local_68;
  undefined8 **local_60;
  long local_58;
  
  lVar1 = tpidr_el0;
  local_58 = *(long *)(lVar1 + 0x28);
  local_88 = param_2;
  ios_base::getloc();
  local_80 = &ctype<char>::id;
  uStack_70 = 0;
  local_78 = locale::id::__init;
  if (ctype<char>::id != -1) {
    local_60 = &local_80;
    local_68 = &local_60;
    __call_once((ulong *)&ctype<char>::id,&local_68,FUN_01208130);
  }
  if (((ulong)(*(long *)(local_90 + 0x18) - *(long *)(local_90 + 0x10) >> 3) <=
       (long)DAT_01794330 - 1U) ||
     (lVar3 = *(long *)(*(long *)(local_90 + 0x10) + ((long)DAT_01794330 - 1U) * 8), lVar3 == 0)) {
                    /* WARNING: Subroutine does not return */
    FUN_00cc7c00();
  }
  __shared_count::__release_shared(local_90);
  lVar2 = (**(code **)(*(long *)(this + 0x10) + 8))(this + 0x10);
  lVar3 = FUN_011dcaa8(&local_88,param_3,lVar2,lVar2 + 0x240,lVar3,param_5,0);
  if (lVar3 - lVar2 < 0x240) {
    lVar3 = (lVar3 - lVar2 >> 3) * -0x5555555555555555;
    *(int *)(param_6 + 0x10) =
         (int)lVar3 +
         ((int)((ulong)(lVar3 / 6 + (lVar3 >> 0x3f)) >> 1) -
         (SUB164(SEXT816(lVar3) * SEXT816(0x2aaaaaaaaaaaaaab),0xc) >> 0x1f)) * -0xc;
  }
  if (*(long *)(lVar1 + 0x28) == local_58) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail(local_88);
}

