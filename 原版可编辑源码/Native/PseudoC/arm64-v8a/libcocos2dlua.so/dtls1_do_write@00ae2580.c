...
LAB_00ae2640:
                    /* try { // try from 00ae2640 to 00be266f has its CatchHandler @ 00ae2640
                       catch() { ... } // from try @ 00ae2640 with catch @ 00ae2640
                       catch() { ... } // from try @ 00ae26a0 with catch @ 00ae2640 */
    iVar4 = 0xd;
    ctx_00 = (EVP_CIPHER_CTX *)param_1->generate_session_id;
  }
  else {
    if ((EVP_CIPHER_CTX *)param_1->generate_session_id != (EVP_CIPHER_CTX *)0x0) {
                    /* try { // try from 00ae2618 to 00be263f has its CatchHandler @ 00ae2514 */
      pEVar8 = EVP_CIPHER_CTX_cipher((EVP_CIPHER_CTX *)param_1->generate_session_id);
      uVar9 = EVP_CIPHER_flags(pEVar8);
      if (((uint)uVar9 >> 0x15 & 1) != 0) goto LAB_00ae2640;
      ctx = *(EVP_MD_CTX **)&param_1->verify_mode;
    }
    md = EVP_MD_CTX_md(ctx);
                    /* catch() { ... } // from try @ 00ae2564 with catch @ 00ae262c */
  ...
  if (ctx_00 != (EVP_CIPHER_CTX *)0x0) {
    ...
    if ((uVar9 & 0xf0007) == 2) {
      iVar7 = EVP_CIPHER_CTX_block_size((EVP_CIPHER_CTX *)param_1->generate_session_id);
                    /* try { // try from 00ae2670 to 00be2673 has its CatchHandler @ 00ae2724 */
      iVar7 = iVar7 << 1;
      goto LAB_00ae267c;
    }
  }
...
LAB_00ae267c:
  ...
  uVar9 = 0;
  bVar3 = true;
                    /* try { // try from 00ae2690 to 00be2693 has its CatchHandler @ 00ae2704 */
  param_1->rwstate = 1;
  while( true ) {
    iVar7 = *(int *)&param_1->d1;
                    /* try { // try from 00ae2698 to 00be269f has its CatchHandler @ 00ae26e0 */
    while( true ) {
      if (iVar7 < 1) {
        return 0;
      }
        ...
        if (((int)uVar9 == 0) && (param_1->version != 0x100)) {
          ...
          *(undefined2 *)(pcVar11 + 7) = 0;
          ...
          uVar6 = uVar5;
                    ...
                    /* catch() { ... } // from try @ 00ae2698 with catch @ 00ae26e0 */
          pcVar11 = pcVar11 + 0xc;
          uVar6 = uVar5 - 0xc;
        }
      ...
      iVar1 = *(int *)((long)&param_1->d1 + 4);
                    /* catch() { ... } // from try @ 00ae2690 with catch @ 00ae2704 */
      p_Var13 = param_1->msg_callback;
