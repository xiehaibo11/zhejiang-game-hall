
void FUN_010fefbc(long *param_1,int param_2,uint param_3,long *param_4)

{
  bool bVar1;
  long lVar2;
  long *plVar3;
  byte bVar4;
  long lVar5;
  int iVar6;
  undefined8 *puVar7;
  uint *puVar8;
  byte *pbVar9;
  ulong uVar10;
  long lVar11;
  int iVar12;
  long lVar13;
  long lVar14;
  long lVar15;
  uint uVar16;
  ulong uVar17;
  uint uVar18;
  uint auStack_574 [258];
  byte local_16c [260];
  long local_68;
  
  lVar5 = tpidr_el0;
  local_68 = *(long *)(lVar5 + 0x28);
  if (3 < param_3) {
    puVar7 = (undefined8 *)*param_1;
    *(undefined4 *)(puVar7 + 5) = 0x34;
    *(uint *)((long)puVar7 + 0x2c) = param_3;
    (*(code *)*puVar7)(param_1);
  }
  plVar3 = param_1 + 0x1d;
                    /* try { // try from 010ff028 to 011ff067 has its CatchHandler @ 010ff07c */
  if (param_2 == 0) {
    plVar3 = param_1 + 0x21;
  }
  lVar15 = plVar3[(int)param_3];
  if (lVar15 == 0) {
    puVar7 = (undefined8 *)*param_1;
    *(undefined4 *)(puVar7 + 5) = 0x34;
    *(uint *)((long)puVar7 + 0x2c) = param_3;
    (*(code *)*puVar7)(param_1);
  }
  lVar13 = *param_4;
  if (lVar13 == 0) {
    lVar13 = (**(code **)param_1[1])(param_1,1,0x620);
    *param_4 = lVar13;
  }
  uVar16 = 0;
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff028 with catch @ 010ff07c
                        */
  lVar14 = 1;
  *(long *)(lVar13 + 0x118) = lVar15;
  do {
    bVar4 = *(byte *)(lVar15 + lVar14);
                    /* try { // try from 010ff090 to 011ff093 has its CatchHandler @ 010ff0d8 */
                    /* try { // try from 010ff094 to 011ff0eb has its CatchHandler @ 010fefa0 */
    if (0x100 < (int)(uVar16 + bVar4)) {
      puVar7 = (undefined8 *)*param_1;
      *(undefined4 *)(puVar7 + 5) = 9;
      (*(code *)*puVar7)(param_1);
    }
    if (bVar4 != 0) {
      memset(local_16c + (int)uVar16,(int)lVar14,(ulong)(bVar4 - 1) + 1);
      uVar16 = uVar16 + bVar4;
    }
    lVar14 = lVar14 + 1;
  } while (lVar14 != 0x11);
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff090 with catch @ 010ff0d8
                        */
  local_16c[(int)uVar16] = 0;
  uVar17 = (ulong)local_16c[0];
  if (uVar17 != 0) {
    iVar6 = 0;
    uVar18 = 0;
    uVar10 = uVar17;
    do {
      if (uVar10 == uVar17) {
                    /* try { // try from 010ff110 to 011ff113 has its CatchHandler @ 010ff154 */
        puVar8 = auStack_574 + (long)iVar6 + 1;
        pbVar9 = (byte *)(((ulong)local_16c | 1) + (long)iVar6);
        do {
          *puVar8 = uVar18;
          bVar4 = *pbVar9;
          uVar18 = uVar18 + 1;
          iVar6 = iVar6 + 1;
          puVar8 = puVar8 + 1;
          pbVar9 = pbVar9 + 1;
        } while (uVar10 == bVar4);
      }
      if (1L << (uVar10 & 0x3f) <= (long)(ulong)uVar18) {
        puVar7 = (undefined8 *)*param_1;
        *(undefined4 *)(puVar7 + 5) = 9;
        (*(code *)*puVar7)(param_1);
      }
      bVar4 = local_16c[iVar6];
      uVar17 = (ulong)bVar4;
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff110 with catch @ 010ff154
                        */
      uVar18 = uVar18 << 1;
      uVar10 = uVar10 + 1;
    } while (bVar4 != 0);
  }
  if (*(char *)(lVar15 + 1) == '\0') {
    uVar10 = 0xffffffffffffffff;
    uVar17 = 0;
  }
  else {
    *(ulong *)(lVar13 + 0x98) = -(ulong)auStack_574[1];
    uVar17 = (ulong)*(byte *)(lVar15 + 1);
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff1e8 with catch @ 010ff180
                        */
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 8) = uVar10;
  if (*(char *)(lVar15 + 2) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    *(ulong *)(lVar13 + 0xa0) = uVar17 - auStack_574[uVar17 + 1];
    uVar17 = uVar17 + *(byte *)(lVar15 + 2);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x10) = uVar10;
                    /* try { // try from 010ff1c8 to 011ff1cb has its CatchHandler @ 010ff2d4 */
  if (*(char *)(lVar15 + 3) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xa8) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
                    /* try { // try from 010ff1e4 to 011ff1e7 has its CatchHandler @ 010ff2f8 */
                    /* try { // try from 010ff1e8 to 011ff30b has its CatchHandler @ 010ff180 */
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 3);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x18) = uVar10;
  if (*(char *)(lVar15 + 4) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xb0) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 4);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x20) = uVar10;
  if (*(char *)(lVar15 + 5) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xb8) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 5);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x28) = uVar10;
  if (*(char *)(lVar15 + 6) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xc0) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 6);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x30) = uVar10;
  if (*(char *)(lVar15 + 7) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 200) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 7);
    uVar10 = (ulong)auStack_574[uVar17];
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff1c8 with catch @ 010ff2d4
                        */
  }
  *(ulong *)(lVar13 + 0x38) = uVar10;
  if (*(char *)(lVar15 + 8) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
                    /* catch(type#1 @ 00000000) { ... } // from try @ 010ff1e4 with catch @ 010ff2f8
                        */
    *(ulong *)(lVar13 + 0xd0) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 8);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x40) = uVar10;
  if (*(char *)(lVar15 + 9) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xd8) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 9);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x48) = uVar10;
  if (*(char *)(lVar15 + 10) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xe0) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 10);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x50) = uVar10;
  if (*(char *)(lVar15 + 0xb) == '\0') {
                    /* catch() { ... } // from try @ 010ff3e4 with catch @ 010ff3b8 */
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xe8) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 0xb);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x58) = uVar10;
  if (*(char *)(lVar15 + 0xc) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xf0) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
                    /* try { // try from 010ff3dc to 011ff3e3 has its CatchHandler @ 010ff428 */
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 0xc);
                    /* try { // try from 010ff3e4 to 011ff443 has its CatchHandler @ 010ff3b8 */
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x60) = uVar10;
  if (*(char *)(lVar15 + 0xd) == '\0') {
                    /* catch() { ... } // from try @ 010ff3dc with catch @ 010ff428 */
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0xf8) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 0xd);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x68) = uVar10;
  if (*(char *)(lVar15 + 0xe) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
                    /* catch() { ... } // from try @ 010ff478 with catch @ 010ff444 */
    *(ulong *)(lVar13 + 0x100) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 0xe);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x70) = uVar10;
  if (*(char *)(lVar15 + 0xf) == '\0') {
    uVar10 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
                    /* try { // try from 010ff470 to 011ff477 has its CatchHandler @ 010ff4c8 */
                    /* try { // try from 010ff478 to 011ff4e3 has its CatchHandler @ 010ff444 */
    *(ulong *)(lVar13 + 0x108) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
    uVar17 = (long)iVar6 + (ulong)*(byte *)(lVar15 + 0xf);
    uVar10 = (ulong)auStack_574[uVar17];
  }
  *(ulong *)(lVar13 + 0x78) = uVar10;
  if (*(char *)(lVar15 + 0x10) == '\0') {
    uVar17 = 0xffffffffffffffff;
  }
  else {
    iVar6 = (int)uVar17;
    *(ulong *)(lVar13 + 0x110) = (long)iVar6 - (ulong)auStack_574[(long)iVar6 + 1];
                    /* catch() { ... } // from try @ 010ff470 with catch @ 010ff4c8 */
    uVar17 = (ulong)auStack_574[(long)iVar6 + (ulong)*(byte *)(lVar15 + 0x10)];
  }
                    /* catch() { ... } // from try @ 010ff51c with catch @ 010ff4e4 */
  *(ulong *)(lVar13 + 0x80) = uVar17;
  *(undefined8 *)(lVar13 + 0x88) = 0xfffff;
  memset((void *)(lVar13 + 0x120),0,0x400);
  uVar17 = (ulong)*(byte *)(lVar15 + 1);
  if (*(byte *)(lVar15 + 1) != 0) {
    uVar17 = 0;
    iVar6 = 1;
    do {
                    /* try { // try from 010ff514 to 011ff51b has its CatchHandler @ 010ff570 */
      iVar12 = 0x81;
                    /* try { // try from 010ff51c to 011ff58b has its CatchHandler @ 010ff4e4 */
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 7);
      do {
        *(undefined4 *)(lVar13 + 0x520 + lVar14 * 4 + -0x400) = 1;
        iVar12 = iVar12 + -1;
        *(undefined1 *)(lVar13 + 0x520 + lVar14) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
        lVar14 = lVar14 + 1;
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 1);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
  if (*(char *)(lVar15 + 2) != '\0') {
    uVar17 = (ulong)(int)uVar17;
    iVar6 = 1;
    do {
                    /* catch() { ... } // from try @ 010ff514 with catch @ 010ff570 */
      iVar12 = 0x41;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 6);
      do {
                    /* catch() { ... } // from try @ 010ff5cc with catch @ 010ff58c */
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 2;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 2);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
                    /* try { // try from 010ff5c4 to 011ff5cb has its CatchHandler @ 010ff640 */
  if (*(char *)(lVar15 + 3) != '\0') {
    uVar17 = (ulong)(int)uVar17;
                    /* try { // try from 010ff5cc to 011ff65b has its CatchHandler @ 010ff58c */
    iVar6 = 1;
    do {
      iVar12 = 0x21;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 5);
      do {
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 3;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 3);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
  if (*(char *)(lVar15 + 4) != '\0') {
    uVar17 = (ulong)(int)uVar17;
    iVar6 = 1;
    do {
                    /* catch() { ... } // from try @ 010ff5c4 with catch @ 010ff640 */
      iVar12 = 0x11;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 4);
      do {
                    /* catch() { ... } // from try @ 010ff698 with catch @ 010ff65c */
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 4;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 4);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
                    /* try { // try from 010ff690 to 011ff697 has its CatchHandler @ 010ff70c */
  if (*(char *)(lVar15 + 5) != '\0') {
                    /* try { // try from 010ff698 to 011ff727 has its CatchHandler @ 010ff65c */
    uVar17 = (ulong)(int)uVar17;
    iVar6 = 1;
    do {
      iVar12 = 9;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 3);
      do {
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 5;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 5);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
  if (*(char *)(lVar15 + 6) != '\0') {
    uVar17 = (ulong)(int)uVar17;
                    /* catch() { ... } // from try @ 010ff690 with catch @ 010ff70c */
    iVar6 = 1;
    do {
      iVar12 = 5;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 2);
      do {
                    /* catch() { ... } // from try @ 010ff76c with catch @ 010ff728 */
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 6;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 6);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
                    /* try { // try from 010ff764 to 011ff76b has its CatchHandler @ 010ff7e8 */
  if (*(char *)(lVar15 + 7) != '\0') {
    uVar17 = (ulong)(int)uVar17;
                    /* try { // try from 010ff76c to 011ff803 has its CatchHandler @ 010ff728 */
    iVar6 = 1;
    do {
      iVar12 = 3;
      lVar14 = (long)(int)(auStack_574[uVar17 + 1] << 1);
      do {
        *(undefined4 *)(lVar13 + lVar14 * 4 + 0x120) = 7;
        iVar12 = iVar12 + -1;
        lVar11 = lVar13 + lVar14;
        lVar14 = lVar14 + 1;
        *(undefined1 *)(lVar11 + 0x520) = *(undefined1 *)(lVar15 + uVar17 + 0x11);
      } while (1 < iVar12);
      uVar17 = uVar17 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 7);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
  if (*(char *)(lVar15 + 8) != '\0') {
    lVar14 = (long)(int)uVar17;
    iVar6 = 1;
    do {
      lVar11 = (long)(int)auStack_574[lVar14 + 1];
                    /* catch() { ... } // from try @ 010ff764 with catch @ 010ff7e8 */
      iVar12 = 2;
      do {
        *(undefined4 *)(lVar13 + lVar11 * 4 + 0x120) = 8;
        iVar12 = iVar12 + -1;
        lVar2 = lVar13 + lVar11;
                    /* catch() { ... } // from try @ 010ff848 with catch @ 010ff804 */
        lVar11 = lVar11 + 1;
        *(undefined1 *)(lVar2 + 0x520) = *(undefined1 *)(lVar15 + lVar14 + 0x11);
      } while (1 < iVar12);
      lVar14 = lVar14 + 1;
      bVar1 = iVar6 < (int)(uint)*(byte *)(lVar15 + 8);
      iVar6 = iVar6 + 1;
    } while (bVar1);
  }
  if ((param_2 != 0) && (0 < (int)uVar16)) {
    uVar17 = (ulong)uVar16;
    pbVar9 = (byte *)(lVar15 + 0x11);
    do {
                    /* try { // try from 010ff840 to 011ff847 has its CatchHandler @ 010ff8d8 */
                    /* try { // try from 010ff848 to 011ff8f3 has its CatchHandler @ 010ff804 */
      if (0xf < *pbVar9) {
        puVar7 = (undefined8 *)*param_1;
        *(undefined4 *)(puVar7 + 5) = 9;
        (*(code *)*puVar7)(param_1);
      }
      uVar17 = uVar17 - 1;
      pbVar9 = pbVar9 + 1;
    } while (uVar17 != 0);
  }
  if (*(long *)(lVar5 + 0x28) == local_68) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}

