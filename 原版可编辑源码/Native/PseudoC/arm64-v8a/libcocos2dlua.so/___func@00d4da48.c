
/* std::__ndk1::__function::__func<std::__ndk1::__bind<void (cocos2d::Skybox::*)(cocos2d::Mat4
   const&, unsigned int), cocos2d::Skybox*, cocos2d::Mat4 const&, unsigned int&>,
   std::__ndk1::allocator<std::__ndk1::__bind<void (cocos2d::Skybox::*)(cocos2d::Mat4 const&,
   unsigned int), cocos2d::Skybox*, cocos2d::Mat4 const&, unsigned int&> >, void ()>::~__func() */

void __thiscall
std::__ndk1::__function::
__func<std::__ndk1::__bind<void(cocos2d::Skybox::*)(cocos2d::Mat4_const&,unsigned_int),cocos2d::Skybox*,cocos2d::Mat4_const&,unsigned_int&>,std::__ndk1::allocator<std::__ndk1::__bind<void(cocos2d::Skybox::*)(cocos2d::Mat4_const&,unsigned_int),cocos2d::Skybox*,cocos2d::Mat4_const&,unsigned_int&>>,void()>
::~__func(__func<std::__ndk1::__bind<void(cocos2d::Skybox::*)(cocos2d::Mat4_const&,unsigned_int),cocos2d::Skybox*,cocos2d::Mat4_const&,unsigned_int&>,std::__ndk1::allocator<std::__ndk1::__bind<void(cocos2d::Skybox::*)(cocos2d::Mat4_const&,unsigned_int),cocos2d::Skybox*,cocos2d::Mat4_const&,unsigned_int&>>,void()>
          *this)

{
                    /* try { // try from 00d4da48 to 00e4dacb has its CatchHandler @ 00d4d8e8 */
  *(undefined ***)this = &PTR____func_016d50f8;
  cocos2d::Mat4::~Mat4((Mat4 *)(this + 0x20));
  return;
}

