...
LAB_00b9f7d8:
                    /* try { // try from 00b9f7e8 to 00c9f847 has its CatchHandler @ 00b9f7e8
                       catch() { ... } // from try @ 00b9f7e8 with catch @ 00b9f7e8
                       catch() { ... } // from try @ 00b9f84c with catch @ 00b9f7e8
                       catch() { ... } // from try @ 00b9f884 with catch @ 00b9f7e8
                       catch() { ... } // from try @ 00b9fa60 with catch @ 00b9f7e8
                       catch() { ... } // from try @ 00b9fab0 with catch @ 00b9f7e8 */
        ERR_put_error(0x2e,0xa5,0x41,"crypto/cms/cms_pwri.c",0xa1);
        EVP_CIPHER_CTX_free((EVP_CIPHER_CTX *)0x0);
        if (val == (ASN1_VALUE *)0x0) goto LAB_00b9f810;
      }
      else {
        pAVar6 = ASN1_item_new((ASN1_ITEM *)CMS_PasswordRecipientInfo_it);
        *(ASN1_VALUE **)(val + 8) = pAVar6;
        if (pAVar6 == (ASN1_VALUE *)0x0) goto LAB_00b9f7d8;
        *(undefined4 *)val = 3;
        X509_ALGOR_free(*(X509_ALGOR **)(pAVar6 + 0x10));
        pXVar7 = X509_ALGOR_new();
        *(X509_ALGOR **)(pAVar6 + 0x10) = pXVar7;
        if (pXVar7 == (X509_ALGOR *)0x0) goto LAB_00b9f7d8;
        ...
        *(ASN1_TYPE **)(*(long *)(pAVar6 + 0x10) + 8) = pAVar4;
        if ((pAVar4 == (ASN1_TYPE *)0x0) ||
           ...
           , pAVar8 == (ASN1_STRING *)0x0)) goto LAB_00b9f7d8;
