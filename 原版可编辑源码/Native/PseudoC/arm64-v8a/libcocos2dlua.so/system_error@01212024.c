
/* std::__ndk1::system_error::system_error(std::__ndk1::error_code, std::__ndk1::basic_string<char,
   std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> > const&) */

void __thiscall
std::__ndk1::system_error::system_error
          (system_error *this,undefined8 param_2,undefined8 param_3,basic_string *param_4)

{
  long lVar1;
  undefined8 local_68;
  undefined8 uStack_60;
  basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>> local_58 [16];
  void *local_48;
  system_error local_40 [16];
  void *local_30;
  long local_28;
  
  lVar1 = tpidr_el0;
  local_28 = *(long *)(lVar1 + 0x28);
  local_68 = param_2;
  uStack_60 = param_3;
  basic_string<char,std::__ndk1::char_traits<char>,std::__ndk1::allocator<char>>::basic_string
            (local_58,param_4);
  __init(local_40,&local_68,local_58);
  runtime_error::runtime_error((runtime_error *)this,(basic_string *)local_40);
  if (((byte)local_40[0] & 1) != 0) {
    operator_delete(local_30);
  }
  if (((byte)local_58[0] & 1) != 0) {
    operator_delete(local_48);
  }
  *(undefined ***)this = &PTR__system_error_01731238;
  *(undefined8 *)(this + 0x18) = uStack_60;
  *(undefined8 *)(this + 0x10) = local_68;
  if (*(long *)(lVar1 + 0x28) == local_28) {
    return;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}

