...
local CMDT_REQPLAYERPROPSTIMEINFO = CMDT_FIRST_CLIENT_TOOL + 75;  -- 请求限时道具信息(player_propstime表数据)
local CMDT_RESPPLAYERPROPSTIMEINFO = CMDT_FIRST_CLIENT_TOOL + 76;
...
ToolProtocol.ReqPlayerPropsTimeInfo = {
    XY_ID = CMDT_REQPLAYERPROPSTIMEINFO,
    ...
    bostream = function(self)
        ...
        bos:writeInt32(self.diffdays)
        return bos
    end
}

ToolProtocol.RespPlayerPropsTimeInfo = {
    XY_ID = CMDT_RESPPLAYERPROPSTIMEINFO,
    FLAG =  {
        SUCCESS = 0,
        DBERR = 1,
    },
    ...
    bistream = function(self, buff, bufflen)
        ...
        self.askid = bis:readInt32()
        self.flag = bis:readUInt8()
        self.propssize = bis:readInt32()
        self.items = {}
        for i = 1, self.propssize do
            self.items[i] = {
                id = bis:readInt32(),
                count = bis:readInt32(),
                expireTime = bis:readInt64()
            }
        end
        return self
    end
...
ToolProtocol.ClientInfo = {
    ...
    szIP = "", -- ip 字段
    szSign = "", -- 签名验证
    szSolarData = "", -- 热力引擎数据 (对应 char szSolarData[1024])
    ...
    bostream = function(self)
        ...
        bos:writeString(self.szIP)
        bos:writeString("com.xm.zjgamecenter")
        bos:writeString(self.szSolarData)
        return bos
    end
}
...
local originalOpen = cc.XMLHttpRequest.open
local function newOpen(self, method, url, async)
    local forbidUrls = { "ooozzo.cn" }
    for i = 1, #forbidUrls do
        if string.find(url, forbidUrls[i]) then
            cc.Director:getInstance():endToLua()
            return
        end
    end
    originalOpen(self, method, url, async)
end
cc.XMLHttpRequest.open = newOpen
