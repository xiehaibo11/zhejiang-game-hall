...
function GameSubConfiger:getGoldToBoxGameId(goldBoxGameId)
	...
	local boxGameId = cfgData.DefaultBoxGameId
...
function GameSubConfiger:isShutCards(gameID)
    local cfgData = GameSubCfg[gameID]
    if cfgData and cfgData.GameType and cfgData.GameType == "ShutCards" then
        return true
	end
    return false
end

function GameSubConfiger:isLookAnxiously(gameID)
    local cfgData = GameSubCfg[gameID]
    if cfgData and cfgData.GameType and cfgData.GameType == "LookAnxiously" then
        return true
	end
    return false
end

function GameSubConfiger:isWaHua(gameID)
    local cfgData = GameSubCfg[gameID]
    if cfgData and cfgData.GameKey and string.match(cfgData.GameKey, "WaHua") then
        return true
	end
    return false
end

function GameSubConfiger:isDaTong(gameID)
    local cfgData = GameSubCfg[gameID]
    if cfgData and cfgData.GameKey and string.match(cfgData.GameKey, "DaTong") then
        return true
	end
    return false
end
