...
--进度更新自定义事件
local EVENT_LianYun_PERCENT_CHANGED = "hotupdate.LianYunUpate.ProgressChanged"
local EVENT_LianYun_START_UPDATE = "hotupdate.LianYunUpate.startUpdate"
local EVENT_LianYun_UPDATE_SUCCESS = "hotupdate.LianYunUpate.updateSuccess"
local EVENT_LianYun_UPDATE_FAILED = "hotupdate.LianYunUpate.updateFailed"
...
function HotUpdateScene:start(gameID, gameName)
    ...
    --显示更新界面
    local param = {}
    self:dispatchPercentEvent(EVENT_LianYun_START_UPDATE, param)
end
...
function HotUpdateScene:needHotUpdate(tag, oldVersion, newVersion)
    ...
    local param = {}
    param.percent = 0
    self:dispatchPercentEvent(EVENT_LianYun_PERCENT_CHANGED, param)
end

function HotUpdateScene:hotUpdateSuccess(tag)
    if HotUpdateScene.isHotUpdate then return end
    local param = {}
    self:dispatchPercentEvent(EVENT_LianYun_UPDATE_SUCCESS, param)
end
...
function HotUpdateScene:hotUpdateFaile(message, gameKey)
    ...
    --隐藏更新界面
    local param = {}
    self:dispatchPercentEvent(EVENT_LianYun_UPDATE_FAILED, param)
