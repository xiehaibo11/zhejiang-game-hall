...
local KW_UI_PROP_CSB_PATH = GameSceneDefine.KW_GAME_CSB_PATH .. "Prop/Prop.csb"
...
local KW_TEXT_REMAIND_PROP = "KW_TEXT_REMAIND_PROP"
...
local KW_UI_PLAYER_GOLD = "KW_UI_PLAYER_GOLD"
local KW_UI_PLAYER_PROP = "KW_UI_PLAYER_PROP"
...
function UIProp:ctor()
    ...
    self._usePropMode = UIProp.usePropMode.useGold
end

function UIProp:init(node,usePropMode)
    local csbNode = cc.CSLoader:createNode(KW_UI_PROP_CSB_PATH)
    self._rootNode = csbNode:getChildByName(KW_ROOT_LAYER)
    self._layer3DPos = csbNode:getChildByName("KW_ROOT_LAYER_3D_POS")
    ...
    usePropMode = usePropMode or UIProp.usePropMode.useGold
    self:setRoomMode(usePropMode)
end

function UIProp:setRoomMode(usePropMode)
    if usePropMode == UIProp.usePropMode.useProp then
        self._usePropMode = UIProp.usePropMode.useProp
        Game.UIFunction.setVisible(self._rootNode,KW_UI_PLAYER_PROP,true)
        Game.UIFunction.setVisible(self._rootNode,KW_UI_PLAYER_GOLD,false)
    else
        self._usePropMode = UIProp.usePropMode.useGold
        Game.UIFunction.setVisible(self._rootNode,KW_UI_PLAYER_PROP,false)
        Game.UIFunction.setVisible(self._rootNode,KW_UI_PLAYER_GOLD,true)
    end
end

function UIProp:getUsePropMode()
    return self._usePropMode
end
...
function UIProp:setRemaindProp(propNum)
    self._remaindProp = propNum
    local textNode = ccui.Helper:seekWidgetByName(self._rootNode,KW_TEXT_REMAIND_PROP)
    if textNode then
        textNode:setString(propNum)
    end
end
...
function UIProp:getRemaindProp()
    return self._remaindProp
end
...
function UIProp:resetLayerPos()
    if self._layer3DPos ~= nil then
        self._rootNode:setPositionX(self._layer3DPos:getPositionX())
    end
end
