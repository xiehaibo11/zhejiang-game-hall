...
function BagSysModule:ctor()
    ...
    self._peakRaceTickets = {} -- 巅峰赛数据
    self._enterCards = {} -- 入场卡数据
    self:initData()
end
...
function BagSysModule:getReqConfig()
    return {
        ...
        ReqDoubleCards = { reqPath = "lobby.Req.BagSysNew.ReqListLabelRequest", callBack = self.respDoubleCards },   
        ReqSelfBagData50 = {reqPath = "lobby.Req.BagSysNew.ReqListLabelRequest", callBack = self.onReqSelfBagDataCallBack50},
        ReqBagPackData50 = {reqPath = "lobby.Req.BagSysNew.ReqListLabelRequest", callBack = self.onReqBagPackData50},
        ReqPropsAttr = {reqPath = "lobby.Req.BagSysNew.ReqGetPropsAttr", callBack = self.onRespPropsAttr}, 
    }
end
...
--请求自己背包道具
function BagSysModule:reqSelfBagData(isReqCard)
    if XH.isEmbeddedApp() then
        return self:reqSelfBagData50()
    end
...
function BagSysModule:reqSelfBagData50()
    self:startReq("ReqSelfBagData50")
end

function BagSysModule:onReqSelfBagDataCallBack50(cell, ctype, data)
    if ctype == XH.Req.TYPE.SUCCESS then
        data = convert_keys(data)
        local count_prop_info = {}
        local time_prop_info = {}
        for _, v in ipairs(data.props) do
            v.remaining = tonumber(v.remaining)
            if v.count > 0 then
                count_prop_info[#count_prop_info+1] = {propsid=v.prop_id, propscnt=v.count}
            elseif v.remaining > 0 then
                time_prop_info[#time_prop_info+1] = {propsid=v.prop_id, endtime=v.remaining}
            end
            v.propsid = v.prop_id
            v.propscnt = v.count
            v.propsname = v.name
            -- v.props_overview = v.propsOverview
            v.endtime = v.remaining
            -- v.props_tag_id = v.propsTagId
            v.props_state = v.status == "IN_USE" and 1 or 0
            -- v.props_img = {}
            -- for i,vv in ipairs(v.propsImg) do
            --     v.props_img[i] = {package_id=vv.packageId, img=vv.img}
            -- end
            v.is_time_prop = v.remaining and v.remaining > 0
        end
        -- self._allBagProps = data.props
        local selfBagData = {count_prop_info=count_prop_info, time_prop_info=time_prop_info, allProps=data.props}
        self._bagSysData:setSelfBagData(selfBagData)
        -- xn.Utils.sendDispatchEvent(ENotifySubject.kNotifySubject_Web, en_kNotifySubject_Web.EN_NOTIFY_MYPROPINFO, nil, true)
        self:dispatchEvent({name = self.EVENT_SELF_BACK_PACK_DATA})
...
function BagSysModule:reqBagPackData(pcnt, areaid, numid, propsid)
    if XH.isEmbeddedApp() then
        for i = 1, #numid do
            self:reqBagPackData50(pcnt, areaid[i], numid[i], propsid)
        end
        return
    end
    local ReqBackpackData = require("lobby.Req.BagSysNew.ReqBackpackData")
...
function BagSysModule:reqBagPackData50(pcnt, areaid, numid, propsid)
    self:startReq("ReqBagPackData50", nil, areaid, numid)
end

function BagSysModule:onReqBagPackData50(cell, ctype, data)
    if ctype == XH.Req.TYPE.SUCCESS then
        data = convert_keys(data)
        local count_prop_info = {}
        local time_prop_info = {}
        ...
        local selfBagData = { count_prop_info = count_prop_info, time_prop_info = time_prop_info, allProps = data.props }
        if data.numid then
            self._bagSysData:setPlayerBagData(selfBagData, data.numid)
            self:dispatchEvent({ name = self.EVENT_PLAYER_BAG_DATA, data = { numid = data.numid, data = selfBagData } })
        end
    end
end
    ...
    if ppi then
        ...
        if count_prop_info then
            for _ , propinfo in ipairs(count_prop_info) do
                ...
                table.insert(propInfo, {id = propinfo.propsid, count = propinfo.propscnt})
            end
        end
    end
    XH.playerData:dispatchEvent({name = XH.playerData.EVENT_PLAYER_DROP_CHANGED, msg = propInfo})
end
...
--操作道具
function BagSysModule:reqOperateProps(operate_props, propsid, isNotShowTip, autoReqBagData)
    ...
    reqOperateProps.autoReqBagData = autoReqBagData
...
function BagSysModule:onReqOperatePropsCallBack(cell, ctype, data)
     if ctype == XH.Req.TYPE.SUCCESS then
        self:dispatchEvent({name = self.EVENT_OPERATE_PROPS, data = data, info = cell._reqData})
...
function BagSysModule:onReqOperatePropsWithOutTipCallBack(cell, ctype, data)
    ...
    if cell.autoReqBagData == nil or cell.autoReqBagData == true then
        self:reqSelfBagData()
    end
end
    ...
    if propid then
        ...
        for _,propInfo in pairs(devideBaseConfig) do
            if propid == propInfo.propsid then
                ...
                if propInfo.category_one and string.find(propInfo.category_one, "装扮") and propInfo.propstype and  propInfo.propstype ~= 20 and not XH.isEmbeddedApp() then
                    return true
                end
...
function BagSysModule:reqDoubleCards(showLoading,callback)
    if showLoading then
        XH.TipTool.showLoading()
    end
    self.cardCallback = callback
    self:startReq("ReqDoubleCards")
end
...
function BagSysModule:respDoubleCards(cell, type, data)
    ...
    if type == XH.Req.TYPE.SUCCESS then
        local cardsData = {} -- 加倍卡
        local peakRaceData = {} -- 巅峰赛
        if data and data.props and #data.props > 0 then
            for i = 1, #data.props do
                if data.props[i].count ~= 0 then
                    if data.props[i].propId == BagConfig.PROP_ID_DOUBLE_CARD then
                        cardsData[#cardsData + 1] = self:transferDataToZJ(data.props[i], true)
                    elseif data.props[i].propId == BagConfig.PROP_ID_PEAK_RACE then
                        peakRaceData[#peakRaceData + 1] = self:transferDataToZJ(data.props[i], true)
                    elseif data.props[i].propId >= 150755 and data.props[i].propId <= 150760 then
                        self._enterCards[#self._enterCards + 1] = self:transferDataToZJ(data.props[i], true)
                    end
                end
            end
        end
        self._doubleCards = cardsData
        self._peakRaceTickets = peakRaceData
        table.sort(self._peakRaceTickets, function(a, b)
        ...
        self:setDoubleCards({})
        self:setPeakRaceTickets({})
    end
    self:dispatchEvent( { name = self.EVENT_UPDATE_DOUBLE_CARD})
    if self.cardCallback then
        self.cardCallback()
        self.cardCallback = nil
    end
end
...
function BagSysModule:transferDataToZJ(data, isTimeProp)
    local devideBaseConfig = self:getBagData():getDevideBagConfigData()
    local bagData = {}
    if devideBaseConfig and devideBaseConfig[BagConfig.PROP_ID_DOUBLE_CARD] and data.propId == BagConfig.PROP_ID_DOUBLE_CARD then
        bagData = devideBaseConfig[BagConfig.PROP_ID_DOUBLE_CARD]
    elseif devideBaseConfig and devideBaseConfig[BagConfig.PROP_ID_PEAK_RACE] and data.propId == BagConfig.PROP_ID_PEAK_RACE then
        bagData = devideBaseConfig[BagConfig.PROP_ID_PEAK_RACE]
    elseif data.propId >= 150755 and data.propId <= 150760 then
        bagData.propsid = data.propId
    end
    local transferData = {
        ...
        addtime = 0, -- 获得时间
        endtime = tonumber(data.remaining) or 0, -- 道具结束时间
    }
    if data.propsOverview then
        transferData.props_overview = string.split(data.propsOverview, "到期日期")[1]
    end
    return transferData
end
...
function BagSysModule:getEnterCardCount(propId)
    if self._enterCards then
        for i = 1, #self._enterCards do
            if self._enterCards[i].propsid == propId then
                return self._enterCards[i].propscnt
            end
        end
    end
    return 0
...
function BagSysModule:setPeakRaceTickets(data)
    self._peakRaceTickets = data
end

function BagSysModule:getPeakRaceTickets()
    return self._peakRaceTickets or {}
end
...
function BagSysModule:reqPropsAttr(propids)
    self:startReq("ReqPropsAttr", propids or BagConfig.PROP_ID_GETATTR)
end

function BagSysModule:onRespPropsAttr(cell, ctype, data)
    if ctype == XH.Req.TYPE.SUCCESS then
        data = convert_keys(data)
        self._bagSysData:setBasePropInfo(data.props)
    end
end
return BagSysModule
