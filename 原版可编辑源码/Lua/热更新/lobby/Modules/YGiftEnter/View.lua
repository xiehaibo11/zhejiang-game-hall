...
function YGiftEnterView:ctor(param)
    ...
    local createType = self:getGiftEnterModule():getCreateType() or 0
...
function YGiftEnterView:getGiftEnterModule()
    local module = XH.lobby:getModule("YGiftEnter")
    if self._gameId == 30579 then
        module = XH.lobby:getModule("YGiftEnterV2")
    end
    return module
end
...
function YGiftEnterView:initUI()
    for i = 1, 3 do
        ...
        if self._info[i] then
            item:getChildByName("_KW_YH_1"):setVisible(self._info[i].limit  == 1)
            item:getChildByName("_KW_YH_2"):setVisible(self._info[i].limit == 1)
    ...
    elseif #self._info == 1 then
        self._KW_GOLD_LIST_1:setPositionX(self._KW_GOLD_LIST_2:getPositionX())
    end
end

function YGiftEnterView:onTouchEventBuyPropId(send, eventType, info)
    local activityId = self:getGiftEnterModule():getActivityId()
    if activityId == nil then
        XH.TipTool.showToast("暂未获取到配置，请点击重试~")
        return
    end
    local productId = YGiftBankruptcyDefine.ACTLIST.Enter .. "-" .. activityId .. "-" .. info.goods_id
    if self._gameId == 30579 then
        productId = YGiftBankruptcyDefine.ACTLIST.EnterV2 .. "-" .. activityId .. "-" .. info.goods_id
    end
    self:throwClickData(info)
    XH.lobby:getModule("YGiftBankruptcy"):reqBuyPropByProductId(productId,self._gameId,self._level,info.limit)
    self:close()
end
