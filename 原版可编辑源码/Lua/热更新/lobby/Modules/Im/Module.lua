...
function IMModule:onReqCreateGroup(cell, ctype, data)
    if ctype == XH.Req.TYPE.SUCCESS then 
        ...
        elseif data.flag == 6 then
            XH.TipTool.showToast("您已被封禁加入/创建比赛场 ，无法进行该操作，如有疑问请联系客服。")
        end
    else
        XH.TipTool.showToast("操作失败，请重试~")
    end
...
function IMModule:joinRoom(roomid, bHost)
    ...
    if roomid == position.roomID and XH.gameManager then
...
function IMModule:createShareInfo(data)
    ...
    XH.lobby:getModule("Share"):setShareScene(XH.teaHouseManager:isInTeaHouse() and "比赛场" or "包厢", "房间牌友列表")
...
function IMModule:onTouchShare(callBackFunc)
    ...
    XH.lobby:getModule("Share"):setShareScene("大厅", "牌友列表")
...
function IMModule:judgeSceneName()
    ...
    if inTeahouse then
        sceneName = XH.gameManager and "比赛场房间" or "比赛场"
    else
