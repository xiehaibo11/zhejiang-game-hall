...
function NewIMListView:on_KW_UI_ACTION_ADD_GROUP(send, eventType)
    ...
    if  XH.playerData:getPlayerIsAgent() or XH.areaData:getVirtualAgentState() then
        ...
        XH.lobby:getModule("Im"):ReqRegisterAgent(XH.areaData:getApiHubRid(),XH.playerData:getPlayerPhone(),XH.playerData:getWeChatURL(),XH.playerData:getNickName())        
    end
end
...
function NewIMListView:onGroupListChanged(event)
    local imData = self:getData()
    if not imData then
        return
    end
    if imData:getLable() == imData.KW_LIST_TYPE.TABLELIST then
        if event.bNotUpdateUI then
            local groupList = imData:getGroupList()
            local bExsit = false
...
function NewIMListView:updateGroupFilter()
    ...
    if #self:getData():getTeahouseList() > 0 then
        table.insert( filterList, {name = "比赛场",groupType = self.GROUP_TYPE.TEAHOSE, nPasswd = self.GROUP_TYPE.TEAHOSE} )
    end
    if #self:getData():getFriendList() > 0 then
...
function NewIMListView:onTouchRoomInfo(data,send, eventType)
    ...
    local data = {
        block_label = sceneName,
        game_id = XH.areaData:getAreaID() .. "_" .. data.gameid,
        room_id = data.game_roomid,
        players = data.chairs,
    }
...
function NewIMListView:getHeadFrameSize()
    return cc.size(240, 240)
end
