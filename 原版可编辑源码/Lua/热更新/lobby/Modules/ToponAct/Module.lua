...
ToponActModule.EVENT_UPDATE_RED_POINR = "EVENT_UPDATE_RED_POINR"
...
function ToponActModule:respSign(cell, ctype, data)
    ...
    if ctype == XH.Req.TYPE.SUCCESS then
        if data and data.res and data.res.reward then
            local actData = {}
            if data.res.reward.id == XH.areaData:getPropLimitedTimeRoomCardID() then
                data.res.reward.count = data.res.reward.count / XH.areaData:getPropLimitedTimeRoomCardRatio()
            end
            actData[#actData + 1] = {
...
function ToponActModule:transferData50(data)
    ...
    for i = 1, #data.bases do
        ...
        if data.bases[i].reward.id == XH.areaData:getPropLimitedTimeRoomCardID() then
            data.bases[i].reward.count = data.bases[i].reward.count / XH.areaData:getPropLimitedTimeRoomCardRatio()
        end
        local tmpData = {
        ...
        if data.bases[i].multiple == 0 then
            ...
            if data.bases[i].ext then
                ...
                if ok and res_tab and res_tab.read_ad then
                    local showCnt = res_tab.read_ad
                    if data.bases[i].reward.id == XH.areaData:getPropLimitedTimeRoomCardID() then
                        showCnt = showCnt / XH.areaData:getPropLimitedTimeRoomCardRatio()
                    end
                    actData.actInfo.newSginInfo.adAwardInfo[#actData.actInfo.newSginInfo.adAwardInfo].prize_name = showCnt .. data.bases[i].reward.name
                end
            end
        end
    end
    ...
    self:dispatchEvent( { name = self.EVENT_UPDATE_RED_POINR })
    return actData
end
...
function ToponActModule:onToponAdsShowCallBack(event)
    ...
    print("ToponActModule CallBack, code:" .. code)
    if (code == XH.ThirdDefine.TOPONSDK_CALLBACK_CODE.TOPON_FLAG_SDK_JLSUCCESS) then
        ...
        if not self._throwDataLock then
            XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.ws24120406)
            self._throwDataLock = true
    ...
    elseif (code == XH.ThirdDefine.TOPONSDK_CALLBACK_CODE.TOPON_FLAG_SDK_CLOSE) then
        self:dispatchEvent( { name = ToponActModule.KW_TOPON_ACT_FULSH_LOBBY_WAIT, data = "" })
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.ws24120404)
    elseif (code == XH.ThirdDefine.TOPONSDK_CALLBACK_CODE.TOPON_FLAG_SDK_SHOWSUCCESS) then
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.ws24120403)
    elseif (code == XH.ThirdDefine.TOPONSDK_CALLBACK_CODE.TOPON_FLAG_SDK_CLICKED) then
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.ws24120405)
    end
end
---------------------------initActInfo-----------------------------
