...
function IdentityCheckNView:getBindingInfo()
    return {
        ...
        ["_KW_IMG_TITLE"] = {varName = "_imageTitle"},
        ["_KW_TEXT_SMALL_TITLE"] = {varName = "_textSmallTitle"},
        ...
        ["_KW_REWARD_ITEM"] = {varName = "_rewardItem"},
        ["_REWARD_TIPS"] = {varName = "_rewardTips"},
        ["_KW_ZFUB_ICON"] = {varName = "_zFuBIcon"},
...
function IdentityCheckNView:ctor(param)
    ...
    self._isSupplement = param.isSupplement or false -- 补充实名
    self._isForceRealName = param.forceRealName or false -- 强实名
    ...
    self:postShowData()
    if self._isSupplement then
        if self._imageTitle then
            self._imageTitle:loadTexture("hall/Image/window/RealName/Img_biaoti.png", ccui.TextureResType.plistType)
            self._imageTitle:ignoreContentAdaptWithSize(true)
        end
        if self._textSmallTitle then
            self._textSmallTitle:setString("您的实名信息存在异常，根据防沉迷相关规定，必须进行实名认证才能完整体验游戏，请尽快补充实名")
        end
        self._btnChangeArea:setVisible(false)
    else
        if self._imageTitle then
            self._imageTitle:loadTexture("hall/Image/window/RealName/title.png", ccui.TextureResType.plistType)
        end
        if self._textSmallTitle then
            self._textSmallTitle:setString("根据国家相关规定，必须进行实名认证才能完整体验游戏。")
        end
        self._btnChangeArea:setVisible(true)
    end
end

function IdentityCheckNView:onEnter()
    XH.NewThrowDataManager:AnalyseStartData("实名认证开始")
end

function IdentityCheckNView:onGetPrizeInfo(event)
    local awards = event.data
    local awardNums = #awards
    self._rewardTips:setVisible(awardNums > 0)
    
    local awardItem = self._rewardItem
    local listMargin = self._rewardList:getItemsMargin()
    local itemSize = awardItem:getContentSize()
    if awardNums < 5 then
        self._rewardList:setContentSize(itemSize.width * awardNums + listMargin * (awardNums - 1), itemSize.height)
    end
    self._rewardList:setScrollBarEnabled(false)
    for i, _ in ipairs(awards) do
        local item = awardItem:clone()
        local prop_icon = item:getChildByName("_KW_PRO")
        local size = cc.size(80, 80)
        local headImage = XH.RemoteImage.new():setPosition(prop_icon:getPositionX(), prop_icon:getPositionY()):ignoreContentAdaptWithSize(false):setContentSize(size.width, size.height):addTo(item)
        headImage:setUrl(awards[i].propImgUrl or "")
        local pro_text = item:getChildByName("_KW_PRO_TXT")
        local showPropCnt = awards[i].propCount
        if awards[i].propId == XH.areaData:getPropLimitedTimeRoomCardID() then
            if XH.areaData:getPropLimitedTimeRoomCardRatio() then
                showPropCnt = showPropCnt / XH.areaData:getPropLimitedTimeRoomCardRatio()
            end
        end

        pro_text:setText(awards[i].propName.."x"..showPropCnt)
...
function IdentityCheckNView:onDefendRequest(strName, strIDNum)
    ...
    local isAutoBindPhone = false
    XH.lobby:getModule("IdentityCheck"):reqDefendIdentity(strName, strIDNum, isAutoBindPhone, self._isSupplement)
end
...
function IdentityCheckNView:initLayerNode(isAutoBindUI, isRealnameIng)
    ...
    -- self._inputName:setFontSize(34)
    -- self._inputName:setPlaceholderFontColor(cc.c3b(170, 170, 170))
    -- self._inputName:setFontColor(cc.c3b(170, 170, 170))
    ...
    -- self._inputNumber:setFontSize(34)
    -- self._inputNumber:setPlaceholderFontColor(cc.c3b(170, 170, 170))
    -- self._inputNumber:setFontColor(cc.c3b(170, 170, 170))
    ...
    -- 新增切换地区按钮
    if self._isForceRealName then
        self._btnClose:setVisible(false)
    elseif XH.playerData:getPassiveValue() == 2 and not XH.lobby:getModule("PersonalCenter"):getFakeIdentify() then
        self._btnClose:setVisible(false)
    ...
    --
    if self._zFuBIcon and self._zFuBPanel then
        local size = cc.size(174, 173)
        local headImage = XH.RemoteImage.new():setPosition(self._zFuBIcon:getPositionX(), self._zFuBIcon:getPositionY()):ignoreContentAdaptWithSize(false):setContentSize(size.width, size.height):addTo(self._zFuBPanel)
        headImage:setUrl("https://asset-iop.oss-cn-hangzhou.aliyuncs.com/zhejiang/pic/256294d3bfd1399316096b59797b6a52.png")
    end
end
...
function IdentityCheckNView:onTouchEventClose(send, eventType)
    ...
    if self.compulsoryRealName then
        local callback = function()
            XH.lobby:getModule("GuideNew"):checkShowGuide()
        end
        XH.TipTool.showTip({type = XH.TIP_LAYER_TYPE.OK, bTop = true, funcOK = callback, funcCancel = callback, funcClose = callback}, "完成实名认证才能游戏和充值哦，\n\n请尽快完成。")
    end
...
function IdentityCheckNView:onTouchEventChooseZF(send, eventType)
    ...
    XH.lobby:getModule("IdentityCheck"):setIsSdk()
    self:postClickData()
...
function IdentityCheckNView:postShowData()
    if XH.lobby and XH.lobby:getModule("UserPath") then
        ...
        XH.lobby:getModule("UserPath"):throwDataRepeat(XH.NewThrowDataDefine.xyh24081405, eventData, true, self._isSupplement)
    end
end

function IdentityCheckNView:postClickData()
    if XH.lobby and XH.lobby:getModule("UserPath") then
        local eventData = {
            block_label =  XH.lobby:getModule("IdentityCheck"):getIsSdk() and "支付宝" or "手动",
            item_id = "不勾选",
            customize = {
                choice = XH.lobby:getModule("UserPath"):firstStart()
            }
        }
        XH.lobby:getModule("UserPath"):throwDataRepeat(XH.NewThrowDataDefine.xyh25011001, eventData, true, self._isSupplement)
    end
end
