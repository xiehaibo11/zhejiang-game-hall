local ReqSpreadInviteClick = class("ReqSpreadInviteClick", XH.ReqHttp)
local UrlConf = require("app.Config.UrlConf")
local PromoteConfig = require("lobby.Modules.Promote.Config")

function ReqSpreadInviteClick:start(teaNumber, timeoutTime)
    if ReqSpreadInviteClick.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEAHOUSE_SPREAD_INVITE_CLICK)
...
function ReqSpreadInviteClick:onHttpCallBack(eType, status, response)
    if status == 200 then
