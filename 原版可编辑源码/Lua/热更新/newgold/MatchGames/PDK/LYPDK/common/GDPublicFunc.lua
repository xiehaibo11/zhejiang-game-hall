...
function _M.getCardColor(card)
    local isLaizi = false
    if card > 1600 then 
        card = card -  1600
        isLaizi = true
    end
    return math.floor(card/16) , isLaizi
end


function _M.isSameColor(data  , exceptLaizi)

    local color = nil

    if data and type(data[1]) == "table" then 
        local _data =  {}
	    GDPublicFunc.toUnidimensionalTable( data , _data)
        data = _data
    end

    for i,v in ipairs(data) do

        local c =  GDPublicFunc.getCardColor(v)

        local islaizi  =  g_gameGlobal:isGuandanLaizi(v)

        if not (exceptLaizi and islaizi) then 
            if not color then 
                color = c
            else
                if color ~= c then 
...
-- 获取牌的逻辑值和花色
function _M.getCardLogicValue(card , exceptLaizi )
    ...
    --癞子的逻辑值为1
    if  g_gameGlobal:isGuandanLaizi(card) and not exceptLaizi then 
        return 1
    end
    ...
    local isLegal = true
    if value == 0 or ( (color > 4 and color < 100) or color > 103  ) then 
        isLegal = false
    end 
...
--  排序 逻辑值
function _M.sortLogicCard(logicCards , model)

    if next(logicCards) == nil then
        return {}
    end 

    local sort_Func = function(a_logic,b_logic) 

        if g_gameGlobal:isJiPai(a_logic , true) then
            a_logic = 15
        end
        if g_gameGlobal:isJiPai(b_logic  , true) then
            b_logic = 15
        end
	...
	table.sort(logicCards,sort_Func)
end



-- 按牌值的逻辑排序，model == 1为降序，否则为升序，默认相同牌值按花色从小到大排序
--isHandSort,是否为手牌显示，手牌显示癞子牌要排在小王后面
function _M.sortByCardLogic(data,model , exceptlaizi,isHandSort )
	...
	local sort_Func = function(a,b) 
		local a_logic = _M.getCardLogicValue(a , exceptlaizi)
		local b_logic = _M.getCardLogicValue(b , exceptlaizi)

        if isHandSort then
            if g_gameGlobal:isJiPai(a) then
                a_logic = 15
            end
            if g_gameGlobal:isJiPai(b) then
                b_logic = 15
            end
        end
