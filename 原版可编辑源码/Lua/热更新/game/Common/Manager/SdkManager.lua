local SdkManager = XH.sdkManager

function SdkManager:isVoiceLimitVersion()
    return not self:isSupportFunction("fastvo_start_record")
end

function SdkManager:stardRecordWithNumid(numid)
...
function SdkManager:stopRecording()
    if device.platform == "windows" then
        return print("windows not suport")
    end
...
function SdkManager:cancelRecording()
    if device.platform == "windows" then
        return print("windows not suport")
    end
...
function SdkManager:playRecordVoice(url)
    if device.platform == "windows" then
        return print("windows not suport")
    end
