...
local PATH_SHUTCARDS = "res/audio/ShutCards"
local KW_PATH_SOUND_FILE = PATH_SHUTCARDS .. "/Base/"
local KW_PATH_GAMESOUND_FILE = PATH_SHUTCARDS .. "/GameLayer/"
...
local FILE_THREE_WITH_TWO_MP3 = "px_5_sandaier.mp3"
...
function SoundManager:playSoundCards(sex, cardType, cardPower, bFangYan, gameID)
    ...
    if cardType >= CardEnum.CARDS_TYPE.CT_3_JUNKO_2_EXCESS_1_CONTINUOUS and 
    cardType <= CardEnum.CARDS_TYPE.CT_3_JUNKO_2_EXCESS_5_CONTINUOUS then
        if bMan then
            path = KW_PATH_GAMESOUND_FILE .. gameID .. "/" .. sexPath[1]
        else
            path = KW_PATH_GAMESOUND_FILE .. gameID .. "/" .. sexPath[2]
        end
        SoundManager:playSound3with2(path)
    end
    if cardType > CardEnum.CARDS_TYPE.CT_4_JUNKO_1_EXCESS_0_CONTINUOUS and
        cardType < CardEnum.CARDS_TYPE.CT_4_JUNKO_3_EXCESS_0_CONTINUOUS then
...
function SoundManager:getSpeakConfigPath(gameID)
    return "game.ShutCards.GameSpeakConfig.GameSpeak_" .. gameID
end

function SoundManager:playSound3with2(path)
    local sound_path = path .. FILE_THREE_WITH_TWO_MP3
    if cc.FileUtils:getInstance():isFileExist(sound_path) then
