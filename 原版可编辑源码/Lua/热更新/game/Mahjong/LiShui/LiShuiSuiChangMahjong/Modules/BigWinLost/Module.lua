...
function BigWinLostModule:onMsgTotalResult(msgData)
    ...
    for seat = 0, CF.roomData:getMaxPlayer() - 1 do
        ...
        if player then
            ...
            if msgData.drawnCount[seat] and  msgData.drawnCount[seat] > 0 then
                tmpStrType[#tmpStrType + 1] = "流局" .. msgData.drawnCount[seat] .. "次"
            end

            if msgData.fangGangCount[seat] and msgData.fangGangCount[seat] > 0 then
                tmpStrType[#tmpStrType + 1] = "放杠" .. msgData.fangGangCount[seat] .. "次"
            end

            if msgData.sanQingSeat == seat then
                if msgData.isMenQianQing then
                    tmpStrType[#tmpStrType + 1] = "门前清"
                else
                    tmpStrType[#tmpStrType + 1] = "三清"
                end
            end

            tmpStrType[#tmpStrType + 1] = "最大 " .. msgData.maxFanNum[seat] .. " 番" or ""
