...
function WinLostView:getProxyEvents()
    ...
    local eventList = WinLostView.super.getProxyEvents(self)
    eventList[#eventList+1] = { module = winLostData, eventKeyName = "EVENT_SETTLE_CLOSE", callBack = "onRemoveSettleWindow" }
    return eventList
end
...
function WinLostView:onRemoveSettleWindow(event)
	self:close()
end
