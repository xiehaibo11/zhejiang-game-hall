local BigWinLostModule = CF.gameClass("BigWinLostModule", "game.Mahjong.ShengZhou.ShengZhouMahjong.Modules.BigWinLost.Module")
...
function BigWinLostModule:onMsgTotalResult(msgData)
    ...
    for seat = 0, CF.roomData:getMaxPlayer() - 1 do
        ...
        if player then
            ...
            local tmpStrType = {"胡 " .. msgData.maxHuCount[seat] .. " 次", "最大 " .. msgData.maxFanNum[seat] .. " 番"}
            ...
            if msgData.maxFanCount[seat] > 0 then
                ...
                if CF.roomData:getChairs() == 2 then
                    tmpStrType[#tmpStrType + 1] = ""
                    for j = 2, msgData.maxFanCount[seat] do
                        tmpStrType[#tmpStrType] = tmpStrType[#tmpStrType] .. ( j == 2 and "" or" " ) .. ( WinLostFanConfig.KW_WINLOST_FAN_TYPE[msgData.maxFanName[seat][j]] or "" )
                    end
                else
                    for j = 2, msgData.maxFanCount[seat] do
                        if j % 2 == 0 then
