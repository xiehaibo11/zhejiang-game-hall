...
-- 通过等级和充值获取奖励
function YGiftEnterModule:getGiftInfoByLevel(level, isRecharge)
    local info = self:getActInfo()
    if info == nil then
        return
    end
    for i = 1, #info do
        if info[i].session_id == level then
            if isRecharge then
                if info[i].normal_goods and #info[i].normal_goods > 0 then
                    return info[i].normal_goods
                end
            else
                if info[i].prefer_goods and #info[i].prefer_goods > 0 then
                    return info[i].prefer_goods
                end
            end
        end
    end
...
function YGiftEnterModule:reqPlayerInfo(session_id, gameID)
    ...
    self:startReq("ReqJoinRoomGift", self:getActivityId(), session_id, 10)
end
...
function YGiftEnterModule:respPlayerInfo(req, type, data)
    ...
    if type == XH.Req.TYPE.SUCCESS then
        ...
        -- 如果有对应配置就弹
        local info = self:getGiftInfoByLevel(self._roomLevel, data.isRecharged)
        if info then
