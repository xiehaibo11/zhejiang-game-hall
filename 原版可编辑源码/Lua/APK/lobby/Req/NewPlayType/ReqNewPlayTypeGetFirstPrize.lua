local ReqNewPlayTypeGetFirstPrize = class("ReqNewPlayTypeGetFirstPrize", XH.ReqHttp)
...
function ReqNewPlayTypeGetFirstPrize:start(teaNumber, timeoutTime)
    if ReqNewPlayTypeGetFirstPrize.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEA_NEW_PLAY_TYPE_ACT_GET_FIRST_PRIZE)
...
function ReqNewPlayTypeGetFirstPrize:onHttpCallBack(eType, status, response)
    if status == 200 then
...
return ReqNewPlayTypeGetFirstPrize