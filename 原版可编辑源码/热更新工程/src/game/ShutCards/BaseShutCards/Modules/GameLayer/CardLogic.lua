...
--从剩余牌中获取带牌
function CardLogic:getWithCards(restCardsID, withnum, startpos)
    local restCards = CardLogic:ID2MyCards(restCardsID)
    local orderCards = self:sortCard(restCards, GameDefine.KW_NUM_SORT_BY_COUNT)
    if withnum >= #orderCards then
        return false,orderCards
    end

    if startpos + withnum - 1 <= #orderCards then
        local tempcards = {}
        tempcards = MyCardsType:pushCards(tempcards, orderCards, startpos, startpos + withnum - 1)
        local bHavNext = (startpos + withnum - 1 < #orderCards)
        return bHavNext, tempcards
    else
        return false,{}
    end
end
