...
local UIMahHandAreaBase = CF.gameClass("UIMahHandAreaBase", CF.ViewBase)
...
function UIMahHandAreaBase:ctor(localSeat, config)
    ...
    self._combs = {}
    self._maxHandMahCount = CF.GameDefine.MAX_HAND_COUNT_TYPE.FOURTEEN
    self._UIConfig = nil
...
-- 获取麻将牌类型
function UIMahHandAreaBase:_getHandMahType(bStand, mahValue)
...
function UIMahHandAreaBase:setMaxHandMahCount(maxHandMahCount)
    for _, count in pairs(CF.GameDefine.MAX_HAND_COUNT_TYPE) do
...
function UIMahHandAreaBase:flushMahBack()
    ...
    for i = 1, #self._handMahs do
        handMahsData[i] = {mahValue = CF.GameDefine.MAH_VALUE.BACK, bStand = false}
    end
    self:_showHandMahs(handMahsData)
    -- 设置单放牌
    self:_showDanFang(CF.GameDefine.MAH_VALUE.BACK, false)
    self:_updateHandMahsPosition()
end
