local MahAlgorithm = CF.gameClass("MahAlgorithm")
...
--明杠
function MahAlgorithm:findExposedKong(handMahs, inMah)
    local combs = {}
    local tempCombs = {}
    local count = self:findMahCount(handMahs, inMah)
    if count == 3 then
...
function MahAlgorithm:isWan(mah)
    ...
    if mah >= CF.GameDefine.MAH_VALUE.WAN_1 and mah <= CF.GameDefine.MAH_VALUE.WAN_9 then
        return true
    else
        return false
    end
...
function MahAlgorithm:isTiao(mah)
    ...
    if mah >= CF.GameDefine.MAH_VALUE.TIAO_1 and mah <= CF.GameDefine.MAH_VALUE.TIAO_9 then
        return true
    else
        return false
    end
...
function MahAlgorithm:isTong(mah)
    ...
    if mah >= CF.GameDefine.MAH_VALUE.TONG_1 and mah <= CF.GameDefine.MAH_VALUE.TONG_9 then
        return true
    else
        return false
    end
...
function MahAlgorithm:getNextMahOfSameType(mah) 
    if self:isWan(mah) == true or self:isTiao(mah) == true or self:isTong(mah) ==true then
        if mah == CF.GameDefine.MAH_VALUE.WAN_9 or mah ==  CF.GameDefine.MAH_VALUE.TIAO_9 or mah ==  CF.GameDefine.MAH_VALUE.TONG_9 then
            return 0
        end
        return mah + 1
    end
...
function MahAlgorithm:getPreviousMahOfSameType(mah) 
    if  self:isWan(mah) == true or 
        ...
        if mah == CF.GameDefine.MAH_VALUE.WAN_1 or 
            mah ==  CF.GameDefine.MAH_VALUE.TIAO_1 or 
            mah ==  CF.GameDefine.MAH_VALUE.TONG_1 then
            return 0
        end
        return mah - 1
    end
