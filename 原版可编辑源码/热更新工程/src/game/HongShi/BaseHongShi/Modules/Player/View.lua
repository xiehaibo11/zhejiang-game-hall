...
function PlayerView:getProxyEvents()
    ...
    eventTable[#eventTable + 1] = {module = CF.game:getModule("Player"), eventKeyName = "EVENT_UPDATE_LIGHT_TEN", callBack = "onLightTen"}
    eventTable[#eventTable + 1] = {module = CF.game:getModule("GameLayer"), eventKeyName = "EVENT_LEFT_CARDS_CHANGED", callBack = "onLeftCardNumChanged"}
...
function PlayerView:clearPlayerHead(localSeat) 
    ...
    if playerHead then
        ...
        playerHead:updateLightTen(false)
    end
end

function PlayerView:onLightTen(event)
    local msg = event.msg
    if not msg then
        return
    end
    local localSeat = event.msg.localSeat
    local bFlag = event.msg.bFlag
    local nType = event.msg.nType
    local playerHead = self._playerHeads[localSeat]
    if playerHead then
        playerHead:updateLightTen(bFlag,nType)
    end
end

function PlayerView:onLeftCardNumChanged(event)
    local msg = event.msg
    if not msg then
        return
    end
    ...
    local playerHead = self._playerHeads[localSeat]
    if playerHead then
        local leftCardNum = self._gameData:getHandCardNum(msg.seat)
        playerHead:setLeftCardNum(leftCardNum)
    end
end
