...
function IMListView:getBindingInfo()
    return {
        ...
        ["_KW_IMG_HAVE_AWARD2"] = { varName = "_KW_IMG_HAVE_AWARD2" },
        ["_KW_IMG_HAVE_AWARD3"] = { varName = "_KW_IMG_HAVE_AWARD3" },
...
function IMListView:ctor(openType) 
...
function IMListView:initUI()
    ...
    if not cc.UserDefault:getInstance():getBoolForKey("IMListLayer_First_Open_" .. date, false) and self._openType ~= XH.LOBBY_TYPE.XIUXIAN then
...
function IMListView:initUIInGame()
    self._openBtnPic:setOpacity(0)
    self._readyPanel:setOpacity(0)
...
function IMListView:initTableView(parentNode, viewName)
    ...
    local tableViewLayout = tableView:getChildren()[1] -- tableview下面还有一个layout，才是cell
...
function IMListView:throwDataClickEnd()
    ...
    if self._friendPanel then
        ...
        if not bMoveIn then
            ...
            actSequence = cc.Sequence:create(actSpawn, cc.CallFunc:create(function()
                                ...
                                self._readyPanel:setOpacity(255)
...
function IMListView:hideListEntrance()
    if self._openBtnPic then
        self._openBtnPic:setOpacity(0)
    end
    if self._friendPanel then
        self._friendPanel:setVisible(false)
    end
    if self._readyPanel then
        self._readyPanel:setOpacity(0)
        self._readyPanel:setVisible(false)
    end
    if self._openBtnPicInGame then
...
function IMListView:showListEntrance()
    if XH.gameManager then
        ...
        XH.UITool.runLobbyAction(self._readyPanel, {"", "commonFadeIn"})
        self._readyPanel:setVisible(true)
        XH.UITool.runLobbyAction(self._openBtnPic, {"", "commonFadeIn"})
    end
end
...
function IMListView:setIconHaveAwardVisible(visible)
    local iconList = { self._KW_IMG_HAVE_AWARD2, self._iconHaveAward, self._KW_IMG_HAVE_AWARD3 }
    for i = 1, #iconList do
        if iconList[i] then
            ...
            if iconList[i] == self._KW_IMG_HAVE_AWARD3 and CF and CF.roomData and CF.roomData:getIsSeer() then
...
function IMListView:reqHasRecallAward()
    ...
    req:addReqCallBack(
        self,
        function(tager, _, nType, vData)
            if nType == XH.Req.TYPE.SUCCESS and tager and not tolua.isnull(tager) then
...
function IMListView:onBtnFriendData(send, eventType)
    if eventType == ccui.TouchEventType.began then
        XH.audioManager:play("BUTTON_CLICK")
        send:setScale(0.95)
        send:setColor(cc.c3b(160,160,160))
...
function IMListView:on_BTND_WILL_JOIN(tableInfo, send, eventType)
    ...
    local data = {
        block_label = sceneName,
        game_id = XH.areaData:getAreaID() .. "_" .. tableInfo.gameid,
        room_id = tableInfo.game_roomid,
        players = tableInfo.chairs,
    }
...
function IMListView:on_KW_BTND_INVITE_ALL(send, eventType)
    ...
    if CF ~= nil and CF.roomData ~= nil then
        ...
        local inviteCnt = 0
        if allData and allData._friendListInfo then
            for i = 1, #allData._friendListInfo do
                if allData._friendListInfo[i] and (allData._friendListInfo[i].player_state == XH.IMProtocol.PLAYERSTATE.PS_ONLINE or allData._friendListInfo[i].player_state == XH.IMProtocol.PLAYERSTATE.PS_WAIT) then
                    inviteCnt = inviteCnt + 1
                end
            end
        end
        data = {
...
function IMListView:setReadyInfo(node, data, index)
    ...
    -- 设置头像
    local headNode = self:getRemoteImg(node, "_KW_IMG_HEAD")
    if headNode then
...
function IMListView:getRemoteImg(node, childNode)
    local tmp = XH.UITool.seekNodeByName(node, childNode)
    if tmp then
        tmp:removeAllChildren()
        local headImg = XH.RemoteImage.new()
        :setPosition(tmp:getContentSize().width / 2, tmp:getContentSize().height / 2)
        :ignoreContentAdaptWithSize(false)
        :setContentSize(tmp:getContentSize().width, tmp:getContentSize().height)
...
--    XH.UITool.setText(node, "_KW_TEST_IDX", index + 1)
    ...
    self:initFrienDataBtn(node)
end

function IMListView:initFrienDataBtn(node, data, index)
    local btnFriendData = XH.UITool.seekNodeByName(node, "_KW_UI_DATA")
    if btnFriendData then
        btnFriendData:addTouchEventListener(handler(self, self.onBtnFriendData))
    end
end
...
function IMListView:setFrameUrl(node, item, url)
    ...
    if node then
        ...
        propFrame = XH.RemoteImage.new()
            ...
            :ignoreContentAdaptWithSize(false)
            :setContentSize(self:getHeadFrameSize() or KW_FRAME_DEFULT_SIZE)
            :addTo(node)
        if item then
...
function IMListView:setTableViewItem(parent, item)
    ...
    childItem:setCascadeOpacityEnabled(true)
...
function IMListView:onUpdateStatus()
    ...
    local friendListWaiting = self:getData():getFriendListWaiting()
    local isShow = false
    for _,v in pairs(friendListWaiting) do 
...
function IMListView:getHeadFrameSize()
    return KW_FRAME_DEFULT_SIZE
end
