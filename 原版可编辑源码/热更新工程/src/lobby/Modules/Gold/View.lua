...
function GoldView:on_KWA_BTND_BACk(send, eventType)
    XH.lobby:getModule("AutoPop"):onBackToLobby("goldlayer")
...
function GoldView:initGameListView()
    ...
    if goldGames and #goldGames > 0 then
        self._gameListView:setInnerContainerSize(cc.size(570 * (#goldGames + 1), self._gameListView:getContentSize().height))
        ...
        for i = 1, #goldGames do
            ...
            tmpCreateBtn:setName("KWA_BTND_CREATE_" .. goldGames[i].LeisureID)
...
function GoldView:initBuyuPanel()
    local btn = self.KWA_BTND_CREATE_JINBICHANG_ITEM:clone()
    self._gameListView:addChild(btn)
    XH.UITool.setText(btn, "_KW_LABEL_ITEM_TITLE", "捕鱼")
    local node = XH.UITool.seekNodeByName(btn, "_KW_LABEL_ITEM_LOGO")
    node:setVisible(false)
    local pos = cc.p(node:getPosition())

    local params = {
        path = "animation/Lobby/Base/rrby_icon/",
        tex = "zzb_buyu_xfrk.json",
        ske = "zzb_buyu_xfrk.atlas",
    ...
    if armature then
        ...
        armature:setPosition(cc.p(pos.x, pos.y))
    end

    btn:addTouchEventListener(handler(self, self.onRRBYClicked))
end

function GoldView:onRRBYClicked(send, eventType)
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.lianyun2024092403)
...
function GoldView:on_KWA_BTND_CREATE_JINBICHANG(send, eventType)
    ...
    if isNewUser == false then
        XH.throwDataManager:throwDataClick(XH.ThrowDataDefine.NEWPLAYER_GUIDE_12,
                                           {GameID = roomConf.roomInfo.GameID, GameName = roomConf.roomInfo.GameTitle})
    end
    ...
    if roomConf.roomInfo.GameID == 999999 then
        ...
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.GUANDAN_GOLD_CLICK, nil,
                                         {area_id = XH.areaData:getAreaID(), channel_id = XH.SysTool:getChannelID()})
        return
    end
...
function GoldView:initCreateBtnInfo(buttonNode, roomInfo)
    ...
    if roomInfo.GameID == 999999 or roomInfo.GameID == 30579 then
        local iconNew = XH.UITool.seekNodeByName(buttonNode, "_KW_ICON_NEW")
        if iconNew then
            XH.SysTool.createShakeAni(iconNew)
            iconNew:setVisible(true)
        end
        XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.GUANDAN_ICON_SHOW, nil,
                                         {area_id = XH.areaData:getAreaID(), channel_id = XH.SysTool:getChannelID()})
    end
...
function GoldView:moveInAnimation()
    if self._goldRoot then
        ...
        local sequenceAction = cc.Sequence:create(actSpawn, cc.CallFunc:create(function()
...
function GoldView:runCountDownAcitvity(node, leftTime)
    ...
    timeNode:runAction(cc.RepeatForever:create(cc.Sequence:create(cc.CallFunc:create(function()
        leftTime = leftTime - 1
        if leftTime >= 0 then
