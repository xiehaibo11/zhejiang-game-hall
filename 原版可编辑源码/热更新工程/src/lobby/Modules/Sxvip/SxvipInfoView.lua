...
function SxvipInfoView:ctor(param, oriInfo)
    SxvipInfoView.super.ctor(self)
    self._buyData = param or {}
    self._oriData = oriInfo
...
function SxvipInfoView:getSubScription()
    ...
    if self._oriData and self._oriData.tags and self._oriData.tags[1] and self._oriData.tags[1].key and self._oriData.tags[1].key == "Subscription" then
        return true
    end
    return false
end
...
function SxvipInfoView:onBtnBuy(send, event)
    ...
    XH.lobby:getModule("Sxvip"):reqBuy(self._buyData, nil, true, nil, self._oriData)
    self:close()
end
