...
RepluginModule.PLUGIN_LIST = {
    RRBY = "61c550c2-3d61-4375-86bd-4dc5a9a4d558", -- 人人捕鱼
    CARDB = "05e83702-04d2-42bb-b074-2e9b09715775", -- 十三张
}
...
function RepluginModule:isSupportRePlugin()
    if device.platform == "android" and not XH.isEmbeddedApp() then
...
function RepluginModule:addRepluginListener()
    if device.platform ~= "android" or XH.isEmbeddedApp() then
        return
    end
...
function RepluginModule:setLastPlugin(pluginid)
    self._pluginName = pluginid
end

function RepluginModule:getLastPlugin()
    return self._pluginName or RepluginModule.PLUGIN_LIST.RRBY
end
...
function RepluginModule:invokeFuncNameandParameters(method, param)
    repluginSdk.invokeFuncNameandParameters(method, param)
end

function RepluginModule:deleteDownloadCacheAndIgnoreSilentDownload(pluginid)
    repluginSdk.deleteDownloadCacheAndIgnoreSilentDownload(pluginid)
end
