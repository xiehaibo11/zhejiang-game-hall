...
local KW_PROP_DOUBLE_CARD = "double_card"
local KW_PROP_PEAK_RACE_TICKETS = "peak_race_tickets"
...
function BagSysData:getSelfDevideCategoryDataTest()
    ...
    for _,prop in pairs(devideBackConfig) do
        local tag = prop.props_tag_id 
        if tag and tag ~= "gift" and tag ~= "hide" then
            categoryDevideData[tag] = categoryDevideData[tag] or {}
            local tmpTimeProp = clone(prop)
            tmpTimeProp.addtime = prop.addtime or 0
            tmpTimeProp.propscnt = 123
            table.insert(categoryDevideData[tag], tmpTimeProp)
        end
    end
...
function BagSysData:getDevideCategoryData(playerBagData)
    ...
    if next(doubleCards) then
        categoryDevideData[KW_PROP_DOUBLE_CARD] = categoryDevideData[KW_PROP_DOUBLE_CARD] or {}
        for _,doubleCard in pairs(doubleCards) do
            local tmpTimeProp = clone(doubleCard)
            tmpTimeProp.is_time_prop = true
            table.insert(categoryDevideData[KW_PROP_DOUBLE_CARD], tmpTimeProp)
            table.insert(allprop,tmpTimeProp)
        end
    end

    -- 巅峰赛
    local peakRaceTickets = XH.lobby:getModule("BagSysNew"):getPeakRaceTickets()
    if next(peakRaceTickets) then
        categoryDevideData[KW_PROP_PEAK_RACE_TICKETS] = categoryDevideData[KW_PROP_PEAK_RACE_TICKETS] or {}
        for _,peakRaceTicket in pairs(peakRaceTickets) do
            local tmpTimeProp = clone(peakRaceTicket)
            tmpTimeProp.is_time_prop = true
            table.insert(categoryDevideData[KW_PROP_PEAK_RACE_TICKETS], tmpTimeProp)
            table.insert(allprop,tmpTimeProp)
        end
    end
...
function BagSysData:getSelfDevideCategoryData()
    if XH.isEmbeddedApp() then
        return self:getSelfDevideCategoryData50()
    end
...
function BagSysData:isTimeProp(propData)
    ...
    if XH.isEmbeddedApp() and propData.can_use then
        return true
    end
...
function BagSysData:getPropInfo(propID)
    ...
    for _, v in pairs(self._selfBagData.allProps or {}) do
        if v.prop_id == propID then
            return v
        end
    end
    for _, v in pairs(self._basePropInfo or {}) do
...
function BagSysData:setBasePropInfo(info)
    for _,v in pairs(info) do
        self._basePropInfo[v.propsid] = v
    end
end

function BagSysData:getSelfDevideCategoryData50()
    local allprop = {}
    local categoryDevideData = {}
    for _, v in pairs(self._selfBagData.allProps or {}) do
        categoryDevideData[v.props_tag_id] = categoryDevideData[v.props_tag_id] or {}
        local tmpTimeProp = clone(v)
        table.insert(categoryDevideData[v.props_tag_id],tmpTimeProp)
        table.insert(allprop,tmpTimeProp)
    end
    categoryDevideData["all_prop"] = allprop
    return categoryDevideData
end
return BagSysData