...
PlayerBillModule.EVENT_HALL_BILL_UPDATE_TOTALLIST_GOLD = "EVENT_HALL_BILL_UPDATE_TOTALLIST_GOLD"
...
function PlayerBillModule:getReqConfig()
    return {
        ...
        ReqGoldPlayBack = {reqPath = "lobby.Req.PlayerBill.ReqGoldPlayBack", callBack = self.onReqShareGameCallBack},
    }
end
...
function PlayerBillModule:initData()
    ...
    self._totalBillInfoGold = {}			--金币战绩数据
end
...
--请求回放
function PlayerBillModule:startPlayBack(shareID, is50)
    ...
    if is50 then
        self:startReq("ReqGoldPlayBack", shareID, 10)
    else
...
function PlayerBillModule:onReqShareGameCallBack(cell, type, data)
	...
	if type == XH.Req.TYPE.SUCCESS then
        ...
        param.is50 = cell._is50
        param.chaircnt = data and data.chairCnt
...
function PlayerBillModule:onReqCurTimeCallBack(req, type, data)
    if type == XH.Req.TYPE.TIMEOUT then
        ...
        if cc.UserDefault:getInstance():getStringForKey("btn_tab_bill", XH.PlayerBillTab.battle) == XH.PlayerBillTab.battle then
            self:readPlayerBillInfo(PlayerBillModule.SHOW_BILL_DAY)
        else
            self:reqGoldBill(PlayerBillModule.SHOW_BILL_DAY)
        end
    end
end
...
function PlayerBillModule:reqGoldBill(curBillTimeIndex, curPage)
    ...
    local param = {
        ...
        timestamp =  tostring(nReqTime),
        curPage = curPage or 1
    }

    local ReqGoldBill = require("lobby.Req.PlayerBill.ReqGoldBill")
    local reqGoldBill = ReqGoldBill.new()
    reqGoldBill:addReqCallBack(self, self.respGoldBill)
    reqGoldBill:start(param, curBillTimeIndex, 10)
end

function PlayerBillModule:respGoldBill(cell, type, data)
    if data and data.resp and data.resp.errCode == 0 then
        if data.curPage == 1 then
            self._totalBillInfoGold = {}
        end
        for i, zhanji in pairs(data.zhanjis) do
            table.insert(self._totalBillInfoGold, zhanji)
        end
        if data.curPage >= data.totalPage then
            self:dispatchEvent({ name = PlayerBillModule.EVENT_HALL_BILL_UPDATE_TOTALLIST_GOLD })
        else
            self:reqGoldBill(data.curBillTimeIndex, data.curPage + 1)
        end
    end
end

function PlayerBillModule:getGoldBillInfo()
    return self._totalBillInfoGold
end
