...
function RecallListItem:setAward(count)
    ...
    elseif self._propID == XH.areaData:getPropLimitedTimeRoomCardID() then
        self._iconAward:loadTexture("recall_res_icon_fk.png", ccui.TextureResType.plistType)
        count = count / XH.areaData:getPropLimitedTimeRoomCardRatio()
    else
...
function RecallListItem:onBtnEventInvate()
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.zh25032003, { item_id = XH.areaData:getAreaID() .. "_" .. self._userID, page = scene })
end
