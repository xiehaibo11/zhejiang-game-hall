...
function LobbyShareView:ctor()
    ...
    XH.NewThrowDataManager:throwData(XH.NewThrowDataDefine.ohfx25070901 ,{page = "大厅"})
end
...
function LobbyShareView:initUI()
    ...
    if shareInfo ~= nil and shareInfo.award ~= nil and #shareInfo.award > 0 then
        for i = 1, #shareInfo.award do
            ...
            local showPropCnt = shareInfo.award[i].propCount
            if shareInfo.award[i].propId == XH.areaData:getPropLimitedTimeRoomCardID() then
                if XH.areaData:getPropLimitedTimeRoomCardRatio() then
                    showPropCnt = showPropCnt / XH.areaData:getPropLimitedTimeRoomCardRatio()
                end
            end
            temp:getChildByName("_KW_TEXT_NAME"):setString("x" .. showPropCnt)
