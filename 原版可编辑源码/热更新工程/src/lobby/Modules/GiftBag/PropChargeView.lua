--钻石不够的充值弹窗,会主动计算差的钻石进行推荐
local PropChargeView = class("PropChargeView", XH.ViewBase)
local ShopConfig = require("lobby.Modules.Shop.Config")

function PropChargeView:getCSBPath()
    return "cocosStudio/hall/CSB/vip/VipChargeLayer.csb"
end

function PropChargeView:getBindingInfo()
    return {
...
function PropChargeView:getProxyEvents()
    return {
        { module = XH.lobby:getModule("Shop"), eventKeyName = "EVNET_SHOPMANAGER_SHOW_SHOP", callBack = "onEventProductList" },
    }
end

function PropChargeView:ctor(param)
    self._canTouchBackGround = false
    param = param or { }
    PropChargeView.super.ctor(self, param)
    self:initData(param)
end

function PropChargeView:initData(param)
...
-- 初始化界面UI
function PropChargeView:updateLayerUI()
...
function PropChargeView:dolayout()
...
function PropChargeView:onEventProductList(data)
...
function PropChargeView:onTouchEventClose(send, eventType)
    self:close()
end

function PropChargeView:onTouchEventConfirm(send, eventType)
    if self._buyProduct then
        XH.lobby:getModule("Shop"):reqBuyDiamond(self._buyProduct, nil,self._giftType, nil, nil, nil, "gift")
    end
    self:close()
end

return PropChargeView