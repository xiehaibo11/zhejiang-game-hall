...
function _M.sortData(data,model)
    ...
    if type(data) ~= "table" or next(data) == nil then
        print_debug_stack("DDZPublicFunc sortData");
...
function _M.getCardLogicValue(card)
    ...
    if isLegal == false then
        print_debug_stack("DDZPublicFunc getCardLogicValue");
    ...
    if card == 0x42 or card == 0x41 then        -- 大鬼 or  小鬼
        value = 15 + value
...
function _M.toUnidimensionalTable(data,result)
	...
	for k, v in pairs(data) do 
		...
		if type(v) == "table" then 
			local tmp = _M.toUnidimensionalTable(v,result)
		else
