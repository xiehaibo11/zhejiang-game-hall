...
local UIMah = NG.GAME.gameClass("UIMah2D", "newgold.GoldGames.GoldGameMah.MahXueLiu.Modules.GameMain.MahLayer.ViewBase.UIMahBase")
...
function UIMah:ctor(mahType, config)
    ...
    self._mahValue = NG.GAME.GameDefine.MAH_VALUE.NONE
    ...
    -- 打出是否可能会承包
    self._bPreBao = false
    if config.isSimple then
        self:_initRootNode()
        self._isSimple = true
        self._simpleConfig = config
        self:createSimple(config)
    end
...
function UIMah:_initView()
    ...
    self:_setBackPatternImgVisible(not NG.GAME.settingData:getIsClearModel())
end

function UIMah:_initEvent()
    self._settingDataProxy = cc.EventProxy.new(NG.GAME.settingData,self)
        :addEventListener(NG.GAME.settingData.EVENT_CLEAR_MODEL, handler(self,self.onEventClearModel))--纯净模式
        :addEventListener(NG.GAME.settingData.EVENT_MAHJONG_CONFIG, handler(self,self.onEventMahJongConfig))
end
...
function UIMah:onEventMahJongConfig(event)
    ...
    local isShow = not NG.GAME.settingData:getIsClearModel()
...
function UIMah:_resetBackPatternImg()
     ...
     self._mahBackPatternImg = self:_createMahImg(GroundImgType.BackPattern, ImgSource.Download)
end
...
function UIMah:_updateFace()
    ...
    if not self._mahFaceImg then
        if self._isSimple then
            self:createSimple(self._simpleConfig)
            return
        end
...
function UIMah:_isNeedLessenFaceScale()
    if self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TOLEFT
    or self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TORIGHT then
        return true
    end
    return false
end
...
function UIMah:_createMahImg(keyStr, imgSource)
    ...
    local newImg = self:_getNewImg(imgSource, frameName, layoutConfig)
    if not newImg then
        return
    end
...
function UIMah:_getFrameName(keyStr, frameNameStr, shapeStr)
    ...
    elseif keyStr == GroundImgType.BackPattern then
        ...
        if backPatternType ~= "" and type(backPatternType) == "table" then
            if self._mahType == NG.GAME.GameDefine.MAH_TYPE.STAND_FACE_BACKWARD then
                frameName = backPatternType[1] or ""
            elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_VERTICAL then
                frameName = backPatternType[1] or ""
            elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_DOWN_HORIZONTAL then
...
function UIMah:_getNewImg(imgSource, frameName, layoutConfig)
    local newImg
...
function UIMah:_updateLayoutAndSize()
    ...
    local sizeTop = topImg:getContentSize()
    local sizeBottom = bottomImg:getContentSize()
    local by = bottomImg:getPositionY()
...
function UIMah:_updateJokerIconPosition()
    ...
    elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TOUP then
        self._jokerIcon:setPosition(jockerIconSize.width/2, selfSize.height-jockerIconSize.height/2)
    elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_VERTICAL_TODOWN then
        self._jokerIcon:setPosition(selfSize.width - jockerIconSize.width/2, thick+jockerIconSize.height/2)
    elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TOLEFT then
        self._jokerIcon:setPosition(jockerIconSize.height/2, thick+jockerIconSize.width/2)
    elseif self._mahType == NG.GAME.GameDefine.MAH_TYPE.LIE_FACE_UP_HORIZONTAL_TORIGHT then
...
function UIMah:_createIconImg(iconType)
    ...
    if iconType == IconType.Joker then
        local shapeType = self:_getConfigValueByKey(UIMahConfig.MahSettingKey.ShapeType)
        if shapeType == NG.GAME.GameDefine.MAH_SHAPE_TYPE.SQUARE then
            frameName = UIMahConfig.MahJokerIconFrameName.Square
        else
            frameName = UIMahConfig.MahJokerIconFrameName.Circle
        end
        if UIMahConfig.JokerSet and UIMahConfig.JokerSet[NG.GAME.roomTableData:getGameID()] then
            frameName = UIMahConfig.JokerSet[NG.GAME.roomTableData:getGameID()]
        end
...
function UIMah:_updateJokerIcon()
    ...
    if UIMahConfig.JokerColorSet and UIMahConfig.JokerColorSet[NG.GAME.roomTableData:getGameID()] then
...
function UIMah:_updateTingIcon()
...
function UIMah:showTingIcon(bShow)
...
function UIMah:createSimple(config)
    if config.plist then
        MahLogic.addPlistSpriteFrames(config.plist)
    end
    if not self._mahFaceGroundImg2 then
        self._mahFaceGroundImg2 = ccui.ImageView:create()
        self._mahFaceGroundImg2:loadTexture(config.bg, ccui.TextureResType.plistType)
        self:_addToRootNode(self._mahFaceGroundImg2, UIMahConfig.MahZorderConf.Face)
    end
    if not self._mahFaceImg then
        self._mahFaceImg = ccui.ImageView:create()
        self:_addFaceSpriteFrames()
        self._mahFaceImg:loadTexture(self:_getFaceFrameName(), ccui.TextureResType.plistType)
        self._mahFaceImg:setScale(config.faceScale or 1)
        self:_addToRootNode(self._mahFaceImg, UIMahConfig.MahZorderConf.Face)
    end
end
return UIMah