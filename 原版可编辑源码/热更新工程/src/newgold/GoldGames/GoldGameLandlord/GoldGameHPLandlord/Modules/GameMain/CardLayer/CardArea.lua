...
--创建一张牌
function CardArea:createCard(cardId, cardIndex)
    local card = Card:new()
...
function CardArea:setSpecialColorCardIDs(cardIDs)
...
--播放发牌动画
function CardArea:playOutCardAnimation()
    local delayTime = 0
    for i = #self._cards, 1, -1 do
        ...
        self._cards[i]:runAction(
            cc.Sequence:create(
                ...
                cc.ScaleTo:create(0.15, 1.4),
                cc.ScaleTo:create(0.15, 1),
                cc.CallFunc:create(
                    function()
                        if i == 1 then
                            self:onPlayOutCardAniEnd()
                        end
                    end
                )
            )
...
function CardArea:onPlayOutCardAniEnd()
...
function CardArea:onPlaySendCardAniEnd()
...
--设置显示牌数据
function CardArea:setShowCards(cardIDs, aniBool, aniType)
    self:clearCards()
    ...
    for index, cardId in ipairs(cardIDs) do
        local card = self:createCard(cardId, index)
    ...
    if aniBool then
        if aniType and aniType == "out" then
            self:playOutCardAnimation()
        else
