...
local HandCardArea = NG.GAME.gameClass("HandCardArea", CardArea)
...
--设置手牌可操作
function HandCardArea:setShowCards(cardIDs, aniBool)
    HandCardArea.super.setShowCards(self, cardIDs, aniBool)
...
function HandCardArea:onCardTouchEvent(send, eventType)
    --正在执行发牌动画，不允许操作
    if self._bPlayingSendCardAni then
        return
    end
...
function HandCardArea:dealCardSlide(movePos)
    ...
    if hitCard and hitCard:getTag() ~= self._currentTouchIndex then
...
function HandCardArea:dealCardSelectEvent(haveUnSelect)
    ...
    local isShowFire = CardLayerFunction.isAllBoom(selectCardIDs)
    self:showFireOnSelectedCards(isShowFire)
end
...
function HandCardArea:setDragCards(cardIDs, movePos)
    ...
    local tempPos = self:convertToNodeSpace(movePos) --世界坐标转换为本地坐标
    self._dragCardArea:setPosition(tempPos)
    self._dragCardArea:setShowCards(cardIDs)
...
function HandCardArea:resetCards()
    ...
    self:dealCardSelectEvent(false)
end
...
function HandCardArea:showFireOnSelectedCards(isShowFire)
    for _, card in ipairs(self._cards) do
        card:showFire(isShowFire and card:isSelect())
    end
end

local MoveBoomLineTip = "MoveBoomLineTip"
function HandCardArea:clearMoveBoomLineTipAni()
    self:removeChildByName(MoveBoomLineTip)
end

function HandCardArea:showMoveBoomLineTipAni(lineNum)
    self:clearMoveBoomLineTipAni()
    local aniName = "animation"..lineNum
    local spineNode = NG.UITool.playDargonBonesSpine("NewGoldRes/Spine/HPLandlord/GameMain/", "Hp_zds.json", "Hp_zds.atlas", aniName, false)
    if spineNode then
        spineNode:setName(MoveBoomLineTip)
        local cardSize = self:getContentSize()
        spineNode:setScale(3)
        spineNode:setAnchorPoint(cc.p(0.5, 0.5))
        spineNode:setPosition(0, 190)
        self:addChild(spineNode)
    end
end

function HandCardArea:onPlaySendCardAniEnd()
    HandCardArea.super.onPlaySendCardAniEnd(self)
    local handCards = self:getCardIDs()
    local booms = NG.goldGame:getModule("GameMain"):getTipLogic().gameLogic:getNormalBoomGroup(handCards)
    self:showMoveBoomLineTipAni(#booms)
    self:showBoomEffectAni(handCards, booms)

    NG.msgManager:sendMsgEndWait()
end

function HandCardArea:setBombCards(bombCards)
    local boomPowers = {}
    for k, v in pairs(bombCards) do
        boomPowers[#boomPowers + 1] = GameMainDefine.ucCardPower[v[1]]
    end
    table.sort(boomPowers)
    local showData = {}
    local lastPower = -1
    local line = 1
    for i, power in ipairs(boomPowers) do
        if lastPower + 1 == power and lastPower ~= GameMainDefine.CardPower.CP_A then
            line = line + 1
            if i == #boomPowers then
                showData[#showData + 1] = {power = power, line = line}
            end
        elseif lastPower > 0 then
            showData[#showData + 1] = {power = lastPower, line = line}
            line = 1
            if i == #boomPowers then
                showData[#showData + 1] = {power = power, line = line}
            end
    ...
    local removeFromTable = function (toShowData, removecardID)
        local removePower = GameMainDefine.ucCardPower[removecardID]
        for index, v in ipairs(toShowData) do
            local tempLine = v.line
            if v.power == removePower then
                table.remove(toShowData, index)
                return true, tempLine
            end
        end
        return false, 0
    end
    for cardIndex = #self._cards, 1, -1 do
        local cardID = self._cards[cardIndex]:getCardID()
        local isRemove, line = removeFromTable(showData, cardID)
        if isRemove then
            self._cards[cardIndex]:showBombLine(true, line)
        else
            self._cards[cardIndex]:showBombLine(false)
        end
    end
end

-- 播放框动画
function HandCardArea:showBoomEffectAni()
    self:flushBoomEffectNodes(true)
    local handCards = self:getCardIDs()
    local booms = NG.goldGame:getModule("GameMain"):getTipLogic().gameLogic:getNormalBoomWithoutKing(handCards)
    if #booms == 0 then
        return
    end
    local boomPowers = {}
    ...
    if #showData > 0 then
        for i, v in ipairs(showData) do
            tempNode = tempNode + 1
            self:setCombBombEffect(v.power, v.line, tempNode)
        end
    end
end

local KW_COMB_BOMB_EFFECT = "KW_COMB_BOMB_EFFECT"
function CardArea:flushBoomEffectNodes(isRemove)
    for i = 1, 13 do
        local node = self:getChildByName(KW_COMB_BOMB_EFFECT .. i)
        if node ~= nil then
            if isRemove then
                self:removeChildByName(KW_COMB_BOMB_EFFECT .. i)
            else
...
function CardArea:setCombBombEffect(power, line, nodeIdx)
    local showCard = nil
    local addWidth = 0
    local distanceX = self:getCardDistanceX()
    local scale = 1.18
    for cardIndex = #self._cards, 1, -1 do
        if GameMainDefine.ucCardPower[self._cards[cardIndex]:getCardID()] == power then
            showCard = self._cards[cardIndex]
            if GameMainDefine.ucCardPower[self._cards[1]:getCardID()] == power or GameMainDefine.ucCardPower[self._cards[cardIndex]:getCardID()] - line + 1 == GameMainDefine.ucCardPower[self._cards[1]:getCardID()] then
                addWidth = (self._cards[1]:getContentSize().width - distanceX) / scale
            end
            break
        end
    end
    line = line * 4
    local cardZhaImg = ccui.ImageView:create()
    cardZhaImg:loadTexture("newgold_hp_landlord_CardZha.png", ccui.TextureResType.plistType)
    cardZhaImg:setAnchorPoint(cc.p(1, 0.5))
    cardZhaImg:ignoreContentAdaptWithSize(false)
    cardZhaImg:setScale9Enabled(true)
    cardZhaImg:setScale(scale)
    cardZhaImg:setPosition(showCard:getPositionX() + (line - 1) * distanceX + addWidth-7, showCard:getPositionY())
    cardZhaImg:setContentSize((line * distanceX + addWidth + 50) / scale, cardZhaImg:getContentSize().height)
    cardZhaImg:setName(KW_COMB_BOMB_EFFECT .. nodeIdx)
    cardZhaImg:setOpacity(0)
    self:addChild(cardZhaImg)
    -- 闪烁
    local actSequence2 = cc.Sequence:create(cc.FadeIn:create(0.2), cc.DelayTime:create(0.5), cc.FadeOut:create(0.7),
    cc.FadeIn:create(0.2), cc.DelayTime:create(0.5), cc.FadeOut:create(0.7),
    cc.FadeIn:create(0.2), cc.DelayTime:create(0.5), cc.FadeOut:create(0.7),
    cc.FadeIn:create(0.2), cc.DelayTime:create(0.5), cc.FadeOut:create(0.7),
    cc.CallFunc:create(function(cardZhaImg)
        if cardZhaImg ~= nil then
            cardZhaImg:setVisible(false)
        end
    end))
    cardZhaImg:runAction(actSequence2)
end
