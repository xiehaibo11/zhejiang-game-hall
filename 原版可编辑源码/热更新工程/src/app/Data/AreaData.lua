...
function AreaData:initData(data)
    ...
    self._propChangeCard = data.propList.propChangeCard or 0 --换牌卡id
    self._propSmallRoomCardID = data.propList.smallRoomCardID      --加倍/洗牌道具（小房卡）
    self._propLimitedTimeRoomCardID = data.propList.limitedTimeRoomCardID      --限时绑定房卡
    ...
    self._propLimitedTimeRoomCardRatio = self._propFreeRoomCardRatio
    ...
    self._usedNewGPGames = data.usedNewGPGames
...
function AreaData:getPropLimitedTimeRoomCardID()
    return self._propLimitedTimeRoomCardID
end
...
function AreaData:getPropLimitedTimeRoomCardRatio()
    return self._propLimitedTimeRoomCardRatio
end

function AreaData:changeTimeCardToSmallCard(num)
    return num * (self._propSmallRoomCardRatio or 1) / self._propLimitedTimeRoomCardRatio
end
...
function AreaData:getLocationGameRuleKeyword()
    ...
    elseif lobbyID == XH.LOBBY_ID.SHAOXING3D then --绍兴
		rtn = "bCheckPos"
    elseif lobbyID == XH.LOBBY_ID.YUYAO then --余姚
		rtn = "bCheckPos"    
	end
	return rtn
end
...
function AreaData:getTenantid()
    if XH.isEmbeddedApp() then
        return XH.PluginModule.getTenantID()
    end
    return 4
end
...
function AreaData:isUsedNewGPGames(gameID)
    if gameID == 30588 then
        return true
    end
    if self._usedNewGPGames == nil then
        return false
    end
    for i = 1, #self._usedNewGPGames do
        if self._usedNewGPGames[i] == gameID then
            return true
        end
    end
    return false
...
function AreaData:tranformRoomCard(propID, propNum)
    if self:getPropList().smallRoomCardID and propID == self:getPropRoomCardID() then
        local smallRoomCardRatio = self:getPropSmallRoomCardRatio()
        if smallRoomCardRatio then
            propNum = math.floor(propNum * smallRoomCardRatio)
            propID = self:getPropSmallRoomCardID()
        end
    end
