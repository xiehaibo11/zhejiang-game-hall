...
function TipTool.showLoading(hideTime, showLoading)
    if NG and NG.goldLobby and NG.goldLobby:getModule("PeakRaceDispatch"):isInGame() then
        -- 如果是在巅峰赛游戏区就不要加节点了
        return
    end
    local LoadingManager = require("app.Tip.LoadingManager")
    LoadingManager.getInstance():addOnce(hideTime, showLoading)
end
...
function TipTool.removeAllToast()
    local ToastManager = require("app.Tip.ToastManager")
    ToastManager.getInstance():removeAllToast()
end
...
function TipTool.showTip(info, msg, ...)
    ...
    args.enumType = type
    args.msg = msg
    args.funcOK = info.funcOK
    args.funcCancel = info.funcCancel
    args.funcClose = info.funcClose
    args.showCheckBox = info.bShowCheckBox
    args.bLeft = info.bLeft
    args.richText = info.richText
    local popNode = XH.SceneManager:getInstance():getPopNode()
    local tipView = require("app.Tip.MessageBoxView").new(args)
    if info.bCrossScene and popNode then
