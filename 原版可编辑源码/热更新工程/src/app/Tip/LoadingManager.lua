...
function LoadingManager:onDirectorBeforeUpdate(sender)
    if self._loadingState == LoadingState.add then
        if not self._loadingLayer or tolua.isnull(self._loadingLayer) then
            self._loadingLayer = require("app.Tip.LoadingLayer").new(self._showLoading)
            self._showLoading = nil
        ...
        -- 竖屏处理
        if XH.viewManager:isViewExist("ScoringAssistantView") and self._loadingLayer and self._loadingLayer._csbRoot and self._loadingLayer._csbRoot:getChildByName("_LOADING_ANI_PANEL") then
            self._loadingLayer._csbRoot:getChildByName("_LOADING_ANI_PANEL"):setRotation(270)
        end
        self._loadingState = LoadingState.default
...
function LoadingManager:addOnce(hideTime, showLoading)
