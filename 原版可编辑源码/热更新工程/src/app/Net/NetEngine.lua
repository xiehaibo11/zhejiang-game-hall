...
function NetEngine:onMessage(msgid, buff, len, processid, groupID)
    ...
    -- end
    if groupID then
        print("【"..groupID.."】NetEngine onMessage processid = " .. processid .. " msgid = " .. msgid)
    else
        print("【0】NetEngine onMessage msgid = " .. msgid)
    end
...
function NetEngine:sendProtocol(protocol, processID, appID, groupId)
    ...
    if tcp == nil or tcp:getLinkState() ~= XH.SRS_LINK_STATE.LINK_STATE_SUCCESS then
        if XH.SRSProtocol.PlayerConnect.XY_ID == protocol.XY_ID then 
...
function NetEngine:startTcpPB(groupId, protocolData)
    ...
    newTcp:removeAllLinkStateScriptFunc()
...
function NetEngine:getTcpConnectInfoByGroupId(groupId)
    ...
    if XH.areaData:isSupportSRS50() then
        local tipList = XH.LOCAL_TCP_LIST_50
        if XH.userDefault:getValue(XH.userDefault.KEY_ID.KW_DATA_IS_ABROAD_SRS, false) then
            tipList =  XH.LOCAL_TCP_LIST_50_ABROAD
        end
        for k,v in pairs(tipList) do
            if k == groupId then
                list = tipList[groupId]
...
function NetEngine:testReconnect(groupId)
    local newTcp = self:getSRSBySRSGroupID(groupId)
    if newTcp then
        newTcp:_onConnect(false)
    end
end

function NetEngine:setDelayMsg(isDelay)
    for k, v in pairs(self._tcpList) do
        v:setIsDelayDealMsg(isDelay)
    end
end
