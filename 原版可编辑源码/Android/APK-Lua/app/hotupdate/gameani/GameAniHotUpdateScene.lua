...
--进度更新自定义事件
local EVENT_GAMEANI_PERCENT_CHANGED = "hotupdate.GameAniUpate.ProgressChanged"
local EVENT_GAMEANI_START_UPDATE = "hotupdate.GameAniUpate.startUpdate"
...
function HotUpdateScene:start(gameType)
    if gameType == nil or gameType == 0 then
        return
    end
    ...
    self:dispatchPercentEvent(EVENT_GAMEANI_START_UPDATE, param)
end
...
function HotUpdateScene:needHotUpdate(tag, oldVersion, newVersion)
    ...
    local param = {}
    param.percent = 0
    self:dispatchPercentEvent(EVENT_GAMEANI_PERCENT_CHANGED, param)
end
