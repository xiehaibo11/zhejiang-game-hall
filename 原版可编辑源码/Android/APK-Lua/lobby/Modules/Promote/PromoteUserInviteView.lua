...
function PromoteUserInviteView:setInviterName()
    ...
    self._txtMsgLable2:setText(XH.StringTool.replaceMatchStr(string.format("邀请您加入他的亲友圈【%s】", self._info.teaPasswd)))
...
function PromoteUserInviteView:isInTeaHouse()
    ...
    if teahouseData ~= nil then
        ...
        for teaid, info in pairs(teahouseInfo) do
...
function PromoteUserInviteView:onTouchEventAgree(send, eventType)
    ...
    if self._info.tableId ~= nil then
        XH.teaHouseManager:reJoinTeaHouse(self._info.teaPasswd, self._info.tableId)
    elseif self._info.modeId ~= nil then
        XH.teaHouseManager:reJoinTeaHouse(self._info.teaPasswd, nil, nil, nil, self._info.modeId)
    end
    if self.close then
        self:close()
    end
