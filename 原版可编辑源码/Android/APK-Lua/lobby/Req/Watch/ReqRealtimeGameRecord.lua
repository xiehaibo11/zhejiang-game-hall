...
function ReqRealtimeGameRecord:start(roomid, offset, gameid, isDelay, timeout, playercount)
    ...
    if lobbyJsonData and lobbyJsonData.watch1006 then
		local reqRealtimeGameRecord = XH.MatchLinkProtocol.ReqRealtimeGameRecord:new()
		...
		self:sendMsg(reqRealtimeGameRecord, XH.MatchLinkProtocol.RespRealtimeGameRecord, XH.areaData:getSrsGroupID(), 0)
		return
	end
