local ReqSpreadInviteList = class("ReqSpreadInviteList", XH.ReqHttp)
local UrlConf = require("app.Config.UrlConf")
local PromoteConfig = require("lobby.Modules.Promote.Config")

function ReqSpreadInviteList:start(teaNumber, page, timeoutTime)
    if ReqSpreadInviteList.super.start(self, timeoutTime) == false then
        return
    end
    ...
    table.insert(data, "page=" .. page)
    table.insert(data, "reqUrl=" .. UrlConf.URL_SECOND_DOMAIN.REQ_TEAHOUSE_SPREAD_INVITE_LIST)
...
function ReqSpreadInviteList:onHttpCallBack(eType, status, response)
    if status == 200 then
