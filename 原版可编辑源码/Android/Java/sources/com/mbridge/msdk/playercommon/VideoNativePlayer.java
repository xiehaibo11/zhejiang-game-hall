...
public class VideoNativePlayer implements Player.EventListener {
    ...
    private void postOnPlayProgressMSOnMainThread(final long j, final long j2) {
        try {
            if (this.mHandler != null) {
                this.mHandler.post(new Runnable() {
                    @Override
    ...
    private void postOnPlayStartOnMainThread(final int i) {
        try {
            if (this.mHandler != null) {
                this.mHandler.post(new Runnable() {
                    @Override
