...
public class HorizontalSwipeLayout extends FrameLayout {
    ...
    */
    private boolean c(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            c.d("HorizontalSwipeLayout", string);
            if (this.aMs != 0) {
            }
        } else {
            if (action != 2) {
            ...
            float fAbs2 = Math.abs(motionEvent.getY() - this.fy);
            if (fAbs > this.aMr && fAbs > fAbs2) {
                if (x > 0.0f) {
                    this.aMs = 1;
                } else {
                    this.aMs = 2;
    ...
    */
    private boolean d(MotionEvent motionEvent) {
        ...
        if (action != 0) {
            if (action == 1) {
                c.d("HorizontalSwipeLayout", "onTouchEvent ACTION_UP mDragState=" + this.aMs);
                ...
                float fAbs = Math.abs(x);
                float fAbs2 = Math.abs(motionEvent.getY() - this.fy);
                if (this.aMs == 0 && fAbs > this.aMr && fAbs > fAbs2) {
                    if (x > 0.0f) {
                        this.aMs = 1;
                    } else {
                        this.aMs = 2;
        ...
        sb.append(x);
        c.d("HorizontalSwipeLayout", sb.toString());
        if (this.aMs != 0) {
        }
    }
