...
public abstract class HuaweiMobileServicesUtil {
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i) {
        ...
        if (HwBuildEx.VERSION.EMUI_SDK_INT < 5 && packageManagerHelper.getPackageVersionCode(hMSPackageName) < 20500000 && ReadApkFileUtil.isCertFound(hmsPath)) {
        ...
        if (PackageManagerHelper.PackageStates.NOT_INSTALLED.equals(hMSPackageStates)) {
            String packageSignature2 = packageManagerHelper.getPackageSignature(hMSPackageName);
