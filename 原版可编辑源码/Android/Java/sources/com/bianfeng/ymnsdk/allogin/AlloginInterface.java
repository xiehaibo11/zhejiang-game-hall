...
public class AlloginInterface extends YmnChannelInterface {
    ...
    private void preLogin(final AlloginCallBack alloginCallBack) {
        ...
        try {
            onPreLoginRequest(getActivity(), YmnURLManagerV2.getHost(HostConstant.login) + "/pre_login", mapToJson(treeMap), new getPreLoginCallback() {
                @Override
    ...
    class 2 implements AlloginCallBack {
        2() {
        }
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        AlloginInterface.this.tryRunOnUiThreadOrJustRun(new Runnable() {
                            @Override
                            public void run() {
    ...
    class 3 implements AlloginCallBack {
        3() {
        }

        class 1 implements Runnable {
            final String val$autoInfo;
                        ...
                        public void run() {
                            if (str.equals("9000") && str2.equals(BasicPushStatus.SUCCESS_CODE)) {
                                try {
                                    ...
                                    CheckApi.checkLogin(AlloginInterface.this.getActivity(), jSONObject2.toString(), new CheckCallBack() {
                                        @Override
    ...
    private void onPreLoginRequest(Activity activity, String str, String str2, final getPreLoginCallback getprelogincallback) {
        ...
        try {
            httpHelper.createHttpRequest(str, str2, new HttpListener() {
                @Override
