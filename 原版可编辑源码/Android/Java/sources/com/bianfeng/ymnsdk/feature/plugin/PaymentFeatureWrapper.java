        ...
        public void onActionResult(ActionSupportV2.ResponseResult result) {
            ...
            if (result.isOk()) {
                try {
                    ...
                    if (status == 1) {
                        this.a.sendResult(200, status + "|" + entity.toString());
                    } else {
                        this.a.sendResult(201, status + "|" + this.b);
                    }
                    return;
                } catch (YmnsdkException e) {
                    this.a.sendResult(201, "201| 查询数据解析出错");
                    return;
                }
            }
            boolean isNotNeedRetry = this.c.retryRequest(this.d);
            if (isNotNeedRetry) {
                this.a.sendResult(201, "201|" + result.messageFail());
            }
        }
    }
