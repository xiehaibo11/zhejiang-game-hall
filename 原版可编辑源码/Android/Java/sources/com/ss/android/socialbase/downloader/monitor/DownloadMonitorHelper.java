...
public class DownloadMonitorHelper {
    ...
    */
    public static void monitorDownloadConnect(IDownloadHeadHttpConnection iDownloadHeadHttpConnection, String str, String str2, long j, String str3, int i, IOException iOException, DownloadInfo downloadInfo) {
        ...
        if (downloadInfo == null || (iOptInt = (downloadSettingObtain = DownloadSetting.obtain(downloadInfo.getId())).optInt(DownloadSettingKeys.MONITOR_DOWNLOAD_CONNECT, 0)) <= 0 || TextUtils.isEmpty(str)) {
            return;
        }
        int responseCode = -1;
        try {
            if (iDownloadHeadHttpConnection != null) {
                try {
                    responseCode = iDownloadHeadHttpConnection.getResponseCode();
                } catch (Throwable th) {
                    th.printStackTrace();
                }
                hostIp = iDownloadHeadHttpConnection instanceof AbsDownloadHttpConnection ? ((AbsDownloadHttpConnection) iDownloadHeadHttpConnection).getHostIp() : null;
            }
            String requestLog = "";
            ...
            try {
                jSONObject.put(DownloadSettingKeys.SETTING_TAG, downloadSettingObtain.optString(DownloadSettingKeys.SETTING_TAG));
                ...
                if (errorMessage != null) {
                    jSONObject.put("error_msg", DownloadUtils.getFixLengthString(errorMessage, downloadSettingObtain.optInt(DownloadSettingKeys.KEY_EXCEPTION_MSG_LENGTH, 500)));
                }
            ...
            if ((iOptInt == 1 || iOptInt == 3) && (downloadMonitorListener = DownloadComponentManager.getDownloadMonitorListener()) != null) {
        ...
        try {
            ...
            if (z) {
                ...
                if (baseException != null) {
                    errorCode = !DownloadUtils.isNetworkConnected(DownloadComponentManager.getAppContext()) ? 1049 : baseException.getErrorCode();
                    errorMessage = baseException.getErrorMessage();
                    jSONObject2 = new JSONObject();
                    if (iDownloadHttpConnection == null) {
                        i2 = iDownloadHttpConnection instanceof IDefaultDownloadHttpConnection ? 0 : 1;
                        ...
                        j6 = j;
                        requestLog = iDownloadHttpConnection instanceof AbsDownloadHttpConnection ? ((AbsDownloadHttpConnection) iDownloadHttpConnection).getRequestLog() : "";
                        r12 = Contains;
                    } else {
                        j6 = j;
                        requestLog = "";
                    ...
                    jSONObject2.put(MonitorConstants.URL_PATH, path);
                    jSONObject2.put(MonitorConstants.URL_LAST_PATH_SEGMENT, lastPathSegment);
                    jSONObject2.put(MonitorConstants.NET_LIB, i2);
                    jSONObject2.put(MonitorConstants.HIT_CDN_CACHE, r12);
                    jSONObject2.put("status_code", errorCode);
                    jSONObject2.put(MonitorConstants.REQUEST_LOG, requestLog);
                    if (errorMessage != null) {
                        jSONObject2.put("error_msg", DownloadUtils.getFixLengthString(errorMessage, downloadSetting.optInt(DownloadSettingKeys.KEY_EXCEPTION_MSG_LENGTH, 500)));
                    }
                    ...
                    if (i != 1 || i == 3) {
                        ...
                        if (downloadMonitorListener == 0) {
                            str5 = str;
                            downloadMonitorListener.monitorEvent(str5, jSONObject2, null, null);
                        } else {
                            str5 = str;
                        }
                        i3 = 2;
                    ...
                    if (i != i3 || i == 3) {
                        DownloadComponentManager.getEventListener().onEvent(downloadInfo.getId(), str5, jSONObject2);
                    }
                    return;
                }
                errorCode = 0;
            ...
            jSONObject2.put(MonitorConstants.DOWNLOAD_SEC, nanos);
            jSONObject2.put(MonitorConstants.DOWNLOAD_MB, d3);
            if (nanos > 0.0d) {
            }
            if (z3) {
            }
