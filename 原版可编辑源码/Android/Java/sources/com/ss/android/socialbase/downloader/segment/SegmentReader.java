    ...
    public void run() {
        ...
        try {
            try {
                ...
                while (true) {
                    segmentObtainSegment = this.host.obtainSegment(this, this.urlRecord);
                    if (segmentObtainSegment == null) {
                        Logger.i(TAG, "no more segment, thread_index = " + this.threadIndex);
                        break;
                    }
                    this.curSegment = segmentObtainSegment;
                    try {
                        try {
                            if (!download(segmentObtainSegment)) {
                                if (!this.closed) {
                                    Logger.e(TAG, "download segment failed, segment = " + segmentObtainSegment + ", thread_index = " + this.threadIndex + ", failedException = " + this.failedException);
                                    break;
                                }
                                break;
                            }
                            this.succeedSegments.add(segmentObtainSegment);
                            this.curSegment = null;
                            iSegmentCallback2 = this.host;
                        } catch (Throwable th) {
                            this.curSegment = null;
                            this.host.unObtainSegment(this, segmentObtainSegment);
                            throw th;
                        }
                    } catch (SegmentApplyException e) {
                        Logger.e(TAG, "run: SegmentApplyException, e = " + e);
                        if (this.segmentApplyRetryTimes >= 50) {
                            Logger.e(TAG, "segment apply failed " + this.segmentApplyRetryTimes + "times, thread_index = " + this.threadIndex);
                            this.curSegment = null;
                            ISegmentCallback iSegmentCallback3 = this.host;
                            iSegmentCallback3.unObtainSegment(this, segmentObtainSegment);
                        } else {
                            this.segmentApplyRetryTimes++;
                            this.curSegment = null;
                            iSegmentCallback2 = this.host;
                        }
                    }
                    iSegmentCallback2.unObtainSegment(this, segmentObtainSegment);
                }
                iSegmentCallback3.unObtainSegment(this, segmentObtainSegment);
