...
public class DownloadTask {
    ...
    public void asyncDownload(final IDownloadStartCallback iDownloadStartCallback) {
        DownloadThreadPool.executeOP(new Runnable() {
            @Override
            public void run() {
    ...
    public void addListenerToDownloadingSameTask() {
        Logger.d(IDownloadTask.TAG, "same task just tryDownloading, so add listener in last task instead of tryDownload");
        DownloadInfo downloadInfo = this.downloadInfo;
    ...
    private void setChunkCalculator() {
        if (this.downloadInfo.getThrottleNetSpeed() > 0) {
            chunkStategy(new IChunkCntCalculator() {
                @Override
