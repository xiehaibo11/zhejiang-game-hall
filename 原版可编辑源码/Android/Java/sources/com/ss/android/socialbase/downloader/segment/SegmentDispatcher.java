...
public class SegmentDispatcher implements DownloadDnsManager.Callback, ISegmentCallback {
    ...
    private final DownloadWatchDog.IWatcher connectWatcher = new DownloadWatchDog.IWatcher() {
    ...
    private final DownloadWatchDog.IWatcher readWatcher = new DownloadWatchDog.IWatcher() {
        @Override
        public long onScheduleWatch() {
    ...
    private boolean trySwitchNextUrlForReader(SegmentReader segmentReader) {
        synchronized (this) {
    ...
    */
    private void applySegmentLocked(SegmentReader segmentReader, Segment segment) throws SegmentApplyException {
        ...
        long endOffset;
        boolean z;
        Logger.i(TAG, "applySegment: start " + segment);
        if (segment.owner == segmentReader) {
            Logger.i(TAG, "applySegment: " + segmentReader + " is already the owner of " + segment);
            return;
        }
        if (segment.owner != null) {
            Logger.e(TAG, "applySegment: " + segment + " is already has an owner:" + segment.owner);
            throw new SegmentApplyException(1, "segment already has an owner");
        }
        if (segmentReader.getStartOffsetInConnection() != segment.getCurrentOffsetRead()) {
            throw new SegmentApplyException(5, "applySegment");
        ...
        long startOffset = segment.getStartOffset();
        int iIndexOfSegmentLocked = indexOfSegmentLocked(startOffset);
        if (iIndexOfSegmentLocked == -1 || (segment2 = this.dispatchedSegments.get(iIndexOfSegmentLocked)) == null) {
            Logger.e(TAG, "applySegment: " + segment + " not exist! segmentIndex = " + iIndexOfSegmentLocked);
            throw new SegmentApplyException(2, "segment not exist");
        }
        if (segment2 == segment) {
            j = startOffset;
        } else if (segment2.getStartOffset() == segment.getStartOffset()) {
            ...
            if (remainReadBytes > 0) {
                ...
                if (segmentReader2 != null) {
                    ...
                    Log.i(TAG, "applySegmentLocked: has same segment, but owner is normal, abort. segmentInList = " + segment2);
                } else {
                    j = startOffset;
                }
                if (segmentReader2 != null) {
                    Log.i(TAG, "applySegmentLocked: has same segment,and owner too slow, segmentInList = " + segment2);
                    segmentReader2.reconnect(true);
                } else {
                    Log.i(TAG, "applySegmentLocked: has same segment and no owner, segmentInList = " + segment2);
                }
                segment.setEndOffset(segment2.getEndOffset());
                segment.setCompetitor(segment2.getCompetitor());
                this.dispatchedSegments.set(iIndexOfSegmentLocked, segment);
                z = true;
                if (!z) {
                    Logger.e(TAG, "applySegment: " + segment + " not exist! but has another same segment, segmentInList = " + segment2);
        ...
        while (true) {
            ...
            if (endOffset2 <= 0 || endOffset2 >= j) {
                if (downloadBytes <= 0 && segment3.getCurrentOffsetRead() > j) {
                    Logger.e(TAG, "applySegment:prev's current has over this start, prev = " + segment3 + ", segment = " + segment);
                ...
                if (segment3.owner == null) {
                    ...
                    Logger.i(TAG, "applySegment: prev set end, prev = " + segment3);
                    if (segment3.getDownloadBytes() > 0) {
                        Log.d(TAG, "applySegmentLocked:q break");
                        break;
                    }
                } else {
                    long j2 = j - 1;
                    if (segment3.owner.adjustSegmentEndOffset(j2)) {
                        ...
                        Logger.i(TAG, "applySegment: adjustSegmentEndOffset succeed, prev = " + segment3);
                    } else {
                        Logger.e(TAG, "applySegment: adjustSegmentEndOffset filed, prev = " + segment3);
        ...
        for (int i2 = iIndexOfSegmentLocked + 1; i2 < size; i2++) {
            ...
            if (segment4.getDownloadBytes() > 0 || segment4.owner != null) {
                ...
                if (endOffset3 <= 0 || endOffset3 >= startOffset2) {
                    ...
                    Logger.i(TAG, "applySegment: segment set end:" + j3 + ", later = " + segment4);
                }
                Log.d(TAG, "applySegmentLocked: break 2");
                ...
                Logger.i(TAG, "applySegment: OK " + segment);
            }
        }
        endOffset = segment.getEndOffset();
        if (endOffset <= 0) {
    ...
    private SegmentReader findEarliestConnectTimeoutReader(boolean z, long j, long j2) {
