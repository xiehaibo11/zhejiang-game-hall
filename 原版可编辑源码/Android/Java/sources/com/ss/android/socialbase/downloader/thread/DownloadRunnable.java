...
public class DownloadRunnable implements IDownloadRunnableCallback, Runnable {
    ...
    */
    private int calculateChunkCount(long j, List<DownloadChunk> list) {
        int iCalculateChunkCount;
        int iCalculateChunkCount2;
        if (isMultiChunkDownloadAvailable()) {
            if (!this.canResumeFromCache) {
                ...
                if (iChunkCntCalculator != null) {
                    iCalculateChunkCount2 = iChunkCntCalculator.calculateChunkCount(j);
                } else {
                    iCalculateChunkCount2 = this.globalChunkCalculator.calculateChunkCount(j);
                }
                ...
                if (iChunkAdjustCalculator != null) {
                    iCalculateChunkCount = iChunkAdjustCalculator.calculateChunkCount(iCalculateChunkCount2, currentNetworkQuality);
                } else {
                    iCalculateChunkCount = this.globalChunkAdjustCalculator.calculateChunkCount(iCalculateChunkCount2, currentNetworkQuality);
                }
            } else if (list != null) {
                iCalculateChunkCount = list.size();
            } else {
                iCalculateChunkCount = this.downloadInfo.getChunkCount();
            }
            if (iCalculateChunkCount <= 0) {
            }
        } else {
            iCalculateChunkCount = 1;
        }
        if (Logger.debug()) {
            Logger.d(TAG, String.format("chunk count : %s for %s contentLen:%s", String.valueOf(iCalculateChunkCount), this.downloadInfo.getName(), String.valueOf(j)));
        }
        return iCalculateChunkCount;
    }
    ...
    */
    private void checkTaskCache() throws DownloadFileExistException {
        Throwable th;
        DownloadFileExistException downloadFileExistException;
        boolean z = false;
        boolean z2 = true;
        try {
            try {
                try {
                    ...
                    if (this.downloadCache != null) {
                        if (DownloadExpSwitchCode.isSwitchEnable(2048)) {
                        ...
                        if (downloadInfo == null || downloadInfo.isNewTask()) {
                            ...
                            boolean z3 = DownloadExpSwitchCode.isSwitchEnable(4096) && downloadInfo != this.downloadInfo;
                            try {
                                if (savePath.equals(downloadInfo.getSavePath()) && DownloadUtils.isFileDownloaded(downloadInfo, false, md5)) {
                                    try {
                                        ...
                                        downloadFileExistException = e;
                                    } catch (Throwable th2) {
                                        th = th2;
                                        z = z3;
                                        if (this.downloadTask != null) {
                                            DownloadMonitorHelper.monitorSendWithTaskMonitor(this.downloadTask.getMonitorDepend(), this.downloadInfo, new BaseException(1003, DownloadUtils.getErrorMsgWithTagPrefix(th, "checkTaskCache")), this.downloadInfo.getStatus());
                                        }
                                        if (z) {
                                        }
                                    }
                    ...
                    try {
                        ...
                        if (!z2 || this.setting.optBugFix(DownloadSettingKeys.BugFix.FIX_FILE_EXIST_UPDATE_DOWNLOAD_INFO)) {
            ...
            } catch (DownloadFileExistException e6) {
                throw e6;
            } catch (Throwable th5) {
                th = th5;
                z2 = false;
            ...
            if (0 != 0) {
                if (1 != 0) {
                    try {
                        if (this.setting.optBugFix(DownloadSettingKeys.BugFix.FIX_FILE_EXIST_UPDATE_DOWNLOAD_INFO)) {
                            this.downloadCache.updateDownloadInfo(this.downloadInfo);
                        }
    ...
    */
    public void checkSpaceOverflow(long j) throws BaseException {
        ...
        RandomAccessOutputStream randomAccessOutputStreamCreateOutputStream = DownloadUtils.createOutputStream(this.downloadInfo, tempPath, tempName, -1);
        try {
            ...
            DownloadSetting downloadSettingObtain = DownloadSetting.obtain(this.downloadInfo.getId());
            if (downloadSettingObtain.optInt(DownloadSettingKeys.SPACE_FILL_PART_DOWNLOAD, 0) == 1) {
                this.curBytesNeedCheckSpaceOverFlow = 0L;
                if (totalBytes <= 0) {
                    totalBytes = this.downloadInfo.getTotalBytes() - this.downloadInfo.getCurBytes();
                }
                if (availableSpaceBytes < totalBytes) {
                    Logger.w(TAG, "checkSpaceOverflow: contentLength = " + DownloadUtils.byteToMb(j) + "MB, downloaded = " + DownloadUtils.byteToMb(length) + "MB, required = " + DownloadUtils.byteToMb(totalBytes) + "MB, available = " + DownloadUtils.byteToMb(availableSpaceBytes) + "MB");
                    if (availableSpaceBytes > 0) {
                        int iOptInt = downloadSettingObtain.optInt(DownloadSettingKeys.SPACE_FILL_MIN_KEEP_MB, 100);
                        if (iOptInt > 0) {
                            j3 = availableSpaceBytes - (((long) iOptInt) * 1048576);
                            Logger.w(TAG, "checkSpaceOverflow: minKeep = " + iOptInt + "MB, canDownload = " + DownloadUtils.byteToMb(j3) + "MB");
                            if (j3 <= 0) {
                        ...
                        if (downloadSettingObtain.optInt(DownloadSettingKeys.DOWNLOAD_WHEN_SPACE_NEGATIVE, 0) != 1) {
                            StringBuilder sb = new StringBuilder();
                            sb.append("availableSpace ");
                            sb.append(availableSpaceBytes == 0 ? ContainerUtils.KEY_VALUE_DELIMITER : "<");
                            sb.append(" 0");
                            throw new BaseException(DownloadErrorCode.ERROR_GET_AVAILABLE_SPACE, sb.toString());
                        }
                        j2 = j;
                        z = false;
                    }
                }
                randomAccessOutputStreamCreateOutputStream.setLength(j);
                DownloadUtils.safeClose(randomAccessOutputStreamCreateOutputStream);
            }
            if (availableSpaceBytes <= 0) {
                StringBuilder sb2 = new StringBuilder();
                sb2.append("availableSpace ");
                sb2.append(availableSpaceBytes == 0 ? ContainerUtils.KEY_VALUE_DELIMITER : "<");
                sb2.append(" 0");
                throw new BaseException(DownloadErrorCode.ERROR_GET_AVAILABLE_SPACE, sb2.toString());
            }
            if (availableSpaceBytes < totalBytes) {
                throw new DownloadOutOfSpaceException(availableSpaceBytes, totalBytes);
            }
    ...
    public RetryCheckStatus onRetry(BaseException baseException, long j) {
        ...
        if (baseException != null && baseException.getErrorCode() == 1047) {
            if (this.forbiddenHandler == null || this.downloadInfo.isForbiddenRetryed()) {
                ...
                AbsDownloadForbiddenCallback absDownloadForbiddenCallback = new AbsDownloadForbiddenCallback() {
                    @Override
            ...
            IDownloadDiskSpaceCallback iDownloadDiskSpaceCallback = new IDownloadDiskSpaceCallback() {
                @Override
    ...
    */
    private DownloadChunk getUnCompletedSubChunk(DownloadChunk downloadChunk, int i) {
        ...
        if (!downloadChunk.hasChunkDivided() && retainLength > DownloadConstants.MIN_CHUNK_REUSE_SIZE && this.downloadInfo.isNeedReuseChunkRunnable()) {
            List<DownloadChunk> listDivideChunkForReuse = downloadChunk.divideChunkForReuse(this.downloadInfo.getChunkCount(), this.downloadInfo.getTotalBytes());
            if (listDivideChunkForReuse != null) {
                Iterator<DownloadChunk> it = listDivideChunkForReuse.iterator();
                while (it.hasNext()) {
            ...
            if (z && downloadChunk.hasChunkDivided()) {
                for (i2 = 1; i2 < downloadChunk.getSubChunkList().size(); i2++) {
                    DownloadChunk downloadChunk3 = downloadChunk.getSubChunkList().get(i2);
                    if (downloadChunk3 != null) {
                        Logger.d(TAG, "check can checkUnCompletedChunk -- chunkIndex:" + downloadChunk3.getChunkIndex() + " currentOffset:" + downloadChunk3.getCurrentOffset() + "  startOffset:" + downloadChunk3.getStartOffset() + " contentLen:" + downloadChunk3.getContentLength());
