...
public final class StreamAllocation {
    ...
    public HttpCodec newStream(OkHttpClient okHttpClient, Interceptor.Chain chain, boolean z) {
        try {
            HttpCodec httpCodecNewCodec = findHealthyConnection(chain.getConnectTimeoutMillis(), chain.readTimeoutMillis(), chain.writeTimeoutMillis(), okHttpClient.pingIntervalMillis(), okHttpClient.retryOnConnectionFailure(), z).newCodec(okHttpClient, chain, this);
    ...
    private RealConnection findConnection(int i, int i2, int i3, int i4, boolean z) throws Throwable {
        ...
        routeDatabase().connected(realConnection2.getRoute());
        synchronized (this.connectionPool) {
