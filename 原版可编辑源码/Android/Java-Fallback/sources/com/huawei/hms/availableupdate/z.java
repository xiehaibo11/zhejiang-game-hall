package com.huawei.hms.availableupdate;

public class z extends com.huawei.hms.availableupdate.p {
    public z() {
            r0 = this;
            r0.<init>()
            return
    }

    public final void a(android.content.Intent r5) {
            r4 = this;
            android.app.Activity r0 = r4.b()
            android.content.pm.PackageManager r0 = r0.getPackageManager()
            r1 = 65536(0x10000, float:9.1835E-41)
            java.util.List r0 = r0.queryIntentActivities(r5, r1)
            if (r0 == 0) goto L53
            boolean r1 = r0.isEmpty()
            if (r1 != 0) goto L53
            java.util.ArrayList r1 = new java.util.ArrayList
            r1.<init>()
            java.lang.String r2 = "com.huawei.appmarket.tv"
            r1.add(r2)
            java.lang.String r2 = "com.huawei.appmarket.car"
            r1.add(r2)
            java.lang.String r2 = "com.huawei.appmarket"
            r1.add(r2)
            int r2 = r0.size()
            r3 = 1
            if (r2 != r3) goto L32
            return
        L32:
            java.util.Iterator r0 = r0.iterator()
        L36:
            boolean r2 = r0.hasNext()
            if (r2 == 0) goto L53
            java.lang.Object r2 = r0.next()
            android.content.pm.ResolveInfo r2 = (android.content.pm.ResolveInfo) r2
            android.content.pm.ActivityInfo r3 = r2.activityInfo
            java.lang.String r3 = r3.packageName
            boolean r3 = r1.contains(r3)
            if (r3 == 0) goto L36
            android.content.pm.ActivityInfo r0 = r2.activityInfo
            java.lang.String r0 = r0.packageName
            r5.setPackage(r0)
        L53:
            return
    }

    @Override
    public void a(com.huawei.hms.availableupdate.q r3) {
            r2 = this;
            java.lang.String r0 = "HiappWizard"
            java.lang.String r1 = "Enter onCancel."
            com.huawei.hms.support.log.HMSLog.i(r0, r1)
            boolean r3 = r3 instanceof com.huawei.hms.availableupdate.a0
            if (r3 == 0) goto Le
            r2.d()
        Le:
            return
    }

    @Override
    public void a(java.lang.Class<? extends com.huawei.hms.availableupdate.q> r3) {
            r2 = this;
            r2.a()
            java.lang.Object r3 = r3.newInstance()     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            com.huawei.hms.availableupdate.q r3 = (com.huawei.hms.availableupdate.q) r3     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            java.lang.String r0 = r2.h     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            boolean r0 = android.text.TextUtils.isEmpty(r0)     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            if (r0 != 0) goto L25
            boolean r0 = r3 instanceof com.huawei.hms.availableupdate.a0     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            if (r0 == 0) goto L25
            java.lang.String r0 = "hms_update_title"
            java.lang.String r0 = com.huawei.hms.utils.ResourceLoaderUtil.getString(r0)     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            r2.h = r0     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            r0 = r3
            com.huawei.hms.availableupdate.a0 r0 = (com.huawei.hms.availableupdate.a0) r0     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            java.lang.String r1 = r2.h     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            r0.a(r1)     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
        L25:
            r3.a(r2)     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            r2.d = r3     // Catch: java.lang.IllegalStateException -> L2b java.lang.IllegalAccessException -> L2d java.lang.InstantiationException -> L2f
            goto L4a
        L2b:
            r3 = move-exception
            goto L30
        L2d:
            r3 = move-exception
            goto L30
        L2f:
            r3 = move-exception
        L30:
            java.lang.StringBuilder r0 = new java.lang.StringBuilder
            r0.<init>()
            java.lang.String r1 = "In showDialog, Failed to show the dialog."
            r0.append(r1)
            java.lang.String r3 = r3.getMessage()
            r0.append(r3)
            java.lang.String r3 = r0.toString()
            java.lang.String r0 = "HiappWizard"
            com.huawei.hms.support.log.HMSLog.e(r0, r3)
        L4a:
            return
    }

    @Override
    public void b(com.huawei.hms.availableupdate.q r3) {
            r2 = this;
            java.lang.String r0 = "HiappWizard"
            java.lang.String r1 = "Enter onDoWork."
            com.huawei.hms.support.log.HMSLog.i(r0, r1)
            boolean r0 = r3 instanceof com.huawei.hms.availableupdate.a0
            if (r0 == 0) goto L28
            r3.b()
            boolean r3 = r2.c()
            if (r3 != 0) goto L28
            r3 = 0
            boolean r3 = r2.a(r3)
            r0 = 8
            if (r3 != 0) goto L23
            int r3 = r2.f
            r2.c(r0, r3)
            goto L28
        L23:
            int r3 = r2.f
            r2.a(r0, r3)
        L28:
            return
    }

    public final boolean c() {
            r8 = this;
            java.lang.String r0 = "HiappWizard"
            android.app.Activity r1 = r8.b()
            r2 = 0
            if (r1 == 0) goto L78
            boolean r3 = r1.isFinishing()
            if (r3 == 0) goto L10
            goto L78
        L10:
            java.lang.String r3 = r8.g
            boolean r3 = android.text.TextUtils.isEmpty(r3)
            if (r3 == 0) goto L19
            return r2
        L19:
            android.content.Intent r3 = new android.content.Intent     // Catch: android.content.ActivityNotFoundException -> L73
            r3.<init>()     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.String r4 = "com.huawei.appmarket.intent.action.AppDetail"
            r3.setAction(r4)     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.String r4 = "APP_PACKAGENAME"
            java.lang.String r5 = r8.g     // Catch: android.content.ActivityNotFoundException -> L73
            r3.putExtra(r4, r5)     // Catch: android.content.ActivityNotFoundException -> L73
            boolean r4 = com.huawei.hms.android.SystemUtils.isTVDevice()     // Catch: android.content.ActivityNotFoundException -> L73
            r5 = 1
            if (r4 == 0) goto L63
            com.huawei.hms.utils.PackageManagerHelper r4 = new com.huawei.hms.utils.PackageManagerHelper     // Catch: android.content.ActivityNotFoundException -> L73
            r4.<init>(r1)     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.String r6 = "com.hisilicon.android.hiRMService"
            int r4 = r4.getPackageVersionCode(r6)     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.StringBuilder r6 = new java.lang.StringBuilder     // Catch: android.content.ActivityNotFoundException -> L73
            r6.<init>()     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.String r7 = "version "
            r6.append(r7)     // Catch: android.content.ActivityNotFoundException -> L73
            r6.append(r4)     // Catch: android.content.ActivityNotFoundException -> L73
            java.lang.String r6 = r6.toString()     // Catch: android.content.ActivityNotFoundException -> L73
            com.huawei.hms.support.log.HMSLog.i(r0, r6)     // Catch: android.content.ActivityNotFoundException -> L73
            if (r4 != r5) goto L63
            java.lang.String r4 = "startActivity"
            com.huawei.hms.support.log.HMSLog.i(r0, r4)     // Catch: android.content.ActivityNotFoundException -> L73
            r4 = 268435456(0x10000000, float:2.524355E-29)
            r3.addFlags(r4)     // Catch: android.content.ActivityNotFoundException -> L73
            r8.a(r3)     // Catch: android.content.ActivityNotFoundException -> L73
            r1.startActivity(r3)     // Catch: android.content.ActivityNotFoundException -> L73
            return r5
        L63:
            java.lang.String r4 = "startActivityForResult"
            com.huawei.hms.support.log.HMSLog.i(r0, r4)     // Catch: android.content.ActivityNotFoundException -> L73
            r8.a(r3)     // Catch: android.content.ActivityNotFoundException -> L73
            int r4 = r8.getRequestCode()     // Catch: android.content.ActivityNotFoundException -> L73
            r1.startActivityForResult(r3, r4)     // Catch: android.content.ActivityNotFoundException -> L73
            return r5
        L73:
            java.lang.String r1 = "can not open hiapp"
            com.huawei.hms.support.log.HMSLog.e(r0, r1)
        L78:
            return r2
    }

    public void d() {
            r2 = this;
            int r0 = r2.f
            r1 = 13
            r2.c(r1, r0)
            return
    }

    @Override
    public int getRequestCode() {
            r1 = this;
            r0 = 2005(0x7d5, float:2.81E-42)
            return r0
    }

    @Override
    public void onBridgeActivityCreate(android.app.Activity r2) {
            r1 = this;
            super.onBridgeActivityCreate(r2)
            com.huawei.hms.update.ui.UpdateBean r2 = r1.c
            if (r2 != 0) goto L8
            return
        L8:
            r0 = 5
            r1.f = r0
            boolean r2 = r2.isNeedConfirm()
            if (r2 == 0) goto L1f
            java.lang.String r2 = r1.h
            boolean r2 = android.text.TextUtils.isEmpty(r2)
            if (r2 != 0) goto L1f
            java.lang.Class<com.huawei.hms.availableupdate.a0> r2 = com.huawei.hms.availableupdate.a0.class
            r1.a(r2)
            goto L39
        L1f:
            boolean r2 = r1.c()
            if (r2 != 0) goto L39
            r2 = 0
            boolean r2 = r1.a(r2)
            r0 = 8
            if (r2 != 0) goto L34
            int r2 = r1.f
            r1.c(r0, r2)
            goto L39
        L34:
            int r2 = r1.f
            r1.a(r0, r2)
        L39:
            return
    }

    @Override
    public void onBridgeActivityDestroy() {
            r0 = this;
            super.onBridgeActivityDestroy()
            return
    }

    @Override
    public boolean onBridgeActivityResult(int r2, int r3, android.content.Intent r4) {
            r1 = this;
            boolean r0 = r1.e
            if (r0 == 0) goto Ld
            com.huawei.hms.activity.IBridgeActivityDelegate r0 = r1.b
            if (r0 == 0) goto Ld
            boolean r2 = r0.onBridgeActivityResult(r2, r3, r4)
            return r2
        Ld:
            int r3 = r1.f
            r4 = 5
            r0 = 0
            if (r3 != r4) goto L32
            int r3 = r1.getRequestCode()
            if (r2 != r3) goto L32
            java.lang.String r2 = r1.g
            int r3 = r1.i
            boolean r2 = r1.a(r2, r3)
            if (r2 == 0) goto L29
            int r2 = r1.f
            r1.c(r0, r2)
            goto L30
        L29:
            int r2 = r1.f
            r3 = 8
            r1.c(r3, r2)
        L30:
            r2 = 1
            return r2
        L32:
            return r0
    }

    @Override
    public void onBridgeConfigurationChanged() {
            r0 = this;
            super.onBridgeConfigurationChanged()
            return
    }

    @Override
    public void onKeyUp(int r2, android.view.KeyEvent r3) {
            r1 = this;
            boolean r0 = r1.e
            if (r0 == 0) goto Lc
            com.huawei.hms.activity.IBridgeActivityDelegate r0 = r1.b
            if (r0 == 0) goto Lc
            r0.onKeyUp(r2, r3)
            return
        Lc:
            r3 = 4
            if (r3 != r2) goto L2a
            java.lang.String r2 = "HiappWizard"
            java.lang.String r3 = "In onKeyUp, Call finish."
            com.huawei.hms.support.log.HMSLog.i(r2, r3)
            android.app.Activity r2 = r1.b()
            if (r2 == 0) goto L2a
            boolean r3 = r2.isFinishing()
            if (r3 != 0) goto L2a
            r3 = 0
            r0 = 0
            r2.setResult(r3, r0)
            r2.finish()
        L2a:
            return
    }
}
